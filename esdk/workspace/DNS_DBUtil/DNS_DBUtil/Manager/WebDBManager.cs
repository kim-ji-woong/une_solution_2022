    ...
    public class WebDBManager
    {
        ...
        public virtual bool BatchCommit()
        {
            ...
            try
            {
                ...
                // 정수형 데이터는 Dictionary로 처리하면 문자열로 인식된다.
                // 따라서, 아래처럼 해야한다.
                JObject obj = new JObject();
                obj.Add("transactionKey", m_nBatchCode);
        ...
        private ArrayList GetBatchProcedure(string strProcedureName, List<string> fieldNames, List<string> fieldValues)
        {
            try
            {
                ...
                /*JObject obj = new JObject();
                obj.Add("procedureName", strProcedureName);
                obj.Add("fieldNames", GetArrayJsonString(fieldNames));
                obj.Add("fieldValues", GetArrayJsonString(fieldValues));
                obj.Add("transactionKey", m_nBatchCode);
                ...
                Dictionary<string, object> values = new Dictionary<string, object>
                {
                    ...
                    { "fieldNames", fieldNames },
                    { "fieldValues", fieldValues }/*,
                    // 정수형 타입과 실수형 타입을 구분할 방법이 없으므로 정수형 타입은 따로 처리한다.
                    { "transactionKey", m_nBatchCode.ToString() }*/
                };
                ...
                json2 = json2.Substring(1, json2.Length - 2).Trim();

                string json = JsonManager.Serialize(values);
                json = json.Substring(0, json.Length - 1) + ", " + json2 + "}";
