...
import PoiInfo from './popups/poiInfo';
...
export class Contents3D extends Component {
	...
	static poiDatas = {
		"0": {
			"rnjs2955": Contents3D.makePoiData(25, 9, "권혜진", "사업팀"),
			"zxz3410": Contents3D.makePoiData(45, 15, "송다예 대리", "사업팀"),
			"kyran": Contents3D.makePoiData(68, 19, "이경한 부장", "사업팀"),
			"sywzzss": Contents3D.makePoiData(65, 12, "서유원 차장", "사업팀"),
			"kdr1820": Contents3D.makePoiData(-16, 9, "김다래 대리", "개발1팀"),
			"su1562": Contents3D.makePoiData(-28, 9, "이수현 과장", "개발1팀"),
			"jsj930406": Contents3D.makePoiData(-45, 15, "전수진", "개발1팀"),
			"kjw": Contents3D.makePoiData(-55, 18, "김지웅 이사", "개발1팀"),
		}
	}

	// horzAngle : poi의 수평방향 초기위치(degree, -90 ~ 90)
	// vertAngle : poi의 수직방향 초기위치(degree, -90 ~ 90)
	static makePoiData(horzAngle, vertAngle, name, team) {
		return {
			"horz": horzAngle,
			"vert": vertAngle,
			"name": name,
	...
	init() {
		...
		this.addPois();
		//this.poiManager.addPOI(this.scene, POIManager.icon.etc, "test poi", 0, 0, 1);
	}

	addPois() {
		this.poiManager.removeAll(this.scene);
		const poiDatas = Contents3D.poiDatas[this.currentPointIndex];

		if (poiDatas) {
			for (const poiName in poiDatas) {
				const poiData = poiDatas[poiName];
				this.poiManager.addPOI(this.scene, POIManager.icon.etc, poiName, poiData.horz, poiData.vert, 90 - this.lon, this.lat);
			}
		}
    }
	...
	setArrowModels() {
		...
		if (normalCount > 0 && selectedCount > 0 && normalCount === selectedCount) {
			...
			for (let i = 0/*movePoints.length*/; i < this.arrowModels_normal.length; i++) {
	...
	onPointerMove(event) {
		if (this.isUserInteracting === true) {
			...
			const rotateCameraHorz = 90 - this.lon;
			const rotateCameraVert = this.lat;
			this.poiManager.rotateCamera(rotateCameraHorz, rotateCameraVert);
		}
	...
	onClick(event) {
		...
		const prevSelectedPoi = this.state.selectedPoi;
		let selectedPoi = this.poiManager.getPoi(raycaster);

		if (selectedPoi) {
			const poiDatas = Contents3D.poiDatas[this.currentPointIndex];

			if (poiDatas) {
				selectedPoi = poiDatas[selectedPoi.name];
			}
			else {
				selectedPoi = null;
            }
		...
		if (selectedPoi === prevSelectedPoi) {
			return;
		}
		else {
			this.setState({ selectedPoi: selectedPoi, clickPos: { x: x, y: y } });
        }
	}
		...
		return (
			<div>
				...
				</div>
				{
					...
					<PoiInfo poiData={this.state.selectedPoi} clickPos={this.state.clickPos}/>
				}
				{
