...
namespace Vacation.BLL
{
    ...
    public class ScheduleManager
    {
        ...
        public static void SetSystemInfo(string strSystemMail, string strNoticeMail, string strSystemCode, string strURL)
        {
        ...
        private bool CheckIgnoreSVRequest(DateTime dtNow)
        {
            ...
            strCondition = string.Format("{0} in ({1})", SpecialVacationResponse.Fields.RequestID, string.Join(",", requestIDs.ToArray()));
        ...
        private Dictionary<SpecialVacationRequest, List<SpecialVacationResponse>> GetSVRequestResponses(IEnumerable<SpecialVacationRequest> requests, IEnumerable<SpecialVacationResponse> responses)
        {
        ...
        private bool TimeoutSVResponse(SpecialVacationResponse response, Response.ResponseType result, DateTime dtNow)
        {
            ...
            if (m_dataManager.GetUpdate().Update<SpecialVacationResponse>(response, null, out strErrorMessage) == false)
            {
        ...
        private bool TimeoutSVRequest(SpecialVacationRequest request, Dictionary<int, CompanyMember> dicRequestMembers)
        {
            ...
            // 취소처리
            if (m_dataManager.GetUpdate().Update<SpecialVacationRequest>(request, null, out strErrorMessage) == false)
            {
        ...
        private bool CheckTimeoutRequest(DateTime dtNow)
        {
            ...
            foreach (Request request in requests)
            {
                ...
                {
                    if (firstDate.Type == Date.DateType.AM && dtNow.Hour >= 12)
                    {
                        requestIDs.Add(request.ID);
            ...
            strCondition = string.Format("{0} in ({1})", Response.Fields.RequestID, string.Join(",", requestIDs));
        ...
        private bool CheckIgnoreRequest(DateTime dtNow)
        {
            ...
            strCondition = string.Format("{0} in ({1})", Response.Fields.RequestID, string.Join(",", requestIDs.ToArray()));
            IEnumerable<Response> responses = m_dataManager.GetSelect().Select<Response>(strCondition, out strErrorMessage);
        ...
        private Dictionary<int, CompanyMember> GetMembers(IEnumerable<int> memberIDs)
        {
            ...
            string strErrorMessage;
            IEnumerable<CompanyMember> members = m_dataManager.GetSelect().Select<CompanyMember>(strConditions, out strErrorMessage);
        ...
        {
            string strErrorMessage;
            request.Response = (int)Response.ResponseType.Timeout;

            // 취소처리
            if (m_dataManager.GetUpdate().Update<Request>(request, null, out strErrorMessage) == false)
            {
            ...
            if (dicRequestMembers.TryGetValue(request.MemberID, out member))
            {
                float fTotalDays;
                string strPeriod;
                RequestManager.GetYearDays(Date.StringToDateList(request.Days, request.Year), out fTotalDays, out strPeriod);
        ...
        // Request에 해당하는 Response들을 시간순으로 정렬하여 리턴한다.
        private Dictionary<Request, List<Response>> GetRequestResponses(IEnumerable<Request> requests, IEnumerable<Response> responses)
        {
        ...
        private bool CheckNewYear(DateTime dtNow)
        {
            ...
            foreach (CompanyMember member in members)
            {
                string strCondition = string.Format("{0} = {1} and {2} = {3}",
                    History.Fields.MemberID, member.ID,
                    History.Fields.Year, dtNow.Year);
                History history = m_dataManager.GetSelect().SelectFirst<History>(strCondition, out strErrorMessage);
        ...
        private bool CheckNextVacation(DateTime dtNow)
        {
            ...
            ArrayList arrDatas = m_joinManager.JoinCompanyMemberHistory(null, dicConditions, null, out strErrorMessage);
        ...
        private void UpdateMonthVacation(CompanyMember member, History history, DateTime dtNow)
        {
            ...
            if (dtNow.Month == 1 && dtNow.Year == 1)
            {
                ...
                history = m_dataManager.GetSelect().SelectFirst<History>(strCondition, out strErrorMessage);
            ...
            history.NextVacationDay = nextVacationDay;
            m_dataManager.GetUpdate().Update<History>(history, null, out strErrorMessage);
        }
        ...
        private bool CheckVacationNotice(DateTime dtNow)
        {
            ...
            string strCondition = string.Format("{0} = {1} and {2} is null",
                Request.Fields.Response, (int)Response.ResponseType.Permit,
                Request.Fields.MailSendTime);
                ...
                foreach (Request request in requests)
                {
                    ...
                    if (span.TotalDays <= nNoticeDays)
                    {
                        if (SendNoticeEmail(request, strDays, rootTeam))
                        {
                            ...
                            m_dataManager.GetUpdate().Update<Request>(request, null, out strErrorMessage);
                        }
                    }
                }
            }
        ...
        private bool CheckTodayVacationNotice(DateTime dtNow)
        {
            ...
            strAdditionalCondition += string.Format(" and {0} = {1}", Request.Fields.Response, (int)Response.ResponseType.Permit);

            IEnumerable<Request> requests = m_dataManager.GetSelect().Select<Request>(strAdditionalCondition, out strErrorMessage);
        ...
        {
            string strCondition = string.Format("{0} is null", RegularTeam.Fields.ParentID);
            ...
            RegularTeam team = AccountManager.GetFirst(teams);
        ...
        private bool ReadOptions(out int nNoticeDays, out string strErrorMessage)
        {
            ...
            string strCondition = string.Format("{0} in ('{1}')", VacationOption.Fields.PropertyName, strNoticeDays);
