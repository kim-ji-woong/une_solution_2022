...
namespace Vacation.BLL
{
    public class KakaoManager
    {
        ...
        private static void Send(string strMessage, string strPhoneNumber, string strTmpltCode)
        {
            ...
            IEnumerable<OptionKakaoInfo> infos = m_dataManager.GetSelect().Select<OptionKakaoInfo>(null, out strErrorMessage);

            if (infos == null)
                return;

            OptionKakaoInfo info = AccountManager.GetFirst(infos);
