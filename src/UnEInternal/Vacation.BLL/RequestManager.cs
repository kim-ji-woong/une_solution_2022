...
namespace Vacation.BLL
{
    ...
    public static class RequestManager
    {
        public static RequestSpecialVacationResult CreateSpecialVacationResult(IDataManager dataManager, ApplicationUser requestManager, List<ApplicationUser> responseManagers, List<ApplicationUser> users, float fDays, Response.ResponseType response, string strRequestDescription)
        {
            ...
            CreateManager createManager = new CreateManager(dataManager);

            SpecialVacationRequest request = new SpecialVacationRequest();
            request.Days = fDays;
            request.RequestTime = dtNow;
            request.RequestManagerID = requestManager.ID;
            request.MemberIDs = string.Join(",", userIDs.ToArray());
            request.ResponseManagerIDs = string.Join(",", managerIDs.ToArray());

            if (response == Response.ResponseType.None)
                request.Response = null;
            else
                request.Response = (int)response;

            request.RequestDescription = strRequestDescription;

            int addID;
            string strErrorMessage;
            createManager.CreateSpecialVacationRequest(request, out addID, out strErrorMessage);

            if (createManager.CreateSpecialVacationRequest(request, out addID, out strErrorMessage) == false)
        ...
        private static bool UpdateVacationDays(IDataManager dataManager, ApplicationUser user, Request request, RollbackManager rollback, ref string strChangedVacation, out string strErrorMessage)
        {
            ...
            for (int i=0;i<nDayCount;i++)
            {
                ...
                else if (nDay == today)
                {
                    ...
                    else// if (date.Type == Date.DateType.Normal)
                    {
                        if (dtNow.Hour > VacationManager.BeginWorkHour || (dtNow.Hour == VacationManager.BeginWorkHour && dtNow.Minute >= VacationManager.BeginWorkMinute))
                        {
                            if (dtNow.Hour < 12)
                            {
                                ...
                                am.Type = Date.DateType.AM;
                                days.Insert(i, am);
                                nDayCount++;

                                date.Type = Date.DateType.PM;
                            }
