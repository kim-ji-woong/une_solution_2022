...
namespace Vacation.BLL
{
    ...
    public class VacationManager
    {
        ...
        private void ReadOptions()
        {
            ...
            string strCondition = string.Format("{0} in ('{1}', '{2}', '{3}')", strBaseVacationDays, strReservationMonth, strRequestTimeout);

            string strErrorMessage;

            IEnumerable<VacationOption> options = m_dataManager.GetSelect().Select<VacationOption>(strCondition, out strErrorMessage);
        ...
        public RequestManagerResult GetManager(string strUserID, List<Date> requestDays)
        {
            ...
            ArrayList arrMembers = m_joinManager.JoinCompanyMemberJobLevelRegularTeam(dicConditions, null, null, null, out strErrorMessage);
            ...
            foreach (Date date in requestDays)
            {
                ...
                if (dicHistories.TryGetValue(date.Year, out history) == false)
                {
                    string strCondition = string.Format("{0} = {1} and {2} = {3}",
                        Model.History.Fields.MemberID, member.ID,
                        Model.History.Fields.Year, date.Year);
                    history = selectManager.SelectFirst<Model.History>(strCondition, out strMessage);
                ...
                float fDay = date.Type == Date.DateType.AM || date.Type == Date.DateType.PM ? 0.5f : 1;
            ...
            ArrayList arrResult = m_joinManager.JoinCompanyMemberJobLevelRegularTeam(dicConditions1, null, dicConditions3, null, out strErrorMessage);
        ...
        private bool GetHistoryDays(Model.History history)
        {
            ...
            {
                string strCondition = string.Format("{0} = {1} and {2} = {3}",
                    Request.Fields.MemberID, history.MemberID,
                    Request.Fields.Year, history.Year);
                ...
                string strErrorMessage;
                IEnumerable<Request> requests = m_dataManager.GetSelect().Select<Request>(strCondition, out strErrorMessage);
                ...
                {
                    if (request.Response != null && (int)request.Response == (int)Response.ResponseType.Permit)
                    {
                        List<Date> dates = Date.StringToDateList(request.Days, request.Year);
                    ...
                    else if (request.Response != null && (int)request.Response == (int)Response.ResponseType.Processing)
                    {
                        List<Date> dates = Date.StringToDateList(request.Days, request.Year);
        ...
        private Models.Vacation.History GetVacationHistory(string userID, CompanyMember member, Dictionary<int, Model.History> dicMemberHistories, Dictionary<int, IEnumerable<SpecialVacation>> dicMemberSpecialVacations, Dictionary<int, SpecialVacationRequest> dicSpecialVacationRequests, IEnumerable<SpecialVacationResponse> svResponses, Dictionary<int, IEnumerable<Request>> dicLastYearMemberRequests, Dictionary<int, IEnumerable<Request>> dicMemberRequests, IEnumerable<Response> responses, Dictionary<int, CompanyMember> allMembers, Dictionary<int, RegularTeam> allTeams, Dictionary<int, JobLevel> allLevels, int year, int month, int day, bool createIfNotExist = true)
        {
            float fTotalDays = 0;
            List<SpecialVacation> specialVacations = new List<SpecialVacation>();
            List<Request> requests = new List<Request>();
            ...
            if (member == null)
            {
                string strCondition = string.Format("{0} = '{1}'", CompanyMember.Fields.UserID, userID);
                //Dictionary<CompanyMember.Fields, object> dicConditions = new Dictionary<CompanyMember.Fields, object>();
                //dicConditions[CompanyMember.Fields.UserID] = userID;

                IEnumerable<CompanyMember> members = selectManager.Select<CompanyMember>(strCondition, out strErrorMessage);

                if (members == null)
                    return GetFailVacationHistory(strErrorMessage);

                member = AccountManager.GetFirst(members);
            ...
            else
            {
                string strCondition = string.Format("{0} = {1} and {2} = {3}",
                    Model.History.Fields.MemberID, member.ID,
                    Model.History.Fields.Year, year);
                history = selectManager.SelectFirst<Model.History>(strCondition, out strErrorMessage);
            }
            ...
            IEnumerable<SpecialVacation> svs = null;
            ...
            {
                string strCondition = string.Format("{0} = {1} and {2} >= '{3}-01-01 00:00:00' and {2} <= '{3}-12-31 23:59:59'",
                    SpecialVacation.Fields.MemberID, member.ID,
                    SpecialVacation.Fields.CreateTime, year);
                svs = selectManager.Select<SpecialVacation>(strCondition, out strErrorMessage);
            ...
            foreach (SpecialVacation vacation in svs)
            {
                ...
                {
                    string strCondition = string.Format("{0} = {1}", SpecialVacationRequest.Fields.ID, vacation.RequestID);

                    request = selectManager.SelectFirst<SpecialVacationRequest>(strCondition, out strErrorMessage);
            ...
            if (svResponses == null)
            {
                string strCondition = string.Format("{0} in ({1})",
                    SpecialVacationResponse.Fields.RequestID, string.Join(", ", svRequestIDs.ToArray()));
                svResponses = selectManager.Select<SpecialVacationResponse>(strCondition, out strErrorMessage);
            }
            ...
            IEnumerable<Request> _requestList = null;

            if (dicMemberRequests != null)
            {
                if (dicMemberRequests.TryGetValue(member.ID, out _requestList) == false)
                {
                    _requestList = new List<Request>();
                    dicMemberRequests[member.ID] = _requestList;
                }
            ...
            {
                bool isNullable;
                string strCondition = string.Format("({0} = {1} or {2} = {1}) and {3} = {4}",
                    Request.GetFieldName(Request.Fields.Year, out isNullable),
                    year,
                    Request.GetFieldName(Request.Fields.Year2, out isNullable),
                    Request.Fields.MemberID, member.ID);

                _requestList = selectManager.Select<Request>(strCondition, out strErrorMessage);
            }
            ...
            requestList.AddRange(_requestList);
            ...
            {
                string strCondition = string.Format("{0} in ({1})",
                    Response.Fields.RequestID, string.Join(", ", requestIDs.ToArray()));
                responses = selectManager.Select<Response>(strCondition, out strErrorMessage);
            }
        ...
        // 작년에 사용한 연차중에서 연말에 두 년도에 결처있는 Request가 있는지 확인
        private void AddRequestFromLastYear(List<Request> requestList, int memberID, Dictionary<int, IEnumerable<Request>> dicLastYearMemberRequests)
        {
            if (dicLastYearMemberRequests != null)
            {
                IEnumerable<Request> prevRequestList;
        ...
        private List<VacationDetail> ParseVacationDetails(List<Request> requests, Dictionary<Request, List<Response>> dicResponses, Dictionary<int, CompanyMember> dicManagers, Dictionary<CompanyMember, JobLevel> dicManagerLevels, Dictionary<CompanyMember, RegularTeam> dicManagerTeams)
        {
            ...
            foreach (Request request in requests)
            {
                if (request.Response != null &&
                    ((int)request.Response == (int)Response.ResponseType.Deny ||
                    (int)request.Response == (int)Response.ResponseType.Cancel ||
                    (int)request.Response == (int)Response.ResponseType.Timeout))
                    continue;
                ...
                if (dicResponses.TryGetValue(request, out responses))
                {
                    ...
                    detail.Dates.AddRange(Date.StringToDateList(request.Days, request.Year));
        ...
        private ArrayList SelectCompanyMemberJobLevelRegularTeam(int memberID, Dictionary<int, CompanyMember> allMembers, Dictionary<int, RegularTeam> allTeams, Dictionary<int, JobLevel> allLevels, ISelect selectManager, out string strErrorMessage)
        {
            ...
            else
            {
                ...
                arrDatas = m_joinManager.JoinCompanyMemberJobLevelRegularTeam(dicConditions, null, null, null, out strErrorMessage);
            }

            return arrDatas;
        }

        private bool SetManager(Dictionary<int, CompanyMember> dicManagers, Dictionary<CompanyMember, JobLevel> dicManagerLevels, Dictionary<CompanyMember, RegularTeam> dicManagerTeams, int nManagerID, Dictionary<int, CompanyMember> allMembers, Dictionary<int, RegularTeam> allTeams, Dictionary<int, JobLevel> allLevels, ISelect selectManager, ref string strErrorMessage)
        {
        ...
        private bool SetManager(Dictionary<int, CompanyMember> dicManagers, Dictionary<CompanyMember, JobLevel> dicManagerLevels, Dictionary<CompanyMember, RegularTeam> dicManagerTeams, string strManagerIDs, Dictionary<int, CompanyMember> allMembers, Dictionary<int, RegularTeam> allTeams, Dictionary<int, JobLevel> allLevels, ISelect selectManager, ref string strErrorMessage)
        {
            int nManagerID;
            string[] managerIDs = strManagerIDs.Split(',');

            foreach (string strManagerID in managerIDs)
            {
                if (int.TryParse(strManagerID.Trim(), out nManagerID))
                {
        ...
        public RequestVacationResult RequestVacation(string strUserID, List<Date> requestDays, string strRequestDescription)
        {
            ...
            string strCondition = string.Format("{0} = {1}", JobLevel.Fields.ID, member.JobLevelID);
            JobLevel memberLevel = m_dataManager.GetSelect().SelectFirst<JobLevel>(strCondition, out strErrorMessage);
        ...
        private void CheckManagerRequests(List<Request> requests, int nManagerID)
        {
            ...
            for (int i=nRequestID-1;i>=0;i--)
            {
                ...
                string[] managerIDs = request.ManagerIDs.Split(',');
                ...
                foreach (string strManagerID in managerIDs)
                {
                    if (int.TryParse(strManagerID.Trim(), out managerID) && managerID == nManagerID)
                    {
                        find = true;
                        break;
                    }
        ...
        public ManagerRequestData RequestManagerData(string strManagerUserID, int year)
        {
            ...
            string strAdditionalConditions = string.Format("{0} like '%{1}%' and {2} >= {3}",
                ...
                Request.GetFieldName(Request.Fields.Year, out isNullable),
                year);
            IEnumerable<Request> _requests = selectManager.Select<Request>(strAdditionalConditions, out strErrorMessage);

            if (_requests == null)
                return GetManagerRequestData(null, null, null, null, strErrorMessage);

            List<Request> requests = new List<Request>();
            requests.AddRange(_requests);
        ...
        private List<WaitingRequestSpecialVacation> GetRequestSpecialVacations(CompanyMember manager, List<CompletedRequestSpecialVacation> completes, out string strErrorMessage)
        {
            ...
            IEnumerable<SpecialVacationRequest> _requests = m_dataManager.GetSelect().Select<SpecialVacationRequest>(strAdditionalConditions, out strErrorMessage);
            ...
            for (int i=nRequestCount-1;i>=0;i--)
            {
                ...
                string[] responseManagerIDs = request.ResponseManagerIDs.Split(',');

                int responseManagerID;
                bool find = false;

                foreach (string strResponseManagerID in responseManagerIDs)
                {
                    if (int.TryParse(strResponseManagerID.Trim(), out responseManagerID) && responseManagerID == manager.ID)
                    {
                        find = true;
                        break;
                    }
        ...
        private List<CompletedRequest> GetCompletedRequest(string strManagerUserID, List<Request> requests, ref List<Response> responses, Dictionary<int, Models.Account.ApplicationUser> dicUsers, out string strErrorMessage)
        {
            ...
            if (responses == null)
            {
                string strCondition = string.Format("{0} in ({1})", Response.Fields.RequestID, string.Join(", ", requestIDs.ToArray()));
                IEnumerable<Response> _responses = m_dataManager.GetSelect().Select<Response>(strCondition, out strErrorMessage);

                if (_responses == null)
                    return null;

                responses.Clear();
                responses.AddRange(_responses);
            }
            ...
            foreach (Response response in responses)
            {
                if (response.Result != Response.ResponseType.None && response.Result != Response.ResponseType.Timeout)
                {
                    ...
                    if (request.Response != null && (int)request.Response == (int)Response.ResponseType.None)
                        continue;
        ...
        private List<CompletedRequestSpecialVacation> GetCompletedRequestSpecialVacation(string strManagerUserID, List<SpecialVacationRequest> requests, ref List<SpecialVacationResponse> responses, Dictionary<int, Models.Account.ApplicationUser> dicUsers, out string strErrorMessage)
        {
            ...
            if (responses == null)
            {
                string strCondition = string.Format("{0} in ({1})", SpecialVacationResponse.Fields.RequestID, string.Join(",", requestIDs.ToArray()));
                IEnumerable<SpecialVacationResponse> _responses = m_dataManager.GetSelect().Select<SpecialVacationResponse>(strCondition, out strErrorMessage);
        ...
        private List<WaitingRequest> GetWaitingRequest(int nManagerID, List<Request> requests, ref List<Response> responses, Dictionary<int, Models.Account.ApplicationUser> dicUsers, out string strErrorMessage)
        {
            ...
            foreach (Response response in responses)
            {
                ...
                if (response.Result == Response.ResponseType.None)
                {
                    ...
                    if (request.Response != null && (int)request.Response != (int)Response.ResponseType.Processing)
                        continue;
        ...
        private bool SetWaitingRequestData(WaitingRequest data, Request request, List<Response> responses, Dictionary<int, Models.Account.ApplicationUser> dicUsers, out string strErrorMessage)
        {
            ...
            data.RequestID = request.ID;
            data.RequestTime = request.RequestTime;
            data.RequestDays.AddRange(Date.StringToDateList(request.Days, request.Year));
        ...
        private ProcessRequestResult ProcessSpecialVacationRequest(int nRequestID, bool permit, string strManagerUserID, string strManagerDescription)
        {
            ...
            string strErrorMessage;
            string strCondition = string.Format("{0} = {1}", SpecialVacationRequest.Fields.ID, nRequestID);
            SpecialVacationRequest request = selectManager.SelectFirst<SpecialVacationRequest>(strCondition, out strErrorMessage);
            ...
            strCondition = string.Format("{0} = {1} and {2} = {3}",
                SpecialVacationResponse.Fields.RequestID, nRequestID,
                SpecialVacationResponse.Fields.ManagerID, manager.ID);
            ...
            IEnumerable<SpecialVacationResponse> responses = selectManager.Select<SpecialVacationResponse>(strCondition, out strErrorMessage);

            if (responses == null)
                return GetProcessRequestResult(null, null, strErrorMessage);

            SpecialVacationResponse response = AccountManager.GetFirst(responses);
            ...
            if (m_dataManager.GetUpdate().Update<SpecialVacationResponse>(response, "", out strErrorMessage) == false)
        ...
        private ProcessRequestResult ProcessRequest(int nRequestID, bool permit, string strManagerUserID, string strManagerDescription)
        {
            ...
            string strErrorMessage;
            string strCondition = string.Format("{0} = {1}", Request.Fields.ID, nRequestID);
            Request request = selectManager.SelectFirst<Request>(strCondition, out strErrorMessage);
            ...
            strCondition = string.Format("{0} = {1} and {2} = {3}",
                Response.Fields.RequestID, nRequestID,
                Response.Fields.ManagerID, manager.ID);
            ...
            IEnumerable<Response> responses = selectManager.Select<Response>(strCondition, out strErrorMessage);

            if (responses == null)
                return GetProcessRequestResult(null, null, strErrorMessage);

            Response response = AccountManager.GetFirst(responses);
            ...
            if (m_dataManager.GetUpdate().Update<Response>(response, null, out strErrorMessage) == false)
                result = GetProcessRequestResult(null, null, strErrorMessage);
            else
            {
                Models.Account.ApplicationUser _manager = RequestManager.GetApplicationUser(m_dataManager, manager.ID, null, out strErrorMessage);
        ...
        {
            IEnumerable<CompanyMember> members = m_dataManager.GetSelect().Select<CompanyMember>(null, out strErrorMessage);

            if (members == null)
                return null;

            IEnumerable<RegularTeam> teams = m_dataManager.GetSelect().Select<RegularTeam>(null, out strErrorMessage);
        ...
        {
            string strCondition = string.Format("{0} = {1}", RegularTeam.Fields.ParentID, nTeamID);
            ...
            IEnumerable<RegularTeam> teams = m_dataManager.GetSelect().Select<RegularTeam>(strCondition, out strErrorMessage);
        ...
        private bool GetAllMemberVacationHistories(int lastYear, Dictionary<int, Model.History> dicLastYearHistories, Dictionary<int, Model.History> dicThisYearHistories, Dictionary<int, Model.History> dicNextYearHistories, out string strErrorMessage)
        {
            ...
            IEnumerable<Model.History> histories = m_dataManager.GetSelect().Select<Model.History>(strCondition, out strErrorMessage);
        ...
        private bool GetAllMemberSpecialVacations(int lastYear, Dictionary<int, List<SpecialVacation>> dicLastYearSpecialVacations, Dictionary<int, List<SpecialVacation>> dicThisYearSpecialVacations, Dictionary<int, List<SpecialVacation>> dicNextYearSpecialVacations, out string strErrorMessage)
        {
            ...
            IEnumerable<SpecialVacation> specialVacations = m_dataManager.GetSelect().Select<SpecialVacation>(strCondition, out strErrorMessage);
        ...
        private bool GetAllSpecialVacationRequestNResponse(int year, Dictionary<int, SpecialVacationRequest> dicSpecialVacationRequests, List<SpecialVacationResponse> specialVacationResponses, out string strErrorMessage)
        {
            ...
            IEnumerable<SpecialVacationRequest> requests = m_dataManager.GetSelect().Select<SpecialVacationRequest>(strCondition, out strErrorMessage);
            ...
            IEnumerable<SpecialVacationResponse> responses = m_dataManager.GetSelect().Select<SpecialVacationResponse>(strCondition, out strErrorMessage);
        ...
        private List<Models.Account.ApplicationUser> GetTeamMembers(RegularTeam team, Dictionary<int, List<Models.Vacation.History>> dicMemberHistory, Dictionary<int, CompanyMember> dicCompanyMembers, Dictionary<int, List<CompanyMember>> dicTeamCompanyMembers, Dictionary<int, RegularTeam> dicTeams, Dictionary<int, JobLevel> dicJobLevels, DateTime timestamp, out string strErrorMessage)
        {
            ...
            IEnumerable<Response> responses = m_dataManager.GetSelect().Select<Response>(strCondition, out strErrorMessage);

            if (responses == null)
                return null;

            Dictionary<int, IEnumerable<SpecialVacation>> _dicLastYearSpecialVacations = new Dictionary<int, IEnumerable<SpecialVacation>>();
            Dictionary<int, IEnumerable<SpecialVacation>> _dicThisYearSpecialVacations = new Dictionary<int, IEnumerable<SpecialVacation>>();
            Dictionary<int, IEnumerable<SpecialVacation>> _dicNextYearSpecialVacations = new Dictionary<int, IEnumerable<SpecialVacation>>();

            Dictionary<int, IEnumerable<Request>> _dicLastYearMemberRequests = new Dictionary<int, IEnumerable<Request>>();
            Dictionary<int, IEnumerable<Request>> _dicThisYearMemberRequests = new Dictionary<int, IEnumerable<Request>>();
            Dictionary<int, IEnumerable<Request>> _dicNextYearMemberRequests = new Dictionary<int, IEnumerable<Request>>();

            foreach (var pair in dicLastYearSpecialVacations)
            {
                _dicLastYearSpecialVacations[pair.Key] = pair.Value;
            }

            foreach (var pair in dicThisYearSpecialVacations)
            {
                _dicThisYearSpecialVacations[pair.Key] = pair.Value;
            }

            foreach (var pair in dicNextYearSpecialVacations)
            {
                _dicNextYearSpecialVacations[pair.Key] = pair.Value;
            }

            foreach (var pair in dicLastYearMemberRequests)
            {
                _dicLastYearMemberRequests[pair.Key] = pair.Value;
            }

            foreach (var pair in dicThisYearMemberRequests)
            {
                _dicThisYearMemberRequests[pair.Key] = pair.Value;
            }

            foreach (var pair in dicNextYearMemberRequests)
            {
                _dicNextYearMemberRequests[pair.Key] = pair.Value;
            }
            ...
            foreach (CompanyMember member in teamMembers)
            {
                if (dicJobLevels.TryGetValue(member.JobLevelID, out level))
                {
                    ...
                    Models.Vacation.History historyLastYear = GetVacationHistory(member.UserID, member, dicLastYearHistories, _dicLastYearSpecialVacations, dicSpecialVacationRequests, specialVacationResponses, null, _dicLastYearMemberRequests, responses, dicCompanyMembers, dicTeams, dicJobLevels, timestamp.Year - 1, 12, 31, false);
                    Models.Vacation.History historyThisYear = GetVacationHistory(member.UserID, member, dicThisYearHistories, _dicThisYearSpecialVacations, dicSpecialVacationRequests, specialVacationResponses, _dicLastYearMemberRequests, _dicThisYearMemberRequests, responses, dicCompanyMembers, dicTeams, dicJobLevels, timestamp.Year, timestamp.Month, timestamp.Day);
                    Models.Vacation.History historyNextYear = GetVacationHistory(member.UserID, member, dicNextYearHistories, _dicNextYearSpecialVacations, dicSpecialVacationRequests, specialVacationResponses, _dicThisYearMemberRequests, _dicNextYearMemberRequests, responses, dicCompanyMembers, dicTeams, dicJobLevels, timestamp.Year + 1, 1, 1);
        ...
        private List<Models.Account.ApplicationUser> GetTeamMembers(RegularTeam team, Dictionary<int, List<Models.Vacation.History>> dicMemberHistory, DateTime timestamp, out string strErrorMessage)
        {
            ...
            ArrayList arrResult = m_joinManager.JoinCompanyMemberJobLevelRegularTeam(dicConditions, null, null, null, out strErrorMessage);
        ...
        private MemberVacationHistory GetMemberVacationHistory(MemberTeam rootTeam, Dictionary<int, List<Models.Vacation.History>> dicMemberHistories, string strMessage, DateTime dtNow)
        {
            ...
            else
            {
                ...
                int minYear = selectManager.GetMinimumHistoryYear(memberIDs, out strErrorMessage);
        ...
        public ResponseVacationList GetVacationList(string strUserID, int year)
        {
            ...
            string strErrorMessage;
            ArrayList arrDatas = m_joinManager.JoinCompanyMemberRequest(strAdditionalCondition, out strErrorMessage);
            ...
            {
                request = (Request)arrDatas[i + 1];

                if (request.Response != null &&
                    ((int)request.Response == (int)Response.ResponseType.Cancel ||
                    (int)request.Response == (int)Response.ResponseType.Deny ||
                    (int)request.Response == (int)Response.ResponseType.None ||
                    (int)request.Response == (int)Response.ResponseType.Timeout))
                    continue;
        ...
        private VacationInfo GetVacationInfo(Request request, List<Response> responses, Dictionary<int, ApplicationUser> dicManagers)
        {
            ...
            float fTotalDays;
            string strPeriod;
            Dictionary<int, float> dicDays = RequestManager.GetYearDays(dates, out fTotalDays, out strPeriod);

            vacation.Days.AddRange(dates);
            vacation.DaysDescription = GetDaysDescription(dates.Count, fTotalDays, strPeriod);
