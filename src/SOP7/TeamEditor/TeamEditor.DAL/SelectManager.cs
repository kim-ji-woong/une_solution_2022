...
namespace TeamEditor.DAL
{
    public class SelectManager : QueryManager, ISelect
    {
        ...
        {
            m_dataMgr = dataMgr;
            m_dbManager = dataMgr.GetDBManager() as DirectDBManager;
        ...
        public List<Regular> SelectRegulars(out string strErrorMessage)
        {
            ...
            for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
            {
                Regular model = ReadRegular(arrResults, i, out strErrorMessage);
        ...
        private Regular ReadRegular(ArrayList arrResult, int index, out string strErrorMessage)
        {
            ...
            foreach (Regular.Fields field in Regular.Fields.GetValues(typeof(Regular.Fields)))
            {
                ...
                else if (field == Regular.Fields.SiteID)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
        ...
        */
        ...
        {
            return JoinRegularRegularMember(strAdditionalConditions, null, out strErrorMessage);
        }

        public ArrayList JoinRegularRegularMember(string strAdditionalConditions, int? topNCount, out string strErrorMessage)
        {
            strErrorMessage = null;

            string strRegularTableName = Regular.GetTableName();
            string strRegularMemberTableName = RegularMember.GetTableName();

            int nRegularFieldCount, nRegularMemberFieldCount;

            string strRegularFields = GetFieldNames<Regular.Fields>(strRegularTableName, out nRegularFieldCount);
            string strRegularMemberFields = GetFieldNames<RegularMember.Fields>(strRegularMemberTableName, out nRegularMemberFieldCount);

            int nFieldsCount = nRegularFieldCount + nRegularMemberFieldCount;

            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("Select {0}, {1} ", strRegularFields, strRegularMemberFields);
            sb.AppendFormat("  From {0}, {1} ", strRegularTableName, strRegularMemberTableName);
            sb.AppendFormat(" Where {0}.{1} = {2}.{3} ", strRegularTableName, Regular.Fields.ID, strRegularMemberTableName, RegularMember.Fields.RegularID);
            ...
            {
                Regular regular = ReadRegular(arrResult, i, out strErrorMessage);
                ...
                RegularMember regularMember = ReadRegularMember(arrResult, i + nRegularFieldCount, out strErrorMessage);
