...
namespace TeamEditor.BLL.Excel.Reader
{
    using Rollback;
    ...
    public class RegularMemberReader : ExcelReader
    {
        ...
        public RegularMemberReader(string strFilePath, SDMS.IDAL.IDataManager sdmsDataManager, IDataManager teamDataManager, SOPManager.IDAL.IDataManager sopDataManager)
            : base(strFilePath, teamDataManager)
        {
        ...
        protected override bool UpdateData(List<SheetData> sheetDatas, out string strErrorMessage)
        {
            ...
            Dictionary<Regular, List<RegularMember>> dicTeamMembers = ReadDB(m_dataManager, out dicIDMembers, out dicPhoneNumberMembers, out dicEmailMembers, out dicJobLevels, out strErrorMessage);
            ...
            bool result = CheckData(dicTeamMembers, dicIDMembers, dicPhoneNumberMembers, dicEmailMembers, dicJobLevels, sheetDatas, out strErrorMessage);
        ...
        private bool CheckData(Dictionary<Regular, List<RegularMember>> dicTeamMembers, Dictionary<string, RegularMember> dicIDMembers, Dictionary<string, RegularMember> dicPhoneNumberMembers, Dictionary<string, RegularMember> dicEmailMembers, Dictionary<string, int> dicJobLevels, List<SheetData> sheetDatas, out string strErrorMessage)
        {
            strErrorMessage = null;
            ...
            {
                MakeSheetRegularTeamMembers(sheet, dicSheetRegularTeamMembers, dicTeamPath, dicIDMembers, dicPhoneNumberMembers, dicEmailMembers, dicJobLevels, dicColumnIndex);
                // 첫번째 Sheet만 사용한다.
                break;
            }
        ...
        private bool DeleteTempRegularTeam()
        {
            ...
            return m_dataManager.GetDeleteManager().DeleteRegular(m_nTempTeamID, out strErrorMessage);
        }
        ...
        private bool SetTempRegularTeam(string strMemberIDs, RollbackManager rollback)
        {
            ...
            Regular tempTeam = m_dataManager.GetSelectManager().SelectRegular(m_nTempTeamID, out strErrorMessage);
            ...
            if (tempTeam == null)
            {
                ...
                if (m_dataManager.GetCreateManager().AddRegular(tempTeam) == false)
                    return false;
                else
                {
            ...
            List<RegularMember> members = m_dataManager.GetSelectManager().SelectRegularMembers(strCondition, out strErrorMessage);
            ...
            foreach (RegularMember member in members)
            {
                ...
                if (m_dataManager.GetUpdateManager().UpdateRegularMember(updateMember, out strErrorMessage) == false)
                    return false;
                else
                ...
                foreach (RegularTeam team in teams)
                {
                    if (team.ID <= 0)
                    {
                        if (team.ParentTeam == null || (team.ParentTeam != null && team.ParentTeam.ID > 0))
                        {
                            ...
                            if (m_dataManager.GetCreateManager().AddRegular(team) == false)
                                return false;
                            else
        ...
        private bool DeleteRegularTeams(string strNotDeletingTeamIDs, RollbackManager rollback)
        {
            ...
            if (strNotDeletingTeamIDs.Length > 0)
            {
                ...
                teams = m_dataManager.GetSelectManager().SelectRegulars(null, strCondition, out strErrorMessage);
            }
            else
            {
                teams = m_dataManager.GetSelectManager().SelectRegulars(out strErrorMessage);
            }
            ...
            foreach (Regular team in teams)
            {
                if (m_dataManager.GetDeleteManager().DeleteRegular(team.ID, out strErrorMessage) == false)
                    return false;
                else
                    rollbackTeams.Add(team);
            }
        ...
        private bool DeleteRegularMembers(string strNotDeletingMemberIDs, RollbackManager rollback)
        {
            ...
            if (strNotDeletingMemberIDs.Length > 0)
            {
                ...
                members = m_dataManager.GetSelectManager().SelectRegularMembers(strCondition, out strErrorMessage);
            }
            else
            {
                members = m_dataManager.GetSelectManager().SelectRegularMembers(out strErrorMessage);
            }
        ...
        private void MakeSheetRegularTeamMembers(SheetData sheet, Dictionary<RegularTeam, List<RegularMember>> dicSheetRegularTeamMembers, Dictionary<string, RegularTeam> dicTeamPath, Dictionary<string, RegularMember> dicIDMembers, Dictionary<string, RegularMember> dicPhoneNumberMembers, Dictionary<string, RegularMember> dicEmailMembers, Dictionary<string, int> dicJobLevels, Dictionary<string, int> dicColumnIndex)
        {
            ...
            for (int i = 0; i < nValueCount; i++)
            {
                ...
                if (strMemberID != null)
                {
                    ...
                    if (CheckValidMemberID(strMemberID) == false)
                        continue;
                }
        ...
        private void SetJobLevelID(RegularMember member, string strJobLevel, Dictionary<string, int> dicJobLevels)
        {
            if (strJobLevel != null && strJobLevel.Length > 0)
            {
                ...
                {
                    string strErrorMessage;
                    int nID = m_dataManager.GetSelectManager().GetMaxID(Options.TableName, out strErrorMessage) - 1;

                    if (strErrorMessage != null && strErrorMessage.Length > 0)
                        return;

                    int nID2 = m_dataManager.GetSelectManager().GetMaxID(Options.TableName, out strErrorMessage, "PropertyName = '" + JobLevelProperty + "'") - 1;
                    ...
                    if (nID2 > 0)
                    {
                        List<Options> options = m_dataManager.GetSelectManager().SelectOptions("ID = " + nID2.ToString(), out strErrorMessage);
                    ...
                    if (m_dataManager.GetCreateManager().AddOptions(option) == false)
                        return;
        ...
        private Dictionary<Regular, List<RegularMember>> ReadDB(IDataManager dataManager, out Dictionary<string, RegularMember> dicIDMembers, out Dictionary<string, RegularMember> dicPhoneNumberMembers, out Dictionary<string, RegularMember> dicEmailMembers, out Dictionary<string, int> dicJobLevels, out string strErrorMessage)
        {
            strErrorMessage = null;
