...
namespace TeamEditor.BLL
{
    public class SaveManager
    {
        ...
        public ResponseUpdateRegularMember UpdateRegularMember(RequestUpdateRegularMember req)
        {
            ...
            // 핸드폰 번호 암호화 처리
            if (req.Member.PhoneNumber != null && req.Member.PhoneNumber.Length > 0)
            {
                req.Member.PhoneNumber = LoadManager.EncryptString(req.Member.PhoneNumber);
            }
            ...
            if (req.Member.ID > 0)
            {
                ...
                dicConditions[User.Fields.MemberID] = req.Member.ID;
                ...
                else if (users.Count > 0)
                {
                    ...
                    // 사번이 제거가 된다면 계정을 삭제
                    if (req.Member.MemberID == null || req.Member.MemberID == "")
                    {
                        if (RemoveAccountUser(req.Member.ID, rollback) == false)
                        {
                            rollback.Rollback(m_sdmsDataManager, m_dataManager, m_sopDataManager);
                            res.Message = "UpdateRegularMember 에러 (RemoveAccountUser 실패)";
                            res.Success = false;
                            return res;
                        }
                        else
                    ...
                    else
                    {
                        ...
                        if (req.Member.MemberID != user.UserID)
                        {
                            bIsUpdate = true;
                            user.UserID = req.Member.MemberID;
                        }

                        // 이름이 수정된다면 NickName 수정
                        if (req.Member.MemberName != user.NickName)
                        {
                            bIsUpdate = true;
                            user.NickName = req.Member.MemberName;
                        ...
                        {   // 업데이트
                            ...
                            {
                                rollback.Rollback(m_sdmsDataManager, m_dataManager, m_sopDataManager);
                                res.Message = "UpdateRegularMember 에러(UpdateUser 실패)";
                                res.Success = false;
                                return res;
                            }
                        }
        ...
        public ResponseUpdateRegularMember UpdateNipaRegularMember(RequestUpdateRegularMember req)
        {
            string strErrorMessage = null;
            ResponseUpdateRegularMember res = new ResponseUpdateRegularMember();
        ...
        public bool UpdateRegularMemberData(Dictionary<string, List<RegularMember>> dicPathRegularMember, out string strErrorMessage, int? nSiteID = null, Dictionary<int, Regular> dicIDRegulars = null, Dictionary<int, RegularMember> dicIDRegularMembers = null)
        {
            ...
            if (AddRegular(nSiteID, dicAddRegulars.Values, out dicChangeRegularID, rollback, out strErrorMessage) == false)
            {
                rollback.Rollback(m_sdmsDataManager, m_dataManager, m_sopDataManager);
                return false;
            }
        ...
        private bool RemoveAccountUser(int nUserID, RollbackManager rollback)
        {
            ...
            string strCondition = string.Format("{0} in ({1})", User.GetFieldName(User.Fields.MemberID, out isNullable), nUserID);
            ...
            {
                // 연동 계정 옵션 제거
                strCondition = string.Format("{0} in ({1})", Option.GetFieldName(Option.Fields.UserID, out isNullable), user.ID);
                ...
                string strSQL = $"update {ActionStepHistory.TableName} set {ActionStepHistory.Fields.LastAccessedUserID}=null where {ActionStepHistory.Fields.LastAccessedUserID}={user.ID}";
                if (m_sopDataManager.GetSelectManager().GetResultData(strSQL, out strErrorMessage) == null)
                    return false;

                strSQL = $"update {ComponentHistory.TableName} set {ComponentHistory.Fields.AccessedUserID}=null where {ComponentHistory.Fields.AccessedUserID}={user.ID}";
                if (m_sopDataManager.GetSelectManager().GetResultData(strSQL, out strErrorMessage) == null)
                    return false;

                // 연동 계정 세션 제거
                strCondition = string.Format("{0} in ({1})", Session.GetFieldName(Session.Fields.AccountUserID, out isNullable), user.ID);
        ...
        private bool AddRegular(int? nSiteID, ICollection<Regular> regulars, out Dictionary<int, int> dicChangeRegularID, RollbackManager rollback, out string strErrorMessage)
        {
            strErrorMessage = "";
            ...
            foreach (Regular regular in regulars)
            {
                ...
                {   // 부모 regular ID를 아직 모른다면 
                    ...
                    {   // 부모 regular가 아직 추가되지 않았다면 
                        regular.ParentTeamID = AddParentRegular(nSiteID, (int)regular.ParentTeamID, regulars, dicChangeRegularID, rollbackTeams, out strErrorMessage);
        ...
        private int AddParentRegular(int? nSiteID, int nParentTeamID, ICollection<Regular> regulars, Dictionary<int, int> dicChangeRegularID, List<Regular> rollbackTeams, out string strErrorMessage)
        {
            strErrorMessage = "";
        ...
        public MessageResult MoveMembers(RequestMoveMembers data)
        {
            if (data.TargetRegularTeamID != null)
                return MoveRegularMembers((int)data.TargetRegularTeamID, data.RegularMemberIDs);
            else if (data.TargetTemporaryTeamID != null)
                return MoveTemporaryMembers((int)data.TargetTemporaryTeamID, data.TemporaryMemberIDs);

            return MakeMessageResult(false, "옮겨갈 대상 팀이 지정되지 않았습니다.");
        }

        private MessageResult MoveTemporaryMembers(int targetTemporaryTeamID, List<int> temporaryMemberIDs)
        {
            if (temporaryMemberIDs.Count == 0)
                return MakeMessageResult(false, "옮길 비상조직원들이 선택되지 않았습니다.");

            string strCondition = string.Format("{0} in ({1})", TemporaryMember.Fields.ID, string.Join(",", temporaryMemberIDs));

            Dictionary<TemporaryMember.Fields, object> dicSets = new Dictionary<TemporaryMember.Fields, object>();
            dicSets[TemporaryMember.Fields.TeamID] = targetTemporaryTeamID;

            string strErrorMessage;
            if (m_dataManager.GetUpdateManager().UpdateTemporaryMember(dicSets, null, strCondition, out strErrorMessage))
                return MakeMessageResult(true, "");

            return MakeMessageResult(false, strErrorMessage);
        }

        private MessageResult MoveRegularMembers(int targetRegularTeamID, List<int> regularMemberIDs)
        {
            if (regularMemberIDs.Count == 0)
                return MakeMessageResult(false, "옮길 직원들이 선택되지 않았습니다.");

            string strCondition = string.Format("{0} in ({1})", RegularMember.Fields.ID, string.Join(",", regularMemberIDs));

            Dictionary<RegularMember.Fields, object> dicSets = new Dictionary<RegularMember.Fields, object>();
            dicSets[RegularMember.Fields.RegularID] = targetRegularTeamID;

            string strErrorMessage;
            if (m_dataManager.GetUpdateManager().UpdateRegularMember(dicSets, null, strCondition, out strErrorMessage))
                return MakeMessageResult(true, "");

            return MakeMessageResult(false, strErrorMessage);
        }

        public MessageResult MoveTeams(RequestMoveTeams data)
        {
            if (data.TargetRegularParentTeamID != null)
                return MoveRegularTeams((int)data.TargetRegularParentTeamID, data.RegularTeamIDs);
            else if (data.TargetTemporaryParentTeamID != null)
                return MoveTemporaryTeams((int)data.TargetTemporaryParentTeamID, data.TemporaryTeamIDs);

            return MakeMessageResult(false, "옮겨갈 대상 팀이 지정되지 않았습니다.");
        }

        private MessageResult MoveTemporaryTeams(int targetParentTeamID, List<int> temporaryTeamIDs)
        {
            if (temporaryTeamIDs.Count == 0)
                return MakeMessageResult(false, "옮길 팀들이 선택되지 않았습니다.");

            string strCondition = string.Format("{0} in ({1})", Temporary.Fields.ID, string.Join(",", temporaryTeamIDs));

            Dictionary<Temporary.Fields, object> dicSets = new Dictionary<Temporary.Fields, object>();
            dicSets[Temporary.Fields.ParentTeamID] = targetParentTeamID;

            string strErrorMessage;
            if (m_dataManager.GetUpdateManager().UpdateTemporary(dicSets, null, strCondition, out strErrorMessage))
                return MakeMessageResult(true, "");

            return MakeMessageResult(false, strErrorMessage);
        }

        private MessageResult MoveRegularTeams(int targetParentTeamID, List<int> regularTeamIDs)
        {
            if (regularTeamIDs.Count == 0)
                return MakeMessageResult(false, "옮길 팀들이 선택되지 않았습니다.");

            string strCondition = string.Format("{0} in ({1})", Regular.Fields.ID, string.Join(",", regularTeamIDs));

            Dictionary<Regular.Fields, object> dicSets = new Dictionary<Regular.Fields, object>();
            dicSets[Regular.Fields.ParentTeamID] = targetParentTeamID;

            string strErrorMessage;
            if (m_dataManager.GetUpdateManager().UpdateRegular(dicSets, null, strCondition, out strErrorMessage))
                return MakeMessageResult(true, "");

            return MakeMessageResult(false, strErrorMessage);
        }

        private MessageResult MakeMessageResult(bool success, string message)
        {
            MessageResult result = new MessageResult();
