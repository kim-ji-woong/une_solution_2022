...
namespace TeamEditor.BLL
{
    public class LoadManager
    {
        ...
        public List<RegularMember> LoadRegularMembers(int? nSiteID)
        {
            string strErrorMessage;
            ...
            if (nSiteID.HasValue && nSiteID > 0)
            {
                //dicConditions[Regular.Fields.SiteID] = nSiteID.Value;
                strAdditionalConditions = string.Format("{0} = {1} or {0} is NULL", Regular.Fields.SiteID, nSiteID.Value);
            }
            ...
            if (regulars?.Count > 0)
            {
                string strRegularIDs = string.Join(",", regulars.Select(p => p.ID));

                regularMembers = m_dataManager.GetSelectManager().SelectRegularMembers($"{RegularMember.Fields.RegularID} in ({strRegularIDs})", out strErrorMessage);
                if (regularMembers == null)
                    return null; 
            }
            ...
            foreach (RegularMember regularMember in regularMembers)
            {
                if (regularMember.PhoneNumber != null)
                    regularMember.PhoneNumber = DecryptString(regularMember.PhoneNumber);
            }
        ...
        public List<Regular> LoadRegulars(int? nSiteID)
        {
            string strErrorMessage;
            string strAdditionalConditions = "";
        ...
        public List<Temporary> LoadTemporaries(bool bIsNormal, int? nSiteID)
        {
            string strErrorMessage;
            ...
            if (nSiteID.HasValue && nSiteID > 0)
                dicConditions[Temporary.Fields.SiteID] = nSiteID.Value;

            List<Temporary> temporaries = m_dataManager.GetSelectManager().SelectTemporaries(dicConditions, out strErrorMessage);
            if (temporaries == null)
                return null;
