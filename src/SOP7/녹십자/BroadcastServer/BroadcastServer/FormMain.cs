...
namespace BroadcastServer
{
    ...
    public partial class FormMain : Form, IServiceOwner
    {
        ...
        private const int OutPD2_EquipmentID = 1934;
        private const int OutUT_EquipmentID = 1935;
        private const int PD2_ZoneID = 11;
        private const int UT_ZoneID = 34;
        ...
        private void CheckAlarms()
        {
            ...
            foreach (CurrentAlarm alarm in alarms)
            {
                ...
                if (UseAlarmBroadcast())
                {
                    ...
                    Building building = GetAlarmBuilding(alarm, out strErrorMessage);
        ...
        private Building GetAlarmBuilding(CurrentAlarm alarm, out string strErrorMessage)
        {
            strErrorMessage = "";

            string strAdditionalConditions = string.Format("{0}.{1} = {2}", SDMS.Model.History.SensorZoneHistory.TableName, SDMS.Model.History.SensorZoneHistory.Fields.ID, alarm.SensorZoneHistoryID);

            ArrayList arrayList = m_dataManager.GetSelectManager().JoinEquipmentZoneSensorReactionHistorySensorZoneSensorZoneHistory2(null, null, null, null, strAdditionalConditions, out strErrorMessage);
            ...
            {
                strErrorMessage = "해당 알람에 대한 정보가 조회되지 않습니다.";
                return null;
            }

            Building building = null;

            if (arrayList[0] is EquipmentZone &&
                arrayList[1] is SDMS.Model.History.SensorReactionHistory &&
                arrayList[2] is SDMS.Model.Sensor.SensorZone &&
                arrayList[3] is SDMS.Model.History.SensorZoneHistory &&
                arrayList[4] is Zone)
            {
                EquipmentZone eq = arrayList[0] as EquipmentZone;
                SDMS.Model.History.SensorReactionHistory srh = arrayList[1] as SDMS.Model.History.SensorReactionHistory;
                SDMS.Model.Sensor.SensorZone sz = arrayList[2] as SDMS.Model.Sensor.SensorZone;
                SDMS.Model.History.SensorZoneHistory szh = arrayList[3] as SDMS.Model.History.SensorZoneHistory;
                Zone z = arrayList[4] as Zone;

                int nZoneID = -1;

                if (eq.ID == OutPD2_EquipmentID)
                    nZoneID = PD2_ZoneID;
                else if (eq.ID == OutUT_EquipmentID)
                    nZoneID = UT_ZoneID;
                else
                    nZoneID = z.ID;
                ...
                if (nZoneID != -1)
                {
                    strAdditionalConditions = string.Format("{0}.{1} = {2}", Zone.TableName, Zone.Fields.ID, nZoneID.ToString());
                    ArrayList arrayList2 = m_dataManager.GetSelectManager().JoinBuildingGroupBuildingZone(null, null, null, strAdditionalConditions, out strErrorMessage);

                    if (arrayList2 == null)
                    {
                        return null;
                    }
                    else if (arrayList2.Count == 0 || arrayList2.Count < 3)
                    {
                        strErrorMessage = "해당 알람에 대한 건물 정보가 조회되지 않습니다.";
                        return null;
                    }

                    if (arrayList2[0] is BuildingGroup &&
                        arrayList2[1] is Building &&
                        arrayList2[2] is Zone)
                    {
                        BuildingGroup bg = arrayList2[0] as BuildingGroup;
                        Building b = arrayList2[1] as Building;
                        Zone zone = arrayList2[2] as Zone;
