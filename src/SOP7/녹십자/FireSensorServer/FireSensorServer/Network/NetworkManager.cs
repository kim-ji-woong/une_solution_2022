...
namespace FireSensorServer.Network
{
    public class NetworkManager
    {
        ...
        private void ReadSettings()
        {
            ...
            string strDbHost = ReadString("dbHost");
            if (strDbHost == null)
                return;

            string strDbId = ReadString("dbId");
            if (strDbId == null)
                return;

            string strDbPw = ReadString("dbPw");
            if (strDbPw == null)
                return;
            ...
            m_sensorManager = new Data.SensorManager((int)dbType, strDbHost, strDBName, strDbId, strDbPw, (int)siteID);
        ...
        private void RunSiemensServer(int nPort)
        {
            if (nPort > 0)
            {
                ...
                m_logger.Write("RunSiemensServer Success");
            }
            else
                m_logger.Write("RunSiemensServer Fail : " + nPort);
        }
        ...
        private void ConnectionThread()
        {
            ...
            while (m_runThread)
            {
                try
                {
                    ...
                    {
                        m_logger.Write("[Siemens Server Connect Fail] " + m_strSiemensServerIP + "/" + m_nSiemensServerPort);
                    }
        ...
        public void SendSensorData(int tagNo, EventType eventType, ClientType clientType, DateTime dtEvent, string strPosition, string strMessage, string strTagNo)
        {
            try
            {
                ...
                if (tag != null)
                {
                    ...
                    if (eventType == EventType.DetectSensor)
                ...
                {
                    m_logger.Write("[" + GetClientTypeString(clientType) + "] Unknown TagNo : " + strTagNo + " / int tagNo : " + tagNo);
                    System.Diagnostics.Trace.WriteLine("[" + GetClientTypeString(clientType) + "] Unknown TagNo : " + strTagNo);
                }
            }
            catch (Exception ex)
            {
                m_logger.Write("[ERROR] SendSensorData(int, EventType, ClientType, DateTime, string, string, string) : " + ex.Message);
            }
        }
