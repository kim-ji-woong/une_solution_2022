...
namespace ERPReadServer
{
    ...
    public class SAPConnectorInterface
    {
        ...
        public bool TestConnection(string destinationName)
        {
            ...
            try
            {
                Logger.Instance.Write("TestConnection-1");
                rfcDestination = RfcDestinationManager.GetDestination(destinationName);
                if (rfcDestination != null)
                {
                    Logger.Instance.Write("TestConnection-2");
                    rfcDestination.Ping();
                    result = true;
                }
            }
        ...
        {
            Logger.Instance.Write("RetrieveCustomers-1");
            ...
            try
            {
                ...
                Logger.Instance.Write("RetrieveCustomers-2");
                RfcRepository rfcRepository = rfcDestination.Repository;

                IRfcFunction rfcFunction = rfcRepository.CreateFunction(strFunctionName);
                Logger.Instance.Write("RetrieveCustomers-3");
                rfcFunction.SetValue(strParamName, strParamValue);
                Logger.Instance.Write("RetrieveCustomers-4");
                rfcFunction.Invoke(rfcDestination);
                Logger.Instance.Write("RetrieveCustomers-5");
                ...
                Logger.Instance.Write("RetrieveCustomers-6");
            }
            catch (Exception ex)
            {
