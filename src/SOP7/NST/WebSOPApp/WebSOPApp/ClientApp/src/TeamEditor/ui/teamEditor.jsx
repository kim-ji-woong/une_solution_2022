...
class TeamEditor extends Component {
	...
	async init() {
		...
		if (userInfo === null || userInfo === undefined) {
			userInfo = ProjectResource.SiteID ? JSON.parse(window.localStorage.getItem(SessionString.Key.account + "_" + ProjectResource.SiteID.toString())) : null;

			if (userInfo) {
				ProjectResource.SiteID = userInfo.siteID;
			}
			else {
				return;
			}
	...
	async initTeamData(selectedSiteID) {
		...
		let regularDatas = await TeamEditController.DisplayRegular(selectedSiteID);
		let temporaryDatas = await TeamEditController.DisplayTemporary(true, selectedSiteID);
		let temporaryEmergencyDatas = await TeamEditController.DisplayTemporary(false, selectedSiteID);
	...
	editTeam = async (team, chgName) => {
		...
		const nodeData = { ID: team.ID, TeamName: chgName, ParentTeamID: team.ParentTeamID, SiteID: this.state.selectedSiteID };
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
