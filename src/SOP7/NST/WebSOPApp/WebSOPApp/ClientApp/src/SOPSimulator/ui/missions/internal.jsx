...
class Internal extends Component {
    ...
    showConfirmDialog(isSMS, isEmail, isBroadcast, isSiren) {
        ...
        if (!isBroadcast && this.props.currentSection.receivers === null || this.props.currentSection.receivers.length === 0) {
            ...
            if (isSMS) {
                ...
                if (this.props.commonSettings.UseBroadcast === 'false') {
                ...
                this.checkNRunBroadcast(isSMS, isEmail, isSiren, strMessage);
                //this.confirmDialogData = [isSMS, isEmail, isBroadcast, isSiren];
                return;
            }
    ...
    async checkNRunBroadcast(isSMS, isEmail, isSiren, strMessage) {
        const result = await SopSimulatorController.checkRunBroadcast();

        if (!result?.success) {
            if (result.message) {
                this.props.showConfirmDialog("에러", result.message, ["확인"]);
            }
            return;
        }
        else {
            if (result.runBroadcast) {
                this.props.showConfirmDialog("알림", "현재 방송이 진행중입니다.", ["확인"]);
            }
            else {
                this.confirmDialogData = [isSMS, isEmail, true, isSiren];
                this.props.showConfirmDialog('알림', strMessage, ['상황 전파', '취소'], this.onProgressSpread);
            }
        }
    }
    ...
    render() {
        ...
        if (this.props.sectionData.isBroadcast)
            tagUI.push(<span key='broadcast' className={uis.flag + " " + uis.flagBroadcast}>방송</span>);
        if (this.props.sectionData.isSMS)
            tagUI.push(<span key='sms' className={uis.flag + " " + uis.flagSms}>문자</span>);
        ...
        return (
            <div className={uis.sectionBox + boxClassName + " " + uneStyles.sectionBox} id={this.props.id}>
                ...
                <div className={uis.sendMessage + " " + uneStyles.sendMessage}>
                    ...
                    {
                        (this.props.sectionData.isBroadcast) ?
                            <div><button type="button" onClick={() => this.showConfirmDialog(false, false, true, false)}><i className={uneStyles.mic}></i></button></div>
                            : <></>
                    }
                    {/*
                        (this.props.sectionData.isBroadcast) ?
