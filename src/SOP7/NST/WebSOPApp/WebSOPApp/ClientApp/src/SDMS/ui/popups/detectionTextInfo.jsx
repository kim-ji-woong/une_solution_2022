...
class DetectionTextInfo extends Component {
    ...
    render() {
        ...
        const strMonth = month < 9 ? "0" + month : month.toString();
        const day = date.getDate();
        const strDay = day < 9 ? "0" + day : day.toString();
        const today = date.getFullYear() + "-" + strMonth + "-" + strDay;

        const alarms = this.props.sensorAlarms;
        for (let i = 0; i < alarms.length; i++) {
            const alarm = alarms[i];
            const alarmDate = alarm.dtTime.substring(0, 10);
