...
export class POIManager {
    ...
    getSensorPOI(sensorType, zoneID, sensorID) {
        ...
        let sprite = this.sensorPOIs[key];

        if (!sprite) {
            if (sensorType === "etc")
                sprite = this.getEtcSensorPOI(zoneID, sensorID);
            else if (sensorType === "psm")
                sprite = this.getPsmSensorPOI(zoneID, sensorID);
        }

        return sprite;
    }

    getEtcSensorPOI(zoneID, sensorID) {
        const sensorTypes = [];
        sensorTypes.push(SDMSMainMenu.Detect_Sensor);
    ...
    getPsmSensorPOI(zoneID, sensorID) {
        const sensorTypes = [];
        sensorTypes.push(SDMSMainMenu.O2_Sensor);
        sensorTypes.push(SDMSMainMenu.H2_Sensor);
        sensorTypes.push(SDMSMainMenu.CO_Sensor);
    ...
    updateWorkers(workers, zoneID) {
        if (zoneID === 0 || zoneID) {
            ...
            if (zoneWorkers) {
                const _zoneWorkers = {};
                _workers.zones[zoneID] = _zoneWorkers;

                for (const workerID in zoneWorkers) {
                    _zoneWorkers[workerID] = zoneWorkers[workerID];
                }

                this.setWorkerIcons(_workers, zoneID);
            }
            else {
                this.clearWorkerIcons();
            }
        }
    }
}
