...
class SDMS extends React.Component/*<Props, State>*/ {
    ...
    constructor(props/*: Props*/) {
        ...
        store.subscribe(function () {
            ...
            if (data !== null && data !== undefined) {
                ...
                if (data.actionType === 'PSM_SENSORS') {
                    this.setPSMSensorStatus(store.getState());
                }
            }
        }.bind(this));

        ProjectResource.initUserAuthor();
    }
    ...
    setMobileUsers(storeValue) {
        ...
        if (this.poiManager) {
            this.poiManager.updateWorkers(workers, this.state.currentView?.zoneID);
        }
    }

    setPSMSensorStatus(storeValue) {
        if (storeValue.psmSensors) {
            if (this.isSamePSMSensors(storeValue.psmSensors) === false) {
                this.setState({ psmSensors: storeValue.psmSensors });
            }
        }
    }
    ...
    isSamePSMSensors(psmSensors) {
        const oldSensors = [...this.state.psmSensors];

        const oldCount = oldSensors.length;
        const newCount = psmSensors.length;

        if (oldCount !== newCount) {
            for (let i = 0; i < newCount; i++) {
                const newSensor = psmSensors[i];
        ...
        const selectedPsmSensors = {};
        const originSelectedPsmSensors = [...this.state.selectedPsmSensors];

        for (const sensor of originSelectedPsmSensors) {
            selectedPsmSensors[sensor.id.toString()] = sensor;
        }
        ...
        for (let i = 0; i < newCount; i++) {
            ...
            const selectedSensor = selectedPsmSensors[newSensor.id.toString()];
    ...
    async showAlarm(alarm, targetCCTVMenu) {
        ...
        await this.getEquipZoneCCTV(alarm.equipZoneID, targetCCTVMenu);

        if (sensorType === "psm")
            this.onSelectMenu(SDMSMainMenu.Menu_Show_Alarm, [zoneID, sensorType, orgSensorID, alarmDepth, isAlarm, alarm.materialTypeString.toLowerCase()]);
        else
        ...
        for (let key in this.alarmInfo) {
            ...
            if (this.state.visiblePopups[key] && alarmInfo && alarmInfo[1]) {
                if (alarmInfo[1].sensorZoneHistoryID === selectedAlarm.sensorZoneHistoryID ||
                    ...
                    this.state.visiblePopups.cctvInfo = true;
    ...
    async getPopupState() {
        ...
        const userInfo = ProjectResource.SiteID ? JSON.parse(window.localStorage.getItem(SessionString.Key.account + "_" + ProjectResource.SiteID.toString())) : null;
    ...
    async processMenu(menu/*: Common.NullableNumber*/, param/*: any*/) {
        ...
        else if (menu === SDMSMainMenu.Menu_ToggleSOP) {
            window.open('/sop-simulator', '_blank');
            return;
        }
        else if (menu === SDMSMainMenu.Menu_ToggleSOPEdit) {
            window.open('/sop-manager', '_blank');
            return;
        }
        else if (menu === SDMSMainMenu.Menu_ToggleTeamEditor) {
            window.open('/team-editor', '_blank');
            return;
        }

        if (menu >= SDMSMainMenu.Menu_ToggleStatusInfo && menu <= SDMSMainMenu.Menu_ToggleSensorStatus) {
    ...
    togglePopup(menu) {
        ...
        else if (menu === SDMSMainMenu.Menu_ToggleSensorStatus) {
            visiblePopups.sensorStatus = !visiblePopups.sensorStatus;
        }
        else if (menu === SDMSMainMenu.Menu_ToggleSOP) {
            visiblePopups.sop = !visiblePopups.sop;
        }
        else if (menu === SDMSMainMenu.Menu_ToggleSOPEdit) {
            visiblePopups.sopEdit = !visiblePopups.sopEdit;
        }
        else if (menu === SDMSMainMenu.Menu_ToggleTeamEditor) {
            visiblePopups.teamEditor = !visiblePopups.teamEditor;
        }
    ...
    getPsmSensor(sensorID) {
        const psmSensors = [...this.state.psmSensors];
    ...
    containsSelectedSensor(sensor) {
        const psmSensors = [...this.state.selectedPsmSensors];

        for (const psmSensor of psmSensors) {
            if (sensor.id === psmSensor.id) {
                return [psmSensors, true];
            }
        }

        return [psmSensors, false];
    }
    ...
    onSelectSensorPOI = (sensorID, sensorType, poi, poiManager, onlyOne) => {
        ...
        if (!isEditMode) {
            ...
            if (sensor) {
                ...
                this.setState({ selectedPsmSensors: selectedSensors, visiblePopups });
            }
        }
    }
    ...
    changeToAllSensor = () => {
        ...
        this.setState({ selectedPsmSensors: [], visiblePopups });
    }
    ...
    getPsmSensorsForSensorStatus() {
        const psmSensors = [...this.state.psmSensors];
        const selectedSensors = [...this.state.selectedPsmSensors];
    ...
    render() {
        ...
        const [psmSensors, isAll] = this.getPsmSensorsForSensorStatus();
        ...
        return (
            <div className={styles.bodyArea} style={{ MozUserSelect: 'none', WebkitUserSelect: 'none' }}>
                <Contents3D
                    ...
                    setPoiManager={this.setPoiManager}
                ...
                {
                    this.state.visiblePopups.sensorStatus && !ProjectResource.isModelViewer &&
                    <SensorStatus
                        popupType="sensorStatus"
                        setVisiblePopup={this.setVisiblePopup}
                        setActiveDragPopup={this.setActiveDragPopup}
