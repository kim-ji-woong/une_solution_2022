...
class StatusInfoBuilding extends Component {
    ...
    getSensorList(sensorTypeName, zone) {
        const originSensors = this.props.sensorList[sensorTypeName];
        const sensors = [];

        if (originSensors) {
            for (const sensor of originSensors) {
                if (sensor.zoneID === zone.id && (sensor.visible === true || this.props.searchText === '')) {
                    sensors.push(sensor);
                }
            }
        }
    ...
    setZoneUI(zoneDatas, ui) {
        ...
        for (var i = 0; i < zoneDatas.length; i++) {
            ...
            if (this.props.sensorList) {
                fireSensors = this.getSensorList('fireSensors', zone);
                coSensors = this.getSensorList(SDMSMainMenu.CO_Sensor + "Sensors", zone);
                o2Sensors = this.getSensorList(SDMSMainMenu.O2_Sensor + "Sensors", zone);
                h2Sensors = this.getSensorList(SDMSMainMenu.H2_Sensor + "Sensors", zone);
                ch4Sensors = this.getSensorList(SDMSMainMenu.CH4_Sensor + "Sensors", zone);
                detectSensors = this.getSensorList(SDMSMainMenu.Detect_Sensor + "Sensors", zone);
                cctvs = this.getSensorList("cctvs", zone);
                /*fireSensors = this.props.sensorList['fireSensors'].filter(x => x.zoneID === zone.id && (x.visible === true || this.props.searchText === ''));
