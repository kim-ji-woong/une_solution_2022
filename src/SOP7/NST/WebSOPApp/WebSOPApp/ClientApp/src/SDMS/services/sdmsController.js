...
export class SDMSController {
    ...
    static async requestSaveIndoorModelViewport(modelName/*: string*/, cameraData/*: Frontend.PerspectiveCameraData2*/, zoneID)/*: Backend.MessageResult | null*/ {
        try {
    ...
    static StartWatchTimer() {
        ...
        // 1초에 한번씩 실행
        SDMSController.WatchPSMSensors();
        let timerPSMSensors = setTimeout(function tick() {
            SDMSController.WatchPSMSensors();
            timerPSMSensors = setTimeout(tick, 1000);
        }, 1000);
    }
    ...
    static async WatchPSMSensors() {
        let result = await SDMSController.requestPSMSensors();
        result = result === null || result.success === false ? [] : result.sensors;
        store.dispatch({ type: 'PSM_SENSORS', psmSensors: result });
    }

    static async requestPSMSensors() {
        try {
            const jsonData = SdmsJsonManager.makeRequestPSMSensors();

            const res = await fetch('Safety/Sensor/RequestData', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
