...
class Contents3D extends React.Component/*<Props, State>*/ {
    ...
    componentDidUpdate() {
        this.props.setPoiManager(this.poiManager);
    }
    ...
    static processMenu(props, state) {
        if (props.command) {
            if (props.command.menu == SDMSMainMenu.Menu_Save_BuildingGroup_Viewport) {
                ...
                const [zoneID, sensorType, sensorID, alarmLevel, isAlarm, psmSensorType] = props.command.menuParameter;
                state.prevInstance.showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, psmSensorType);
            }
    ...
    showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, psmSensorType) {
        ...
        if (sensorType === "psm" && psmSensorType && psmSensorType.length > 0)
            this.showIndoor(modelData[0], modelData[2], zoneID, psmSensorType, sensorID);
        else
            this.showIndoor(modelData[0], modelData[2], zoneID, sensorType, sensorID);
        ...
        if (model) {
            ...
            if (sensorPOI) {
                ...
                this.poiManager.selectPOI(sensorPOI);
    ...
    postMoveCamera(mode/*: number*/, fov/*: number*/, far/*: number*/, near/*: number*/, param/*: any*/) {
        ...
        if (param) {
            if (param.zoneID !== null && param.zoneID !== undefined) {
                ...
                if (param.sensorID !== null && param.sensorID !== undefined) {
                    const poi = this.poiManager.getSensorPOI(param.sensorType, param.zoneID, param.sensorID);

                    if (poi) {
                        this.poiManager.selectPOI(poi);
                        //this.props.onSelectSensorPOI(param.sensorID, param.sensorType, poi, this.poiManager, true);
                    }
                }
            }
        }
    ...
    showIndoor(modelFile/*: string*/, cameraOptions/*: Frontend.PerspectiveCameraData*/, zoneID/*: number*/, sensorType/*: string*/, sensorID/*: number*/) {
    ...
    async saveOutdoorModelViewport(camera/*: THREE.Camera*/, targetPoint/*: THREE.Vector3*/) {
        ...
        const result = await SDMSController.requestSaveIndoorModelViewport(modelName, cameraData, this.props.currentView.zoneID);
    ...
    onClickPOI(poi, event) {
        ...
        if (type === SDMSMainMenu.CCTV_Type) {
            ...
            type === SDMSMainMenu.H2_Sensor ||
            type === SDMSMainMenu.CH4_Sensor ||
            type === SDMSMainMenu.CO_Sensor) {
    ...
    getAlarmElements() {
        ...
        if (!this.props.alarmSound || !selectedAlarm || !selectedAlarm.isAlarm) {
            return (
                <></>
            );
        }

        if (selectedAlarm.alarmDepth/*this.state.alarm*/ === Contents3D.ALARM_2) {
            return (
        ...
        else if (selectedAlarm.alarmDepth/*this.state.alarm*/ === Contents3D.ALARM_3) {
            return (
                <audio autoPlay={true} loop={true}
        ...
        else if (selectedAlarm.alarmDepth/*this.state.alarm*/ === Contents3D.ALARM_4) {
            return (
                <audio autoPlay={true} loop={true}
    ...
    render() {
        ...
        const isIndoor = this.isIndoor();
        const currentZoneName = this.props.currentView.zoneName;
        ...
        return (
            <main className={styles.appWrap}>
                ...
                <input ref={this.refTooltip} type="text" className={tooltipClassName} value={this.tooltipText} onChange={() => this.onChangeTooltipText()} />
                {
                    isIndoor &&
                    <div id={styles.dsMap}>
                        <h3 className={styles.dsmTitle}>{currentZoneName}</h3>
                    </div>
                }
            </main>
        );
