...
namespace SoulbrainSensorTester
{
    public partial class FormMain : Form, IFormMain
    {
        ...
        public FormMain()
        {
            ...
            // 트리 작성
            SensorTreeNode fireSensorTree = m_dataManager.MakeSensorTree(Facility.FacilityType.FIRE_SENSOR);
            if (fireSensorTree != null)
                sensorTreeView.Nodes.Add(fireSensorTree);

            SensorTreeNode psmSensorTree = m_dataManager.MakeSensorTree(Facility.FacilityType.PSM_SENSOR);
            if (psmSensorTree != null)
                sensorTreeView.Nodes.Add(psmSensorTree);

            SensorTreeNode etcSensorTree = m_dataManager.MakeSensorTree(Facility.FacilityType.ETC);
