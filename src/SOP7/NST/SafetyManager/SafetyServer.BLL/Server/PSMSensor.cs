...
namespace SafetyServer.BLL.Server
{
    ...
    public class PSMSensor : BaseServer
    {
        ...
        private static ConcurrentQueue<UserResetDatas> m_queueUserResets = new ConcurrentQueue<UserResetDatas>();
        ...
        private static bool m_runUserResets = false;
        ...
        private Result ProcessUserReset(ArrayList arrDatas, bool userReset)
        {
            ...
            m_queueUserResets.Enqueue(new UserResetDatas(arrDatas, userReset));

            Task task = ProcessUserResets();
        ...
        private async Task ProcessUserResets()
        {
            if (m_runUserResets)
                return;

            m_runUserResets = true;

            // 이 함수를 비동기로 만든다.
            await Task.Yield();

            string strErrorMessage;
            UserResetDatas userResetData;

            while (m_queueUserResets.TryDequeue(out userResetData))
            {
                ArrayList arrDatas = userResetData.ArrDatas;
                bool userReset = userResetData.UserReset;
                ...
                if (arrDatas.Count >= 2 && arrDatas[0] is int && arrDatas[1] is int)
                {
                    ...
                    if (sensorZone == null || group == null)
                    {
                        strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessUserResets Error1 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.UNKNOWN_SENSOR_ID));
                    }
                    ...
                    foreach (KeyValuePair<SensorZone, int> pair in group.GetSensors())
                    {
                        ...
                        {
                            WriteLog(strErrorMessage);
                            System.Diagnostics.Trace.WriteLine("PSM.ProcessUserResets Error2 : " + strErrorMessage);
                            continue;
                            //return new MessageResult(false, strErrorMessage);
                        }
                    ...
                    WriteLog(strErrorMessage);
                    System.Diagnostics.Trace.WriteLine("PSM.ProcessUserResets Error3 : " + strErrorMessage);
                    continue;
                    //return new MessageResult(false, ErrorMessageType.ToMessage(nResult));
                }

                strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.INVALID_MESSAGE);
                WriteLog(strErrorMessage);
                System.Diagnostics.Trace.WriteLine("PSM.ProcessUserResets Error4 : " + strErrorMessage);
                //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.INVALID_MESSAGE));
            }

            m_runUserResets = false;
        }
        ...
        {
            // 동기화 문제를 피하기 위하여 직접 처리하지 않고 Queue에 쌓는다.
            m_queueSensorDatas.Enqueue(new SensorDatas(arrDatas, isReal, clearAlarm));
            ...
            while (m_queueSensorDatas.TryDequeue(out sensorData))
            {
                ...
                if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
                {
                    ...
                    if (group == null)
                    {
                        ...
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error1 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    if (sensorZone == null)
                    {
                        MessageResult result = GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(result.Message);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error2 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    {
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error3 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, strErrorMessage);
                    }
                    ...
                    if (nSensorData > 0)
                    {
                        ...
                        strErrorMessage = ErrorMessageType.ToMessage(nResult);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error4 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(nResult));
                    }
                    else
                    {
                        ...
                        strErrorMessage = ErrorMessageType.ToMessage(nResult);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error5 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(nResult));
                    }
                }
                ...
                System.Diagnostics.Trace.WriteLine("PSM.ProcessSensorDatas Error6 : " + _strErrorMessage);
                //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.INVALID_MESSAGE));
            }
            ...
            while (m_queueManualReports.TryDequeue(out arrDatas))
            {
                                    ...
                                    && arrDatas[4] is int && arrDatas[5] is string && arrDatas[6] is string)
                {
                    ...
                    if (nSensorZoneID < Header.ManualReportDefaultID)
                    {
                        strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error1 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.UNKNOWN_SENSOR_ID));
                    }
                    ...
                    if (group == null)
                    {
                        MessageResult result = GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(result.Message);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error2 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    if (sensorZone == null)
                    {
                        MessageResult result = GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(result.Message);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error3 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    if (alarm != null)
                    {
                        strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.ALREADY_PROCESSED);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error4 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.ALREADY_PROCESSED));
                    }
                    ...
                    if (alarm == null)
                    {
                        strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.DB_EXCEPTION);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error5 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.DB_EXCEPTION));
                    }
                ...
                strErrorMessage = ErrorMessageType.ToMessage(ErrorMessageType.INVALID_MESSAGE);
                WriteLog(strErrorMessage);
                System.Diagnostics.Trace.WriteLine("PSM.ProcessManualReports Error6 : " + strErrorMessage);
                //return new MessageResult(false, ErrorMessageType.ToMessage(ErrorMessageType.INVALID_MESSAGE));
            }
            ...
            while (m_queueClearManualReports.TryDequeue(out arrDatas))
            {
                if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
                {
                    ...
                    if (group == null)
                    {
                        MessageResult result = GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(result.Message);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessClearManualReports Error1 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    if (sensorZone == null)
                    {
                        MessageResult result = GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                        WriteLog(result.Message);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessClearManualReports Error2 : " + result.Message);
                        continue;
                        //return GetErrorMessageResult(ErrorMessageType.UNKNOWN_SENSOR_ID);
                    }
                    ...
                    if (alarm.IsManual)
                    {
                        ...
                        strErrorMessage = ErrorMessageType.ToMessage(nResult);
                        WriteLog(strErrorMessage);
                        System.Diagnostics.Trace.WriteLine("PSM.ProcessClearManualReports Error3 : " + strErrorMessage);
                        continue;
                        //return new MessageResult(false, ErrorMessageType.ToMessage(nResult));
                    }
                }
        ...
        private int ChangeAlarm(AlarmData currentAlarm, SensorZoneGroup group, SensorZone sensorZone, PSM sensor, int nSensorData, bool isReal, ref AlarmData alarm, ref AlarmData prevAlarm)
        {
            ...
            if (IsChangedAlarmDepth(sensorZoneDatas, group, sensorZone, nSensorData, out nAlarmDepth, out alarmSensorZone))
            {
                ...
                alarm.Status = (int)SensorReactionHistory.ReactionTypes.CHANGE_ALARM_DEPTH;
        ...
        public int RemoveAlarm(SensorZoneGroup group, SensorZone sensorZone, bool isReal)
        {
            ...
            SensorZoneHistory.DetectionType detectionStatus = isReal ? SensorZoneHistory.DetectionType.Real : SensorZoneHistory.DetectionType.Test;
            SensorReactionHistory.ReactionTypes reactionType = SensorReactionHistory.ReactionTypes.END_STATUS;//isReal ? SensorReactionHistory.ReactionTypes.END_STATUS : SensorReactionHistory.ReactionTypes.USER_RESET;
        ...
        private class SensorDatas
        {
            ...
            public SensorDatas(ArrayList arrDatas, bool isReal, bool clearAlarm)
            {
                m_arrDatas = arrDatas;
                m_isReal = isReal;
        ...
        private class UserResetDatas
        {
            private ArrayList m_arrDatas = null;
            private bool m_userReset = false;
            ...
            public bool UserReset
            {
                get { return m_userReset; }
                set { m_userReset = value; }
            }

            public UserResetDatas()
            {
            }

            public UserResetDatas(ArrayList arrDatas, bool userReset)
            {
                m_arrDatas = arrDatas;
                m_userReset = userReset;
            }
        }
    }
}
