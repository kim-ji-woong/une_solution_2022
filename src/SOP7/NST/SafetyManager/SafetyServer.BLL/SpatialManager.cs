...
namespace SafetyServer.BLL
{
    ...
    public class SpatialManager
    {
        ...
        public ResponseMobieUserList GetMobileUserList(Safety.IDAL.IDataManager safetyDataManager, string strMemberIDFormat)
        {
            string strErrorMessage;
            bool isNullable;
            string strCondition = strMemberIDFormat != null && strMemberIDFormat.Length > 0 ? string.Format("{0} like '{1}'", RegularMember.GetFieldName(RegularMember.Fields.MemberID, out isNullable), strMemberIDFormat) : null;
            ...
            Dictionary<int, RegularMemberInfo> dicRegularMemberInfos = GetRegularMemberInfos(safetyDataManager, members, out strErrorMessage);

            if (dicRegularMemberInfos == null)
                return new ResponseMobieUserList(false, strErrorMessage);
            ...
            foreach (RegularMember member in members)
            {
                ...
                if (dicTeams.TryGetValue(member.RegularID, out team))
                {
                    ...
                    RegularMemberInfo memberInfo;

                    if (dicRegularMemberInfos.TryGetValue(member.ID, out memberInfo))
                    {
                        // 데이터가 없으면 안전장구 알람이 발생하지 않은 상태이기 때문에 모두 착용한 것으로 간주한다.
                        user.Helmet = memberInfo.Helmet == null ? true : (bool)memberInfo.Helmet;
                        user.Belt = memberInfo.Belt == null ? true : (bool)memberInfo.Belt;
                        user.Shoes = memberInfo.Shoes == null ? true : (bool)memberInfo.Shoes;
                    }
                    else
                    {
                        user.Helmet = true;
        ...
        private Dictionary<int, RegularMemberInfo> GetRegularMemberInfos(Safety.IDAL.IDataManager safetyDataManager, List<RegularMember> members, out string strErrorMessage)
        {
            string strMemberIDs = "";
            ...
            Dictionary<int, RegularMemberInfo> dicRegularMemberInfos = new Dictionary<int, RegularMemberInfo>();

            if (members.Count == 0)
            {
                strErrorMessage = null;
                return dicRegularMemberInfos;
            }

            bool isNullable;
            string strCondition = string.Format("{0} in ({1})", RegularMemberInfo.GetFieldName(RegularMemberInfo.Fields.MemberID, out isNullable), strMemberIDs);
            List<RegularMemberInfo> regularMemberInfos = safetyDataManager.GetSelectManager().SelectSopTeamRegularMemberInfos(null, strCondition, out strErrorMessage);

            if (regularMemberInfos == null)
                return null;

            foreach (RegularMemberInfo info in regularMemberInfos)
            {
                dicRegularMemberInfos[info.MemberID] = info;
            }
