...
namespace SafetyServer.BLL.Data.Models
{
    public class SafetyAlarm : AlarmData
    {
        ...
        private static void SetAlarmInfo(Dictionary<int, SafetyAlarm> dicAlarms, IDataManager dataManager, TeamEditor.IDAL.IDataManager teamDataManager)
        {
            ...
            SetEtcAlarmInfo(dicAlarms, dataManager, teamDataManager, sensorZones, dicSensorZones);
            SetPsmAlarmInfo(dicAlarms, dataManager, teamDataManager, sensorZones, dicSensorZones);
        }

        private static void SetEtcAlarmInfo(Dictionary<int, SafetyAlarm> dicAlarms, IDataManager dataManager, TeamEditor.IDAL.IDataManager teamDataManager, List<SensorZone> sensorZones, Dictionary<int, int?> dicSensorZones)
        {
            ...
            string strAdditionalConditions = string.Format("{0} in ({1})", SDMS.Model.Sensor.ETC.GetFieldName(SDMS.Model.Sensor.ETC.Fields.ID, out isNullable), strOriginSensorIDs);
            List<SDMS.Model.Sensor.ETC> etcSensors = strOriginSensorIDs.Length > 0 ? dataManager.GetSelectManager().SelectETCSensors(null, strAdditionalConditions, out strErrorMessage) : new List<SDMS.Model.Sensor.ETC>();

            if (etcSensors == null)
            {
                if (strErrorMessage != null)
                    System.Diagnostics.Trace.WriteLine("SetEtcAlarmInfo Fail : " + strErrorMessage);
                return;
            }
            ...
            Dictionary<int, SDMS.Model.Sensor.ETC> dicETCSensors = new Dictionary<int, SDMS.Model.Sensor.ETC>();

            string strRegularMemberIDs = "";

            foreach (SDMS.Model.Sensor.ETC sensor in etcSensors)
            {
                ...
                if (dicSensorZones.TryGetValue(pair.Value.SensorZoneID, out originSensorID))
                {
                    if (originSensorID != null)
                    {
                        ...
                        if (dicETCSensors.TryGetValue((int)originSensorID, out sensor))
                        {
                            ...
                            {
                                pair.Value.m_strAlarmType = "감지센서";
                                pair.Value.m_strAlarmInfo = pair.Value.ZoneName + ", " + sensor.Name;
                            }
                        }
                    }
                }
        ...
        private static void SetPsmAlarmInfo(Dictionary<int, SafetyAlarm> dicAlarms, IDataManager dataManager, TeamEditor.IDAL.IDataManager teamDataManager, List<SensorZone> sensorZones, Dictionary<int, int?> dicSensorZones)
        {
            string strOriginSensorIDs = "";

            foreach (SensorZone sensorZone in sensorZones)
            {
                if (sensorZone.SensorType == (int)dnsData.Sensor.Facility.FacilityType.PSM_SENSOR)
                {
                    if (strOriginSensorIDs.Length == 0)
                        strOriginSensorIDs = sensorZone.OrgSensorID.ToString();
                    else
            ...
            bool isNullable;
            string strErrorMessage = null;
            string strAdditionalConditions = string.Format("{0} in ({1})", PSM.GetFieldName(PSM.Fields.ID, out isNullable), strOriginSensorIDs);
            List<PSM> psmSensors = strOriginSensorIDs.Length > 0 ? dataManager.GetSelectManager().SelectPSMSensors(null, strAdditionalConditions, out strErrorMessage) : new List<PSM>();

            if (psmSensors == null)
            {
                if (strErrorMessage != null)
                    System.Diagnostics.Trace.WriteLine("SetPsmAlarmInfo Fail : " + strErrorMessage);
                return;
            }

            // Key : PsmSensor ID
            Dictionary<int, PSM> dicPSMSensors = new Dictionary<int, PSM>();

            foreach (PSM sensor in psmSensors)
            {
                dicPSMSensors[sensor.ID] = sensor;
            }
            ...
            foreach (KeyValuePair<int, SafetyAlarm> pair in dicAlarms)
            {
                ...
                if (dicSensorZones.TryGetValue(pair.Value.SensorZoneID, out originSensorID))
                {
                    if (originSensorID != null)
                    {
                        ...
                        if (dicPSMSensors.TryGetValue((int)originSensorID, out sensor))
                        {
                            pair.Value.m_strAlarmType = "가스누출";
                            pair.Value.m_strAlarmInfo = pair.Value.ZoneName + ", " + sensor.Name;
                        }
                    }
                }
