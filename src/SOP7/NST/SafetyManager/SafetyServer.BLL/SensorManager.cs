...
namespace SafetyServer.BLL
{
    ...
    public class SensorManager
    {
        ...
        // Key : PSMSensor ID
        private Dictionary<int, PSM> m_dicPsmSensors = new Dictionary<int, PSM>();
        // Key : EtcSensor ID
        private Dictionary<int, ETC> m_dicEtcSensors = new Dictionary<int, ETC>();
        ...
        public MessageResult ProcessAreaAlarm(string strMemberID, string strCameraID, DateTime timeStamp, int nAlarmLevel, string strMessage)
        {
            ...
            {
                // strMemberID를 DB에서 찾지 못하였으므로 알람처리는 하지 않는다.
                // 다만, 이 정보를 netvision app server쪽으로 전달은 한다.
        ...
        public MessageResult ProcessNoEquipmentAlarm(Safety.IDAL.IDataManager safetyDataManager, string strMemberID, string strCameraID, DateTime timeStamp, bool helmet, bool shoes, bool belt, int nAlarmLevel, string strMessage)
        {
            int nSensorZoneID, nSensorTagID;
            string strErrorMessage = null;
            ...
            if (UpdateRegularMemberInfo(safetyDataManager, strMemberID, helmet, shoes, belt, out strErrorMessage) == false)
                return new MessageResult(false, strErrorMessage);
        ...
        private bool UpdateRegularMemberInfo(Safety.IDAL.IDataManager safetyDataManager, string strMemberID, bool helmet, bool shoes, bool belt, out string strErrorMessage)
        {
            Dictionary<RegularMember.Fields, object> dicConditions = new Dictionary<RegularMember.Fields, object>();
            dicConditions[RegularMember.Fields.MemberID] = strMemberID;

            List<RegularMember> members = m_mainManager.TeamDataManager.GetSelectManager().SelectRegularMembers(dicConditions, null, out strErrorMessage);
            ...
            if (members.Count == 0)
            {
                strErrorMessage = string.Format("올바르지 않은 MemberID({0}) 입니다.", strMemberID);
                return false;
            }

            RegularMember member = members[0];

            Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object> dicConditions2 = new Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object>();
            dicConditions2[Safety.Model.Sop.Team.RegularMemberInfo.Fields.MemberID] = member.ID;

            List<Safety.Model.Sop.Team.RegularMemberInfo> regularMemberInfos = safetyDataManager.GetSelectManager().SelectSopTeamRegularMemberInfos(dicConditions2, null, out strErrorMessage);

            if (regularMemberInfos == null)
                return false;

            if (regularMemberInfos.Count == 0)
            {
                Safety.Model.Sop.Team.RegularMemberInfo regularMemberInfo = new Safety.Model.Sop.Team.RegularMemberInfo();
                regularMemberInfo.MemberID = member.ID;
                regularMemberInfo.Belt = belt;
                regularMemberInfo.Helmet = helmet;
                regularMemberInfo.Shoes = shoes;
                regularMemberInfo.LoginStatus = false;
                regularMemberInfo.ZoneID = null;
                regularMemberInfo.X = null;
                regularMemberInfo.Y = null;

                regularMemberInfo = safetyDataManager.GetCreateManager().CreateSopTeamRegularMemberInfo(regularMemberInfo, out strErrorMessage);

                if (regularMemberInfo == null)
                    return false;
            }
            else
            {
                Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object> dicConditions3 = new Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object>();
                dicConditions3[Safety.Model.Sop.Team.RegularMemberInfo.Fields.MemberID] = member.ID;

                Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object> dicSets = new Dictionary<Safety.Model.Sop.Team.RegularMemberInfo.Fields, object>();
                dicSets[Safety.Model.Sop.Team.RegularMemberInfo.Fields.Belt] = belt;
                dicSets[Safety.Model.Sop.Team.RegularMemberInfo.Fields.Helmet] = helmet;
                dicSets[Safety.Model.Sop.Team.RegularMemberInfo.Fields.Shoes] = shoes;

                if (safetyDataManager.GetUpdateManager().UpdateSopTeamRegularMemberInfo(dicSets, dicConditions3, null, out strErrorMessage) == false)
                    return false;
            }
        ...
        private bool LoadSensorZones()
        {
            ...
            foreach (SensorZone sensorZone in sensorZones)
            {
                ...
                SensorZoneGroup group = GetSensorZoneGroup(sensorZone);
        ...
        private void LoadPSMSensors()
        {
            ...
            if (psmSensors != null)
            {
                ...
                {
                    m_dicPsmSensors[psmSensor.ID] = psmSensor;
                }
            }
        }
        ...
        {
            string strErrorMessage;
            List<ETC> etcSensors = m_mainManager.SDMSDataManager.GetSelectManager().SelectETCSensors(null, null, out strErrorMessage);

            if (etcSensors != null)
            {
                foreach (ETC etcSensor in etcSensors)
                {
                    m_dicEtcSensors[etcSensor.ID] = etcSensor;
                }
            }
        }
        ...
        public SensorZoneGroup GetSensorZoneGroup(EquipmentZone equipZone, int nOrgSensorID, int sensorType)
        {
            if (sensorType == (int)Facility.FacilityType.ETC || sensorType == (int)Facility.FacilityType.PSM_SENSOR)
            {
                if (sensorType == (int)Facility.FacilityType.ETC)
                {
                    ETC etcSensor;

                    // ETC Sensor의 경우 각 센서타입별로 SensorZoneGroup을 생성한다.
                    if (m_dicEtcSensors.TryGetValue((int)nOrgSensorID, out etcSensor))
                    {
                        if (etcSensor.MaterialType != null)
                            sensorType = (int)etcSensor.MaterialType;
                    }
                }
                else if (sensorType == (int)Facility.FacilityType.PSM_SENSOR)
                {
                    PSM psmSensor;

                    // PSM Sensor의 경우 각 센서타입별로 SensorZoneGroup을 생성한다.
                    if (m_dicPsmSensors.TryGetValue(nOrgSensorID, out psmSensor))
                    {
                        if (psmSensor.MaterialType != null)
                            sensorType = (int)psmSensor.MaterialType;
                    }
                }
            }
        ...
        public SensorZoneGroup GetSensorZoneGroup(SensorZone sensorZone/*int nEquipZoneID, Facility.FacilityType sensorType*/)
        {
            int nSensorType = sensorZone.SensorType;

            if (sensorZone.OrgSensorID != null && (nSensorType == (int)Facility.FacilityType.ETC || nSensorType == (int)Facility.FacilityType.PSM_SENSOR))
            {
                if (nSensorType == (int)Facility.FacilityType.ETC)
                {
                    ...
                    // ETC Sensor의 경우 각 센서타입별로 SensorZoneGroup을 생성한다.
                    if (m_dicEtcSensors.TryGetValue((int)sensorZone.OrgSensorID, out etcSensor))
                    {
                        if (etcSensor.MaterialType != null)
                            nSensorType = (int)etcSensor.MaterialType;
                    }
                }
                else if (nSensorType == (int)Facility.FacilityType.PSM_SENSOR)
                {
                    PSM psmSensor;

                    // PSM Sensor의 경우 각 센서타입별로 SensorZoneGroup을 생성한다.
                    if (m_dicPsmSensors.TryGetValue((int)sensorZone.OrgSensorID, out psmSensor))
                    {
                        if (psmSensor.MaterialType != null)
                            nSensorType = (int)psmSensor.MaterialType;
                    }
                }
            }

            long nID = SensorZoneGroup.ToID(sensorZone.EquipZoneID, nSensorType);
            return GetSensorZoneGroup(nID, sensorZone.EquipZoneID, null, nSensorType);
        }

        private SensorZoneGroup GetSensorZoneGroup(long nSensorZoneGroupID, int nEquipZoneID, EquipmentZone equipZone, int sensorType)
        {
        ...
        public SensorZone GetSensorZone(int nOrgSensorID, Facility.FacilityType sensorType, EquipmentZone equipZone, out SensorZoneGroup group)
        {
            ...
            if (equipZone != null)
            {
                group = GetSensorZoneGroup(equipZone, nOrgSensorID, (int)sensorType);
