...
class SopSimulator extends Component {
    constructor(props) {
        ...
        this.state = {
            selectedSiteID: null,
            content: SopSimulatorResource.menu.SOP_불러오기,
            sopTabIndex: -1,
    ...
    async loadSOP(storeValue) {
        ...
        if (sopHistories === null || sopHistories.length === 0) {
            this.setState({ sopDatas: [], viewSopDatas: [], content: SopSimulatorResource.menu.SOP_불러오기, sopTabIndex: -1, currentSopTabKey: '', selectedSiteID });
        }
        else {
            ...
            if (closeSOPs.length > 0) {
                ...
                if (this.confirmDialogData) {
                    let message = i18n.t('sopSimulator.formText.대기시간 자동 종료시간이 경과되어 SOP를 종료합니다');
                    if (this.confirmDialogData.sensorZoneHistoryID && this.confirmDialogData.sensorZoneHistoryID > 0) {
                        message = i18n.t('sopSimulator.formText.신호 복구 후 자동 종료시간이 경과되어 SOP를 종료합니다');
                    }
                    this.showConfirmDialog(i18n.t('sopSimulator.formText.알림'), message, [i18n.t('common.확인')], this.confirmDialogCloseSOP);
                }
            }
            ...
            if (viewSopHistoriesDump.length === 0) {
                ...
                this.setState({
                    content: SopSimulatorResource.menu.SOP_실행,
    ...
    async loadTeamDatas() {
        ...
        teamDatas.regularMember = await TeamEditController.DisplayRegularMember(this.selectedSiteID);
    ...
    changeSelectSiteID = (siteID) => {
        ...
        if (ProjectResource.SiteID !== ProjectResource.Site.GG_A || _siteID === NaN)
            return;

        this.setState({ selectedSiteID: _siteID });
    }
    ...
    */
    ...
    async openDB(versionID, alarm) {        
        ...
        if (sopDataResult && sopDataResult.success) {
            ...
            if (sopTabIndex > -1) {
                this.setState({ content: SopSimulatorResource.menu.SOP_실행, sopDatas: loadSopDatas, sopTabIndex: sopTabIndex });
            }
            else {                                
    ...
    closeSopData = (index) => {
        if (!this.state.viewSopDatas[index].position) {
            ...
            if (sopTabIndexTemp < 0) {
                this.setState({ sopDatas: sopDatasCopy, viewSopDatas: viewSopDatasCopy, sopTabIndex: -1, content: SopSimulatorResource.menu.SOP_불러오기, currentSopTabKey: '' });
            }
            else {
    ...
    onSelectMenu = (menu, param) => {
        ...
        if (menu === SopSimulatorResource.menu.SOP_불러오기) {
            ...
            if (content === SopSimulatorResource.menu.SOP_불러오기 && sopDatas.length !== 0) {
                this.setState({ content: SopSimulatorResource.menu.SOP_실행 });
            } else {
                this.setState({ content: SopSimulatorResource.menu.SOP_불러오기 });
            }   
        }
    }
    ...
    getSiteList = () => {
        ...
        if (userInfo?.levelID === AccountResource.accountLevelID.master) {
            if (siteCount > 1) {
                siteList.push(<a className={'btnList'} onClick={() => this.onChangeSite(-1)}>{i18n.t('common.전체')}</a>);
            }

            for (let i = 0; i < siteCount; i++) {
                const site = sites[i];
                siteList.push(<a className={'btnList'} onClick={() => this.onChangeSite(site.id)}>{site.siteName}</a>);
            ...
            for (let i = 0; i < siteCount; i++) {
                ...
                if (site.id === userInfo.siteID) {
                    siteList.push(<a>{site.siteName}</a>);
                    break;
                }
            }
        }
    ...
    test() {
        ...
        if (sopDatasDump.length > 0) {
            ...
            this.setState({ viewSopDatas: [], content: SopSimulatorResource.menu.SOP_불러오기, sopTabIndex: -1, currentSopTabKey: '' });
        }
    }
    ...
    render() {
        ...
        let ui = <></>;
        if (this.state.content === SopSimulatorResource.menu.SOP_불러오기) {
            ui = <>
                <SopSimulatorSBcall
                    openDB={this.openDB}
        ...
        } else if (this.state.content === SopSimulatorResource.menu.SOP_실행) {
            ui = <>
                <SopSimulatorBody
                    id={'SopSimulatorBody'}
...
export default withTranslation()(SopSimulator);