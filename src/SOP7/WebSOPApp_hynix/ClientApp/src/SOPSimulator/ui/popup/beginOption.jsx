...
class BeginOption extends Component {
    ...
    onClickBegin = () => {
        ...
        if (!position || position.length === 0) {
            alert(i18n.t('sopSimulator.formText.재난 발생 위치를 입력하세요'));
            return;
        }
        else if (position.length > 50) {
            alert(i18n.t('sopSimulator.formText.재난 발생 위치 길이는 50자를 초과할 수 없습니다. (현재:') + position.length + ')');
            return;
        }
        else {
            let beginTime = '';
    ...
    render() {
        ...
        return (
            <BeginOptionComponent id={'sopPop'}>
                <div>
                    <div>
                        <div className={'sqPop'}>
                            <div className={'sqpTop'}>
                                <h4>{i18n.t('sopSimulator.formText.시작 이벤트 옵션')}</h4>
                            ...
                            <div className={'sqpCont'}>
                                <div className={'sqpUp'}>
                                    {
                                        ...
                                        //</select>
                                        <input type="text" id="txtPosition" className={'sqpSel'} onKeyUp={this.beginEnterKey} placeholder={i18n.t('sopSimulator.formText.시작 위치')} />
                                    }
                                    {
                                        //<div className={uis.sqpChk}>
                                        //    <input type="checkbox" name="sqpChk" id="sqpChk" checked="" /><label htmlFor="sqpChk">상황 시작/종료 문자 메시지 사용</label>
                                        //</div>
                                    }
                                </div>
                                <div className={'sqpDown'}>
                                    ...
                                    <ul className={'sqpRdo'}>
                                        <li><input type="radio" name="sqpRdo" id="sqpRdo01" onChange={() => this.onChangeTimeMode(true)} checked={this.state.isCurrent} /><label htmlFor="sqpRdo01">{i18n.t('sopSimulator.formText.현재 시간을 재난발생시간으로 설정')}</label></li>
                                        <li><input type="radio" name="sqpRdo" id="sqpRdo02" onChange={() => this.onChangeTimeMode(false)} /><label htmlFor="sqpRdo02">{i18n.t('sopSimulator.formText.재난 발생 시간 입력')}</label></li>
                                    </ul>
                                    <ul className={'sqpTime'}>
                                        <li>
                                            <select name="" id="" value={this.state.year||''} onChange={(e) => this.onChangeDate('year', e.target)}>
                                                {yearTag}
                                            </select>
                                        </li>
                                        <li>{i18n.t('common.년')}</li>
                                        <li>
                                            <select name="" id="" value={this.state.month || ''} onChange={(e) => this.onChangeDate('month', e.target)}>
                                                {monthTag}
                                            </select>
                                        </li>
                                        <li>{i18n.t('common.월')}</li>
                                        <li>
                                            <select name="" id="" value={this.state.day || ''} onChange={(e) => this.onChangeDate('day', e.target)}>
                                                {dayTag}
                                            </select>
                                        </li>
                                        <li>{i18n.t('common.일')}</li>
                                        <li>
                                    ...
                                    <ul className={'sqpBtn'}>
                                        <li><a className={'bk'} onClick={this.onClickBegin}>{i18n.t('common.시작')}</a></li>
                                        <li><a className={'gry'} onClick={this.onClickClose}>{i18n.t('common.취소')}</a></li>
                                    </ul>
                                </div>
                            </div>{/*<!-- sqpCont -->*/}
                        </div>{/*<!-- sqPop -->*/}
...
export default withTranslation()(BeginOption);