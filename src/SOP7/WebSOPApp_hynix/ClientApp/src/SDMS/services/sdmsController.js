...
export class SDMSController {
            ...
            for (let i = 0; i < receiveAlarmCount; i++) {
                ...
                for (let j = 0; j < temp.length; j++) {
                    ...
                    if (receiveAlarm[i].dtTime == temp[j].dtTime &&
                        ...
                        receiveAlarm[i].sensorZoneHistoryInfos.length == temp[j].sensorZoneHistoryInfos.length &&
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (result.success) {
                    ...
                    sensorCount.manufactureSensorCount = result.manufactureSensorCount;
                    sensorCount.disabledManufactureSensorCount = result.disabledManufactureSensorCount;
                    ...
                    if (!currentData) {
                        ...
                        if (currentData.fireSensorCount !== sensorCount.fireSensorCount ||
                            ...
                            currentData.manufactureSensorCount !== sensorCount.manufactureSensorCount ||
                            currentData.disabledManufactureSensorCount !== sensorCount.disabledManufactureSensorCount ||
    ...
    static StartWatchTimer() {
        ...
        let timerId = setTimeout(async function tick() {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.Magog) {
                await SDMSController.WatchDoorStatus();
            }
            if (ProjectResource.SiteID === ProjectResource.Site.Wonik) {
                await SDMSController.WatchSpeedDetection();
            }
    ...
    static StartWatchTimerElevator(siteID) {
        // 타이머 실행 유무 판단
        if (this.timerCheckElevator == true) return;
        ...
        // 1초에 한번씩 실행 - 엘리베이터 정보
        const tick = async () => {
            if (!this.timerCheckElevator) return;
            await SDMSController.WatchElevatorInfos(siteID);
            if (this.timerCheckElevator) {
                this.timerElevator = setTimeout(tick, 1000);
            }
        };
    ...
    static stopWatchTimer() {
        this.timerCheckElevator = false;

        if (this.timerElevator > 0) {
            clearTimeout(this.timerElevator);
            this.timerElevator = 0;
        }
    }
    ...
    static async _requestGltfModelList(userID, siteIDs) {
        try {
            const jsonData = SdmsJsonManager.makeRequestGltfDataList2(userID, siteIDs);

            const res = await fetch('SDMS/SDMS/RequestGltfDataList', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    static checkGGSite(siteIDs) {
        if (siteIDs) {
            ...
            for (const siteID of siteIDs) {
                ...
                else if (siteID >= ProjectResource.Site.GG_B && siteID <= ProjectResource.Site.GG_H) {
                    ggSite = true;
                }
            }

            // 경기도청 주변의 입주기관일 경우 종합방재실은 무조건 추가한다.
            if (ggSite) {
    ...
    static async requestSensorList(siteIDs) {
        try {
            SDMSController.checkGGSite(siteIDs);
            const jsonData = SdmsJsonManager.makeRequestSensorList(siteIDs);
    ...
    static async requestSaveIndoorModelViewport(modelName, cameraData, zoneID, userID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSaveIndoorModelViewport(modelName, cameraData, zoneID, userID);
    ...
    static async requestSaveOrthoModelViewport(modelName, cameraData, zoneID, userID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSaveOrthoModelViewport(modelName, cameraData, zoneID, userID);
    ...
    static async requestAllClearReport(sensorType) {
        try {
            const jsonData = SdmsJsonManager.makeRequestAllClearReport(sensorType);
    ...
    static async WatchSpeedDetection() {
        // 오늘 차량 과속감지 데이터

        // 방재실 모니터링 시스템에만 적용
        const userInfo = ProjectResource.getUserInfo();
        if (userInfo?.options?.ui?.useEquipZoneAssess === true)
            return;

        let result = await SDMSController.requestWonikSpeedDetections();

        let speedDetectionDatas = [];
    ...
    static async requestWonikSpeedDetections() {
        try {
        ...
        return {
            success: false,
            message: "requestWonikSpeedDetections 실패"
        };
    }
    ...
    static async requestElevators(siteID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestElevators(siteID);
    ...
    static async WatchElevatorInfos(siteID) {
        // 엘리베이터 정보 조회
        let result = await SDMSController.requestElevators(siteID);
    ...
    static async requestWorkerInfo(workerID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestWorkerInfo(workerID);

            const res = await fetch('SDMS/Hynix/RequesWorkerInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return {
            success: false,
            message: "requestWorkerInfo 실패"
        };
    }

    static async requestItemInfo(itemID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestItemInfo(itemID);

            const res = await fetch('SDMS/Hynix/RequesItemInfo', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return {
            success: false,
            message: "requestItemInfo 실패"
        };
    }    

    static async requesAbnormalHistory(workerID, time) {
        try {
            const jsonData = SdmsJsonManager.makeRequesAbnormalHistory(workerID, time);

            const res = await fetch('SDMS/Hynix/RequesAbnormalHistory', {
                method: 'post',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        return {
            success: false,
            message: "requesAbnormalHistory 실패"
        };
    }    
}