...
class SDMSMainMenu extends Component {
    ...
    static Admin_Menu_Viewport = SdmsResource.adminMenu.뷰포트_설정;
    static Admin_Menu_MovePOI = SdmsResource.adminMenu.POI_이동;
    static Admin_Menu_FakeWall = SdmsResource.adminMenu.가벽;
    static Admin_Menu_FireSensor = SdmsResource.adminMenu.화재센서_이동;
    static Admin_Menu_PsmSensor = SdmsResource.adminMenu.누출센서_이동;
    static Admin_Menu_EtcSensor = SdmsResource.adminMenu.기타센서_이동;
    static Admin_Menu_CCTV = SdmsResource.adminMenu.CCTV_이동;

    static Fire_Sensor = "fire";
    static PSM_Sensor = "psm";
    static Etc_Sensor = "etc";
    static Earthquake_Sensor = "earthquake";
    static Blackout_Sensor = "blackout";
    static Strongwind_Sensor = "strongWind";
    static Environment_Sensor = "environment";
    static Manufacture_Sensor = "manufacture";
    static Emergency_Sensor = "emergencyBell";
    static Laser_Sensor = "laser";
    static Door_Sensor = "door";
    ...
    static Worker = "Worker";                                       // 작업자
    static Visitor = "Visitor";                                     // 방문객
    static EquipZoneArea = "equipZoneArea";
    static EquipZoneAssess = "equipZoneAssess";    
    static Natural = "Natural";                                     // 환경
    static Manufacturing_Facilities = "Manufacturing-Facilities";   // 제조설비
    static Field = "Field";                                         // 영역
    static Park = "Park";                                           // 주차차단기
    static Exit = "exit";                                           // 비상구
    static Life = "Life";                                           // 인명구조기구
    static Cardiac = "Cardiac";                                     // 심장제세동기
    static Rescue = "Rescue";                                       // 완강기
    static SpeedDetection = "speedDetection";                       // 과속감지

    static Hynix_ForcedDoorOpen = "강제 문열림";
    static Hynix_CheatedTagging = "대리태깅";
    static Hynix_Untagging = "꼬리물기";
    static Hynix_StealCard = "사원증도용";
    static Hynix_Stranger = "이상행위자";
    static Hynix_EvasionItem = "무인 보안검색 우회";
    static Hynix_NotPermittedPerson = "비인가 구역 출입";
    static Hynix_NotPermittedItem = "비인가 구역 반입";

    static Hynix_Door = "hynixDoor";
    static Hynix_CardReader = "hynixCardReader";
    static Hynix_SmartTagReader = "hynixSmartTagReader";
    ...
    static isEquipZoneArea(text) {
        if (text?.startsWith(SDMSMainMenu.EquipZoneArea)) {
            return true;
        }
    ...
    async savePOIPosition(selectedPOI) {
        ...
        if (sensorType && sensorType.length > 0 && zoneID && sensorID) {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
                [result, message] = await GghController.requestUpdatePOIPosition(sensorType, zoneID, sensorID, poi.object.position.x, poi.object.position.y, poi.object.position.z);
            }
            else {
                [result, message] = await SDMSController.requestUpdatePOIPosition(sensorType, zoneID, sensorID, poi.object.position.x, poi.object.position.y, poi.object.position.z);
            }
    ...
    moveBuildingName() {
        ...
        if (x !== null && x !== undefined && y !== null && y !== undefined && z !== null && z !== undefined) {
            ...
            alert(i18n.t('sdms.sdmsMainMenu.숫자만 입력 가능합니다'));
        }
    }
    ...
    moveSensor() {
        ...
        if (isNaN(x) === false && isNaN(z) === false && sensor) {
            ...
            if (!sensor) {
                alert(i18n.t('sdms.sdmsMainMenu.센서가 선택되지 않았습니다'));
            }
            else {
                alert(i18n.t('sdms.sdmsMainMenu.숫자만 입력 가능합니다'));
            }
    ...
    movePOI() {
        ...
        if (isNaN(x) === false && isNaN(y) === false && isNaN(z) === false && this.props.selectedPOI) {
            ...
            if (!this.props.selectedPOI) {
                alert(i18n.t('sdms.sdmsMainMenu.POI가 선택되지 않았습니다'));
            }
            else {
                alert(i18n.t('sdms.sdmsMainMenu.숫자만 입력 가능합니다'));
            }
    ...
    render() {
        ...
        const selectedPOIName = this.props.selectedPOI ? this.props.selectedPOI[0].object.name : i18n.t('sdms.sdmsMainMenu.선택된 POI 없음');
        const btnMoveBuildingNameText = this.state.equipZoneVisible ? i18n.t('sdms.sdmsMainMenu.설비영역 이동') : i18n.t('sdms.sdmsMainMenu.건물이름 이동');

        return (
            <div className={className}>
                <h2 className={styles.menuTitle}>
                    {i18n.t('sdms.sdmsMainMenu.관리자를 위한 기능')}
                </h2>
                ...
                <div className={styles.horzArea}>
                    <button ref={this.refBGBtn} className={styles.menuBtnViewport} onClick={() => this.saveViewport(SDMSMainMenu.Menu_Save_BuildingGroup_Viewport, this.state.currentBuildingGroupName)}>{i18n.t('sdms.sdmsMainMenu.뷰포트 저장')}</button>
                    <button ref={this.refBuildingViewportBtn} className={styles.menuBtnViewport} onClick={() => this.saveViewport(SDMSMainMenu.Menu_Save_Building_Viewport, this.state.currentBuildingName)}>{i18n.t('sdms.sdmsMainMenu.건물 뷰포트 저장')}</button>
                </div>
                ...
                <input ref={this.refPosY} className={styles.inputText} type="text" placeholder={i18n.t('sdms.sdmsMainMenu.높이')} defaultValue={posY} onChange={this.onChangeText} />
                <div ref={this.refSensorName} className={styles.sensorName + " " + styles.hidden}>{selectedPOIName}</div>
                <button ref={this.refMovePOI} className={styles.menuBtnSensor + " " + styles.hidden} onClick={() => this.movePOI()}>{i18n.t('sdms.sdmsMainMenu.적용')}</button>
                <div className={styles.horzArea}>
                    ...
                    <button ref={this.refBuildingBtn} className={styles.menuBtnBuildingName} onClick={() => this.moveBuilding(this.state.currentBuildingName)}>{i18n.t('sdms.sdmsMainMenu.건물 이동')}</button>
                </div>
                <label ref={this.refCheckEquipZone} className={commonStyles.clickable}>
                    ...
                    {i18n.t('sdms.sdmsMainMenu.설비 영역 보기')}
					</label>
                <div className={sensorClassName}>
                    <button ref={this.refSensorBtn} className={styles.menuBtnSensor} onClick={() => this.moveSensor()}>{i18n.t('sdms.sdmsMainMenu.센서 이동')}</button>
                    <button className={prevClassName} onClick={() => this.showNextSensor(currentSensors, -1)}>{i18n.t('sdms.sdmsMainMenu.이전 센서')}</button>
                    <button className={nextClassName} onClick={() => this.showNextSensor(currentSensors, 1)}>{i18n.t('sdms.sdmsMainMenu.다음 센서')}</button>
                    <button className={alarmClassName} onClick={() => this.showAlarm()}>{i18n.t('sdms.sdmsMainMenu.알람 발생')}</button>
                </div>
            </div>
        );
    }
...
export default hoistStatics(withTranslation()(SDMSMainMenu), SDMSMainMenu);