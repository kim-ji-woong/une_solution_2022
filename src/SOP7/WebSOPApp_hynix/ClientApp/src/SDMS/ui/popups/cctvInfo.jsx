...
class CCTVInfo extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            popupMinWidth: 360,
            popupMinHeight: 380,
            refreshTime: 1000 * 60 * 5,     // 5분 (자동 새로고침 시간)
            reload: 0
        };
    ...
    componentDidMount() {
        ...
        // go2rtc 영상서버 관련 이슈 (간혈적으로 전 영상화면을 표시)
        if (ProjectResource.SiteID === ProjectResource.Site.Wonik || ProjectResource.SiteID === ProjectResource.Site.Soulbrain) {
            setTimeout(() => this.showCCTVs(), 1500);
        }
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        if (this.props.cctvList !== prevProps.cctvList) {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.Wonik) {
                setTimeout(() => this.showCCTVs(), 3000);
            }
...
export default hoistStatics(withTranslation()(CCTVInfo), CCTVInfo);