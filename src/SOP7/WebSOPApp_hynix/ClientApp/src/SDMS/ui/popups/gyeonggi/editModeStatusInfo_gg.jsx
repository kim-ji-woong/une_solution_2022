...
class EditModeStatusInfo_gg extends Component {
    constructor(props) {
        super(props);
        this.state = {
            searchText: '',
    ...
    getBuildingGroupUI() {
        ...
        for (var i = 0; i < buildingGroupList.length; i++) {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
                // 현재 선택된 입주기관이 종합방재실일 경우 외부영역만 표출
    ...
    getPOISensorTypeGG = () => {
        ...
        const { useSensorTypes, selectSiteID } = this.props;
        ...
        let visibleBellPOI = this.props.visibleSensorTypes[SDMSMainMenu.Emergency_Sensor] ? true : false;  
        let visibleParkPOI = this.props.visibleSensorTypes[SDMSMainMenu.Park] ? true : false;  
        let visibleLifePOI = this.props.visibleSensorTypes[SDMSMainMenu.Life] ? true : false; 
        let visibleCardiacPOI = this.props.visibleSensorTypes[SDMSMainMenu.Cardiac] ? true : false; 
        let visibleRescuePOI = this.props.visibleSensorTypes[SDMSMainMenu.Rescue] ? true : false; 
    
        let visibleEditCCTVClassName = (visibleCCTVPOI) ? 'visibleEditCCTV' : 'disableEditCCTV';
        let visibleEditFireClassName = (visibleFirePOI) ? 'visibleEditFire' : 'disableEditFire';
        let visibleEditBellClassName = (visibleBellPOI) ? 'visibleEditEmergencyBell' : 'disableEditEmergencyBell';
        let visibleEditParkClassName = (visibleParkPOI) ? 'visibleEditPark' : 'disableEditPark';
        let visibleEditLifeClassName = (visibleLifePOI) ? 'visibleEditLife' : 'disableEditLife';
        let visibleEditCardiacClassName = (visibleCardiacPOI) ? 'visibleEditCardiac' : 'disableEditCardiac';
        let visibleEditRescueClassName = (visibleRescuePOI) ? 'visibleEditRescue' : 'disableEditRescue';

        if (selectSiteID === ProjectResource.Site.GG_A) {
            poiSensorTypeUI.push(
                <li className={'poiSensorLi'} key='poiSensorType_cctv'><div className={visibleEditCCTVClassName} checked={visibleCCTVPOI} title="CCTV" onClick={() => this.toggleButton(SDMSMainMenu.CCTV_Type)}></div></li>
            );

            if(useSensorTypes?.UseParkingBreaker === true){
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_parkingBreaker'><div className={visibleEditParkClassName} checked={visibleParkPOI} title="주차차단기" onClick={() => this.toggleButton(SDMSMainMenu.Park)}></div></li>);
            }

            if(useSensorTypes?.UseLifeSaving === true){
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_lifeSaving'><div className={visibleEditLifeClassName} checked={visibleLifePOI} title="인명구조기구" onClick={() => this.toggleButton(SDMSMainMenu.Life)}></div></li>);
            }
    
            if(useSensorTypes?.UseCardiacDefibrillator === true){
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_cardiacDefibrillator'><div className={visibleEditCardiacClassName} checked={visibleCardiacPOI} title="심장제세동기" onClick={() => this.toggleButton(SDMSMainMenu.Cardiac)}></div></li>);
            }
    
            if(useSensorTypes?.UseRescueTeam === true){
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_rescueTeam'><div className={visibleEditRescueClassName} checked={visibleRescuePOI} title="완강기" onClick={() => this.toggleButton(SDMSMainMenu.Rescue)}></div></li>);
            }
        }
            ...
            if (useSensorTypes?.UseFire === true) {
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_fire'><div className={visibleEditFireClassName} checked={visibleFirePOI} title="화재센서" onClick={() => this.toggleButton(SDMSMainMenu.Fire_Sensor)}></div></li>);
            }
    
            if(useSensorTypes?.UseEmergencyBell === true){
                poiSensorTypeUI.push(<li className={'poiSensorLi'} key='poiSensorType_emergencyBell'><div className={visibleEditBellClassName} checked={visibleBellPOI} title="비상벨" onClick={() => this.toggleButton(SDMSMainMenu.Emergency_Sensor)}></div></li>);
            }
                ...
                <div className={'edTgt'}>
                    ...
                    <div className={'poiModeWrap'}>
                        ...
                        <ul className={'poiMode'}>
                            ...
                            {
                                selectSiteID === ProjectResource.Site.GG_B ?
                                <li><input type="radio" id="addIcon" onChange={() => this.onClickMode(EditModeManager.AddIcon)} checked={poiEditMode === EditModeManager.AddIcon} /><label htmlFor="addIcon" className={'poiModeText'}>{i18n.t('sdms.editModeStatusInfo.추가')}</label></li>
                                :<></>
                            }
                        </ul>
                    </div>
...
export default withTranslation()(EditModeStatusInfo_gg);