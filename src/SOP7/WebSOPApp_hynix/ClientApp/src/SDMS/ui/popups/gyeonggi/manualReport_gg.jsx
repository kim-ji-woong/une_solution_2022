...
class ManualReport_gg extends Component {
	constructor(props) {
		super(props);
    ...
    async onReport() {
        ...
        const siteID = this.props.getZoneSiteID(this.state.selectedZoneID); 

        const result = await SDMSController.requestManualReport(
            date,
            this.state.selectedFacilityType,
            this.getManualSensorZoneID(siteID),
    ...
    getManualSensorZoneID(siteID) {
        let sensorZoneID = this.state.selectedFacilityType * 10 + 1000000;

        if (siteID <= ProjectResource.Site.GG_B) {
            sensorZoneID += (siteID - ProjectResource.Site.GG_A);
        }
        else {
            sensorZoneID += (siteID - ProjectResource.Site.GG_A - 1);
        }
    ...
    onChangeFacilityType = (value) => {
        this.setState({ 
            selectedFacilityType: Number(value),
            selectedBuildingGroupID:this.state.selectedBuildingGroupID,
        });
    ...
    render() {
        ...
        facilityTypeUI.push(<option key='facilityType_fire' value={SdmsResource.facilityType.FIRE}>화재</option>);
        facilityTypeUI.push(<option key='facilityType_blackout' value={SdmsResource.facilityType.BLACKOUT}>전력</option>);
        facilityTypeUI.push(<option key='facilityType_waterLevel' value={SdmsResource.facilityType.WaterLevel}>침수</option>);
        facilityTypeUI.push(<option key='facilityType_earthquake' value={SdmsResource.facilityType.Earthquake}>지진</option>);
        facilityTypeUI.push(<option key='facilityType_terror' value={SdmsResource.facilityType.Terror}>테러</option>);
        facilityTypeUI.push(<option key='facilityType_psm' value={SdmsResource.facilityType.PSM_SENSOR}>가스누출</option>);
                    ...
                    {/* <div className={content.boxTypeBluee}> */}
                            ...
                            <colgroup>
                                ...
                                <tbody>
                                        ...
                                        <td>
                                            <>
                                                <ul className={'tel3col'}>
                                                    <li>
                                                        {
                                                            this.state.earthquakeCheck ?
                                                                <p className={'buildingGroupText'}>경기융합타운</p>
                                                            :
                                                            <>
                                                                {
                                                                ...
                                                                }
                                                                {
                                                                    ...
                                                                    <p className={'buildingGroupText'}>경기도청/도의회</p>
                                                                } 
                                                                {
                                                                    userInfo.siteID === ProjectResource.Site.GG_D &&
                                                                    <p className={'buildingGroupText'}>경기도서관</p>
                                                                }  
                                                                {
                                                                    userInfo.siteID === ProjectResource.Site.GG_E &&
                                                                    <p className={'buildingGroupText'}>복합시설관</p>
                                                                }
                                                                {
                                                                    userInfo.siteID === ProjectResource.Site.GG_F &&
                                                                    <p className={'buildingGroupText'}>경기신용보증재단</p>
                                                                }  
                                                                {
                                                                    userInfo.siteID === ProjectResource.Site.GG_G &&
                                                                    <p className={'buildingGroupText'}>경기도교육청</p>
                                                                }  
                                                                {
                                                                    userInfo.siteID === ProjectResource.Site.GG_H &&
                                                                    <p className={'buildingGroupText'}>경기주택도시공사 신사옥</p>
                                                                }       
                                                            </>
                                                        }
                                                    </li>
                                                    <li>
                                                        {
                                                            this.state.earthquakeCheck ?
                                                                <p className={'buildingBlankText'}>-</p> :
                                    ...
                                    <td>
                                        <textarea cols="10" rows="5" className={'w100p' + " " + 'menualTextarea'} onChange={(e) => this.onChangeMemo(e.target.value)} value={this.state.selectedMemo} style={{ resize: 'none' }}>
                            ...
                            <a 
                                onClick={this.state.isFirst ? () => this.onReport() : undefined}
                                className="btnOrange"
                            >
                                {i18n.t('sdms.manualReport.수동 신고')}
                            </a>
...
export default withTranslation()(ManualReport_gg);