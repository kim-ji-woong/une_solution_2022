...
import EntrantInfo from './popups/entrantInfo';
import EntranRoutetInfo from './popups/entranRoutetInfo';
import ForcedDoorOpenInfo from './popups/foredDoorOpenInfo';
import NotPermittedItemInfo from './popups/notPermittedItemInfo';
...
class SDMS extends Component {
    static menu = {
        ...
        // 하이닉스 -------------------------
        entrantInfo: SDMSResource.menu.출입자_상세정보,
        entranRoutetInfo: SDMSResource.menu.출입자_경로_상세보기,
        forcedDoorOpenInfo: SDMSResource.menu.출입거부_이후_강제_문열림,
        notPermittedItemInfo: SDMSResource.menu.물품_상세보기,
        hynixCCTV: SDMSResource.menu.하이닉스_CCTV,
    }
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            popupLayer: {
                ...
                entrantInfoZIndex: 0,
                entranRoutetInfoZIndex: 0,
                forcedDoorOpenInfoZIndex: 0,
                notPermittedItemInfoZIndex: 0,
            },
            popupState: {},
            selectedPOI: null,
            selectedFacility: {
    ...
    initVisibleSensorTypes() {
        ...
        visibleSensorTypes[SDMSMainMenu.Hynix_Door] = true;
        visibleSensorTypes[SDMSMainMenu.Hynix_CardReader] = true;
        visibleSensorTypes[SDMSMainMenu.Hynix_SmartTagReader] = true;
    ...
    componentDidMount() {
        ...
        visiblePopups[SDMS.menu.entrantInfo] = false;
        visiblePopups[SDMS.menu.entranRoutetInfo] = false;
        visiblePopups[SDMS.menu.forcedDoorOpenInfo] = false;
        visiblePopups[SDMS.menu.notPermittedItemInfo] = false;
    ...
    */
    ...
    async changeAlarm(storeValue) {
        if (!storeValue || storeValue?.actionType !== 'SENSOR_ALARM') return;
        ...
        if (selectedAlarm?.isAlarm) {
            this.setHinixCCTVInfo(selectedAlarm.facilityType);

            // 강제 문열림 알람
            if (selectedAlarm.facilityType === SDMSResource.facilityType.ForcedDoorOpen) {
                this.setVisiblePopups(SDMS.menu.forcedDoorOpenInfo, true);
                this.setVisiblePopups(SDMS.menu.entrantInfo, false);
                this.setVisiblePopups(SDMS.menu.entranRoutetInfo, false);
                this.setVisiblePopups(SDMS.menu.notPermittedItemInfo, false);
            }
            // 대리 태깅, 꼬리 물기, 사원증 도용, 이상행위자, 무인 보안 검색 우회 및 적발, 비인가 구역 출입
            else if (
                selectedAlarm.facilityType === SDMSResource.facilityType.CheatedTagging ||
                selectedAlarm.facilityType === SDMSResource.facilityType.Untagging ||
                selectedAlarm.facilityType === SDMSResource.facilityType.StealCard ||
                selectedAlarm.facilityType === SDMSResource.facilityType.Stranger ||
                selectedAlarm.facilityType === SDMSResource.facilityType.EvasionItem ||
                selectedAlarm.facilityType === SDMSResource.facilityType.NotPermittedPerson
            ) {
                this.setVisiblePopups(SDMS.menu.entrantInfo, true);
                this.setVisiblePopups(SDMS.menu.forcedDoorOpenInfo, false);
                this.setVisiblePopups(SDMS.menu.entranRoutetInfo, false);
                this.setVisiblePopups(SDMS.menu.notPermittedItemInfo, false);
            }
            // 비인가 구역 반입
            else if (selectedAlarm.facilityType === SDMSResource.facilityType.NotPermittedItem) {
                this.setVisiblePopups(SDMS.menu.notPermittedItemInfo, true);
                this.setVisiblePopups(SDMS.menu.forcedDoorOpenInfo, false);
                this.setVisiblePopups(SDMS.menu.entrantInfo, false);
                this.setVisiblePopups(SDMS.menu.entranRoutetInfo, false);
            }
    ...
    setHinixCCTVInfo = (facilityType, idx) => {
        let cctvNo = null;

        if (facilityType === SDMSResource.facilityType.Stranger && idx === undefined) return;

        if (facilityType === SDMSResource.facilityType.Stranger && idx) {
            cctvNo = String(idx);
        }
        else if (facilityType === SDMSResource.facilityType.ForcedDoorOpen) {
            cctvNo = "13";
        }
        else if (facilityType === SDMSResource.facilityType.CheatedTagging) {
            cctvNo = "14";
        }
        else if (facilityType === SDMSResource.facilityType.Untagging) {
            cctvNo = "15";
        }
        else if (facilityType === SDMSResource.facilityType.StealCard) {
            cctvNo = "16";
        }
        else if (facilityType === SDMSResource.facilityType.EvasionItem) {
            cctvNo = "17";
        }
        else if (facilityType === SDMSResource.facilityType.NotPermittedPerson) {
            cctvNo = "18";
        }
        else if (facilityType === SDMSResource.facilityType.NotPermittedItem) {
            cctvNo = "19";
        }

        this.setVisiblePopups(SDMS.menu.hynixCCTV, true);
        this.setState({ hynixCCTV: cctvNo });
    }
        ...
        for (let k = 0; k < returnAlarm.length; k++) {
            for (let i = 0; i < returnAlarm[k].alarmSensorZoneIDs.length; i++) {
                ...
                if (sensorZoneID < 1000000) {
                    ...
                    if (returnAlarm[k].facilityType < SDMSResource.facilityType.ForcedDoorOpen) {
            ...
            for (let i = 0; i < moveToSensor.length; i++) {
                for (let j = 0; j < moveToSensor[i].alarmSensorZoneIDs.length; j++) {
                    ...
                    if (sensorZoneID < 1000000) {
                        ...
                        if (moveToSensor[i].facilityType < SDMSResource.facilityType.ForcedDoorOpen) {
                        ...
                        this.processMenu(SDMSMainMenu.Menu_Show_Alarm, [moveToSensor[i].zoneID, SDMS.getFacilityType(moveToSensor[i].facilityType), nOrgSensorID, moveToSensor[i].sensorZoneHistoryID, moveToSensor[i].alarmDepth, moveToSensor[i].isAlarm, null]);
    ...
    async onClickMalfunction(index) {
        ...
        if (alarm.facilityType === SDMSResource.facilityType.ForcedDoorOpen) {
            this.setVisiblePopups(SDMS.menu.forcedDoorOpenInfo, false);
        }
        else if (
            alarm.facilityType === SDMSResource.facilityType.CheatedTagging ||
            alarm.facilityType === SDMSResource.facilityType.Untagging ||
            alarm.facilityType === SDMSResource.facilityType.StealCard ||
            alarm.facilityType === SDMSResource.facilityType.Stranger ||
            alarm.facilityType === SDMSResource.facilityType.EvasionItem ||
            alarm.facilityType === SDMSResource.facilityType.NotPermittedPerson
        ) {
            this.setVisiblePopups(SDMS.menu.entrantInfo, false);
        }
        else if (alarm.facilityType === SDMSResource.facilityType.NotPermittedItem) {
            this.setVisiblePopups(SDMS.menu.notPermittedItemInfo, false);
        }
    ...
    async requestSensorList() {        
        ...
        if (result === null) {
            ...
            if (result.doors) {
                sensorList[SDMSMainMenu.Hynix_Door] = result.doors;
            }
            if (result.cardReaders) {
                sensorList[SDMSMainMenu.Hynix_CardReader] = result.cardReaders;
            }
            if (result.smartTagReaders) {
                sensorList[SDMSMainMenu.Hynix_SmartTagReader] = result.smartTagReaders;
            }
    ...
    setSensorList(site3dOptions, sensorList) {
        if (!sensorList || !site3dOptions) {
            ...
            const doors = sensorList[SDMSMainMenu.Hynix_Door];
            const cardReaders = sensorList[SDMSMainMenu.Hynix_CardReader];
            const smartTagReaders = sensorList[SDMSMainMenu.Hynix_SmartTagReader];
            ...
            if (doors) {
                this.setDoors(doors, site3dOptions);
            }
            if (cardReaders) {
                this.setCardReaders(cardReaders, site3dOptions);
            }
            if (smartTagReaders) {
                this.setSmartTagReaders(smartTagReaders, site3dOptions);
            }
        }
    }
    ...
    setDoors(doors, site3dOptions) {
        const sensorCount = doors.length;

        for (let i = 0; i < sensorCount; i++) {
            const sensor = doors[i];
            sensor.id = sensor.doorID;
            const zone = this.getZone(site3dOptions, sensor.zoneID);

            if (zone) {
                if (!zone.sensors.hynixDoor) {
                    zone.sensors.hynixDoor = [];
                }

                zone.sensors.hynixDoor.push(sensor);
            }
        }
    }

    setCardReaders(cardReaders, site3dOptions) {
        const sensorCount = cardReaders.length;

        for (let i = 0; i < sensorCount; i++) {
            const sensor = cardReaders[i];
            sensor.id = sensor.cardReaderID;
            const zone = this.getZone(site3dOptions, sensor.zoneID);

            if (zone) {
                if (!zone.sensors.hynixCardReader) {
                    zone.sensors.hynixCardReader = [];
                }

                zone.sensors.hynixCardReader.push(sensor);
            }
        }
    }

    setSmartTagReaders(smartTagReaders, site3dOptions) {
        const sensorCount = smartTagReaders.length;

        for (let i = 0; i < sensorCount; i++) {
            const sensor = smartTagReaders[i];
            sensor.id = sensor.smartTagReaderID;
            const zone = this.getZone(site3dOptions, sensor.zoneID);

            if (zone) {
                if (!zone.sensors.hynixSmartTagReader) {
                    zone.sensors.hynixSmartTagReader = [];
                }

                zone.sensors.hynixSmartTagReader.push(sensor);
            }
        }
    }
    ...
    static getFacilityType(facilityType) {
        ...
        if (SDMSResource.isPSMSensorType(facilityType)) {
            ...
            sensorType = SDMSMainMenu.Hynix_ForcedDoorOpen;
        }
        else if (facilityType === SDMSResource.facilityType.CheatedTagging) {
            sensorType = SDMSMainMenu.Hynix_CheatedTagging;
        }
        else if (facilityType === SDMSResource.facilityType.Untagging) {
            sensorType = SDMSMainMenu.Hynix_Untagging;
        }
        else if (facilityType === SDMSResource.facilityType.StealCard) {
            sensorType = SDMSMainMenu.Hynix_StealCard;
        }
        else if (facilityType === SDMSResource.facilityType.Stranger) {
            sensorType = SDMSMainMenu.Hynix_Stranger;
        }
        else if (facilityType === SDMSResource.facilityType.EvasionItem) {
            sensorType = SDMSMainMenu.Hynix_EvasionItem;
        }
        else if (facilityType === SDMSResource.facilityType.NotPermittedPerson) {
            sensorType = SDMSMainMenu.Hynix_NotPermittedPerson;
        }
        else if (facilityType === SDMSResource.facilityType.NotPermittedItem) {
            sensorType = SDMSMainMenu.Hynix_NotPermittedItem;
        }
    ...
    onSelectedAlarm(alarm) {
        ...
        const ENTRANT_KEEP_OPEN = new Set([
            SDMSResource.facilityType.CheatedTagging,
            SDMSResource.facilityType.Untagging,
            SDMSResource.facilityType.StealCard,
            SDMSResource.facilityType.Stranger,
            SDMSResource.facilityType.EvasionItem,
            SDMSResource.facilityType.NotPermittedPerson,
        ]);

        if (!ENTRANT_KEEP_OPEN.has(alarm.facilityType)) {
            this.setVisiblePopups(SDMS.menu.entrantInfo, false);
        }

        if (alarm.facilityType !== SDMSResource.facilityType.Stranger) {
            this.setVisiblePopups(SDMS.menu.entranRoutetInfo, false);
        }

        if (alarm.facilityType !== SDMSResource.facilityType.NotPermittedItem) {
            this.setVisiblePopups(SDMS.menu.notPermittedItemInfo, false);
        }
        ...
        if (visible === undefined) {
            if (menu instanceof Array) {
                ...
                if (menu === SDMS.menu.allCCTV || menu === SDMS.menu.cctv || menu === SDMS.menu.alarmCCTV1 ||
                    menu === SDMS.menu.alarmCCTV2 || menu === SDMS.menu.alarmCCTV3 || menu === SDMS.menu.hynixCCTV) {
                    // CCTV 뷰어 열기 이벤트일 경우
            ...
            if (menu instanceof Array) {
                ...
                // CCTV 뷰어가 다 닫혔을 경우
                if ((menu === SDMS.menu.cctv || menu === SDMS.menu.alarmCCTV1 || menu === SDMS.menu.alarmCCTV2 || menu === SDMS.menu.alarmCCTV3 || menu === SDMS.menu.hynixCCTV) &&
                    ...
                    menus[SDMS.menu.hynixCCTV] === false &&
    ...
    setPopupUI(visiblePopups) {
        ...
        if(!this.isEditMode() && visiblePopups[SDMS.menu.entrantInfo]) {
            popups.push(
                <EntrantInfo
                    key='sdms_popup_entrantInfo'
                    setVisiblePopups={this.setVisiblePopups}
                    setActiveDragPopup={this.setActiveDragPopup}
                    zIndex={this.state.popupLayer.entrantInfoZIndex}
                    popupType={SDMSResource.popupLayer.entrantInfo}
                    popupState={this.state.popupState.entrantInfo}
        ...
        if(!this.isEditMode() && visiblePopups[SDMS.menu.forcedDoorOpenInfo]) {
            popups.push(
                <ForcedDoorOpenInfo
                    key='sdms_popup_forcedDoorOpenInfo'
                    setVisiblePopups={this.setVisiblePopups}
                    setActiveDragPopup={this.setActiveDragPopup}
                    zIndex={this.state.popupLayer.forcedDoorOpenInfoZIndex}
                    popupType={SDMSResource.popupLayer.forcedDoorOpenInfo}
                    popupState={this.state.popupState.forcedDoorOpenInfo}
                    setPopupState={this.setPopupState}
                    selectSiteID={this.state.selectSiteID}
                    selectedAlarm={this.state.selectedAlarm}
                />
        ...
        if(!this.isEditMode() && visiblePopups[SDMS.menu.entranRoutetInfo]) {
            popups.push(
                <EntranRoutetInfo
                    key='sdms_popup_entranRoutetInfo'
                    setVisiblePopups={this.setVisiblePopups}
                    setActiveDragPopup={this.setActiveDragPopup}
                    zIndex={this.state.popupLayer.entranRoutetInfoZIndex}
                    popupType={SDMSResource.popupLayer.entranRoutetInfo}
                    popupState={this.state.popupState.entranRoutetInfo}
                    setPopupState={this.setPopupState}
                    selectSiteID={this.state.selectSiteID}
                    selectedAlarm={this.state.selectedAlarm}
                    setHinixCCTVInfo={this.setHinixCCTVInfo}
                />
            );
        }

        if(!this.isEditMode() && visiblePopups[SDMS.menu.notPermittedItemInfo]) {
            popups.push(
                <NotPermittedItemInfo
                    key='sdms_popup_entranRoutetInfo'
                    setVisiblePopups={this.setVisiblePopups}
                    setActiveDragPopup={this.setActiveDragPopup}
                    zIndex={this.state.popupLayer.notPermittedItemInfoZIndex}
                    popupType={SDMSResource.popupLayer.notPermittedItemInfo}
                    popupState={this.state.popupState.notPermittedItemInfo}
                    setPopupState={this.setPopupState}
                    selectSiteID={this.state.selectSiteID}
                    selectedAlarm={this.state.selectedAlarm}
                />
        ...
        if (!this.isEditMode() && visiblePopups[SDMS.menu.hynixCCTV]) {
            const hynixCCTVs = this.state.hynixCCTV;

            if (hynixCCTVs !== null && hynixCCTVs.length > 0) {
                popups.push(
                    <CCTVInfo key='sdms_popup_hynixCCTV'
                        setVisiblePopups={this.setVisiblePopups}
                        streamServerURL={this.state.streamServerURL}
                        cctvs={this.state.sensorList['cctvs']}
                        setActiveDragPopup={this.setActiveDragPopup}
                        zIndex={this.state.popupLayer.cctvInfoZIndex}
                        popupType={SDMSResource.popupLayer.hynixCCTV}
                        popupState={this.state.popupState.hynixCCTV}
                        setPopupState={this.setPopupState}
                        cctvFullScreenState={this.state.cctvFullScreenState}
                        setCctvFullScreenState={this.setCctvFullScreenState}
                        setCCTVList={this.setCCTVList}
                        ...
                        editModeCCTV={this.state.editModeCCTV}
                        setEditModeCCTV={this.setEditModeCCTV}
                        menu={SDMS.menu.hynixCCTV}
                        selectedCCTVID={this.state.hynixCCTV}
                        cctvList={this.state.hynixCCTV}
                    />
                );
            }
        }
    ...
    showAlarmCCTVPopup(index, menu, visiblePopups, popups) {
        if (visiblePopups[SDMS.menu.allCCTV] && visiblePopups[menu] && !this.isEditMode()) {
            ...
            // 꼬리물기, 대리태깅... 신규 알람은 알람 CCTV창 4개 안나오게
            if (alarm[1].facilityType >= SDMSResource.facilityType.ForcedDoorOpen &&
                alarm[1].facilityType <= SDMSResource.facilityType.CardReader) {
                return;
            }
    ...
    getAlarmData = (sensorZoneHistoryID) => {
        const sensorAlarms = [...this.state.sensorAlarms];

        for (const alarm of sensorAlarms) {
            if (alarm.sensorZoneHistoryID === sensorZoneHistoryID) {
                return alarm;
            }
        }
