...
export class Contents3D extends Component {
    ...
    static processMenu(props, state) {
        ...
        if (props.command) {
            if (props.command.menu == SDMSMainMenu.Menu_Save_BuildingGroup_Viewport) {
                ...
                const [zoneID, sensorType, sensorID, sensorZoneHistoryID, alarmLevel, isAlarm, targetSensorID] = props.command.menuParameter;
    ...
    setLazyAlarmInfo(zoneID, sensorType, sensorID, alarmLevel, isAlarm, sensorZoneHistoryID) {
        // 실내모델 파일 로딩이 끝나지 않아서 보여주지 못했던 알람정보
        this.lazyAlarmData = {
            zoneID: zoneID,
    ...
    showAlarm(zoneID, sensorType, sensorID, sensorZoneHistoryID, alarmLevel, isAlarm, selectedPOI, alarmInfo, dontMoveToPOI = false) {
        ...
        const alarm = sensorZoneHistoryID ? this.props.getAlarmData(sensorZoneHistoryID) : null;
        sensorType = this.getAlarmSensorType(alarm, sensorType);

        let sensor = this.getAlarmSensor(alarm, zoneID, sensorType, sensorID);//this.getSensor(zoneID, sensorType, sensorID);
        ...
        if ((!model || !sensor)) {
            ...
            if (isIndoor === false) {
                ...
                this.setLazyAlarmInfo(zoneID, sensorType, sensorID, alarmLevel, isAlarm, sensorZoneHistoryID);
            }
        }
        else {
            if (!model.camera) {
            ...
            if (sensorPOI) {
                ...
                if (sensor) {
                    if (sensorType === SDMSMainMenu.Etc_Sensor || sensorType === SDMSMainMenu.PSM_Sensor ||
                        ...
                        sensorPOI = this.poiManager.addSensor(sensorType, sensor.id, sensor.x, sensor.y, sensor.z, zoneID, true);
    ...
    getAlarmSensorType(alarm, sensorType) {
        if (alarm?.facilityType) {
            if (alarm.facilityType === SdmsResource.facilityType.ForcedDoorOpen) {
                return SDMSMainMenu.Hynix_Door;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.CheatedTagging) {
                return SDMSMainMenu.Hynix_CardReader;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.Untagging) {
                return SDMSMainMenu.Hynix_SmartTagReader;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.StealCard) {
                return SDMSMainMenu.Hynix_CardReader;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.Stranger) {
                return SDMSMainMenu.Hynix_CardReader;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.EvasionItem) {
                return SDMSMainMenu.Hynix_CardReader;
            }
            else if (alarm.facilityType === SdmsResource.facilityType.NotPermittedPerson) {
                return SDMSMainMenu.Hynix_SmartTagReader;
            }
    ...
    getAlarmSensor(alarm, zoneID, sensorType, sensorID) {
        if (alarm?.facilityType && alarm?.targetSensorID) {
            if (alarm.facilityType === SdmsResource.facilityType.ForcedDoorOpen) {
                return this.getSensor(zoneID, SDMSMainMenu.Hynix_Door, alarm.targetSensorID);
            }
            else if (alarm.facilityType === SdmsResource.facilityType.CheatedTagging) {
                return this.getSensor(zoneID, SDMSMainMenu.Hynix_CardReader, alarm.targetSensorID);
            }
            else if (alarm.facilityType === SdmsResource.facilityType.Untagging) {
                return this.getSensor(zoneID, SDMSMainMenu.Hynix_SmartTagReader, alarm.targetSensorID);
            }
            else if (alarm.facilityType === SdmsResource.facilityType.StealCard) {
                return this.getSensor(zoneID, SDMSMainMenu.Hynix_CardReader, alarm.targetSensorID);
            }
        ...
        return this.getSensor(zoneID, sensorType, sensorID);
    }
    ...
    getAlarmElements() {
        ...
        if (alarmDepth === Contents3D.ALARM_2) {
            ...
            // selectedAlarm.facilityType에 따른 시나리오 알람사운드 적용 
            src = this.getHynixAlarmSound(this.props.selectedAlarm.facilityType, src);
