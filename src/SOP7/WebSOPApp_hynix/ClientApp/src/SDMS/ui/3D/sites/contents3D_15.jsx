...
import { SensorInfoBoxBlue, SensorInfoBoxRed } from '../../../styled/sdmsPopupsStyled';
import { i18n, withTranslation, i18nUtil } from '../../../../language/i18n';

export class Contents3D_15 extends Component {
    constructor(props) {
        super(props);
        ...
        this.state =
        {
            ...
            progressActive: ProjectResource.siteID === ProjectResource.Site.Hydrogen ? false : true,
            progressValue: 0,
            editableInput: false,
            commonSettings: {},
        ...
        // Key : BuildingGroupID
        this.sceneVisibleBgModels = {};
        // Key : BuildingID
        this.sceneVisibleBModels = {};
        // Key : ZoneID
        this.sceneVisibleZNormalModels = {};
        this.sceneVisibleZRedModels = {};
        this.sceneVisibleEtc = null;
    ...
    componentDidMount() {
        ...
        if (_3dOptions.outdoorModel) {
            ...
            this.resizeMethod = () => Contents3D_15.onWindowResize(this.camera, this.renderer);
        ...
        if (ProjectResource.SiteID === ProjectResource.Site.Hydrogen) {
            POIManager.OutdoorPoiScale = 1;
            POIManager.SelectedPoiScale = 1;
        }
    }
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.textPOIManager && prevState.curLanguage !== i18n.language.toString()) {
            this.textPOIManager.visibleTextLayers(i18n.language.toString(), this.props.currentSiteID);
            this.setState({ curLanguage: i18n.language.toString() });
        }
    }
    ...
    static getDerivedStateFromProps(props, state) {
        ...
        /*const movingCamera = */Contents3D_15.processMenu(props, state);
    ...
    static processMenu(props, state) {
        ...
        if (props.isEditMode) {
            Contents3D_15.editModeProcessMenu(props, state);
            return;
        }
        else {
        ...
        if (props.command) {
            if (props.command.menu == SDMSMainMenu.Menu_Save_BuildingGroup_Viewport) {
                ...
                state.prevInstance._moveToBuildingGroup();
                //movingCamera = true;
            }
                ...
                const buildingGroup = props.command.menuParameter;

                if (buildingGroup) {
                    if (Array.isArray(buildingGroup) === false) {
                        state.prevInstance.moveToBuildingGroup(buildingGroup);
                    }
                    else if (buildingGroup.length > 0) {
                        state.prevInstance.moveToBuildingGroup(buildingGroup[0]);
                    }
                }
                else {
                ...
                const building = props.command.menuParameter;
                state.prevInstance.moveToBuilding(building);
                //movingCamera = true;
            }
                ...
                const [zone, floorIndex] = props.command.menuParameter;

                if (zone !== null && zone.buildingID !== undefined && zone.buildingID !== null &&
                    floorIndex !== undefined && floorIndex !== null && isNaN(floorIndex) === false) {
                    const zoneID = state.prevInstance.moveToFloor(zone.buildingID, floorIndex, zone.id);
    ...
    onCompleteOutdoorModelLoading(modelNode, _3dOptions) {
        ...
        this.makeModelStructure();
    }

    // 수소 충전소 > 현황정보 트리에서 선택한 설비 visible scene 구성
    async makeModelStructure() {
        const userInfo = ProjectResource.getUserInfo();
        const dbModels = await SDMSDataManager.getGltfModelList(userInfo.id, this.loadingSiteIDs)

        let check = false;
        for (const sceneChild of this.scene.children) {            
            for (const dbModel of dbModels) {
                if (sceneChild.name === dbModel.modelDatas[0].modelDisplayText) { // main.glb
                    const sceneMain = sceneChild.children[0].children[0];

                    for (const modelB of dbModel.childModels) {
                        for (const mainChild of sceneMain.children) {
                            if (modelB.modelName === mainChild.name) {            // electric resolver
                                this.sceneVisibleBModels[modelB.modelDatas[0].buildingID] = mainChild;

                                for (const modelZ of modelB.childModels) {
                                    for (const ele of mainChild.children) {
                                        if (modelZ.modelName + "_normal" === ele.name) {
                                            this.sceneVisibleZNormalModels[modelZ.modelDatas[0].zoneID] = ele;
                                        } else if (modelZ.modelName + "_red" === ele.name || modelZ.modelName + "_red_(1)" === ele.name) {
                                            this.sceneVisibleZRedModels[modelZ.modelDatas[0].zoneID] = ele;
                                            ele.visible = false; // 초기 red는 비활성화
                                        }
                                    }
                                }
                                //this.buildingModels[1].visible = false;
                            }

                            if (mainChild.name === "etc") {
                                this.sceneVisibleEtc = mainChild;
                            }
    ...
    loadRootModel(contents, nextIndex, files, mode, visible, _3dOptions) {
        ...
        loader.load(fileName, function (object) {
            ...
            if (mode === Contents3D.Mode_Outdoor_All || mode === contents.Mode_Outdoor_Part) {
                ...
                _this.textPOIManager.addBuildingText(_3dOptions.buildings, _3dOptions.siteID, _this.props.site3dOptions);
                Contents3D_15.hideBoundingBoxes(modelNode, _3dOptions.buildingGroups, _3dOptions.buildings);
            }
            ...
            if (isIndoor === false) {
                _this.poiManager.addOutdoorSensors(Contents3D_15.getOutdoorZones(_3dOptions.outdoorZones, _this.props.currentSiteID), _3dOptions.zones, _this.props.visibleSensorTypes);
        ...
        loader.load(fileName, function (object) {
            ...
            obj.traverse((child) => {
                ...
                if (child instanceof THREE.Mesh) {
                    ...
                    const localBox = new THREE.Box3();
                    localBox.expandByObject(child);
                    Contents3D_15.setWorldBox(localBox, worldBox, child);
                }
            });
            ...
            if (mode === Contents3D.Mode_Outdoor_Part/*visible*/) {
                ...
                const facilityGroup = Contents3D_15.showFacilities(modelNode, false, _this.facilityMaps);
                ...
                const exitArrowData = Contents3D_15.showExit(modelNode, false);
    ...
    static showFacilities(modelNode, visible, facilityMaps) {
        ...
        for (let i = 0; i < childCount; i++) {
            const child = Contents3D_15.showFacilities(modelNode.children[i], visible, facilityMaps);
    ...
    static showExit(modelNode, visible) {
        if (modelNode.name.startsWith(Contents3D.ExitArrowGroupTag)) {
            ...
            return [modelNode, Contents3D_15.setArrowDatas(modelNode)];
        }

        const childCount = modelNode.children.length;

        for (let i = 0; i < childCount; i++) {
            const child = Contents3D_15.showExit(modelNode.children[i], visible);
    ...
    moveCamera(delta) {
        ...
        if (this.movingCamera) {
            ...
            if (this.movingCamera.elapsedTime >= this.movingCamera.movingTime) {
                ...
                Contents3D_15.setCamera(this.camera, this.controls, cameraOptions);
                //this.camera.updateProjectionMatrix();
    ...
    _moveToBuildingGroup(buildingGroup) {
        for (const [bKey, bValue] of Object.entries(this.sceneVisibleBModels)) {
            bValue.visible = true;
        }

        for (const [bKey, bValue] of Object.entries(this.sceneVisibleZRedModels)) {
            bValue.visible = false;
        }

        if (this.sceneVisibleEtc !== null) {
            this.sceneVisibleEtc.visible = true;
        }

        this.setVisibleSensorPOI(null, true);
        this.textPOIManager.setBuildingTextAllVisible(true);
    }
    ...
    moveToBuilding(buildings) {
        ...
        if (!Object.keys(this.sceneVisibleBModels).includes(building.id.toString())) {
            return;
        }
        ...
        this.sceneVisibleEtc.visible = false;

        this.setVisibleSensorPOI(building.id, false);
        this.textPOIManager.setBuildingTextVisibleUsingID(building.id, true, true);
    }
    ...
    moveToFloor(buildingID, floorIndex, zoneID) {
        if (!zoneID) {
            return;
        }

        if (!Object.keys(this.sceneVisibleBModels).includes(buildingID.toString())) {
            return;
        }

        for (const [bKey, bValue] of Object.entries(this.sceneVisibleBModels)) {            
            if (Number(bKey) === buildingID) {
                bValue.visible = true;
            }
            else {
                bValue.visible = false;
        ...
        if (!Object.keys(this.sceneVisibleZRedModels).includes(zoneID.toString())) {
            return;
        }

        for (const [bKey, bValue] of Object.entries(this.sceneVisibleZRedModels)) {
            if (Number(bKey) === zoneID) {
                bValue.visible = true;
            }
            else {
                bValue.visible = false;
        ...
        this.setVisibleSensorPOI(buildingID, false);
        this.sceneVisibleEtc.visible = false;
        this.textPOIManager.setBuildingTextVisibleUsingID(buildingID, true, true);
        return zoneID;
    }

    setVisibleSensorPOI(buildingID, visibleTrue) {
        let chkFire = false;
        let chkPsm = false;
        let chkEtc = false;
        let building = null;
        ...
        // 빌딩 그룹 이동시 모든 poi visible true할거임
        if (buildingID !== null) {
            building = this.props._3dOptions.buildingIDs[buildingID];
            if (!building) {
                return;
            }
        ...
        const sceneChildLength = this.scene.children.length;
        for (let i = 0; i < sceneChildLength; i++) {
            const child = this.scene.children[i];
            if (child.name === "sensors_fire" || child.name === "sensors_psm" || child.name === "sensors_etc") {
                if (child.name === "sensors_fire") {
                    chkFire = true;
                } else if (child.name === "sensors_psm") {
                    chkPsm = true;
                } else if (child.name === "sensors_etc") {
                    chkEtc = true;
                }

                const childLength = child.children.length;
                for (let j = 0; j < childLength; j++) {
                    let sensorScene = child.children[j];
                    if (!visibleTrue) {
                        let sensorNms = sensorScene.name.split("_");
                        if (sensorNms.length !== 3) {
                            continue;
                        }

                        const zID = Number(sensorNms[1]);
                        const orgID = Number(sensorNms[2]);
                        let match = false;
                        for (const [bKey, bValue] of Object.entries(zones)) {
                            if (zID === Number(bKey)) {
                                match = true;
                                break;
                            }
                        ...
                        if (match) {
                            sensorScene.visible = true;
                        }
                        else {
                            sensorScene.visible = false;
                        }
                    }
                    else {
                        sensorScene.visible = true;
    ...
    _moveToSensor(zone, model, isIndoor, sensorType, sensorID, zoneID) {
        if (model) {
            ...
            //this.setVisibleSensorPOI(Number(zone[1]), false);
        }
    }
    ...
    getSensorCameraOption(sensor, modelCamera, isIndoor) {
        ...
        const movePos = isIndoor ? [-1.37737, 21.332775, 13.8244] : [-1.37737, 21.332775, 13.8244];//[-5.783151245, 67.45516205, 47.26660156];
    ...
    onClick = (event) => {
        ...
        const poi = this.poiManager.getPOI(event, this.camera, this.props.editMode === Contents3D.Edit_Mode_Text, this.props.editModeParam === TextPOIManager.Mode_RemoveArea);
    ...
    onClickPOI(poi, event) {
        ...
        /*const cameraOption = this.getSensorCameraOption(this.toSensorData(poi.object), this.props._3dOptions.outdoorModel.camera, false);

        if (cameraOption) {
            this.setMovingCamera(cameraOption, Contents3D.Mode_Outdoor_All, null, null);
        }*/
        ...
        if (type === SDMSMainMenu.Fire_Sensor) {
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (Number(id) === sensor.id) {
                    ...
                    if (!zone) {
                        ...
                        datas.push([i18n.t('common.위치') + ' : ' + i18nUtil.convertText(zone.name), true, null]);
                    } else {
                        datas.push([i18n.t('common.위치') + ' : ' + i18nUtil.convertText(zone[3]), true, null]);
                    }


                    if (sensor.sensorSubType === 0) {
                        datas.push([i18n.t('sdms.contents3D_15.감지기 종류') + ' : ' + i18n.t('materialType.열 감지기'), true, null]);
                    } else if (sensor.sensorSubType === 1) {
                        datas.push([i18n.t('sdms.contents3D_15.감지기 종류') + ' : ' + i18n.t('materialType.연기 감지기'), true, null]);
                    } else if (sensor.sensorSubType === 2) {
                        datas.push([i18n.t('sdms.contents3D_15.감지기 종류') + ' : ' + i18n.t('materialType.불꽃 감지기'), true, null]);
                    } else {
                        datas.push([i18n.t('sdms.contents3D_15.감지기 종류') + ' : ' + i18n.t('materialType.일반 감지기'), true, null]);
                    }

                    arrInfo[0] = i18n.t('sdms.contents3D_15.센서정보');
                    //arrInfo[1] = sensor.id + ". " + sensor.name;
                    arrInfo[1] = i18nUtil.convertText(sensor.name);
                    arrInfo[2] = datas;
                    
                    /*if (sensor.departmentPhoneNumber) {
                        datas.push(['중계기 번호 : ' + sensor.departmentPhoneNumber, true, null]);
                    }*/
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (Number(id) === sensor.id) {
                    ...
                    if (!zone) {
                        ...
                        if (sensor.zoneID === 27) {
                            datas.push([i18n.t('common.위치') + ' : Dispenser ' + i18nUtil.convertText(zone[3]), true, null]);
                        }
                        else {
                            datas.push([i18n.t('common.위치') + ' : ' + i18nUtil.convertText(zone[3]), true, null]);
                        }
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (Number(id) === sensor.id) {
                    ...
                    if (sensor.id === 1) {
                        datas.push([i18n.t('sdms.contents3D_15.센서 종류') + ' : Gas leak detector', true, null]);
                    }
                    else {
                        datas.push([i18n.t('sdms.contents3D_15.센서 종류') + ' : ' + typeName, true, null]);
                    }
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (Number(id) === sensor.id) {
                    ...
                    if (sensor.cameraName.indexOf('PTZ') > 0) {
                        datas.push([i18n.t('sdms.contents3D_15.CCTV 종류') + ' : PTZ', true, null]);
                    }
                    else {
                        datas.push([i18n.t('sdms.contents3D_15.CCTV 종류') + ' : ' + sensor.type, true, null]);
                    }
                    ...
                    if (sensor.cameraCompanyName && sensor.cameraCompanyName.length > 0) {
                        datas.push([i18n.t('sdms.contents3D_15.제조사') + ' : ' + sensor.cameraCompanyName, true, null]);
                    }

                    if (sensor.cameraModelName && sensor.cameraModelName.length > 0) {
                        datas.push([i18n.t('sdms.contents3D_15.모델명') + ' : ' + sensor.cameraModelName, true, null]);
                    }
                ...
                if (this.props.editMode === Contents3D.Edit_Mode_Text && this.props.editModeParam === TextPOIManager.Mode_AddArea) {
                    ...
                    if (SDMSMainMenu.isEquipZoneText(poi?.object?.name)) {
                        ...
                        if (equipZoneName) {
                            ...
                            if (this.equipZoneAreaManager.checkDoubleArea(equipZoneID)) {
                                const _message = [equipZoneName + ' ' + i18n.t('sdms.contents3D_15.영역이 존재합니다')];
                                this.props.showConfirmDialog(i18n.t('sdms.contents3D_15.중복'), _message, null, null);
                                return;
                            }
                            ...
                            const message = [equipZoneName + ' ' + i18n.t('sdms.contents3D_15.영역을 생성하시겠습니까?')];
                            this.props.showConfirmDialog(i18n.t('sdms.contents3D_15.영역 생성'), message, [i18n.t('sdms.contents3D_15.예'), i18n.t('sdms.contents3D_15.아니오')], this.onClickAddAreaYesNo);
                        }
                    }  
                    ...
                    if (SDMSMainMenu.isEquipZoneArea(poi?.object?.name)) {
                        ...
                        if (equipZoneName) {
                            ...
                            this.confirmDialogData = poi;
                            const message = [equipZoneName + ' ' + i18n.t('sdms.contents3D_15.영역을 삭제하시겠습니까?')];
                            this.props.showConfirmDialog(i18n.t('sdms.contents3D_15.영역 삭제'), message, [i18n.t('sdms.contents3D_15.예'), i18n.t('sdms.contents3D_15.아니오')], this.onClickRemoveAreaYesNo);
                        }
                    }  
                    ...
                    if (this.doesClickEquipZoneTextInEditTextMode(poi)) {
                        // .TODO: this.props.editModeManager.movePoiMode() 확인 후에 호출되는 것에 대해서 수정이 필요한 부분 
                        // >> POI 편집에서 다른 모드로 전환 후 구역명 모드로 돌아오면 아무 동작이 안됨
                    ...
                    if (sensorType === SDMSMainMenu.CCTV_Type ||
                        ...
                        if (cctv) {
                            this.confirmDialogData = poi;
                            const cameraInfo = cctv.id + '.' + cctv.cameraName;
                            const message = [i18n.t('sdms.contents3D_15.선택한 CCTV를 삭제하시겠습니까?', { cameraName: cameraInfo })]//[`선택한 cctv(${cctv.id}.${cctv.cameraName})를 삭제하시겠습니까?`];
                            this.props.showConfirmDialog(i18n.t('common.확인'), message, [i18n.t('sdms.contents3D_15.예'), i18n.t('sdms.contents3D_15.아니오')], this.onClickDeleteCCTVYesNo);
                        }
                    }
    ...
    getBuildingModel(buildingName) {
        if (!this.props._3dOptions.indoorModels) {
            return;
        }
    ...
    showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, selectedPOI) {
        ...
        this.moveToFloor(_3dOptions.zones[zoneID][1], null, zoneID);
    }
    ...
    addAlarm(zoneID, sensorType, sensorID, alarmLevel) {
        ...
        if (this.currentModel) {
            ...
            if (model && model !== null && model.name !== this.currentModel.name) {
                this.hideAlarms();
            }
    ...
    showIndoor(modelFile, modelDescription, cameraOptions, zoneID, selectedPOI) {
        ...
        if (this.props?.currentView?.zoneID === zoneID && this.prevIndoorModel && this.prevIndoorModel.name === modelFile && this.currentIndoorModel && this.currentIndoorModel.name === modelFile) {
            if (Geometry.getDistance3(this.camera.position.x, this.camera.position.y, this.camera.position.z, cameraOptions.position[0], cameraOptions.position[1], cameraOptions.position[2]) > Geometry.Tolerance ||
                ...
                const param = {
                    method: Contents3D_15.showZoneSensors,
                    methodParam: {
                        contents3D: this,
                        zoneID: zoneID
                    }
        ...
        // 영역 생성 관련
        this.equipZoneAreaManager.setZoneID(zoneID);
        ...
        if (this.lazyAlarmModelData && this.lazyAlarmModelData?.modelName === contents) {
            ...
            if (model) {
                ...
                if (sensorPOI) {
                    ...
                    if (sensor) {
                        if (lazyAlarmData.sensorType === "etc" || lazyAlarmData.sensorType === "psm") {
                ...
                if (lazyAlarmData.isAlarm) {
                    ...
                    if (alarmModels && alarmModels.length > 0 && sensorPOI) {
                        ...
                        this.setState({ alarm: lazyAlarmData.alarmLevel });
                    }
                }
            }
    ...
    showOutdoor(mode) {
        ...
        if (mode === Contents3D.Mode_Indoor && outdoorModels) {
            ...
            // 실내 센서들 제거
            this.poiManager.removeSensors(null);
            this.poiManager.addOutdoorSensors(Contents3D_15.getOutdoorZones(_3dOptions.outdoorZones, this.props.currentSiteID), _3dOptions.zones, this.props.visibleSensorTypes);
    ...
    getVisibleArea() {
        ...
        const useEquipZoneArea = Contents3D_15.useEquipZoneArea();
    ...
    getVisibleAssess() {
        ...
        const useEquipZoneAssess = Contents3D_15.useEquipZoneAssess();
    ...
    getPoiPopup() {
        if (this.props.selectedPOI && this.props.selectedPOI.length > 0) {
            if (!this.props.selectedSensor || !this.props.sensorAlarms || this.props.selectedSensor.length !== 3) {
                return;
            }

            let poiAlarm = null;
            const alarmLength = this.props.sensorAlarms.length;
            for (let i = 0; i < alarmLength; i++) {
                const alarm = this.props.sensorAlarms[i];
                if (alarm.isAlarm && alarm.orgSensorID === this.props.selectedSensor[2] && alarm.zoneID === this.props.selectedSensor[1]) {
                    if (alarm.facilityType === 0 && this.props.selectedSensor[0] === "fire") {
                        poiAlarm = alarm;
                        break;
                    } else if (alarm.facilityType === 21 && this.props.selectedSensor[0] === "etc") {
                        poiAlarm = alarm;
                        break;
                    } else if (alarm.facilityType === 11 && this.props.selectedSensor[0] === "psm") {
                        poiAlarm = alarm;
                        break;
                    }
                }
            ...
            const [x, y] = this.poiManager.getPoiScreenPosition(this.props.selectedPOI[0], 0.5, 0);
            if (x === null || y === null) {
                return null;
            }
            ...
            if (poiAlarm !== null) {
                let sensorName = '';
                let val = '';
                if (poiAlarm.positionName.indexOf('화재센서') > 0) {
                    sensorName = '화재센서'
                } else if (poiAlarm.positionName.indexOf('압력센서') > 0) {
                    sensorName = '압력센서'
                    val = '6MPa';
                } else if (poiAlarm.positionName.indexOf('온도센서') > 0) {
                    sensorName = '온도센서'
                    val ='60℃'
                } else if (poiAlarm.positionName.indexOf('긴급차단장치') > 0) {
                    sensorName = '긴급차단장치'
                } else if (poiAlarm.positionName.indexOf('유량센서') > 0) {
                    sensorName = '유량센서'
                    val = '50kg/hr';
                } else if (poiAlarm.positionName.indexOf('가스센서') > 0) {
                    sensorName = '가스센서'
                }

                return <SensorInfoBoxRed style={{ position: 'absolute', left: x + 'px', top: y + 'px' }}>
                    <span className={'sensorInfoTitleR'}>{sensorName}</span>
                    <span className={'sensorInfoContentsR'}>{val}</span>
                </SensorInfoBoxRed>;
            }
            else {
                let sensorName = '';
                let val = '';
                if (this.props.selectedSensor[0] === 'fire') {
                    sensorName = '화재센서';                    
                } else if (this.props.selectedSensor[0] === 'psm') {
                    sensorName = '가스센서';
                } else if (this.props.selectedSensor[0] === 'etc') {
                    if (this.props.selectedSensor[2] === 1 || this.props.selectedSensor[2] === 3 || this.props.selectedSensor[2] === 7 || this.props.selectedSensor[2] === 10 ||
                        this.props.selectedSensor[2] === 12 || this.props.selectedSensor[2] === 14 || this.props.selectedSensor[2] === 16 || this.props.selectedSensor[2] === 18 ||
                        this.props.selectedSensor[2] === 20 || this.props.selectedSensor[2] === 22 || this.props.selectedSensor[2] === 23 || this.props.selectedSensor[2] === 27) {
                        sensorName = '압력센서';                        
                        val = (Math.floor(Math.random() * 60) + 3) + 'MPa';
                    } else if (this.props.selectedSensor[2] === 2 || this.props.selectedSensor[2] === 24 || this.props.selectedSensor[2] === 29) {
                        sensorName = '긴급차단장치';
                    } else if (this.props.selectedSensor[2] === 4 || this.props.selectedSensor[2] === 6 || this.props.selectedSensor[2] === 8 || this.props.selectedSensor[2] === 11 ||
                        this.props.selectedSensor[2] === 13 || this.props.selectedSensor[2] === 15 || this.props.selectedSensor[2] === 17 || this.props.selectedSensor[2] === 19 ||
                        this.props.selectedSensor[2] === 21 || this.props.selectedSensor[2] === 26 || this.props.selectedSensor[2] === 28) {
                        sensorName = '온도센서';
                        val = (Math.floor(Math.random() * 40) + 30) + '℃';
                    } else if (this.props.selectedSensor[2] === 5 || this.props.selectedSensor[2] === 9 || this.props.selectedSensor[2] === 25) {
                        sensorName = '유량센서';
                ...
                return <SensorInfoBoxBlue style={{ position: 'absolute', left: x + 'px', top: y + 'px' }}>
                    <span className={'sensorInfoTitle'}>{sensorName}</span>
                    <span className={'sensorInfoContents'}>{val}</span>
                </SensorInfoBoxBlue>;
            }
        }
    ...
    render() {
        ...
        if (ProjectResource.SiteID >= ProjectResource.Site.Wonik && ProjectResource.SiteID <= ProjectResource.Site.Wonik_S) {
            this.textPOIManager.setVisibleAssessment(this.getVisibleAssess());  // 구역 등급 표시 설정
        }

        // 알람 영역 표시 여부 
        this.equipZoneAreaManager.showAlarmArea(this.props.selectedAlarm, (this.state.commonSettings?.UseAlarmArea === "true"));  // 알람 영역 표시
        ...
        if (Contents3D_15.useWorkerInfo()) {
            quickButtons.push(
        ...
        if (Contents3D_15.useEquipZoneAssess()) {
            quickButtons.push(
        ...
        //const poiPopup = this.getPoiPopup();
                        ...
                        </div>
                        {
                            ...
                            <>
                                ...
                                <ul ref={this.refQuickButton}>
                                    <li key={"liQuickKey_statusInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+1</span><a id={"dsBot_" + SdmsResource.popupLayer.statusInfo} className={this.getQuickButtonClassName(SDMS.menu.statusInfo) + " " + 'statusInfoIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.statusInfo)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.설비<br/>리스트') }}></em></span></a></li>
                                    <li key={"liQuickKey_dashboard"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+2</span><a id={"dsBot_" + SdmsResource.popupLayer.dashboard} className={this.getQuickButtonClassName(SDMS.menu.dashboard) + " " + 'dashboardIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.dashboard)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.대시<br/>보드') }}></em></span></a></li>
                                    <li key={"liQuickKey_eventInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+3</span><a id={"dsBot_" + SdmsResource.popupLayer.event} className={this.getQuickButtonClassName(SDMS.menu.eventInfo) + " " + 'eventIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.eventInfo)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.이벤트<br/>정보') }}></em></span></a></li>
                                    <li key={"liQuickKey_detectionInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+4</span><a id={"dsBot_" + SdmsResource.popupLayer.detectionInfo} className={this.getQuickButtonClassName(SDMS.menu.detectionInfo) + " " + 'detectionIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.detectionInfo)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.이상<br/>탐지') }}></em></span></a></li>
                                    <li key={"liQuickKey_simulationInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+5</span><a id={"dsBot_" + SdmsResource.popupLayer.simulationInfo} className={this.getQuickButtonClassName(SDMS.menu.simulationInfo) + " " + 'simulationIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.simulationInfo)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.시뮬<br/>레이션') }}></em></span></a></li>
                                    <li key={"liQuickKey_analysisInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+6</span><a id={"dsBot_" + SdmsResource.popupLayer.analysisInfo} className={this.getQuickButtonClassName(SDMS.menu.analysisInfo) + " " + 'analysisIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.analysisInfo)}><span><em dangerouslySetInnerHTML={{ __html: i18n.t('sdms.contents3D_15.위험도<br/>분석') }}></em></span></a></li>
                                    {                                        
...
export default withTranslation()(Contents3D_15);