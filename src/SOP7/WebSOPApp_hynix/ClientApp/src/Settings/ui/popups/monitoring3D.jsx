...
class Monitoring3D extends Component {
	...
	setContentUI = () => {
		...
		if (this.state.tabMenu === i18n.t('common.일반')) {
			contentUI = (
				<NormalTab key="NormalTab" settings={this.props.settings} onChangeNeedToSave={this.props.onChangeNeedToSave} selectedSiteID={this.props.selectedSiteID} deleteOption={this.props.deleteOption} onChangeDeleteOption={this.props.onChangeDeleteOption} />
			);
...
class NormalTab extends Component {
	constructor(props) {
		...
		this.refUseDeleteOptionDeleteAll = React.createRef();
		this.refUseDeleteOptionArchiveDelete = React.createRef();
	...
	componentDidUpdate(prevProps, prevState) {
		...
		if (prevProps.deleteOption !== this.props.deleteOption) {
			this.initDeleteOption();
		}
	}
	...
	initDeleteOption() {
		if (this.props.deleteOption === null || this.props.deleteOption === undefined)
			return;

		let deleteOption = this.props.deleteOption;

		if (deleteOption === SettingsResource.deleteOption.deleteAll) {
			this.refUseDeleteOptionDeleteAll.current.click();
		} else if (deleteOption === SettingsResource.deleteOption.archiveDelete) {
			this.refUseDeleteOptionArchiveDelete.current.click();
		}
	}
	...
	selectDeleteOption = (deleteOption) => {
		if (deleteOption === null || deleteOption === undefined)
			return;
		
		if (deleteOption === SettingsResource.deleteOption.deleteAll ||
			deleteOption === SettingsResource.deleteOption.archiveDelete) {
			this.props.onChangeDeleteOption(deleteOption);
		...
		return (
			<>
				<div className={'stgList'}>
					...
					<div className={'stgName' + " " + 'settingRadio'}>
						<h5>로그파일 삭제</h5>
						<span className={'stgTltp'} data-tooltip="보존기간이 경과한 로그파일에 대해 삭제 옵션을 설정합니다"></span>
						<input type="radio" ref={this.refUseDeleteOptionDeleteAll} name="deleteOption" id="deleteOption" onChange={() => this.selectDeleteOption(SettingsResource.deleteOption.deleteAll)}  />
						<span className={'white'}>&nbsp;&nbsp;전체 삭제&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>

						<input type="radio" ref={this.refUseDeleteOptionArchiveDelete} name="deleteOption" id="deleteOption" onChange={() => this.selectDeleteOption(SettingsResource.deleteOption.archiveDelete)} />
						<span className={'white'}>&nbsp;&nbsp;압축보관 후 삭제 &nbsp;&nbsp;</span>
					</div>
