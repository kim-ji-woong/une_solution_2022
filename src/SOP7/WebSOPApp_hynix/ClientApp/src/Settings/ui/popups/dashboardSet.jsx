...
class DashboardSet extends Component {
	...
	initData() {
		...
		let dashboardBegin = this.props.settings.properties.DashboardBegin;
		let dashboardEnd = this.props.settings.properties.DashboardEnd;
	...
	onChangeDashboardBegin = (date) => {
		...
		this.props.settings.properties.DashboardBegin = korFormat;
	}
	...
	onChangeDashboardEnd = (date) => {
		...
		this.props.settings.properties.DashboardEnd = korFormat;
	}
	...
	onClickDefault = () => {
		...
		this.props.settings.properties.DashboardBegin = "today";
		this.props.settings.properties.DashboardEnd = "today";
    }
	...
	onClickWeek = () => {
		...
		this.props.settings.properties.DashboardBegin = dateValue;
		this.props.settings.properties.DashboardEnd = todayValue;
	}
        ...
        return (
            <>
				<ul className={newStyles.stgTab + " " + newStyles.single}>
					<li><a href="#" className={newStyles.on}>{i18n.t('common.일반')}</a></li>
				</ul> 
				<div className={newStyles.stgList}>
					<div className={newStyles.stgName}>
						<h5>{i18n.t('setting.dashboardSet.표출 데이터 기간 설정')}</h5>
						<span className={newStyles.stgTltp} data-tooltip={i18n.t('setting.dashboardSet.표출 데이터 기간을 설정 합니다')}></span>
						<ul className={newStyles.stgProd}>
							...
							<li><a onClick={this.onClickDefault} >{i18n.t('setting.dashboardSet.기본값으로 변경')}</a></li>
							<li><input type="radio" name="stgDate" id="stgDate01" onClick={this.onClickToday} /><label htmlFor="stgDate01">{i18n.t('setting.dashboardSet.오늘')}</label></li>
							<li><input type="radio" name="stgDate" id="stgDate02" onClick={this.onClickWeek} /><label htmlFor="stgDate02">{i18n.t('setting.dashboardSet.1주')}</label></li>
							<li><input type="radio" name="stgDate" id="stgDate03" onClick={this.onClickMonth} /><label htmlFor="stgDate03">{i18n.t('setting.dashboardSet.1개월')}</label></li>
						</ul>
					</div>
				</div>
            </>
...
export default withTranslation()(DashboardSet);