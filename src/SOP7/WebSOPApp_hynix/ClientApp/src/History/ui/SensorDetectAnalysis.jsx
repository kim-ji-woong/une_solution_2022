...
class SensorDetectHistory extends Component {
	...
	componentDidMount() {
		...
		if (beginDate <= endDate) {
			this.getSensorAnalysisCondition(
				beginDate,
				endDate,
	...
	getSensorAnalysisCondition = async (beginDate, endDate, facilityType, buildingGroupID, buildingID, zoneID) => {
		const result = await HistoryController.requestSensorAnalysisCondition(beginDate, endDate, facilityType, buildingGroupID, buildingID, zoneID);
	...
	async display() {
		...
		if (this.state.searchText.trim() === '') {
			result = await HistoryController.DisplaySensorDetectAnalysis(
				beginDate,
				endDate,
				this.state.facilityType,
				buildingGroupID,
			...
			// 검색어 기반 분석 호출
			result = await HistoryController.DisplaySensorDetectAnalysisQuery(
	...
	async onClickDownload(isCheckedDownload) {
		const title = i18n.t('history.menu.센서 탐지 분석');
		...
		let content1 = i18n.t('history.formText.센서탐지분석 요약1', { selectedDate: this.state.selectedDate, searchZoneName: this.state.searchZoneName, allDetectCount: this.state.allDetectCount, allMalfunctionRate: this.state.allMalfunctionRate});
		let content2 = i18n.t('history.formText.센서탐지분석 요약2', { maxCountSensorName: ((this.state.maxCountSensorName && this.state.maxCountSensorName.length > 0) ? this.state.maxCountSensorName : '-') });
		...
		// column
		let columnRow = worksheet.addRow(['No', i18n.t('history.formText.유형'), i18n.t('common.위치'), i18n.t('history.formText.센서명'), i18n.t('history.formText.탐지 횟수'), i18n.t('history.formText.오작동'), i18n.t('history.formText.현장 복구'), i18n.t('history.formText.오작동률(%)')]);
	...
	getGridData() {
		...
		const data = {
			...
			datasets: [
				{
					label: i18n.t('history.formText.탐지 횟수'),
				...
				{
					type: 'line',
					label: i18n.t('history.formText.누적 탐지율'),
	...
	getSensorTypeUI = () => {
		...
		if (this.props.useSensorTypes?.UseFire === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType0" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.FIRE)} checked={this.state.facilityType === SdmsResource.facilityType.FIRE} /><label htmlFor="hscsType0">{i18n.t('facilityType.화재')}</label></li>);
		}
		if (this.props.useSensorTypes?.UsePSM === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType11" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.PSM_SENSOR)} checked={this.state.facilityType === SdmsResource.facilityType.PSM_SENSOR} /><label htmlFor="hscsType11">{i18n.t('facilityType.누출')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseETC === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType21" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.ETC)} checked={this.state.facilityType === SdmsResource.facilityType.ETC} /><label htmlFor="hscsType21">{i18n.t('facilityType.기타')}</label></li>);
		}

		if (this.props.useSensorTypes?.UseEnvironment === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType117" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.Environment)} checked={this.state.facilityType === SdmsResource.facilityType.Environment} /><label htmlFor="hscsType117">{i18n.t('facilityType.환경설비')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseManufacture === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType118" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.Manufacture)} checked={this.state.facilityType === SdmsResource.facilityType.Manufacture} /><label htmlFor="hscsType118">{i18n.t('facilityType.제조설비')}</label></li>);
		}
		...
		if (this.props.useSensorTypes?.UseEarthquake === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType50" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.Earthquake)} checked={this.state.facilityType === SdmsResource.facilityType.Earthquake} /><label htmlFor="hscsType50">{i18n.t('facilityType.지진')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseStrongWind === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType18" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.STRONG_WIND)} checked={this.state.facilityType === SdmsResource.facilityType.STRONG_WIND} /><label htmlFor="hscsType18">{i18n.t('facilityType.강풍')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseBlackOut === true) {
			sensorTypeUI.push(<li><input type="radio" name="hscsType" id="hscsType17" onChange={() => this.onClickFacilityType(SdmsResource.facilityType.BLACKOUT)} checked={this.state.facilityType === SdmsResource.facilityType.BLACKOUT} /><label htmlFor="hscsType17">{i18n.t('facilityType.정전')}</label></li>);
		}
								...
								<table>
									...
									<thead>
										<tr>
											...
											<th>{i18n.t('history.formText.탐지 횟수')}</th>
											<th>{i18n.t('history.formText.오작동')}</th>
											<th>{i18n.t('history.formText.현장 복구')}</th>
											<th>{i18n.t('history.formText.사용자 복구')}</th>
											<th>{i18n.t('history.formText.오작동률(%)')}</th>
										</tr>
									</thead>
									<tbody>
										{gridUI}
