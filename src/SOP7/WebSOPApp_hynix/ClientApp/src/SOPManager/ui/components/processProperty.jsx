...
class ProcessProperty extends Component {
	...
	static getDerivedStateFromProps(props, state) {
		...
		return {
			...
			teamTreeData: state.teamTreeData,
	...
	async slideCascade(element, bLoad) {
		...
		if (element === this.refReceivers.current) {			
			if (!bLoad && element.classList.contains(ProcessProperty.cssStyles.on)) {
				receiversOn = false;
			}
			else {
				if (!bLoad) {
		...
		for (let i = 0; i < missionCount; i++) {
			...
			if (mission.missionType === ProcessProperty.MissionNormalType) {
				if (nIndex < missionTextCount) {
					...
					mission.missionText = i18nUtil.convertJsonByLanguage(this.state.curLanguage, mission.missionText, missionText);
				}
			}
	...
	onClickAddExternalProgram(add) {
		...
		if (add) {
			...
			if (selectedIndex < 0 || selectedIndex >= programDatas.length) {
				alert(i18n.t('sopManager.formText.추가할 프로그램을 선택하세요'));
				return;
			}
			...
			for (let i = 0; i < parameterCount; i++) {
				...
				if (success === false) {
					alert(i18n.t('sopManager.formText.번째 전달인자 값을 확인하세요', {index:i}), i);
					return;
				}
				else {
					arrDatas.push(data);
						...
						<td className={ProcessProperty.cssStyles.tal}>
							<div className={"scroll-wrapper " + ProcessProperty.cssStyles.sprtTxt + " scrollbar-outer scroll-textarea"} id="pos_relative">
								<div className="scroll-content" id="mission_odd">
									<textarea name="" id="" cols="30" rows="10" className={ProcessProperty.cssStyles.sprtTxt + " scrollbar-outer"} value={i18nUtil.convertText(mission.missionText)} onChange={(event) => this.onMissionTextChange(event, mission)}></textarea>
								</div>
	   ...
	   if (externalProgram.showList && programCount === 0) {
			return (
				<div className={ProcessProperty.cssStyles.sprmDsc}>
					<p>{i18n.t('sopManager.formText.사용할 수 있는 외부 프로그램이 존재하지 않습니다')}</p>
				</div>
			);
	   }
				...
				<div className={ProcessProperty.cssStyles.sprCont + " " + bodyStyles.noDrag}>
					<div className={ProcessProperty.cssStyles.sprTop}>
						<div className={ProcessProperty.cssStyles.sprtTitle}>
							<h4>{i18n.t('sopManager.formText.프로세스')}</h4>
							<p>
						...
						<dl className={ProcessProperty.cssStyles.sprtIpt}>
							...
							<dd><input ref={this.refTitle} type="text" defaultValue={i18nUtil.convertText(this.state.sectionData?.text)} onChange={() => {}} /></dd>
						</dl>
						<dl className={ProcessProperty.cssStyles.sprtIpt}>
									...
									<dl className={ProcessProperty.cssStyles.sprmAcdn}>
										...
										<dd className={this.getReceiversClassName()}>
											<div className={ProcessProperty.cssStyles.sprmTeam}>
												...
												<TreeView treeViewID="sopPropertyTree" treeViewHeight={250} teamTreeData={this.state.teamTreeData} onTreeNodeChanged={this.onTreeNodeChanged} useCheckBox={ProjectResource.treeCascadeMode()} style={{ "height": "250px" }} />
											</div>
										</dd>
										<dt ref={this.refMissions} className={this.getMissionsClassName()} onClick={this.onClickCascade}>{i18n.t('sopManager.formText.임무 내용')}</dt>
										<dd className={this.getMissionsClassName()}>
											<div className={ProcessProperty.cssStyles.sprmUdn}>
												...
												<a className={ProcessProperty.cssStyles.del} onClick={this.onClickDel}>{i18n.t('sopManager.formText.선택 삭제')}</a>
											</div>
											<button className={ProcessProperty.cssStyles.sprmAdd} onClick={this.onClickAddMission}>+ {i18n.t('sopManager.formText.임무 내용 추가')}</button>
											<table className={ProcessProperty.cssStyles.sprmTb}>
												<colgroup>
											...
											<div className={ProcessProperty.cssStyles.sprmDsc}>
												<p dangerouslySetInnerHTML={{ __html: i18n.t('sopManager.formText.외부로 임무 내용이 전파될 수 있으므로, 개인정보 보호를 위해서 특정 개인의 정보 입력을 권장하지 않습니다') }}></p>
											</div>
											<a className={ProcessProperty.cssStyles.sprmIport} onClick={this.onClickExternalPrograms}>{i18n.t('sopManager.formText.외부 프로그램 불러오기')}</a>
											{
											...
											}
											{
												this.state.externalProgram.showList && this.state.externalProgram.datas.length > 0 && (
													<ul className={ProcessProperty.cssStyles.sprmBtn}>
														<li><a className={ProcessProperty.cssStyles.clickable} onClick={() => this.onClickAddExternalProgram(true)}>{i18n.t('common.추가')}</a></li>
														<li><a className={ProcessProperty.cssStyles.clickable} onClick={() => this.onClickAddExternalProgram(false)}>{i18n.t('common.취소')}</a></li>
													</ul>
												)
											}
										</dd>
					...
					<div className={ProcessProperty.cssStyles.sprBot}>
						<a className={ProcessProperty.cssStyles.clickable} onClick={() => this.onClickApply(true)}>{i18n.t('common.확인')}</a>
						<a className={ProcessProperty.cssStyles.clickable} onClick={() => this.onClickApply(false)}>{i18n.t('common.취소')}</a>
					</div>{/*sprBot*/}
				</div>
			);
		}
...
export default withTranslation()(ProcessProperty);