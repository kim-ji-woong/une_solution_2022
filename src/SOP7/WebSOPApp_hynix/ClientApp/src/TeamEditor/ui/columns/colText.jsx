...
class ColText extends Component {
    ...
    formatValue = (value) => {
        ...
        if (!value || !["휴대전화번호", "근무처 전화번호"].includes(this.props.columnName)) {
            return i18nUtil.convertText(value);
        }
        ...
        if (/^02\d{7}$/.test(number)) {
            ...
            return i18nUtil.convertText(formattedNumber);
        }
        ...
        return (
            this.props.isEditMode && this.state.columnName === this.props.editColumnName ?
                ...
                <td id={'td_' + this.props.colID} onMouseDown={() => this.onChangeEditMode(true)}>
                    <span className={styles.fixation + " " + teamEditors.colTextSpan}>{this.formatValue(this.state.value)}</span>
                </td>
            );
    }
}
