...
namespace WebSOPApp.Areas.SOPManager.Controllers
{
    ...
    public class SOPController : Controller
    {
        ...
        [HttpPost]
        public IActionResult UploadXML([FromBody] RequestSaveXml data)
        {
            ResponseOpen result = m_processManager.GetLoadManager().OpenXML(data.XMLData);

            if (result.Success == false)
                return Ok(new ResponseSaveXml(false, result.Message));

            result.SOPData.Version.OwnerID = data.UserID;
            ResponseSave result2 = m_processManager.GetSaveManager().SaveDB(data.UserID, result.SOPData);

            if (result2.Success == false)
                return Ok(new ResponseSaveXml(false, result2.Message));

            string strXmlPath = ResponseSaveXml.GetXmlPath(result.SOPData);

            ResponseSaveXml response = new ResponseSaveXml(true, "");
            response.XmlPath = strXmlPath;
            return Ok(response);
        }
