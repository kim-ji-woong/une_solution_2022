...
import { EtcSensorManager } from './etcSensorManager';
...
export class Contents3D extends Component {
    ...
    static Alarm_Model = ["Alarm_Level1.glb", "Alarm_Level1.glb", "Alarm_Level2.glb", "Alarm_Level3.glb"];
    ...
    // 마우스가 Panel 위에 1초 이상 머무르면 click 이벤트 자동 발생
    static PanelHoverTime = 1000;
    ...
    getOutdoorModelFiles(_3dOptions) {
        ...
        for (const buildingGroupName in _3dOptions.indoorModels) {
            ...
            if (buildingGroup.files) {
                for (const file of buildingGroup.files) {
                    modelFiles.push(file);
                }
            }
            else if (buildingGroup.file) {
                ...
                for (let i = 0; i < sensorCount; i++) {
                    ...
                    if (sensor.name === sensorName) {
                        if (sensor.x !== null && sensor.y !== null && sensor.z !== null) {
                            ...
                            return this.etcSensorManager.getZoneSensor(zoneID, sensorName);
                        }
                    }
                }
            }
    ...
    static processMenu(props, state) {
        ...
        state.prevInstance.checkChangeVisibleSensorTypes(props.visibleSensorTypes, state.prevProps.visibleSensorTypes);
        ...
        if (props.command) {
            if (props.command.menu == SDMSMainMenu.Menu_Save_BuildingGroup_Viewport) {
                ...
                if (selectedPOI && selectedPOI.length === 3) {
                    ...
                    props.setSelectedPOI(sensorType, sensorID, zoneID);
                    //props.setSelectedPOI(selectedPOI[0], selectedPOI[1], selectedPOI[2]);
                }
                //movingCamera = true;
            }
                ...
                const [zoneID, sensorType, facilityID] = props.command.menuParameter;
                state.prevInstance.checkAlarms(zoneID, props.selectedPOI, true);     // 현황정보에 설비를 클릭하여 이동시 알람표시가 뜨지 않아 추가 - K.D.R
    ...
    checkChangeVisibleSensorTypes(newVisibleSensorTypes, oldVisibleSensorTypes) {
        for (const sensorTypeName in newVisibleSensorTypes) {
            const newVisible = newVisibleSensorTypes[sensorTypeName];
            const oldVisible = oldVisibleSensorTypes[sensorTypeName];

            if (newVisible !== oldVisible) {
                if (newVisible === false) {
                    this.pipeManager.clearTypeBoundingBoxModels(sensorTypeName);
                }
            }
        }
    }
    ...
    checkAlarms(zoneID, selectedPOI, noMove) {
        ...
        for (let i = 0; i < alarmCount; i++) {
            ...
            if (alarm.isAlarm && alarm.zoneID === zoneID) {
                ...
                this.showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, selectedPOI, noMove);
                return;
            }
        }
    }
    ...
    init() {
        this.etcSensorManager = new EtcSensorManager(this.props._3dOptions, this.props.facilityInfos);
        this.internalModels = {};
    ...
    checkPanelMouseHover() {
        if (!this.lastMouseEvent) {
            return;
        }

        if (this.props.currentView.zoneID === 10) {
            const current = new Date();
            const timeSpan = current - this.lastMouseMoveTime;

            const event = this.lastMouseEvent;

            if (timeSpan > Contents3D.PanelHoverTime) {
                this.lastMouseEvent = null;
                const intersects = [];

                let clickedModel = this.pipeManager.pickModel(event, intersects, this.state.xRayView);
                const clickedSensor = this.etcSensorManager.onClick(event, this, intersects, clickedModel);

                if ((clickedModel && (clickedModel.name.startsWith(PipeManager.Panel1_Data.beginTag) || clickedModel.name.startsWith(PipeManager.Panel2_Data.beginTag))) ||
                    (clickedSensor?.sensor && clickedSensor.sensor.materialType === SdmsResource.materialType.PanelFacility)) {
                    this.props.showInfoBox(clickedModel?.name, clickedSensor?.sensor, event.nativeEvent.offsetX, event.nativeEvent.offsetY);
                }
                else {
                    this.props.showInfoBox(null, null, null, null);
                }
            }
        }
    }
    ...
    static animate(_this) {
        ...
        if (_this.renderer && _this.scene && _this.camera) {
            if (_this.etcSensorManager) {
                _this.etcSensorManager.setVisible(_this.props.visibleSensorTypes);
            }

            if (_this.state.xRayView) {
                if (_this.pipeManager.runPipeBall(_this.props.visibleSensorTypes, _this.props.currentView?.zoneID, _this.etcSensorManager?.selectedSensorModel, _this.props.selectedPOI, delta) === false)
                    _this.renderer.render(_this.scene, _this.camera);
            }
            else {
                if (_this.pipeManager.showEscape(_this.props.visibleSensorTypes, _this.props.currentView?.zoneID, delta) === false)
                    _this.renderer.render(_this.scene, _this.camera);
            }
        }
    ...
    isIndoor() {
        ...
        if (this.currentModel.name === this.props._3dOptions.outdoorModel.file ||
            this.currentModel.name === XRayManager.getXrayContentsFromModelName(this.props._3dOptions.outdoorModel.file)) {
            return false;
        }
    ...
    onCompleteOutdoorModelLoading(modelNode, _3dOptions) {
        ...
        if (XRayManager.isXrayModel(modelNode) === false) {
    ...
    loadRootModel(contents, nextIndex, files, mode, visible, _3dOptions) {
        ...
        loader.load(fileName, function (object) {
            ...
            _this.loadAnimationModels(object, modelNode, _3dOptions);
            ...
            _this.etcSensorManager.linkZoneSensors(modelNode, false, _3dOptions, mode);

            //_this.outdoorModelTotalCountTemp++;
            _this.onCompleteOutdoorModelLoading(modelNode, _3dOptions);

            _this.loadFile(XRayManager.getXrayContentsFromModel(modelNode), false, _3dOptions.outdoorModel.camera, mode, _3dOptions, XRayManager.setXrayModel, [modelNode, _this.xrayManager, _3dOptions, _this.pipeManager]);
    ...
    async loadAlarmModels(_3dOptions) {
        this.loadAnimationFile(Contents3D.Alarm_Model[Contents3D.ALARM_1 - 1], false, Contents3D.ALARM_1, _3dOptions);
                ...
                if (building && building.floors) {
                    ...
                    for (let i = 0; i < floorCount; i++) {
                        ...
                        if (floor.file && floor.camera) {
                            ...
                            if (floor.files) {
                                for (const file of floor.files) {
                                    this.loadFile(file, false, floor.camera, Contents3D.Mode_Indoor, _3dOptions);
                                }
                            }
                            else
    ...
    loadFile(contents, visible, cameraOptions, mode, _3dOptions, postMethod = null, postMethodParam = null) {
        ...
        if (this.internalTryModels[contents]) {
            this.onAfterLoadFile(contents, _3dOptions, mode, XRayManager.isXrayModel({name: contents}), postMethod, postMethodParam);
            return;
        }
        else {
        ...
        loader.load(fileName, function (object) {
            ...
            // AnimationModel이 있는지 확인한다.
            _this.loadAnimationModels(object, modelNode, mode, _3dOptions);

            const isXrayModeFile = XRayManager.isXrayModel(modelNode);

            if (mode === Contents3D.Mode_Outdoor_All || mode === contents.Mode_Outdoor_Part) {
                Contents3D.hideBoundingBoxes(modelNode, _3dOptions.buildingGroups, _3dOptions.buildings);
            }

            if (isXrayModeFile && mode === Contents3D.Mode_Outdoor_All) {
                _this.pipeManager.setOutdoorPipe(modelNode);
            }

            if (mode === Contents3D.Mode_Outdoor_Part/*visible*/) {
                if (isXrayModeFile === false) {
                    _this.onCompleteOutdoorModelLoading(modelNode, _3dOptions);
            ...
            _this.etcSensorManager.linkZoneSensors(modelNode, isXrayModeFile, _3dOptions, mode);

            _this.onAfterLoadFile(contents, _3dOptions, mode, isXrayModeFile, postMethod, postMethodParam);
            _this.timelog(contents);
            
            if (isXrayModeFile === false) {
                _this.loadFile(XRayManager.getXrayContentsFromModel(modelNode), false, cameraOptions, mode, _3dOptions, XRayManager.setXrayModel, [modelNode, _this.xrayManager, _3dOptions, _this.pipeManager]);
            }
        });
    }

    onAfterLoadFile(contents, _3dOptions, mode, isXrayModeFile, postMethod, postMethodParam) {
        if (mode === Contents3D.Mode_Indoor && this.indoorModelCount > 0 && isXrayModeFile === false) {
            ...
            if (rate >= 100) {
                ...
                this.xrayManager.setXRayModels();
        ...
        if (postMethod) {
            if (postMethodParam !== null) {
                postMethod(modelNode, postMethodParam);
            }
            else {
                postMethod(modelNode);
            }
        }
    }
    ...
    loadAnimationModels(object, modelNode, mode, _3dOptions) {
        if (!object?.animations) {
            return;
        }

        /*if (modelNode.name.startsWith("sphere_")) {
            // 외부모델은 전력선
            this.pipeManager.setPipeBall(modelNode, SDMSMainMenu.Electric_Sensor);
            return;
        }
        else
            return;*/

        if (mode === Contents3D.Mode_Outdoor_All && XRayManager.isXrayModel(modelNode)) {
            this.pipeManager.setPipeBalls(object.animations, null, modelNode, this.props.getEdmsFacility);
            return;
        }

        if (object.animations.length > 0) {
            const isXrayModeFile = XRayManager.isXrayModel(modelNode);
            const normalModelName = isXrayModeFile ? XRayManager.getNormalContentsFromModelName(modelNode.name) : modelNode.name;

            if (mode === Contents3D.Mode_Indoor) {
                const zoneID = this.getZoneIDFromModel(normalModelName, _3dOptions);

                if (zoneID === 0 || zoneID) {
                    this.pipeManager.setPipeBalls(object.animations, zoneID, modelNode, this.props.getEdmsFacility);
                }
            }
            // 애니메이션 모델이 나타나지 않도록 한다.
            /*const mixer = new THREE.AnimationMixer(modelNode);
    ...
    getZoneIDFromModel(modelNodeName, _3dOptions) {
        for (const buildingGroupName in _3dOptions.indoorModels) {
            const buildingGroupModel = _3dOptions.indoorModels[buildingGroupName];
    ...
    postMoveCamera(mode, fov, far, near, param) {
        if (mode === Contents3D.Mode_Indoor) {
            const prevIndoorModels = [...this.prevIndoorModels];

            for (const prevIndoorModel of prevIndoorModels) {
                prevIndoorModel.visible = true;
            }
    ...
    async saveIndoorModelViewport(camera, target) {
        ...
        const result = this.props.isEditMode ? await SDMSController.requestSaveOrthoModelViewport(modelName, cameraData, this.props.currentView.zoneID) : await SDMSController.requestSaveIndoorModelViewport(modelName, cameraData, this.props.currentView.zoneID);
    ...
    moveToSensor(zoneID, sensorType, sensorID) {
        ...
        if (model === this.currentModel || !isIndoor) {
            ...
            const param = {
                method: Contents3D.postMoveToIndoorNSelectSensorPOI,
                methodParam: {
                    contents3D: this,
                    zoneID: zoneID,
                    poi: poi
            ...
            if (this.props.currentView?.zoneID !== zoneID) {
                // 해당 센서의 위치로 이동
                const cameraOption = this.getSensorCameraOption(sensor, model, isIndoor);
                const result = this.showIndoor(model.file, model.modelDisplayText, cameraOption, zoneID, param);

                if (result !== 1 && poi) {
                    Contents3D.selectSensorPOI(this, poi);
                }
            ...
            if (sensors) {
                ...
                for (let i = 0; i < sensorCount; i++) {
                    ...
                    if (sensor.id === sensorID) {
                        ...
                        const parameter = {
                            method: Contents3D.selectSensorPOI,
                            methodParam: {
                                contents3D: this,
                                poi: poi
                            }
                        ...
                        if (!this.currentModel || this.currentModel.name !== model.file) {
                            if (isIndoor) {
                                ...
                                this.showIndoor(model.file, model.modelDisplayText, camera, zoneID, param);
                            }
                            else {
                                //this.showOutdoor(Contents3D.Mode_Outdoor_All);    // setMovingCamera 내에서도 showOutdoor 호출하여 주석처리 - K.D.R
                                this.setMovingCamera(camera, Contents3D.Mode_Outdoor_All, parameter);
                            }
                        }
                        else {
                            if (!isIndoor) {
                                ...
                                this.setMovingCamera(camera, Contents3D.Mode_Indoor, parameter);
                            }
                        }
    ...
    static selectSensorPOI(contents3D, poi) {
        contents3D.poiManager.selectPOI(poi, null, null);
    }

    moveToSensorModel(zoneID, sensorModel) {
        let isIndoor = true;
        let model = null;
        let zone = this.props._3dOptions.zones[zoneID.toString()];
        ...
        if (model === this.currentModel || !isIndoor) {
            this._moveToSensorModel(model, isIndoor, sensorModel, zoneID);
        }
        else {
            // 해당 센서의 위치로 이동
            const cameraOption = this.getSensorModelCameraOption(sensorModel, model, isIndoor);
            ...
            if (movingCameraParam) {
                // 카메라 이동이 끝난후 센서 위치로 이동하도록 한다.
                movingCameraParam.sensorModel = sensorModel;
            }
        }
    }

    _moveToSensorModel(model, isIndoor, sensorModel, zoneID) {
        const camera = this.getSensorModelCameraOption(sensorModel, model, isIndoor);
    ...
    moveToFacility(zoneID, facilityID) {
        ...
        if (facility) {
            ...
            sensorData = this.etcSensorManager.getZoneSensorFromModelName(facility.zoneID, facility.modelName);

            if (sensorData) {
                facility.id = sensorData.sensor?.id;
                facility.materialType = sensorData.sensor?.materialType;
            }

            this.setSelectedFacility(facility);
        }

        // 이전에 선택되었던 InfoBox는 사라지게 한다.
        this.props.showInfoBox(null, null, null);

        this.goToFacility(modelData, sensorData, zoneID);
        /*if (sensorData) {
            this.etcSensorManager.selectSensorModel(this.state.xRayView ? sensorData.model.xray : sensorData.model.normal, this.pipeManager, zoneID);
            this.moveCameraToTarget(null, sensorData);
        }
        else {
            this.etcSensorManager.selectSensorModel(null, this.pipeManager, zoneID);
        }*/
    }

    goToFacility(modelData, sensorData, zoneID) {
        if (!modelData) {
            if (this.props._3dOptions.outdoorZones[zoneID.toString()]) {
                this.showOutdoor(Contents3D.Mode_Outdoor_All);
            }
            ...
            if (sensorData) {
                const sensorModel = this.state.xRayView ? sensorData.model.xray : sensorData.model.normal;
                this.etcSensorManager.selectSensorModel(sensorModel, this.pipeManager, zoneID);
                const cameraOptions = this.moveCameraToTarget(null, sensorData, true);

                const parameter = {
                    method: Contents3D.postMoveToIndoorNSelectFacility,
                    methodParam: {
                        contents3D: this,
                        zoneID: zoneID,
                        model: sensorModel,
                ...
                if (cameraOptions) {
                    ...
                    _modelData[2] = cameraOptions;
                    result = this.showIndoor(_modelData[0], _modelData[1], _modelData[2], zoneID, parameter);
                    moveCamera = true;
                }
                else {
                    result = this.showIndoor(modelData[0], modelData[1], modelData[2], zoneID, parameter);
                }

                if (result !== 1) {
                    Contents3D.selectFacility(this, sensorModel, sensorData.sensor);
                }
            }
            else {
                this.etcSensorManager.selectSensorModel(null, this.pipeManager, zoneID);
    ...
    getSensorCameraOption(sensor, model, isIndoor) {
        ...
        if (usePoiFocus !== SettingResource.usePoiFocus.on) {
            ...
            const cameraData = sensor.edmsFacility?.cameraData;

            if (cameraData) {
                return {
                    far: cameraData.cameraFar,
                    fov: cameraData.cameraFov,
                    near: cameraData.cameraNear,
                    position: [cameraData.cameraPositionX, cameraData.cameraPositionY, cameraData.cameraPositionZ],
                    quaternion: [cameraData.cameraQuaternionX, cameraData.cameraQuaternionY, cameraData.cameraQuaternionZ, cameraData.cameraQuaternionW],
                    rotation: [cameraData.cameraRotationX, cameraData.cameraRotationY, cameraData.cameraRotationZ],
                    targetControl: [cameraData.orbitTargetX, cameraData.orbitTargetY, cameraData.orbitTargetZ]
                };
            }
            else {
                return model.camera;
    ...
    getSensorModelCameraOption(sensorModel, model, isIndoor) {
        const usePoiFocus = this.state.commonSettings?.UsePoiFocus;
        ...
        camera.position = [sensorModel.position.x + movePos[0], sensorModel.position.y + movePos[1], sensorModel.position.z + movePos[2]];
        camera.quaternion = null;
        camera.rotation = [...rotation];
        camera.targetControl = [sensorModel.position.x, sensorModel.position.y, sensorModel.position.z];
        //camera.targetControl = [...model.camera.targetControl];
        camera.fov = model.camera ? model.camera.fov : this.camera.fov;
        camera.near = model.camera ? model.camera.near : this.camera.near;
        camera.far = model.camera ? model.camera.far : this.camera.far;
    ...
    onClick = (event) => {
        const prevSelectedSensorModel = this.etcSensorManager.selectedSensorModel;

        this.optionManager.onPrevClick(event);
        this.etcSensorManager.selectSensorModel(null, this.pipeManager, this.props.currentView?.zoneID);
        ...
        const intersects = [];
        let clickedModel = null, clickedSensor = null;

        //if (this.state.xRayView) {
            clickedModel = this.pipeManager.onClick(event, intersects, this.state.xRayView);
        //}
        //else {
        //    this.props.showInfoBox(null, null, null);
        //}

        // 현재 활성화 상태인 배관에 연결된 설비를 다시 Click할 경우 해당 설비와 배관을 비활성화 상태로 만든다.
        if (PipeManager.isSelectedModel(clickedModel, prevSelectedSensorModel)) {
            if (clickedModel.userData.pair) {
                clickedModel.userData.pair.visible = false;
            }

            clickedModel = null;
        }
        else {
            clickedSensor = this.etcSensorManager.onClick(event, this, intersects, clickedModel);
        }

        this.props.showInfoBox(clickedModel?.name, clickedSensor?.sensor, event.nativeEvent.offsetX, event.nativeEvent.offsetY);

        const selectedSensorModel = this.state.xRayView ? clickedSensor?.model?.xray : clickedSensor?.model?.normal;
        this.etcSensorManager.selectSensorModel(selectedSensorModel, this.pipeManager, this.props.currentView?.zoneID);

        if (this.props.currentView?.zoneID) {
            this.pipeManager.checkSelectedModel(selectedSensorModel);
            this.moveCameraToTarget(clickedModel, clickedSensor);
        }

        /*if (clickedSensor) {
            this.props.showInfoBoxRow(clickedSensor, event.nativeEvent.offsetX, event.nativeEvent.offsetY);
        }
        else {
            this.props.showInfoBoxRow(null, event.nativeEvent.offsetX, event.nativeEvent.offsetY);
        }*/
    ...
    moveCameraToTarget(modelNode, sensorData, onlyData) {
        let cameraData = modelNode?.userData.edmsFacility.cameraData;

        if (!cameraData) {
            cameraData = sensorData?.sensor?.edmsFacility?.cameraData;
        }
        ...
        if (cameraData) {
            ...
            const mode = this.isIndoor() ? Contents3D.Mode_Indoor : Contents3D.Mode_Outdoor_All;
            this.setMovingCamera(cameraOptions, mode);
        }
    ...
    getFacility(facilityID) {
        const facility = this.props.getFacilityModel(facilityID);

        if (facility) {
            facility.name = facility.modelName;
        }

        return facility;

        /*if (modelfacilityName === null) {
            return null;
        }

        return this.facilityMaps[facility.modelName];*/
    }
    ...
    onClickPOI(poi, event) {
        ...
        else if (type === SDMSMainMenu.PSM_Sensor || type === SDMSMainMenu.Etc_Sensor) {
        ...
        if (this.props.editMode === Contents3D.Edit_Mode_MovePOI ||
            ...
            if (this.pickPOI) {
                ...
                if (this.props.editModeManager.movePoiMode()) {
                    ...
                    else if (this.doesClickEquipZoneSensorInEditTextMode(poi)) {
    ...
    showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, selectedPOI, noMove) {
        this.etcSensorManager.selectSensorModel(null, this.pipeManager, zoneID);
        ...
        if (!modelData) {
            if (this.indoorModelCountTemp < this.indoorModelCount) {
                ...
                const buildingGroupName = SDMSDataManager.getBuildingGroupNameFromZoneID(zoneID, this.props._3dOptions);
        ...
        if (isIndoor) {
            ...
            const outdoorModels = this.siteOutdoorModels[this.props._3dOptions.siteID];
        ...
        if (sensor) {
            if (sensorType === SDMSMainMenu.PSM_Sensor || sensorType === SDMSMainMenu.Etc_Sensor) {
        ...
        if ((!model || !sensor)) {
            ...
            if (isIndoor === false) {
                this.setLazyAlarmInfo(zoneID, sensorType, sensorID, alarmLevel, isAlarm);
            } else if (this.showIndoor(modelData[0], modelData[1], modelData[2], zoneID) === 0) {
                this.setLazyAlarmInfo(zoneID, sensorType, sensorID, alarmLevel, isAlarm);
            }
        }
        else {
            ...
            if (model.camera) {
                ...
                if (sensor) {
                    cameraOption = this.getSensorCameraOption(sensor, model, isIndoor);
                }
                else if (sensorType === "etc") {
                    sensorModel = this.etcSensorManager.getSensorModel(zoneID, sensorID, this.state.xRayView);

                    if (sensorModel) {
                        cameraOption = this.getSensorModelCameraOption(sensorModel, model, isIndoor);
                    }
                }

                if (cameraOption && !noMove) {
                    if (isIndoor === false) {
                        this.setMovingCamera(cameraOption, Contents3D.Mode_Outdoor_All, null);
                    } else if (isIndoor === true && this.showIndoor(modelData[0], modelData[1], cameraOption, zoneID) === 0) {
                        this.setLazyAlarmInfo(zoneID, sensorType, sensorID, alarmLevel, isAlarm);
                    }
                }
            }
        ...
        if (model) {
            ...
            if (sensorPOI) {
                ...
                if (!selectedPOI || /*selectedPOI === null ||*/ (selectedPOI && selectedPOI[0] === sensorType && selectedPOI[1] === sensorID && selectedPOI[2] === zoneID)) {
                    ...
                    if (movingCameraParam) {
                        // 카메라 이동이 끝난후 센서 위치로 이동하도록 한다.
                        movingCameraParam.methodParam.sensorType = sensorType;
                        movingCameraParam.methodParam.sensorID = sensorID;
                    }
                }
            }
            else {
                ...
                if (sensor) {
                    if (sensorType === "etc" || sensorType === "psm") {
                        ...
                        if (sensorPOIData) {
                            ...
                            if (!sensorPOI && (sensorPOIData.x === 0 || sensorPOIData.x) && (sensorPOIData.y === 0 || sensorPOIData.y) && (sensorPOIData.z === 0 || sensorPOIData.z)) {
                                sensorPOI = { position: { x: sensorPOIData.x, y: sensorPOIData.y, z: sensorPOIData.z }, virtual: true };
                            }
                        }
                    } else
                    ...
                    if (sensorModel) {
                        ...
                        if (movingCameraParam) {
                            // 카메라 이동이 끝난후 센서 위치로 이동하도록 한다.
                            movingCameraParam.methodParam.sensorModel = sensorModel;
                            //movingCameraParam.sensorModel = sensorModel;
                        }
                    }
                }

                if (sensorPOI && !sensorPOI.virtual) {
                    sensorPOI.visible = true;
                    ...
                    this.etcSensorManager.selectSensorModel(sensorModel, this.pipeManager, zoneID);
                    this.props.onSelectSensorModel(sensorModel, zoneID, sensorType, sensorID);
                }
                else if (sensorPOI && sensorPOI.virtual && sensor) {
                    this.props.selectFacility(sensor, true);
                }
            }
            ...
            if (isAlarm) {
                ...
                if (alarmModels && alarmModels.length > 0 && (sensorPOI || sensorModel)) {
                    this.hideAlarms();

                    const position = sensorPOI ? sensorPOI.position : Contents3D.getModelNodePosition(sensorModel);
                    const alarmModel = this.moveAlarmAnimation(position.x, position.y, position.z, alarmLevel, zoneID);
    ...
    static getModelNodePosition(modelNode, position) {
        if (!modelNode) {
            return position;
        }

        if (!position)
            position = new THREE.Vector3(modelNode.position.x, modelNode.position.y, modelNode.position.z);
        else
            position.set(position.x + modelNode.position.x, position.y + modelNode.position.y, position.z + modelNode.position.z);

        Contents3D.getModelNodePosition(modelNode.parent, position);
        return position;
    }
    ...
    addAlarm(zoneID, sensorType, sensorID, alarmLevel) {
        ...
        if (alarmModels && alarmModels.length > 0 && sensorPOI) {
            const alarmModel = this.moveAlarmAnimation(sensorPOI.position.x, sensorPOI.position.y, sensorPOI.position.z, alarmLevel, zoneID);
    ...
    moveAlarmAnimation(x, y, z, alarmLevel, zoneID) {
        const alarmModels = this.alarmModels[alarmLevel - 1];
        const modelCount = alarmModels.length;

        let scale = POIManager.OutdoorPoiScale;

        if (this.isIndoor()) {
            if (zoneID === 9) {
                // 8층
                scale = POIManager.IndoorPoiScale;
            }
            else if (zoneID === 10) {
                // 전기실
                scale = POIManager.IndoorPoiScale2;
            }
        }

        //const scale = this.isIndoor() ? POIManager.IndoorPoiScale2 : POIManager.OutdoorPoiScale;
    ...
    static showZoneSensors(param) {
        ...
        if (param.zoneID !== null && param.zoneID !== undefined) {
            const poiScale = param.zoneID === 10 ? POIManager.IndoorPoiScale2 : POIManager.IndoorPoiScale;
            contents3D.poiManager.addZoneSensors(param.zoneID, poiScale, contents3D.props._3dOptions.outdoorZones, contents3D.props._3dOptions.zones, contents3D.props.visibleSensorTypes);
            ...
            else if (param.sensorModel) {
                contents3D.moveToSensorModel(param.zoneID, param.sensorModel);
            }
        }
    }
    ...
    showIndoor(modelFile, modelDescription, cameraOptions, zoneID, parameter) {
        if (this.props.isEditMode === false && !this.isIndoor()) {
            this.setPrevOutdoorViewport();
        }

        if (this.props?.currentView?.zoneID === zoneID && this.prevIndoorModels.length > 0 && this.prevIndoorModels[0].name === modelFile && this.currentIndoorModel && this.currentIndoorModel.name === modelFile) {
            if (Geometry.getDistance3(this.camera.position.x, this.camera.position.y, this.camera.position.z, cameraOptions.position[0], cameraOptions.position[1], cameraOptions.position[2]) > Geometry.Tolerance ||
                ...
                const param = parameter ? parameter : {
                    method: Contents3D.postMoveToIndoor,
                    methodParam: {
                        contents3D: this,
                        zoneID: zoneID
                    }
        ...
        if (modelData) {
            ...
            if (this.state.xRayView)
                this.pipeManager.changePipes(zoneID);
            ...
            for (const prevIndoorModel of prevIndoorModels) {
                ...
                if (prevIndoorModel.userData.pair)
                    prevIndoorModel.userData.pair.visible = false;
            }
            ...
            if (this.xrayManager.changeViewMode(model, this.state.xRayView) === false) {
                model.visible = true;
            }
            ...
            // 한층에 여러개의 모델파일이 존재할 경우 modelFile 이외의 나머지 파일들도 불러온다.
            this.addOtherIndoorModels(modelFile, buildingName, zoneID, _3dOptions);
    ...
    // 한층에 여러개의 모델파일이 존재할 경우 modelFile 이외의 나머지 파일들도 불러온다.
    addOtherIndoorModels(modelFile, buildingName, zoneID, _3dOptions) {
        if (buildingName) {
            const indoorModels = _3dOptions.indoorModels[buildingName];

            if (indoorModels) {
                const indoorModel = indoorModels[buildingName];

                if (indoorModel?.floors) {
                    for (const floor of indoorModel.floors) {
                        if (floor.zoneID === zoneID) {
                            if (floor.files) {
                                for (const file of floor.files) {
                                    if (file !== modelFile) {
                                        const modelData = this.internalModels[file];

                                        if (modelData) {
                                            const model = modelData[0];
                                            this.prevIndoorModels.push(model);
                                            model.visible = true;
                                        }
                                    }
                                }
    ...
    static postMoveToIndoor(params) {
        if (params?.contents3D) {
            const _this = params.contents3D;
            _this.setLight(_this.state.xRayView);

            Contents3D.showZoneSensors(params);
        }
    }

    static postMoveToIndoorNSelectFacility(params) {
        Contents3D.postMoveToIndoor(params);
        Contents3D.selectFacility(params.contents3D, params.model, params.sensor);
    }

    static postMoveToIndoorNSelectSensorPOI(params) {
        Contents3D.postMoveToIndoor(params);
        Contents3D.selectSensorPOI(params.contents3D, params.poi);
    }

    static selectFacility(contents3D, model, sensor) {
        if (contents3D && model) {
            const [x, y] = contents3D.globalToScreen(model);

            contents3D.props.showInfoBox(model.name, sensor, x, y);
            contents3D.etcSensorManager.selectSensorModel(model, contents3D.pipeManager, contents3D.props.currentView?.zoneID);
            contents3D.pipeManager.checkSelectedModel(model);
        }
    }

    setLight(xRayView) {
        if (this.isIndoor()) {
            if (xRayView && this.props.currentView?.zoneName !== "전체층") {
                this.setXRayViewLight();
            }
            else {
                this.setNormalLight();
            }
    ...
    setXRayViewLight() {
        this.directionalLightPower = 106;
    ...
    showOutdoor(mode) {
        if (mode === Contents3D.Mode_Outdoor_All && this.state.xRayView)
            this.pipeManager.changePipes(null);
        ...
        if (mode === Contents3D.Mode_Indoor && outdoorModels) {
            outdoorModels.map(model => {
                ...
                if (model.userData.pair)
                    model.userData.pair.visible = false;
            });
        }
        else {
            const prevIndoorModels = [...this.prevIndoorModels];
            ...
            if (this.currentIndoorModel) {
                if (this.currentIndoorModel.userData.pair)
                    this.currentIndoorModel.userData.pair.visible = false;
            ...
            if (outdoorModels) {
                const xrayView = this.state.xRayView;

                outdoorModels.map(model => {
                    if (model.userData?.pair) {
                        if (model.userData?.xrayMode === xrayView) {
                            model.visible = true;
                            model.userData.pair.visible = false;
                        }
                        else {
        ...
        if (mode !== Contents3D.Mode_Indoor) {
            ...
            for (const prevIndoorModel of prevIndoorModels) {
                const animationModel = this.modelAnimations[prevIndoorModel.name];
        ...
        this.setLight(this.state.xRayView);
    }
    ...
    getCurrentBuildingFloors() {
        ...
        for (const zoneID in floors) {
            ...
            if (floorData.length > 0) {
                ...
                /*if (floorIndex < 0) {
                    if (floorData[3].includes('지하'))
                        floorName = '지하' + (-floorIndex) + "층";
                    else
                        floorName = floorData[3];
                }
                else
                    floorName = (floorIndex + 1) + "층";*/
    ...
    toggleViewMode = () => {
        const xRayView = !this.state.xRayView;
        this.xrayManager.toggleViewMode(xRayView);
        this.setLight(xRayView);

        if (xRayView) {
            this.pipeManager.changePipes(this.props.currentView?.zoneID);
        }
        else {
            this.pipeManager.clearAll();
        }

        const viewModeText = xRayView ? "투명모드" : "일반모드";
        this.setState({ xRayView, text: viewModeText });
    }
    ...
    render() {
        ...
        this.poiManager.selectPOI(selectedPOI, this.props.editMode, this.props.editModeParam);
                    ...
                    </div>
                    {
                        ...
                        <div id={styles.dsSoulBot}>
                            ...
                            <ul ref={this.refQuickButton}>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+7</span><a id={"dsBot_" + SdmsResource.popupLayer.weatherInfo} className={this.getQuickButtonClassName(SDMS.menu.weatherInfo)} onClick={() => this.setVisiblePopups(SDMS.menu.weatherInfo)}></a></li>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+1</span><a id={"dsBot_" + SdmsResource.popupLayer.statusInfo} className={this.getQuickButtonClassName(SDMS.menu.statusInfo)} onClick={() => this.setVisiblePopups(SDMS.menu.statusInfo)}></a></li>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+3</span><a id={"dsBot_" + SdmsResource.popupLayer.dashboard} className={this.getQuickButtonClassName(SDMS.menu.dashboard)} onClick={() => this.setVisiblePopups(SDMS.menu.dashboard)}></a></li>
                                <li><span className={styles.modeChange} onClick={() => this.toggleViewMode()}>{this.state.text}</span><a></a></li>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+4</span><a id={"dsBot_" + SdmsResource.popupLayer.event} className={this.getQuickButtonClassName(SDMS.menu.eventInfo)} onClick={() => this.setVisiblePopups(SDMS.menu.eventInfo)}></a></li>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+5</span><a id={"dsBot_" + SdmsResource.popupLayer.miniMap} className={this.getQuickButtonClassName(SDMS.menu.miniMap)} onClick={() => this.setVisiblePopups(SDMS.menu.miniMap)}></a></li>
                                <li><span className={"shortcutKey" + " " + styles.shortCut + " " + styles.hideKey}>Ct+9</span><a id={"dsBot_" + SdmsResource.popupLayer.workerInfo} className={this.getQuickButtonClassName(SDMS.menu.workerInfo)} onClick={() => this.props.setVisiblePopups(SDMS.menu.workerInfo)}></a></li> 
                            </ul>
                        </div>
                    }
                    {
