...
export class POIManager extends IOptionType {
    static OutdoorPoiScale = 16;
    static IndoorPoiScale = 8;
    static IndoorPoiScale2 = 1;
    ...
    registSelectedImage(imgURL) {
        ...
        spriteMaterialSelected.depthTest = false;
        spriteMaterialSelected.depthWrite = false;
    ...
    registDisabledImage(imgURL) {
        ...
        spriteMaterialDisabled.depthTest = false;
        spriteMaterialDisabled.depthWrite = false;
    ...
    registDisabledSelectedImage(imgURL) {
        ...
        spriteMaterialDisabledSelected.depthTest = false;
        spriteMaterialDisabledSelected.depthWrite = false;
    ...
    addSensor(sensorType, sensorID, x, y, z, zoneID, isIndoor) {
        // etc 센서는 표현하지 않는다.(modelNode로 표현되어 있다.)
        if (zoneID === null || zoneID === undefined || sensorType === SDMSMainMenu.Etc_Sensor) {
            return null;
        }
        ...
        const scale = isIndoor ? (zoneID === 10 ? POIManager.IndoorPoiScale2 : POIManager.IndoorPoiScale) : POIManager.OutdoorPoiScale;
    ...
    addSensors(sensorType, sensors, scale, zoneID) {
        ...
        if (!sensors || !zoneID || sensorType === SDMSMainMenu.Etc_Sensor) {
            return sprites;
        }
