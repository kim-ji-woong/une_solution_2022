...
class CCTVInfo extends Component {
    ...
    constructor(props) {
        ...
        this.props = props;
        this.alarmInfo = this.props.alarmInfo;
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (!this.refBody.current || nextProps.cctvList !== nextState.cctvList || this.isSameAlarmInfo(this.alarmInfo, nextProps.alarmInfo) === false) {
            this.alarmInfo = nextProps.alarmInfo;
            return true;
        }
    ...
    isSameAlarmInfo(alarmInfo1, alarmInfo2) {
        if (!alarmInfo1 || !alarmInfo2) {
            return false;
        }

        const count1 = alarmInfo1.length;
        const count2 = alarmInfo2.length;
        ...
        if (count1 === 1) {
            return alarmInfo1[0] === alarmInfo2[0];
        }

        return alarmInfo1[1].sensorZoneHistoryID === alarmInfo2[1].sensorZoneHistoryID;
    }
    ...
    */
    ...
    showCCTVs = () => {
        ...
        for (let i = 0; i < idCount; i++) {
            ...
            const cctv = this.getCCTV(suuid);

            if (cctv?.type && cctv.type.toString().toLowerCase() === "video") {
                this.showVideo(suuid, index, cctv);
            }
            else {
    ...
    showVideo(suuid, index, cctv) {
        ...
        if (video) {
            if (video.children.length > 0) {
                ...
                source.setAttribute("src", cctv.url);

                video.load();

                video.classList.remove(sdms.hidden);
                this.showCCTVInfo(suuid, "cctv" + index);
            }
        }

        const cctvElement = document.getElementById("cctv" + index);

        if (cctvElement) {
            cctvElement.classList.add(sdms.hidden);
        }
    }

    getCCTV(id) {
        const cctvs = this.props.cctvs;

        if (cctvs) {
            for (const cctv of cctvs) {
                if (cctv.id.toString() === id) {
                    return cctv;
                }
            }
        }
    ...
    render() {
        if (!this.state.cctvList || this.state.cctvList.length === 0) {
            return <></>
        }
        ...
        return (
            <div ref={this.refBody} id={this.props.popupType} className={content.viewDashboardBoxD + ' ' + content.viewDashboardCCTV}>
                <PopupDraggable
                                ...
                                <div className={content.viewDashboardCCTVGrid}>
                                    <div className={content.col1row1 + this.getFullScreenClassName(1)}>
                                        <span id="cctv1_span" onDoubleClick={(e) => this.showFullScreenCCTV(1)} onClick={() => this.onClickCCTV(0)}>
                                            ...
                                            <video autoPlay loop id="video1" className={sdms.hidden}>
                                    ...
                                    <div className={content.col2row1 + this.getFullScreenClassName(2)}>
                                        <span id="cctv2_span" onDoubleClick={(e) => this.showFullScreenCCTV(2)} onClick={() => this.onClickCCTV(1)}>
                                            ...
                                            <video autoPlay loop id="video2" className={sdms.hidden}>
                                                <source type="video/mp4" />
                                            </video>
                                        </span>
                                    </div>
                                    <div className={content.col1row2 + this.getFullScreenClassName(3)}>
                                        <span id="cctv3_span" onDoubleClick={(e) => this.showFullScreenCCTV(3)} onClick={() => this.onClickCCTV(2)}>
                                            ...
                                            <video autoPlay loop id="video3" className={sdms.hidden}>
                                                <source type="video/mp4" />
                                            </video>
                                        </span>
                                    </div>
                                    <div className={content.col2row2 + this.getFullScreenClassName(4)}>
                                        <span id="cctv4_span" onDoubleClick={(e) => this.showFullScreenCCTV(4)} onClick={() => this.onClickCCTV(3)}>
                                            ...
                                            <video autoPlay loop id="video4" className={sdms.hidden}>
                                                <source type="video/mp4" />
                                            </video>
                                        </span>
                                    </div>
