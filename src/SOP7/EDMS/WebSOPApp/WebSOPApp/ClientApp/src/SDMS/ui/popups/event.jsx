...
class Event extends Component {
    ...
    keysPressed(e, target) {
        ...
        if (Event.keys[commonKey] && Event.keys[81]) {
            ...
            // 화면전환 퀵버튼
            target.onMoveSelectedAlarm(target.props.selectedAlarm);
    ...
    onMoveSelectedAlarm(alarm) {
        this.props.onMoveSelectedAlarm(alarm);
    }
        ...
        for (var i = 0; i < alarms.length; i++) {
            ...
            grid.push(
                <React.Fragment key={"eventGrid_" + i}>
                    <tr className={rowClassName} onClick={() => this.onSelectedAlarm(alarm)} onDoubleClick={() => this.onMoveSelectedAlarm(alarm)}>
                        <td>{ymd}<br />{hms}{displayNumUI}</td>
                        <td onClick={() => this.onSelectedAlarm(alarm)} onDoubleClick={() => this.onMoveSelectedAlarm(alarm)}>{typeString}</td>
                        <td>{alarmDepth}</td>
                        <td>{positionName}</td>
    ...
    getAlarmInfo() {
        ...
        if (selectedAlarm.facilityType) {
            if (selectedAlarm.facilityType === SDMSResource.facilityType.FIRE) {
                ...
                if (selectedAlarm.materialType === SdmsResource.materialType.FireFacility) {
                    evtClassName = content.evtFIRECONTROL;
                }
                else if (selectedAlarm.materialType === SdmsResource.materialType.AirFacility) {
                    evtClassName = content.evtAIRFAN;
                }
                else if (selectedAlarm.materialType === SdmsResource.materialType.ElectricFacility) {
                    evtClassName = content.evtELECTRIC;
                }
    ...
    getBtnUI = () => {
        let btnUI =
            <ul className={content.eventIconBox}>
                ...
                <li onClick={() => this.onMoveSelectedAlarm(this.props.selectedAlarm)}><span className={"shortcutKey" + " " + content.eventShortCut + " " + uis.hideKey}>Sh+W</span><span className={content.eIcon2}></span><a>화면전환</a></li>
                {
