...
export class SDMSDataManager {
    ...
    static addModel(model) {
        ...
        for (let i = 0; i < modelDataCount; i++) {
            ...
            if (modelData.floorIndex !== null && modelData.floorIndex !== undefined) {
                ...
                const _floor = SDMSDataManager.getFloor(floors, floor.zoneID, floor.floorIndex);

                if (_floor) {
                    // 한 층에 둘 이상의 모델파일이 있으면 files에 담는다.
                    if (_floor.files) {
                        _floor.files.push(floor.file);
                    }
                    else {
                        _floor.files = [_floor.file, floor.file];
                    }
                }
                else
                    floors.push(floor);
                ...
                if (!data.file)
                    ...
                    data.files.push(modelData.modelFile);
                }
    ...
    static getFloor(floors, zoneID, floorIndex) {
        for (const floor of floors) {
            if (floor.zoneID === zoneID && floor.floorIndex === floorIndex) {
                return floor;
            }
        }
