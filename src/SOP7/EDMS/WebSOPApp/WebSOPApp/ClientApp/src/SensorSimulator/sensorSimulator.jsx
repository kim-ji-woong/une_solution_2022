...
class SensorSimulator extends Component {
        ...
        if (result) {
            ...
            if (_3dOptions) {
                ...
                if (result.etcSensors) {
                    this.setSensorList(result.etcSensors, _3dOptions, sensorList);
                }
    ...
    setSensorList(sensors, _3dOptions, sensorList, materialType) {
        for (const sensor of sensors) {
            let _materialType = materialType;

            if (materialType !== 0 && !materialType) {
                _materialType = sensor.materialType;

                if (_materialType !== SdmsResource.materialType.FireFacility &&
                    _materialType !== SdmsResource.materialType.AirFacility &&
                    _materialType !== SdmsResource.materialType.ElectricFacility &&
                    _materialType !== SdmsResource.materialType.PanelFacility) {
                    continue;
                }
            }

            const materialName = SdmsResource.getFacilityTypeString(_materialType);
            let buildingGroups = sensorList[materialName];

            if (!buildingGroups) {
                buildingGroups = {};
                sensorList[materialName] = buildingGroups;
            }
    ...
    getSensorElements() {
        ...
        for (const buildingGroupName of this.getSortedKeys(sensors)) {
            ...
            buildingGroupElements.push(<label for={bgID}>{buildingGroupName}</label>);
            ...
            for (const buildingName in buildingGroupData) {
                ...
                buildingElements.push(<label for={bID}>{buildingName}</label>);
                ...
                for (const zoneName of this.getSortedKeys(buildingData)) {
                    ...
                    zoneElements.push(<input type="checkbox" id={zID} />);
                    zoneElements.push(<label for={zID}>{zoneName}</label>);
                    ...
                    for (const sensorName of this.getSortedKeys(zoneData)) {
                        ...
                        sensorElements.push(
                            <li className={sensor.lastTab}>
                                ...
                                <p className={sensorClassName}><label for={sID} onClick={() => this.onSelectSensor(_sensor)}>{sensorName}</label></p>
                            </li>
                        );
                    }
