            ...
            if (data.Value != "" || data.Key != "")
            {
                try
                {
                    ...
                    if (nIndex > 0)
                    {
                        ...
                        if (siteOption.AutoLogin != null && siteOption.AutoLogin == false)
                            autoLogin = false;

                        result = m_processManager.GetAccountManager().Login(strID, strPW, data.Key, data.IsFullVersion, siteOption.ExternalLogin, autoLogin);
