...
{
    public class ContactProvider : ClientServiceProvider
    {
        private int m_nServerSeqNo = -1;

        private UInt16 m_nTransID = 0;

        private ContactManager m_parentManager = null;
        ...
        public ContactProvider(ContactManager mgr, int nServerSeqNo)
        {
            m_parentManager = mgr;
        ...
        public override void OnReceiveData()
        {
            ...
            try
            {
                ...
                if (FC == FC_ReadDiscrete)
                {
                    int nBitCount = 8;
                    BitArray bitArray = new BitArray(arrData);

                    if (bitArray.Length != nBitCount)
                        return;

                    ContactData contactData = new ContactData();
                    contactData.First_Dry = bitArray[(int)ContactTypes.First_Dry];
                    contactData.Second_Wet = bitArray[(int)ContactTypes.Second_Wet];

                    // 신호 체크
                    m_parentManager.CheckContactSignal(contactData);
                }
            }
            catch (Exception e)
            {
                m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.ContactSignal, m_nServerSeqNo, "ContactProvider OnReceiveData() : " + e.Message);
            }
        }
        ...
        private void ConnectionThread()
        {
            ...
            while (m_runThread)
            {
                    ...
                    if (!this.IsConnected)
                    {
                        lock (this)
                        {
                                ...
                                && m_parentManager.ServerIP != null && m_parentManager.ServerIP != "")
                            {
                                ...
                                {   // 연결 성공
                                    m_bIsConnect = true;
                                    m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.ContactSignal, m_nServerSeqNo, "ConnectionThread() : " + m_parentManager.ServerIP + ":" + m_parentManager.Port.ToString() + " / " + this.IsConnected);

                                    string strError;
                                    if (UpdateServerStatus(m_bIsConnect, out strError) == false)
                                        m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.ContactSignal, m_nServerSeqNo, $"ConnectionThread() : (UpdateServerStatus Error {strError})");
                                } 
                ...
                {
                    m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.ContactSignal, m_nServerSeqNo, "ConnectionThread() : " + e.Message);
                }
            }
        }
        ...
        private void RequestThread()
        {
            while (m_runThread)
            {
                ...
                {
                    m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.ContactSignal, m_nServerSeqNo, "RequestThread() : " + e.Message);
                }
            }
        }
        ...
        public void SendBytes(byte[] CmdBuff)
        {
            ...
            {
                m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.ContactSignal, m_nServerSeqNo, "SendBytes() : " + e.Message);
            }
        }
