import ColComboBox_SelectBuilding from "../basicInfo/columns/colComboBox_SelectBuilding";
...
export class SpaceDataManager {
    ...
    static addZoneToZones(zone, _3dOptions) {
        ...
        for (const equipZoneData of zone.equipmentZoneDatas) {
            ...
            data.push(equipZoneData.id);
            data.push(equipZoneData.zoneName);
            data.push(equipZoneData.textCenter);

            zoneData.equipZones[equipZoneData.id] = data;
        }
    ...
    static addZone(building, zoneName, _3dOptions, buildingGroupList) {
        ...
        if (zone) {
            const equipmentZoneData = SpaceDataManager.makeEquipZoneData(zone);
            zone.equipmentZoneDatas.push(equipmentZoneData);
        }
    ...
    static getZones(zoneDatas, zones, equipZoneDatas) {
        ...
        for (let i = 0; i < zoneCount; i++) {
            ...
            zone.equipmentZoneDatas.push(SpaceDataManager.makeEquipZoneData(zone));
            equipZoneDatas[zone.id] = zone.equipmentZoneDatas;
        }
    }

    // EquipZone에 포함되지 않는 센서를 표현하기 위하여 임시 EquipZone을 만든다.
    static makeEquipZoneData(zone) {
        const equipZoneData = {
            breadcastText: ColComboBox_SelectBuilding.noEquipZone,
            displayText: ColComboBox_SelectBuilding.noEquipZone,
            id: -zone.id,
            zoneName: ColComboBox_SelectBuilding.noEquipZone,
            linkedZoneIDs: [zone.id],
            linkedZoneDatas: [zone]
        };
