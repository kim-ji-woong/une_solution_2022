...
export class POIManager {
        ...
        if (poi) {
            ...
            if (sensorType) {
                ...
                if (sensor) {
                    ...
                    const poiObject = poi.object ? poi.object : poi;
                    this.changePoiObject(poiObject, isSelected, enabled);
                }
            }
    ...
    hideTempPOI() {
        const changedIndoor = POIManager._hideTempPoi(this.tempIndoorPOI);
        const changedOutdoor = POIManager._hideTempPoi(this.tempOutdoorPOI);
    ...
    putTempPOI(tempPOI, sensor, isIndoor, _3dOptions) {
        if (!tempPOI.position) {
            return false;
        }
        ...
        if (!zone) {
            zone = _3dOptions.outdoorZones[sensor.zoneID];
        }
        ...
        if (zone) {
            ...
            let sensorDatas = sensors[sensorTypeName];

            if (!sensorDatas) {
                sensorDatas = [];
                sensors[sensorTypeName] = sensorDatas;
            }
    ...
    selectSensor(sensor) {
        if (sensor) {
            const poi = this.getSensorPOI(sensor.sensorTypeName, sensor.zoneID, sensor.id);

            if (poi) {
                this.selectPOI(poi, false, null);
            }
            else {
                this.selectPOI(null, false, null);
            }
        }
        else {
            this.selectPOI(null, false, null);
