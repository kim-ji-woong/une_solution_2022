...
export class BuildingInfo extends Component {
    ...
    getBuildingClassName(zoneUI) {
        const hasChild = zoneUI.length > 0;

        if (this.state.editMode) {
            return styles.viewList1Depth + " " + styles.hidden;
        }

        if (this.state.expandNode) {
            if (hasChild)
                return styles.viewList1Depth + " " + styles.expand;
            else
                return styles.viewList1Depth + " " + styles.noChild;
        }
        else {
            if (hasChild)
                return styles.viewList1Depth;
            else
                return styles.viewList1Depth + " " + styles.noChild;
        }
    }
    ...
    render() {
        ...
        const buildingClassName = this.getBuildingClassName(zoneUI);//this.state.editMode ? styles.viewList1Depth + " " + styles.hidden : styles.viewList1Depth;
