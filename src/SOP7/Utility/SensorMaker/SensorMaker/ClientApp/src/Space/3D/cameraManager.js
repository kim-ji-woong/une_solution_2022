...
export class CameraManager {
    ...
    static setControlCamera(camera, controls, min, max, cameraOptions, optionOnly) {
        if (min && max) {
            ...
            //CameraManager.setOptionsToCamera(camera, controls, cameraOptions);
    ...
    static setOptionsToCamera(camera, controls, cameraOptions) {
        if (cameraOptions) {
            camera.position.x = cameraOptions.position[0];
            camera.position.y = cameraOptions.position[1];
            camera.position.z = cameraOptions.position[2];

            camera.rotation.x = cameraOptions.rotation[0];
            camera.rotation.y = cameraOptions.rotation[1];
            camera.rotation.z = cameraOptions.rotation[2];

            camera.quaternion.x = cameraOptions.quaternion[0];
            camera.quaternion.y = cameraOptions.quaternion[1];
            camera.quaternion.z = cameraOptions.quaternion[2];
            camera.quaternion.w = cameraOptions.quaternion[3];

            controls.target.x = cameraOptions.targetControl[0];
            controls.target.y = cameraOptions.targetControl[1];
            controls.target.z = cameraOptions.targetControl[2];
