...
export class SpaceBody extends Component {
    ...
    constructor(props) {
        ...
        this.state =
        {
            selectedMenu: SpaceMenus.EditSpaceInfo/*SpaceMenus.EditBasicInfo*/,
            loading: true,
    ...
    async set3DOptions(sensorList) {
        ...
        this.setState({ loading: false, _3dOptions, buildingGroupList, sensorList });
    }
    ...
    setFireSensors(fireSensors, site3dOptions) {
        ...
        for (let i = 0; i < sensorCount; i++) {
            ...
            sensor.sensorTypeName = SpaceDataManager.FireSensorType;
    ...
    setPSMSensors(psmSensors, site3dOptions) {
        ...
        for (let i = 0; i < sensorCount; i++) {
            ...
            sensor.sensorTypeName = SpaceDataManager.PSMSensorType;
            /*let zone = _3dOptions.zones[sensor.zoneID];
    ...
    setEtcSensors(etcSensors, site3dOptions) {
        ...
        for (let i = 0; i < sensorCount; i++) {
            ...
            sensor.sensorTypeName = SpaceDataManager.EtcSensorType;
            /*let zone = _3dOptions.zones[sensor.zoneID];
    ...
    setCCTVs(cctvs, site3dOptions) {
        ...
        for (let i = 0; i < cctvCount; i++) {
            ...
            cctv.sensorTypeName = SpaceDataManager.CCTVType;
    ...
    onRemoveItem = (item, itemType) => {
        if (itemType === SpaceBody.Type_Site) {
            ...
            _3dOptions.indoorModels = {};
            _3dOptions.outdoorModel = {};
            _3dOptions.outdoorZones = {};

            SpaceDataManager.clearBuildingGroup(buildingGroupList, _3dOptions);
            this.setState({ buildingGroupList, _3dOptions, modelList });
            this._onSaveXML(true, this.state.sensorList, _3dOptions.indoorModels, _3dOptions.outdoorModel, modelList, buildingGroupList, _3dOptions.outdoorZones, _3dOptions.siteName);
        }
        else if (itemType === SpaceBody.Type_BuildingGroup) {
            const _3dOptions = { ...this.state._3dOptions };
            const buildingGroupList = [...this.state.buildingGroupList];
    ...
    async makeResultData(result) {
        ...
        this.setState({ loading: false, _3dOptions, sensorList, buildingGroupList: result.buildingGroups, sensorTypes: result.sensorTypes, modelList, tempModelFiles });
    }

    async onSaveXML(bTempSave) {  
        const xmlData = this.makeXMLData(this.state.sensorList, this.state._3dOptions.indoorModels, this.state._3dOptions.outdoorModel, this.state.modelList, this.state.buildingGroupList, this.state._3dOptions.outdoorZones, this.state._3dOptions.siteName);
        if (!xmlData) {
            return;
        }
    ...
    async _onSaveXML(bTempSave, sensorList, indoorModels, outdoorModel, modelList, buildingGroupList, outdoorZones, siteName) {
        const xmlData = this.makeXMLData(sensorList, indoorModels, outdoorModel, modelList, buildingGroupList, outdoorZones, siteName);
        if (!xmlData) {
            return;
        }
    ...
    makeXMLData(sensorList, indoorModels, outdoorModel, modelList, buildingGroupList, outdoorZones, siteName) {
    ...
    onSelectPOI = (poi) => {
        ...
        if (sensorType && (zoneID === 0 || zoneID) && (sensorID === 0 || sensorID)) {
            const zone = this.getZone(this.state._3dOptions, zoneID);
                ...
                if (sensors) {
                    for (const sensor of sensors) {
                        if (sensor.id === sensorID) {
                            this.addSelectedSensorNodes([sensor]);
                            this.poiManager.selectPOI(poi, false, null);
                            break;
                        }
                    }
                }
    ...
    addSelectedSensorNodes = (sensors) => {
        ...
        if (this.state.selectedMenu === SpaceMenus.EditSensorList) {
            this.setState({ selectedSensorNodes: sensors });
        }
        else {
            if (sensors && sensors.length === 1) {
                this.poiManager.selectSensor(sensors[0]);
            }

            this.poiManager.hideTempPOI();
            this.setState({ selectedSensorNodes: sensors, poiEditCommand: this.getPoiEditCommand(sensors) });
        }
    }

    getPoiEditCommand(sensors) {
        ...
        for (const sensor of sensors) {
            if ((sensor.zoneID === 0 || sensor.zoneID) && (sensor.buildingID === 0 || sensor.buildingID)) {
                if (sensor.zoneID !== this.state.currentView?.zoneID) {
                    continue;
                }

                const buildingData = this.state._3dOptions.buildingIDs[sensor.buildingID];

                if (buildingData && buildingData.length > 1) {
                    const buildingGroupName = buildingData[1];
                    command.push([buildingGroupName, sensor.buildingID, sensor.zoneID, sensor.equipZoneID, sensor]);
                }
            }
        }
        ...
        return (
            <div className={styles.bodyArea}>
                ...
                <SpaceContents
                    ...
                    poiEditCommand={this.state.poiEditCommand}
