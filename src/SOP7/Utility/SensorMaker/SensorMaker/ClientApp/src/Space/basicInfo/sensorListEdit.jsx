...
export class SensorListEdit extends Component {
    ...
    static LastSelectedControlType = { none: -1, gridView: 0, treeView:1 }
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            searchTextGridView: '',
            viewSensors: [],
            sensorListCount: null
        }

        this.selectedRows = [];

        this.nIndexLastSelectedRow = -1;
        this.lastSelectedControl = SensorListEdit.LastSelectedControlType.none; // 마지막으로 선택한 컨트롤 treeview, gridview (drag&drop 기능 적용할 곳 구분)
        ...
        this.refSearchText2 = React.createRef();
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevProps.sensorList !== null && prevState.sensorListCount === null) {
            bChange = true;
        }
        else {
            if ((prevProps.sensorList.fire && prevProps.sensorList.fire.length !== prevState.sensorListCount.fire) ||
                (prevProps.sensorList.psm && prevProps.sensorList.psm.length !== prevState.sensorListCount.psm) ||
                (prevProps.sensorList.etc && prevProps.sensorList.etc.length !== prevState.sensorListCount.etc) ||
                (prevProps.sensorList.cctv && prevProps.sensorList.cctv.length !== prevState.sensorListCount.cctv)) {
                bChange = true;
            }
        }
    ...
    initPageIndex(searchText, sensorType, applyEquipZone) {
        ...
        if (this.props.sensorList.fire) {
            sensorListCount.fire = this.props.sensorList.fire.length;
        }
        if (this.props.sensorList.psm) {
            sensorListCount.psm = this.props.sensorList.psm.length;
        }
        if (this.props.sensorList.etc) {
            sensorListCount.etc = this.props.sensorList.etc.length;
        }
        if (this.props.sensorList.cctv) {
            sensorListCount.cctv = this.props.sensorList.cctv.length;
        }
        ...
        if (sensors) {
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (searchText.length === 0 ||
                    (searchText.length > 0 && ((sensor.name && sensor.name.toLowerCase().includes(searchText)) || (sensor.positionName && sensor.positionName.toLowerCase().includes(searchText)) || (sensor.tagNo && sensor.tagNo.toString().toLowerCase().includes(searchText))))
                    || this.checkSearchText(searchText, sensor)) {
    ...
    checkSearchText(searchText, sensor) {
        const search = searchText.toLowerCase();

        if (this.state.curSensorType === SpaceDataManager.FireSensorType) {
            const subTypeName = this.getFireSensorSubTypeName(sensor);

            if (subTypeName && subTypeName.includes(search)) {
                return true;
            }

            const displayName = ColComboBox_SelectBuilding.getDisplayName(sensor.buildingID, this.props.buildingGroupList, sensor.zoneID, sensor.equipZoneID);

            if (displayName && displayName.toLowerCase().includes(search)) {
                return true;
            }
        }
        else if (this.state.curSensorType === SpaceDataManager.PSMSensorType) {
            const [buildingName, zoneName, equipZoneName] = ColComboBox_SelectBuilding.getZoneName(sensor.buildingID, this.props.buildingGroupList, sensor.zoneID, sensor.equipZoneID);

            if (zoneName && zoneName.toLowerCase().includes(search)) {
                return true;
            }

            if (sensor.uniqueKey && sensor.uniqueKey.toLowerCase().includes(search)) {
                return true;
            }

            if (sensor.materialName && sensor.materialName.toLowerCase().includes(search)) {
                return true;
            }
        }
            ...
            if (sensor.type && sensor.type.toLowerCase().includes(search)) {
                return true;
            }

            if (sensor.userID && sensor.userID.toLowerCase().includes(search)) {
                return true;
            }

            if (sensor.password && sensor.password.toLowerCase().includes(search)) {
                return true;
            }

            if (sensor.url && sensor.url.toLowerCase().includes(search)) {
                return true;
            }
        }
    ...
    getFireSensorSubTypeName(sensor) {
        for (const sensorType of this.props.sensorTypes) {
            if (sensorType.id === sensor.sensorType) {
                for (const subType of sensorType.subType) {
                    if (subType.id === sensor.sensorSubType) {
                        return subType.name;
                    }
                }
    ...
    onClickSearch(isTreeView) {
        if (isTreeView) {
            this.onSearch(this.refSearchText.current.value);
        }
        else {
            this.onSearchGridView(this.refSearchText2.current.value);
        }
    }

    onSearch = (text) => {
        if (this.state.searchText.toLowerCase() === text.toLowerCase()) {
            return;
        }
    ...
    onSearchGridView = (text) => {
        if (this.state.searchTextGridView.toLowerCase() === text.toLowerCase()) {
            return;
        }
    ...
    async onDownloadExcel() {
        ...
        await SpaceController.requestDownloadSensorExcelFile(this.state.curSensorType, sensors ? sensors : []);
    }

    onUploadExcel = async(event) => {
        ...
        if (!result[0]) {
            this.refFileUpload.current.value = "";
            alert(result[1]);
            return;
        }

        if (result[2]) {
            let sensors = this.initID(result[2]);
            this.setSpatial(sensors);
    ...
    setSpatial(sensors) {
        const sensorCount = sensors.length;
        for (let i = 0; i < sensorCount; i++) {
            const [buildingID, zoneID, equipZoneID] = this.getSpatialIDs(sensors[i].buildingID, sensors[i].zoneID, sensors[i].equipZoneID);
            sensors[i].buildingID = buildingID;
            sensors[i].zoneID = zoneID;
            sensors[i].equipZoneID = equipZoneID;
        }

        return sensors;
    }

    // buildingID는 없지만 zoneID는 있을 경우
    // buildingID or zoneID는 없지만 equipzoneID는 있을 경우
    getSpatialIDs(pBuildingID, pZoneID, pEquipzoneID) {
        if (pBuildingID === null && pZoneID === null && pEquipzoneID === null) {
            return [null, null, null];
        }

        if (pBuildingID > 0 && pZoneID === null && pEquipzoneID === null) {
            return [pBuildingID, null, null];
        }

        if (pBuildingID > 0 && pZoneID > 0 && pEquipzoneID === null) {
            return [pBuildingID, pZoneID, null];
        }

        if (pBuildingID > 0 && pZoneID > 0 && pEquipzoneID > 0) {
            return [pBuildingID, pZoneID, pEquipzoneID];
        }

        let buildingID = null;
        let zoneID = -1;
        let equipzoneID = null;

        const buildingGroupCount = this.props.buildingGroupList.length;
        for (let j = 0; j < buildingGroupCount; j++) {
            const buildingGroup = this.props.buildingGroupList[j];
            ...
            for (let k = 0; k < buildingCount; k++) {
                const building = buildingGroup.buildingDatas[k];
                
                const zoneCount = building.zoneDatas.length;
                for (let p = 0; p < zoneCount; p++) {
                    const zone = building.zoneDatas[p];
                    if (zone.id === pZoneID) {
                        buildingID = building.id;
                        zoneID = zone.id;

                        if (pEquipzoneID === null) {
                            return [buildingID, zoneID, null];
                        }
                    }
                    ...
                    if (pEquipzoneID !== null) {
                        ...
                        for (let q = 0; q < equipzoneCount; q++) {
                            ...
                            if (equipzone.id === pEquipzoneID) {
                                buildingID = building.id;
                                zoneID = zone.id;
                                equipzoneID = equipzone.id;

                                return [buildingID, zoneID, equipzoneID];
                            }
                        }
                    }
                }                
    ...
    addSelectedNodes = (selectedSensors) => {
        this.lastSelectedControl = SensorListEdit.LastSelectedControlType.treeView;
            ...
            if (buildingGroup.visible) {
                ...
                ui.push(
                    <BuildingGroupNode
                        ...
                        addSelectedNodes={this.addSelectedNodes}
                ...
                <div className={space.listArea}>
                    <div className={space.listBox}>
                        <div className={space.listTitleBox}>
                            ...
                            <span className={space.listSelect} onClick={() => this.onClickSearch(true)}><input ref={this.refSearchText} type="text" onKeyUp={(e) => this.onKeyUpSearch(true, e)} className={space.inputFind} /></span>
                        </div>
                        <div className={space.listContent}>
                            <ul>
                    ...
                    <div className={space.listBox2}>
                        <div className={space.listTitleComboBox}>
                            ...
                            <span className={space.listSelect2} onClick={() => this.onClickSearch(false)}><input ref={this.refSearchText2} type="text" onKeyUp={(e) => this.onKeyUpSearch(false, e)} className={space.listSelectInput} /></span>
                                            ...
                                            
                                            {
                                                ...
                                                (this.state.curSensorType === SpaceDataManager.FireSensorType) ?
                                                    <>
                                                        ...
                                                        <th className={space.fireTh4}>Zone</th>
                                                        <th className={space.fireTh5}>설치장소</th>
                                                    </>
                                                    : null
                                            }
                                            {
                                                ...
                                                (this.state.curSensorType === SpaceDataManager.PSMSensorType) ?
                                                    <>
                                                        ...
                                                        <th className={space.psmTh4}>Zone</th>
                                                        <th className={space.psmTh5}>설치장소</th>
                                                    </>
                                                    : null
                                            }
                                            {
                                                ...
                                                (this.state.curSensorType === SpaceDataManager.EtcSensorType) ?
                                                    <>
                                                        ...
                                                        <th className={space.etcTh4}>Zone</th>
                                                        <th className={space.etcTh5}>설치장소</th>
                                                    </>
                                                    : null
                                            }
                                            {
                                                ...
                                                (this.state.curSensorType === SpaceDataManager.CCTVType) ?
                                                    <>
                                                        ...
                                                        <th className={space.cctvTh7}>Zone</th>
                                                        <th className={space.cctvTh8}>설치장소</th>
                                                    </>
                                                    : null
                                            }
