...
export class BuildingGroupInfo extends Component {
    ...
    hasChild() {
        if (this.props.isOutdoor) {
            const outdoorZones = this.props.buildingGroup;
            return outdoorZones.length > 0;
        }

        const buildingDatas = this.props.buildingGroup?.buildingDatas;
        return buildingDatas && buildingDatas.length > 0;
    }

    getIconClassName() {
        if (!this.props.selectedInfo.buildingGroup) {
            if (this.hasChild())
                return styles.viewListHead;
            else
                return styles.viewListHead + " " + styles.noChild;
        }
        else if (this.props.selectedInfo.buildingGroup === this.props.buildingGroup) {
            if (this.hasChild())
                return styles.viewListHead + " " + styles.expand;
            else
                return styles.viewListHead + " " + styles.noChild;
        }
                ...
                <div className={styles.viewListHeadWrap}>
                    {
                        !this.state.editMode &&
                        <>
                            <span ref={this.refBuildingGroupName} className={this.getIconClassName()} data-show_child={showChild} data-buildinggroupid={buildingGroupID} data-target_class='viewListHead' onClick={(e) => { this.showChildElement(e) }}>{buildingGroupName}</span>
                            {
                                this.props.modeling &&
