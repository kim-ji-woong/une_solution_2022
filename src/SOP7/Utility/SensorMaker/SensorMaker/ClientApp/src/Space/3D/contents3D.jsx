...
export class Contents3D extends Component {
    ...
    setInitialViewport = () => {
        if (this.isIndoor()) {
            ...
            if (zoneID) {
                const cameraOptions = CameraManager.getIndoorModelCamera(SpaceBody.Type_Floor, zoneID, this.props._3dOptions, this.props.editMode);

                if (cameraOptions) {
                    CameraManager.setCameraOptions(this.camera, this.controls, cameraOptions);
    ...
    showIndoorOrtho(modelFile, cameraOrtho, zoneID) {
        if (!cameraOrtho?.position) {
            return;
        }

        const orthoCameraData = { ...cameraOrtho };//this.getOrthoCameraData(zoneID, this.getBuildingIDFromZone(zoneID));
        orthoCameraData.position[1] += 1000;
        ...
        if (modelData) {
            ...
            this.poiManager.addZoneSensors(zoneID, 1, this.props._3dOptions.outdoorZones, this.props._3dOptions.zones, this.state.visibleSensorTypes);
    ...
    getCurrentZoneSelectedSensor() {
        const selectedSensor = this.getSelectedSensor();

        if (selectedSensor) {
            if (this.props.currentView) {
                if (this.props.currentView.zoneID === selectedSensor.zoneID) {
    ...
    onMouseMove = (event) => {
        if (this.state.loading === false && this.camera && this.isEditMode()) {
            const selectedSensor = this.getCurrentZoneSelectedSensor();//this.getSelectedSensor();
    ...
    onClick = (event) => {
        if (this.state.loading === false && this.camera && this.isEditMode()) {
            ...
            if (this.poiManager.selectedPOI) {
                if (this.poiManager.CurrentMode === POIEdit.Menu_Move) {
                ...
                else if (this.poiManager.CurrentMode === POIEdit.Menu_Info) {
                    if (poi !== this.poiManager.selectedPOI) {
                        this.poiManager.selectPOI(poi, this.props.editMode, this.props.editModeParam);
                        this.props.onSelectPOI(poi);
                    }
                }
                else {
                    const selectedSensor = this.getSelectedSensor();
                    ...
                    if (selectedSensor) {
                        if (this.poiManager.putTempPOI(this.poiManager.selectedPOI, selectedSensor, this.isIndoor(), this.props._3dOptions)) {
                            // Temp XML에 저장
                            this.props.onSaveXML(true);
                        }
                        else {
