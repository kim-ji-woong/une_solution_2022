...
namespace SensorMaker.Areas.Space.Controllers
{
    ...
    public class SpaceController : Controller
    {
        ...
        /// <summary>
        /// 파일전송시 크기제한 없애려면 DisableRequestSizeLimit 옵션과 함께
        /// web.config 파일에 아래의 부분이 들어가야 한다.(IIS 사용시)
        /// 개발중일때에는 ProjectFolder/.vs/ProjectName/config/applicationhost.config
        /// <system.webServer>
        ///     <security>
        ///         <requestFiltering>
        ...
        public IActionResult UploadTempModelFile(List<IFormFile> files)
        {
            ...
            Dictionary<string, string> dicParameters = GetParmeters(files);

            string strUserID = null, strUserName = null;

            if (dicParameters.TryGetValue("userID", out strUserID) && dicParameters.TryGetValue("userName", out strUserName))
            //if (value != null && value.Length > 0 && string.Compare(key, "userID", true) == 0)
            {
                int nUserID;

                if (int.TryParse(strUserID, out nUserID))
                {
                    result = ModelFileManager.UploadTempFile(files[0].OpenReadStream(), files[0].FileName, nUserID, strUserName, Startup.TempResourceRootPath, m_processManager.TeamDataManager);
                }
            }
        ...
        private Dictionary<string, string> GetParmeters(List<IFormFile> files)
        {
            ...
            if (files.Count >= 2)
            {
                for (int i=1;i<files.Count;i++)
                {
                    ...
                    string strParamKey = file.FileName.Substring(0, nIndex).Trim();
                    string strParamValue = file.FileName.Substring(nIndex + 1).Trim();

                    dicParameters[strParamKey] = strParamValue;
                }
            }
