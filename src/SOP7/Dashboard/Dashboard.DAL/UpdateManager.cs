...
namespace Dashboard.DAL
{
    ...
    public class UpdateManager : QueryManager, IUpdate
    {
        ...
        public bool UpdateWorkPermit(WorkPermit obj, out string strErrorMessage)
        {
            Dictionary<WorkPermit.Fields, object> dicSets = new Dictionary<WorkPermit.Fields, object>();
            dicSets[WorkPermit.Fields.SpatialType] = obj.SpatialType;
            dicSets[WorkPermit.Fields.SpatialID] = obj.SpatialID;
            dicSets[WorkPermit.Fields.WorkerType] = obj.WorkerType;
            dicSets[WorkPermit.Fields.WorkerCount] = obj.WorkerCount;
            dicSets[WorkPermit.Fields.Comment] = obj.Comment;

            Dictionary<WorkPermit.Fields, object> dicConditions = new Dictionary<WorkPermit.Fields, object>();
            dicConditions[WorkPermit.Fields.ID] = obj.ID;

            return UpdateWorkPermit(dicSets, dicConditions, null, out strErrorMessage);
        }

        public bool UpdateWorkPermit(Dictionary<WorkPermit.Fields, object> dicSets, Dictionary<WorkPermit.Fields, object> dicConditions, string strAdditionalConditions, out string strErrorMessage)
        {
            strErrorMessage = null;
            string strCondition = "";
            string strSets = "";

            if (SetData<WorkPermit.Fields>(ref strSets, dicSets, WorkPermit.GetFieldName, WorkPermit.TableName, ref strErrorMessage) == false)
                return false;
            ...
            return UpdateFromCondition(WorkPermit.TableName, strSets, strCondition, strAdditionalConditions, out strErrorMessage);
        }
