...
namespace EmergencyBell.Network
{
    public class ClientProvider : ClientServiceProvider
    {
        ...
        public void SendData(byte[] bytes, int beginIndex, int len)
        {
            ...
            {
                Logger.Instance.Write("[ERROR] ClientProvider.cs > void SendData(byte[], int, int) :" + ex.Message);
            ...
            try
            {
                ...
                if (dataLength >= 6)
                {
                    ...
                    if (sensorNo > 0)
                    {
                        ...
                        Logger.Instance.Write(strSensorLog);
        ...
        private string WriteBinaryLog(byte[] bytes, int nIndex, int len, string strTag)
        {
            ...
            Logger.Instance.Write(strTag + " : " + strBytesLog);
