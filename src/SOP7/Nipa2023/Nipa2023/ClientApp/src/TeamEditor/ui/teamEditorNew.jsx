...
class TeamEditorNew extends Component {
	...
	async save() {
		...
		if (result === true) {
			this.showConfirmDialog(["데이터가 저장되었습니다."], null, null, 'success');
	...
	checkMemberID = (id, memberID) => {
		...
		if (chk === true) {
			...
			this.showConfirmDialog([memberID + " 사번이 이미 사용 중에 있습니다."], null, null, 'error');
	...
	checkPhoneNumber = (phoneNumber, id) => {
		...
		if (chk === true) {
			...
			this.showConfirmDialog([phoneNumber + " 휴대전화번호가 이미 사용 중에 있습니다."], null, null, 'error');
	...
	onAuthorError = () => {
		this.showConfirmDialog(["해당 로그인 사용자는 권한이 없습니다."], null, null, 'error');
	}
