...
class TeamEditor extends Component {
	...
	removeTeam = () => {
		// 경고 메시지 띄우기
		this.showConfirmDialog(["조직을 삭제하시겠습니까?", "(하위 조직,조직원 및 하위 조직원에 부여된 계정권한도 함께 삭제됩니다.)"], ["취소", "확인"], this.doRemoveTeam, 'error');
	}
	...
	async makeRemoveRegularTeam(selectedTeam, data) {
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (!success) {
				...
				//alert(message);
				this.showConfirmDialog(["조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null, 'error');
				return;
			}
		}
		else {
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(["비상조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null, 'error');
				return;
			}
        }
	...
	editTeam = async (team, chgName) => {
		...
		if (chgName === null || chgName === undefined || chgName.replace(/\s/gi, "") === "") {
			this.showConfirmDialog(["변경할 팀 이름을 다시 확인해주세요."], null, null, 'error');
			return;
		} 
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {			
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(["조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null, 'error');
				return;
            }
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(["비상조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null, 'error');
				return;
			}
	...
	checkEmail = (id, email, target) => {
		...
		if (chk === true) {
			this.showConfirmDialog([email + " 이메일 주소가 이미 사용 중입니다."], null, null, 'error');
			target.value = "";
        }
	...
	checkMemberID = (id, memberID, target) => {
		...
		if (chk === true) {
			// 사번이 중복됨.
			this.showConfirmDialog([memberID + " 사번이 이미 사용 중입니다."], null, null, 'error');
			target.value = "";
		}
	...
	checkPhoneNumber = (id, phoneNumber, target) => {
		...
		if (chk === true) {
			// 휴대전화번호이 중복됨.
			this.showConfirmDialog([phoneNumber + " 휴대전화번호가 이미 사용 중입니다."], null, null, 'error');
			target.value = "";
		}
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (isUpdate) {
				...
				if (!success && message.length > 0) {
					...
					//alert(message);
					this.showConfirmDialog(["조직원 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null, 'error');
					return;
				}
