...
class Internal extends Component {
    ...
    confirmSendSms = () => {
        ...
        if (this.props.sectionData.receivers === null || this.props.sectionData.receivers.length === 0) {
            this.props.showConfirmDialog('전파할 수신자가 없습니다.', null, null, 'error');
            return;
        }

        this.props.showConfirmDialog('메시지를 전송할까요?', ['취소', '발송'], this.sendSms, 'success');
    }
    ...
    showConfirmDialog = (isSMS, isEmail, isBroadcast, isSiren) => {
        ...
        if (this.props.sectionData.message.length === 0) {
            this.props.showConfirmDialog('전파할 내용이 없습니다', null, null, 'error');
            return;
        }
        ...
        if (!isBroadcast && this.props.sectionData.receivers === null || this.props.sectionData.receivers.length === 0) {
            ...
            if (isSMS) {
                ...
                if (!this.props.optionSopNormal.useEmail) {
            ...
            if (strMessage.length > 0) {
                this.props.showConfirmDialog(strMessage, ['상황 전파', '취소'], this.onProgressSpread, 'success');
            }
            else {
    ...
    async onProgressSpread(index) {
        if (index === 0) {
            if (this.confirmDialogData && this.confirmDialogData.length === 4) {
                ...
                if (isSMS) {
                    let useSMS = this.props.optionSopNormal.useSms;
                    if (!useSMS) {
                        useSMS = await this.SaveSetting('UseSMS', 'true');
                    }

                    if (!useSMS) {
                        this.props.showConfirmDialog('문자 전파 설정이 실패했습니다', ['확인'], null, 'error');
                        return;
                    }
                }
                else if (isEmail) {
                    let useEmail = this.props.optionSopNormal.useEmail;
                    if (!useEmail) {
                        useEmail = await this.SaveSetting('UseEmail', 'true');
                    }

                    if (!useEmail) {
                        this.props.showConfirmDialog('메일 전파 설정이 실패했습니다', ['확인'], null, 'error');
                        return;
                    }
                }
                else if (isBroadcast) {
                    let useBroadcast = this.props.optionSopNormal.useBroadcast;
                    if (!useBroadcast) {
                        useBroadcast = await this.SaveSetting('UseBroadcast', 'true');
                    }

                    if (!useBroadcast) {
                        this.props.showConfirmDialog('방송 전파 설정이 실패했습니다', ['확인'], null, 'error');
                        return;
                    }
                }
            ...
            <div className={uis.sectionBox + boxClassName + " " + uneStyles.sectionBox} id={this.props.id}>
                ...
                <dl className={uis.taskDetail + " " + uis.taskSubSection}>
                    <div className={uis.taskSub}>
                        <dt style={{ backgroundColor: '#1D2023', fontSize: '14px', padding: '0 10px' }}>전파메시지</dt>
                    ...
                    <div className={uis.taskSub + " " + uis.taskSubList}>
                        <dt style={{ backgroundColor: '#1D2023', fontSize: '14px', padding: '0 10px' }}>전파 대상자</dt>
