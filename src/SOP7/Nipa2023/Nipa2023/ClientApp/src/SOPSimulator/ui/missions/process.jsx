...
class Process extends Component {
    ...
    showConfirmDialogExternal(mission) {
        ...
        this.props.showConfirmDialog('외부 프로그램을 실행할까요?', ['실행', '취소'], this.onProgress);
    }
    ...
    showConfirmDialog(onClickType, mission) {
        ...
        if (this.props.sectionData.receivers === null || this.props.sectionData.receivers.length === 0) {
            ...
            this.props.showConfirmDialog(strMessage, ['확인'], this.onProgress);
            return;
        }

        if (onClickType === this.onClickSMSType) {
            if (this.props.optionSopNormal.useSms === 'false') {
            ...
            if (containsNormalMission) {
                if (this.props.optionSopNormal.useSms === 'false' && this.props.optionSopNormal.useEmail === 'true') {
                    strMessage = '문자 전파가 사용되지 않음으로 설정되어 있습니다. 문자 전파를 사용함으로 설정하고 문자메시지와 메일을 모두 전송할까요?'
                }
                else if (this.props.optionSopNormal.useSms === 'true' && this.props.optionSopNormal.useEmail === 'false') {
                    strMessage = '메일 전파가 사용되지 않음으로 설정되어 있습니다. 메일 전파를 사용함으로 설정하고 문자메시지와 메일을 모두 전송할까요?'
                }
                else if (this.props.optionSopNormal.useSms === 'false' && this.props.optionSopNormal.useEmail === 'false') {
        ...
        //}
        //else {
        this.props.showConfirmDialog(strMessage, ['상황 전파', '취소'], this.onProgress);
        //}
    }
        ...
        if (index === 0) {
            if (this.confirmDialogData) {
                ...
                if (type === this.onClickSMSType && this.confirmDialogData[1]) {
                    let useSMS = this.props.optionSopNormal.useSms === 'true';
                    if (!useSMS) {
    ...
    render() {
        ...
        } else if (this.props.sectionData.status === SectionData.Status_Run) { // 실행중
            boxClassName = " " + uis.sectionCurrent + " " + uis.borderCurrent;
            textClassName = " " + uis.textCurrent;
        } else if (this.props.sectionData.status === SectionData.Status_Done) { // 완료
...
{/* 
                ...
                <dl className={uis.taskDetail}>
                    ...
                    <dt>행동요령
                        {/* <p className={btnSpreadAllName} onClick={() => this.showConfirmDialog(this.onClickAllType)}>전체전파</p> */}
                    </dt>
                    </span>
                    {missionList}
                </dl>
