...
class BeginOption extends Component {
    ...
    componentDidMount() {
        ...
        $('#sqpRdo01').click(function () {
            $('.' + uis.sqpSelectBox).css({ background: '#B4B4B4' });
    ...
    onClickBegin = () => {
        ...
        if (!position || position.length === 0) {
            this.props.showConfirmDialog(['재난 발생 위치를 입력하세요.'], null, null, 'error');
            return;
        }
        else if (position.length > 50) {
            this.props.showConfirmDialog(['재난 발생 위치 길이는 50자를 초과할 수 없습니다. (현재:' + position.length + ')'], null, null, 'error');
            return;
        }
        else {
            let beginTime = '';
