...
class NewSOPOptions extends Component {
	...
	getSOPData() {
		if (this.state.selectedSubDisasterCategory === this.state.newSubDisasterCategory) {
			...
			if (sdcName.length === 0) {
				this.props.showConfirmDialog([SopManagerResource.ID.messages.inputDisasterType], null, null, 'error');
				return null;
			}
		...
		if (disasterData === this.state.newDisaster) {
			...
			if (disasterName.length === 0) {
				this.props.showConfirmDialog([SopManagerResource.ID.messages.inputSOPName], null, null, 'error');
				return null;
			}
		...
		if (disaster === null) {
			this.props.showConfirmDialog([SopManagerResource.ID.messages.unknownSOPName], null, null, 'error');
			return;
		}
	...
	onClickApply() {
		if (!this.state.selectedDisasterCategory) {
			this.props.showConfirmDialog([SopManagerResource.ID.messages.selectDisasterCategory], null, null, 'error');
			return;
		}
		else if (!this.state.selectedSubDisasterCategory) {
			this.props.showConfirmDialog([SopManagerResource.ID.messages.selectSubDisasterCategory], null, null, 'error');
			return;
		}
		else if (!this.state.selectedDisaster) {
			this.props.showConfirmDialog([SopManagerResource.ID.messages.selectSOPName], null, null, 'error');
			return;
		}
