	...
	onClickAddExternalProgram(add) {
		...
		if (add) {
			...
			if (selectedIndex < 0 || selectedIndex >= programDatas.length) {
				this.props.showConfirmDialog([SopManagerResource.ID.messages.selectAddProgram], null, null, 'error');
				return;
			}
			...
			for (let i = 0; i < parameterCount; i++) {
				...
				if (success === false) {
					...
					this.props.showConfirmDialog([SopManagerResource.format(SopManagerResource.ID.messageFormat.checkNthParameters, i)], null, null, 'error');
					return;
				}
				else {
					arrDatas.push(data);
							...
							<ScrollContent>
								<SprmContProcess>
								   <div className={ProcessProperty.cssStyles.sprmAcdn}>
									    <dt ref={this.refMissions} className={this.getMissionsClassName()}>임무내용 작성</dt>
