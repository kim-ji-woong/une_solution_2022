...
import { SubAside } from '../../SOPManager/styled/managerStyled';
...
class SopManager extends Component {
    ...
    async initUserInfo() {
        ...
        if (userAuthor !== AccountResource.ID.accountLevel.admin) {
            this.showConfirmDialog(["해당 로그인 사용자는 권한이 없습니다."], ["확인"], this.onClickFalseConfirm, this.onClickFalseConfirm, 'error');
        }
    ...
    async deleteDB(params) {
        ...
        if (success) {
            ...
            this.showConfirmDialog(["삭제되었습니다."], null, null, 'success');
        }
        else {
    ...
    getPopup() {
        if (this.state.content === SopManager.menu.open) {
            return <OpenSOPOptions sopData={this.state.sopData} content={this.changeContent} showConfirmDialog={this.showConfirmDialog} handleTooltip={this.handleTooltip} removeTooltip={this.removeTooltip} />;
        }
        else if (this.state.content === SopManager.menu.delete) {
            return <DeleteSOPOptions sopData={this.state.sopData} content={this.changeContent} showConfirmDialog={this.showConfirmDialog} onCloseConfirmDialog={this.onCloseConfirmDialog} handleTooltip={this.handleTooltip} removeTooltip={this.removeTooltip} />;
        }
    ...
    render() {
        ...
        return (
            <div id={styles.subPage} className='UI_Section'>
                ...
                <SopManagerBody menu={this.state.content} menuDatas={this.state.menuDatas} sopData={this.state.sopData} showCascading={this.state.showCascading} changeCascadingMode={this.changeCascadingMode} content={this.changeContent} loginUser={this.state.loginUser} showConfirmDialog={this.showConfirmDialog} onCloseConfirmDialog={this.onCloseConfirmDialog} />
                {/*<Footer />*/}
                {
                    this.getPopup()
                }
