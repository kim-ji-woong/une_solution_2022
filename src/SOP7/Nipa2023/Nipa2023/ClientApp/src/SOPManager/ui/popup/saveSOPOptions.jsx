...
class SaveSOPOptions extends Component {
	...
	async getVersions(sopData) {
		...
		if (disasterVersions === null) {
			this.props.showConfirmDialog([message], ['확인'], this.onClickCancle, 'error');
		...
		if (ok) {
			if (this.refCheckNewVersion.current.checked) {
				...
				if (newVersionName.length === 0) {
					this.props.showConfirmDialog([SopManagerResource.ID.messages.inputSOPVersionName], null, null, 'error');
					return;
				}
