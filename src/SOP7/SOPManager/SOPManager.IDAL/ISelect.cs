...
namespace SOPManager.IDAL
{
    ...
    {
        ArrayList GetResultData(string strSQL, out string strError);
        // Sop.Account
        ...
        ArrayList JoinSopConfigCategory(int? nSiteID, out string strErrorMessage);
        ArrayList JoinSopAloneConfigCategory(int? nSiteID, out string strErrorMessage);
        /// <summary>
        /// 알람 발생시 자동 실행될 SOP가 연결된 정보 조회
        /// </summary>
        /// <param name="versionIDs"></param>
        /// <param name="strErrorMessage"></param>
        /// <returns>SopConfigLinkedSop ID</returns>
        List<LinkedSopCountByVersion> JoinSopCategoryConfigLinkedSop(string strConditions, out string strErrorMessage);

        ArrayList ReadQuery(string strSQL, out string strErrorMessage);

        int GetColumnMaximumLength(string strTableName, string strColumnName, out string strErrorMessage);
    }
}
