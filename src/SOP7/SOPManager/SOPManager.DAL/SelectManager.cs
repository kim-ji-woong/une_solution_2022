...
{   
    ...
    public class SelectManager : QueryManager, ISelect
    {
        ...
        public ArrayList GetResultData(string strSQL, out string strError)
        {
            strError = null;
        ...
        public ArrayList JoinDisasterUserVersion(int disasterID, out string strErrorMessage)
        {
            ...
            if (m_dbManager.DatabaseType == DirectDBManager.DBType.sqlserver)
            {
                // SQL 서버에서 DB Server가 한글 OS가 아닐 경우 Collate 에러가 발생하는것을 방지한다.
                strSQLServerCollate = "collate Korean_Wansung_CI_AS";
            }

            string strCondition = string.Format("{0}.{1} = {2}.{3} and ({4}.{5} = {6}.{7} or {4}.{5} is null) and concat(concat({8} {12}, '/') {12}, {9}) in (Select concat(concat({8} {12}, '/') {12}, {9}) from {0} where {10} = {11})",
        ...
        public ArrayList JoinDisasterCategorySubDisasterCategoryDisasterUserVersion(int versionID, out string strErrorMessage)
        {
            ...
            strSQL += string.Format(" where {0}.{1} = {2}.{3} and {0}.{4} = {5}.{6} and {5}.{7} = {8}.{9} and ({2}.{10} = {11}.{12} or {2}.{10} is null) and {2}.{3} = {13}",
                strDisasterTableName,
                Disaster.GetFieldName(Disaster.Fields.VersionID, out isNullable),
                strVersionTableName,
                Model.Sop.Category.Version.GetFieldName(Model.Sop.Category.Version.Fields.ID, out isNullable),
        ...
        public ArrayList SelectSOPHistory(Dictionary<DisasterCategory.Fields, object> dicConditions1, Dictionary<SubDisasterCategory.Fields, object> dicConditions2, Dictionary<Disaster.Fields, object> dicConditions3, Dictionary<ActionStep.Fields, object> dicConditions4, Dictionary<ActionStepHistory.Fields, object> dicConditions5, string strAdditionalConditions, int? topNCount, out string strErrorMessage)
        {
            ...
            if (dicConditions1 != null && SetCondition<DisasterCategory.Fields>(ref strCondition, dicConditions1, DisasterCategory.GetFieldName, DisasterCategory.TableName, ref strErrorMessage) == false)
                return null;

            if (dicConditions2 != null && SetCondition<SubDisasterCategory.Fields>(ref strCondition, dicConditions2, SubDisasterCategory.GetFieldName, SubDisasterCategory.TableName, ref strErrorMessage) == false)
                return null;

            if (dicConditions3 != null && SetCondition<Disaster.Fields>(ref strCondition, dicConditions3, Disaster.GetFieldName, Disaster.TableName, ref strErrorMessage) == false)
                return null;

            if (dicConditions4 != null && SetCondition<ActionStep.Fields>(ref strCondition, dicConditions4, ActionStep.GetFieldName, ActionStep.TableName, ref strErrorMessage) == false)
                return null;

            if (dicConditions5 != null && SetCondition<ActionStepHistory.Fields>(ref strCondition, dicConditions5, ActionStepHistory.GetFieldName, ActionStepHistory.TableName, ref strErrorMessage) == false)
                return null;
        ...
        public ArrayList JoinSopConfigCategory(int? nSiteID, out string strErrorMessage)
        {
            strErrorMessage = null;

            string strSQL = @"
                select l.ID LinkID, l.FacilityTypeID, l.DisasterCategoryID, l.SubDisasterCategoryID
                     , dc.CategoryName, sdc.SubCategoryName, DisasterName
                     , (select ISNULL(DisplayText, GroupName) from SdmsSpatialBuildingGroup bg where bg.ID=l.LinkedBuildingGroupID) GroupName
	                 , (select ISNULL(DisplayText, BuildingName) from SdmsSpatialBuilding bg where bg.ID=l.LinkedBuildingID) BuildingName
	                 , (select ISNULL(DisplayText, ZoneName) from SdmsSpatialZone bg where bg.ID=l.LinkedZoneID) ZoneName
                     , ISNULL(f.Description, f.TypeName) FacilityTypeName
                     , LinkedBuildingGroupID, LinkedBuildingID, LinkedZoneID
                  from SopConfigLinkedSop l
                 inner join SdmsSensorFacilityType f on l.FacilityTypeID=f.ID
                 inner join SopCategoryDisasterCategory dc on l.DisasterCategoryID = dc.ID
                 inner join SopCategorySubDisasterCategory sdc on dc.ID=sdc.DisasterCategoryID and l.SubDisasterCategoryID=sdc.ID
                 where (1=1) ";

            if (nSiteID != null && nSiteID > 0)
                strSQL += $"And dc.SiteID={(int)nSiteID}";
        ...
        public ArrayList JoinSopAloneConfigCategory(int? nSiteID, out string strErrorMessage)
        {
            strErrorMessage = null;

            string strSQL = @"
                select l.ID LinkID, l.FacilityTypeID, l.DisasterCategoryID, SubDisasterCategoryID
                     , dc.CategoryName, sdc.SubCategoryName, DisasterName
                     , (select ISNULL(DisplayText, GroupName) from SopSpatialBuildingGroup bg where bg.ID=l.LinkedBuildingGroupID) GroupName
	                 , (select ISNULL(DisplayText, BuildingName) from SopSpatialBuilding bg where bg.ID=l.LinkedBuildingID) BuildingName
	                 , (select ISNULL(DisplayText, ZoneName) from SopSpatialZone bg where bg.ID=l.LinkedZoneID) ZoneName
                     , ISNULL(f.DisplayText, f.TypeName) FacilityTypeName
                     , LinkedBuildingGroupID, LinkedBuildingID, LinkedZoneID
                  from SopConfigLinkedSop l
                 inner join SopSensorFacilityType f on l.FacilityTypeID=f.FacilityTypeID
                 inner join SopCategoryDisasterCategory dc on l.DisasterCategoryID = dc.ID
                 inner join SopCategorySubDisasterCategory sdc on dc.ID=sdc.DisasterCategoryID and l.SubDisasterCategoryID=sdc.ID
                 where (1=1) ";
        ...
        public List<LinkedSopCountByVersion> JoinSopCategoryConfigLinkedSop(string strConditions, out string strErrorMessage)
        {
            strErrorMessage = null;
            string strSQL = $@"
                select l.{LinkedSop.Fields.ID}, (select count(*) from {Disaster.TableName} d2 where d2.{Disaster.Fields.SubDisasterCategoryID}= sdc.{SubDisasterCategory.Fields.ID}) count
                  from {DisasterCategory.TableName} dc
                 inner join {SubDisasterCategory.TableName} sdc on dc.{DisasterCategory.Fields.ID}=sdc.{SubDisasterCategory.Fields.DisasterCategoryID}
                 inner join {Disaster.TableName} d on sdc.ID=d.{Disaster.Fields.SubDisasterCategoryID}
                 inner join {LinkedSop.TableName} l on dc.{DisasterCategory.Fields.ID}=l.{LinkedSop.Fields.DisasterCategoryID} 
                                                       and sdc.{SubDisasterCategory.Fields.ID}=l.{LinkedSop.Fields.SubDisasterCategoryID} 
                                                       and d.{Disaster.Fields.DisasterName}=l.{LinkedSop.Fields.DisasterName}
                 where (1=1)";
            ...
            List<LinkedSopCountByVersion> count = new List<LinkedSopCountByVersion>();
            int nResultCount = arrResult.Count;
            for (int i = 0; i < nResultCount - 1; i+=2)
            {
                int nLinkedSopID = WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                int nCount = WebDBManager.GetIntField(arrResult[i + 1].ToString(), -1);

                count.Add(new LinkedSopCountByVersion() { LinkedSopID = nLinkedSopID, Count = nCount });
            }

            return count;
        }

        public ArrayList ReadQuery(string strSQL, out string strErrorMessage)
        {
            strErrorMessage = null;
            ArrayList arrResult = m_dbManager.GetResultData(strSQL);
        ...
        public int GetColumnMaximumLength(string strTableName, string strColumnName, out string strErrorMessage)
        {
            strErrorMessage = null;
            string strSQL = "";
            ...
            if (m_dbManager.DatabaseType == DirectDBManager.DBType.sqlserver)
            {
                strSQL = $"SELECT CHARACTER_MAXIMUM_LENGTH from INFORMATION_SCHEMA.COLUMNS where Table_Name = '{strTableName}' and COLUMN_NAME = '{strColumnName}'";
            }
            else if (m_dbManager.DatabaseType == DirectDBManager.DBType.oracle)
            {
                strSQL = $"SELECT DATA_LENGTH FROM COLS WHERE TABLE_NAME = '{strTableName}' AND COLUMN_NAME='{strColumnName}'";
            }
            else if (m_dbManager.DatabaseType == DirectDBManager.DBType.mysql)
            {
                strSQL = $"SELECT CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='{m_dbManager.DbName}' AND TABLE_NAME = '{strTableName}' AND COLUMN_NAME='{strColumnName}'";
            }
            else
                return -1;
            ...
            if (arrResult.Count > 0)
            {
                VariousData<int> count = WebDBManager.GetIntField(arrResult[0].ToString());
