...
namespace SOPManager.BLL
{
    ...
    public class LoadManager
    {
        ...
        public LoadManager(IDataManager manager, ProcessManager processManager)
        {
            ...
            m_strActionStepNames = InitActionStepNames();
        }
        ...
        /// 알람 단계 명칭 조회
        /// </summary>
        public List<string> InitActionStepNames()
        {
            ...
            List<Common.Model.Option.Options> options =
                m_processManager.CommonDataManager.GetSelectManager().SelectOption(Common.Model.Option.Options.OptionTarget.SOPSimulator, "StandardActionStepNames", out strErrorMessage);
        ...
        public ResponseDisasterCategories DisasterCategories(bool isNormal, int? nSiteID)
        {
            ...
            if (LoadSOPCategories(dicDisasterCategories, dicSubDisasterCategories, dicDisasterActionSteps, dicVersions, actionStepNames, isNormal, nSiteID) == false)
        ...
        public bool LoadSOPCategories(Dictionary<DisasterCategory, List<SubDisasterCategory>> dicDisasterCategories, Dictionary<SubDisasterCategory, List<VersionDisasterData>> dicSubDisasterCategories, Dictionary<Disaster, List<ActionStep>> dicDisasterActionSteps, Dictionary<int, Version> dicVersions, List<string> actionStepNames, bool isNormal, int? nSiteID)
        {
            ...
            string strCondition = nSiteID != null && nSiteID > 0 ? $"SiteID={(int)nSiteID}" : "";
            List<DisasterCategory> disasterCategories = selectManager.SelectDisasterCategories(strCondition, out strErrorMessage);
            if (strErrorMessage != null)
                return false;

            //List<Common.Model.Option.Options> options = null;
            //if (nSiteID != null && nSiteID > 0)
            //    options = m_processManager.CommonDataManager.GetSelectManager().SelectOption(Common.Model.Option.Options.OptionTarget.SOPSimulator, "StandardActionStepNames", (int)nSiteID, out strErrorMessage);
            //else
            //    options = m_processManager.CommonDataManager.GetSelectManager().SelectOption(Common.Model.Option.Options.OptionTarget.SOPSimulator, "StandardActionStepNames", out strErrorMessage);
            //
            //if (options == null || strErrorMessage != null)
            //    return false;

            strCondition = string.Format("isNormal = {0}", isNormal ? 1 : 0);
            if (nSiteID != null && nSiteID > 0)
                strCondition += $" And SiteID={(int)nSiteID}";
            ...
            List<string> standardActionStepNames = InitActionStepNames();
                    ...
                    foreach (KeyValuePair<string, List<Disaster>> pair in dicDisasters)
                    {
                        ...
                        foreach (Disaster disaster in pair.Value)
                        {
                            ...
                            if (dicVersions.TryGetValue(disaster.VersionID, out version))
                            {
                                ...
                                {
                                    User user = m_dataManager.GetSelectManager().SelectUser((int)version.OwnerID, out strErrorMessage);
        ...
        private List<string> GetStandardActionStepNames()
        {
            return m_strActionStepNames;
            //List<string> actionStepNames = new List<string>();
        ...
        public ResponseActionStepDatas GetDefaultActionStepDatas()
        {
            ...
            List<string> standardActionStepNames = GetStandardActionStepNames();
        ...
        public ResponseStepMemberData GetDefaultStepMemberData(int? nSiteID = null)
        {
            string strErrorMessage;
            string strCondition = nSiteID != null && nSiteID > 0 ? $"{Regular.Fields.SiteID}={nSiteID}" : string.Empty;
            List<Regular> regularTeams = m_processManager.TeamDataManager.GetSelectManager().SelectRegulars(null, strCondition, out strErrorMessage);
        ...
        {
            if (nDisasterID < 0)
                return new ResponseDisasterVersions(true, "");

            string strErrorMessage;
            Disaster _disaster = m_dataManager.GetSelectManager().SelectDisaster(nDisasterID, out strErrorMessage);

            if (_disaster == null)
                return new ResponseDisasterVersions(false, strErrorMessage);

            if (m_processManager.GetSaveManager().IsRunningVersion(_disaster.VersionID, m_dataManager))
                return new ResponseDisasterVersions(false, "실행중인 SOP는 편집할 수 없습니다.");
        ...
        {
            //m_processManager.CommonDataManager.GetSelectManager().SelectSites();

            string strErrorMessage;
            SOPData sopData = XMLManager.OpenXML(strXML, -1/*m_dataManager.SiteID*/, out strErrorMessage);
        ...
        {
            return _ParseSpecialMessage(data);
        }

        public static ResponseParseSpecialMessage _ParseSpecialMessage(RequestParseSpecialMessage data)
        {
            string strErrorMessage;
        ...
        public ResponseLoadLinkedSopDatas LoadLinkedSopDatas(int? nSiteID)
        {
            ResponseLoadLinkedSopDatas res = new ResponseLoadLinkedSopDatas();
            try
            {
                ...
                if (IsSopAlone)
                    arrResult = m_processManager.SopDataManager.GetSelectManager().JoinSopAloneConfigCategory(nSiteID, out strError);
                else
                    arrResult = m_processManager.SopDataManager.GetSelectManager().JoinSopConfigCategory(nSiteID, out strError);

                if (arrResult == null)
                    throw new ApplicationException(strError);

                List<LinkedSopData> linkDatas = new List<LinkedSopData>();

                int nResultCount = arrResult.Count;
                for (int i = 0; i < nResultCount - 13; i += 14)
                {
                    VariousData<int> nLinkID = WebDBManager.GetIntField(arrResult[i].ToString());
                    VariousData<int> nFacilityTypeID = WebDBManager.GetIntField(arrResult[i + 1].ToString());
                    VariousData<int> nDisasterCategoryID = WebDBManager.GetIntField(arrResult[i + 2].ToString());
                    VariousData<int> nSubDisasterCategoryID = WebDBManager.GetIntField(arrResult[i + 3].ToString());
                    string strCategoryName = WebDBManager.GetStringField(arrResult[i + 4]);
                    string strSubCategoryName = WebDBManager.GetStringField(arrResult[i + 5]);
                    string strDisasterName = WebDBManager.GetStringField(arrResult[i + 6]);
                    string strBuildingGroupName = WebDBManager.GetStringField(arrResult[i + 7]);
                    string strBuildingName = WebDBManager.GetStringField(arrResult[i + 8]);
                    string strZoneName = WebDBManager.GetStringField(arrResult[i + 9]);
                    string strFacilityTypeName = WebDBManager.GetStringField(arrResult[i + 10]);
                    VariousData<int> nLinkedBuildingGroupID = WebDBManager.GetIntField(arrResult[i + 11].ToString());
                    VariousData<int> nLinkedBuildingID = WebDBManager.GetIntField(arrResult[i + 12].ToString());
                    VariousData<int> nLinkedZoneID = WebDBManager.GetIntField(arrResult[i + 13].ToString());

                    if (nLinkID == null || nFacilityTypeID == null || nDisasterCategoryID == null || nSubDisasterCategoryID == null || strFacilityTypeName == null)
                        continue;

                    LinkedSopData data = new LinkedSopData();
                    data.LinkID = nLinkID.Data;
                    data.FacilityTypeID = nFacilityTypeID.Data;
                    data.DisasterCategoryID = nDisasterCategoryID.Data;
                    data.SubDisasterCategoryID = nSubDisasterCategoryID.Data;
                    data.CategoryName = strCategoryName;
                    data.SubCategoryName = strSubCategoryName;
                    data.DisasterName = strDisasterName;
                    data.ZoneName = string.Concat(strBuildingGroupName, strBuildingName, strZoneName);
                    data.FacilityTypeName = strFacilityTypeName;
                    if (nLinkedBuildingGroupID != null)
                        data.LinkedBuildingGroupID = nLinkedBuildingGroupID.Data;
                    if (nLinkedBuildingID != null)
                        data.LinkedBuildingID = nLinkedBuildingID.Data;
                    if (nLinkedZoneID != null)
                        data.LinkedZoneID = nLinkedZoneID.Data;

                    linkDatas.Add(data);
                }

                res.LinkedSopDatas = linkDatas;
                res.Success = true;
                return res;
            }
            catch (Exception ex)
        ...
        public ResponseLoadLinkedSopVersions LoadLinkedSopVersions(int nSiteID, List<int> versionIDs)
        {
            string strError = null;
            ResponseLoadLinkedSopVersions res = new ResponseLoadLinkedSopVersions();
            try
            {
                string strConditions = $"l.{DisasterCategory.Fields.SiteID}={nSiteID}";
                strConditions += versionIDs != null && versionIDs.Count > 0 ? $" and {Disaster.Fields.VersionID} in ({string.Join(",", versionIDs)})" : string.Empty;

                List<LinkedSopCountByVersion> ids = m_dataManager.GetSelectManager().JoinSopCategoryConfigLinkedSop(strConditions, out strError);
                if (ids == null)
                    throw new ApplicationException(strError);

                res.VersionIDs = ids.Select(p => p.LinkedSopID).ToList();
                res.Success = true;
                return res;
            }
            catch (Exception ex)
