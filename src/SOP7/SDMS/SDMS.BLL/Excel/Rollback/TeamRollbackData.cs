    ...
    public class TeamRollbackData : IRollbackData
    {
        ...
        public bool Rollback(SDMS.IDAL.IDataManager sdmsDataManager, TeamEditor.IDAL.IDataManager teamDataManager, SOPManager.IDAL.IDataManager sopDataManager)
        {
            ...
            if (m_insertUsers != null)
            {
                ...
                {
                    if (sopDataManager.GetCreateManager().CreateUser(user.ID, user.UserLevel, user.UserID, user.Password, user.NickName, user.SiteID, user.PasswordCode) == null)
                        return false;
                }
            }
            ...
            if (m_insertSessions != null)
            {
                ...
                {
                    if (sopDataManager.GetCreateManager().CreateSession(session.AccountUserID, session.SessionKey, session.CreateDate, session.UpdateDate, session.IsAutoLogin, session.BrowserID) == null)
                        return false;
                }
            }
