...
namespace SDMS.BLL.Excel.Writer
{
    public abstract class ExcelWriter
    {
        ...
        protected TeamEditor.IDAL.IDataManager m_teamDataManager = null;

        public ExcelWriter(IDataManager dataManager, TeamEditor.IDAL.IDataManager teamDataManager)
        {
            m_dataManager = dataManager;
            m_teamDataManager = teamDataManager;
        }

        public byte[] Run(List<int> siteIDs, out string strErrorMessage)
        {
            try
            {
                ICollection<SheetData> sheetDatas = ReadSheetDatas(siteIDs, out strErrorMessage);
        ...
        protected abstract ICollection<SheetData> ReadSheetDatas(List<int> siteIDs, out string strErrorMessage);
        protected abstract string GetSubject();

        public static ExcelWriter MakeInstance(DataMode mode, IDataManager dataManager, TeamEditor.IDAL.IDataManager teamDataManager)
        {
            if (mode == DataMode.FacilityInfo)
                return new FacilityInfoWriter(dataManager, teamDataManager);
            else if (mode == DataMode.BuildingData)
                return new BuildingWriter(dataManager, teamDataManager);
            else if (mode == DataMode.BuildingGroupData)
                return new BuildingGroupWriter(dataManager, teamDataManager);
            else if (mode == DataMode.RegularTeamData)
                return new RegularMemberWriter(dataManager, teamDataManager);
