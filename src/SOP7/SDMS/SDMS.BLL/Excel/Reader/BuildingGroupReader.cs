...
namespace SDMS.BLL.Excel.Reader
{
    public class BuildingGroupReader : ExcelReader
    {
        ...
        protected override bool UpdateData(List<SheetData> sheetDatas, int? siteID, out string strErrorMessage)
        {
            ...
            Dictionary<string, List<BuildingGroupData>> dicBuildingGroupDatas = ReadDB(m_dataManager, out dicBuildingGroups, out dicBuildingGroupNames, out strErrorMessage, MakeSiteIDs(siteID));

            if (dicBuildingGroupDatas == null)
            {
                if (strErrorMessage != null)
                {
                    System.Diagnostics.Trace.WriteLine("BuildingGroupReader.UpdateData Fail : " + strErrorMessage);
                }

                strErrorMessage = "건물그룹 DataBase에 데이터 오류가 존재합니다.\r\n시스템 관리자에게 문의해 주세요.";
                return false;
            }
        ...
        public static Dictionary<string, List<BuildingGroupData>> ReadDB(IDataManager dataManager, out Dictionary<int, BuildingGroup> dicBuildingGroups, out Dictionary<string, BuildingGroup> dicBuildingGroupNames, out string strErrorMessage, List<int> siteIDs)
        {
            ...
            string strAdditionalConditions = string.Format("{0} in ({1})", BuildingGroupData.Fields.BuildingGroupID, strBuildingGroupIDs);

            List<BuildingGroupData> datas = dataManager.GetSelectManager().SelectBuildingGroupDatas(null, strAdditionalConditions, out strErrorMessage);
