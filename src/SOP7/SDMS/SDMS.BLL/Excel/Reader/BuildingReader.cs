...
namespace SDMS.BLL.Excel.Reader
{
    public class BuildingReader : ExcelReader
    {
        ...
        protected override bool UpdateData(List<SheetData> sheetDatas, int? siteID, out string strErrorMessage)
        {
            ...
            // Key : Building Name
            Dictionary<string, List<BuildingData>> dicBuildingDatas = ReadDB(m_dataManager, out dicBuildings, out dicBuildingNames, out strErrorMessage, MakeSiteIDs(siteID));

            if (dicBuildingDatas == null)
            {
                if (strErrorMessage != null)
                {
                    System.Diagnostics.Trace.WriteLine("BuildingReader.UpdateData Fail : " + strErrorMessage);
                }

                strErrorMessage = "건물 DataBase에 데이터 오류가 존재합니다.\r\n시스템 관리자에게 문의해 주세요.";
                return false;
            }
        ...
        private List<int> MakeSiteIDs(int? siteID)
        {
            if (siteID == null)
                return null;

            List<int> siteIDs = new List<int>();
            siteIDs.Add((int)siteID);
            return siteIDs;
        }
        ...
        public static Dictionary<string, List<BuildingData>> ReadDB(IDataManager dataManager, out Dictionary<int, Building> dicBuildings, out Dictionary<string, Building> dicBuildingNames, out string strErrorMessage, List<int> siteIDs)
        {
            dicBuildings = null;
            ...
            if (siteIDs != null && siteIDs.Count > 0)
                strCondition = string.Format("{0} in ({1})", BuildingGroup.Fields.SiteID, string.Join(",", siteIDs.ToArray()));
            /*Dictionary<BuildingGroup.Fields, object> dicConditions = new Dictionary<BuildingGroup.Fields, object>();

            if (siteID != null)
                dicConditions[BuildingGroup.Fields.SiteID] = (int)siteID;*/

            List<BuildingGroup> buildingGroups = dataManager.GetSelectManager().SelectBuildingGroups(null, strCondition, out strErrorMessage);

            if (buildingGroups == null || strErrorMessage != null)
                return null;

            string strBuildingGroupIDs = "-1";

            foreach (BuildingGroup group in buildingGroups)
            {
                if (strBuildingGroupIDs == "-1")
                    strBuildingGroupIDs = group.ID.ToString();
                else
                    strBuildingGroupIDs += "," + group.ID.ToString();
            }

            string strAdditionalConditions = string.Format("{0} in ({1})", Building.Fields.BuildingGroupID, strBuildingGroupIDs);

            List<Building> buildings = dataManager.GetSelectManager().SelectBuildings(null, strAdditionalConditions, out strErrorMessage);

            if (buildings == null || strErrorMessage != null)
                return null;

            string strBuildingIDs = "-1";

            foreach (Building data in buildings)
            {
                if (strBuildingIDs == "-1")
                    strBuildingIDs = data.ID.ToString();
                else
                    strBuildingIDs += "," + data.ID.ToString();
            }

            strAdditionalConditions = string.Format("{0} in ({1})", BuildingData.Fields.BuildingID, strBuildingIDs);

            List<BuildingData> datas = dataManager.GetSelectManager().SelectBuildingDatas(null, strAdditionalConditions, out strErrorMessage);
