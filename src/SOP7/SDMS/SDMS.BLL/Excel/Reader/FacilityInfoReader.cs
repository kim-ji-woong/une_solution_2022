...
namespace SDMS.BLL.Excel.Reader
{
    public class FacilityInfoReader : ExcelReader
    {
        ...
        protected override bool UpdateData(List<SheetData> sheetDatas, int? siteID, out string strErrorMessage)
        {
            ...
            Dictionary<string, Dictionary<int, List<InfoData>>> dicBuildingFacilityDatas = ReadDB(out dicFacilityInfo, out strErrorMessage, siteID);
            ...
            bool result = CheckData(dicBuildingFacilityDatas, dicFacilityInfo, sheetDatas, out strErrorMessage);
        ...
        // Key : Building Name
        // Value : Key(Zone ID), Value(Zone별 FacilityInfoData List)
        private Dictionary<string, Dictionary<int, List<InfoData>>> ReadDB(out Dictionary<int, Info> dicFacilityInfos, out string strErrorMessage, int? siteID)
        {
            ...
            List<Building> buildings = m_dataManager.GetSelectManager().SelectBuildings(null, strAdditionalConditions, out strErrorMessage);

            if (buildings == null || strErrorMessage != null)
                return null;

            Dictionary<Zone.Fields, object> dicConditions_Zone = new Dictionary<Zone.Fields, object>();

            if (siteID != null)
                dicConditions_Zone[Zone.Fields.SiteID] = (int)siteID;

            List<Zone> zones = m_dataManager.GetSelectManager().SelectZones(dicConditions_Zone, null, out strErrorMessage);
            ...
            foreach (Zone data in zones)
            {
                if (strZoneIDs == "-1")
                    strZoneIDs = data.ID.ToString();
                else
                    strZoneIDs += "," + data.ID.ToString();
            }

            strAdditionalConditions = string.Format("{0} in ({1})", Info.Fields.ZoneID, strZoneIDs);
            ...
            ArrayList arrDatas = m_dataManager.GetSelectManager().JoinFacilityInfoFacilityInfoData(null, null, strAdditionalConditions, out strErrorMessage);
