...
namespace SDMS.IDAL
{
    ...
    public interface ICreate
    {
        ...
        PSM CreatePSMSensor(string strName, string strPositionName, float? x, float? y, float? z, float? fCurrentData, int nZoneID, int nEquipZoneID, string strDepartment, string strDepartmentPhoneNumber, int? nStatus, string strUniqueKey, int? materialType, float? limitBase, int? limitType, string limitValue);
        ...
        Model.GLTF.PrivateModelData CreateGltfPrivateModelData(int nModelDataID, int nUserID, Vertex3D vCameraPosition, Quaternion qCameraQuaternion, Vertex3D vCameraRotation, int nFov, float fNear, float fFar, Vertex3D vOrbitTarget);
        Model.GLTF.PrivateModelOrthoData CreateGltfPrivateModelOrthoData(int nModelDataID, int nUserID, Vertex3D vCameraPosition, Quaternion qCameraQuaternion, Vertex3D vCameraRotation, Vertex3D vTarget, float fZoom);
        ...
        Sdms2DImage CreateSdms2DImage(Sdms2DImage obj, out string strErrorMessage);
        ETC CreateSdmsSensorETC(ETC obj, out string strErrorMessage);

        Elevator CreateElevator(int doorStatus, int directionStatus, int runStatus, int floorIndex, bool isNormal, string strName, int? groupNo, int siteID, out string strErrorMessage);

        WorkerInfo CreateWorkerInfo(WorkerInfo obj, out string strErrorMessage);

        #region 안전구역평가
        Assessment CreateAssessment(Assessment obj, out string strErrorMessage);
        AssessmentA CreateAssessmentA(AssessmentA obj, out string strErrorMessage);
        AssessmentAItem CreateAssessmentAItem(AssessmentAItem obj, out string strErrorMessage);
        AssessmentAMember CreateAssessmentAMember(AssessmentAMember obj, out string strErrorMessage);
        AssessmentQ CreateAssessmentQ(AssessmentQ obj, out string strErrorMessage);
        AssessmentQItem CreateAssessmentQItem(AssessmentQItem obj, out string strErrorMessage);
        #endregion
    }
}
