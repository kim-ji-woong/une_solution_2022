...
namespace SDMS.DAL
{
    ...
    {
        protected DirectDBManager m_dbManager = null;
        ...
        protected Vertex3D StringToVertex3D(string strVertex)
        {
            ...
            // 독일 문화권 설정에 따른 점과 쉼표의 역할이 달라지기 때문에 예외처리
            if (double.TryParse(tokens[0].Trim(), NumberStyles.Any,  CultureInfo.InvariantCulture, out x) &&
                ...
                double.TryParse(tokens[2].Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out z))
        ...
        protected void SetQuery(ref string strSQL, string strCondition, string strAdditionalConditions)
        {
            string strOrderBy = "";
