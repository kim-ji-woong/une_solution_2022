...
namespace IntegrationServer.Servers.Blackout.GG_F
{
    class ClientProvider : ClientServiceProvider
    {
        ...
        BlackoutGGFManager m_parentManager = null;
        ...
        public ClientProvider(BlackoutGGFManager mgr,int nServerSeqNo)
        {
            m_parentManager = mgr;
        ...
        public override void OnReceiveData()
        {
            ...
            {
                //m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.Blackout_GG_F, m_nServerSeqNo, "ContactProvider OnReceiveData() : " + e.Message);
        ...
        private void ConnectionThread()
        {
            ...
            while (m_runThread)
            {
                    ...
                    if (!this.IsConnected)
                    {
                        lock (this)
                        {
                                ...
                                && m_parentManager.ServerIP != null && m_parentManager.ServerIP != "")
                            {
                                ...
                                {   // 연결 성공
                                    m_bIsConnect = true;
                                    //m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.Blackout_GG_F, m_nServerSeqNo, "ConnectionThread() : " + m_parentManager.ServerIP + ":" + m_parentManager.Port.ToString() + " / " + this.IsConnected);
                                    ...
                                    {
                                        //m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.Blackout_GG_F, m_nServerSeqNo, $"ConnectionThread() : (UpdateServerStatus Error {strError})");
                ...
                {
                    //m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.Blackout_GG_F, m_nServerSeqNo, "ConnectionThread() : " + e.Message);
            ...
            while (m_runThread)
            {
                try
                {
                    if (this.IsConnected)
                    {
                        ...
                        UInt16 nStartAddr = 2000;  // 입력포트 주소
                ...
                {
                    //m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.Blackout_GG_F, m_nServerSeqNo, "RequestThread() : " + e.Message);
