...
namespace IntegrationServer.Servers.Blackout.Gyeyoung
{
    class ClientProvider : ClientServiceProvider
    {
        private static byte FC_ReadHlodingReg = 0x03;
        ...
        GyeyoungManager m_parentManager = null;
        ...
        public ClientProvider(GyeyoungManager mgr, int nServerSeqNo)
        {
            m_parentManager = mgr;
        ...
        public override void OnReceiveData()
        {
            ...
            try
            {
                ...
                if (FC == FC_ReadHlodingReg && nDataLeng == RequestLength * RegisterLength)
                {   // 신호 체크
            ...
            while (m_runThread)
            {
                try
                {
                    if (this.IsConnected)
                    {
                        ...
                        UInt16 nStartAddr = 2;  // .TODO : 입력포트 주소 확인 필요
                        UInt16 nLength = RequestLength; // 데이터 읽을 갯수
                        int nSlaveID = 1;   // .TODO: SlaveID 확인 필요

                        byte[] arrData = MakeRequestMsg(FC_ReadHlodingReg, nSlaveID, m_nTransID, nStartAddr, nLength, IsReverse);
                        SendBytes(arrData);
