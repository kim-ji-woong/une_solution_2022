...
namespace IntegrationServer.Servers.FMS.GG_D
{
    ...
    {
        SumpPitGGDManager m_parentManager = null;
        ...
        public override void OnReceiveData()
        {
            ...
            try
            {
                ...
                if (FC == FC_Discrete && nDataLeng == RequestLength / DividBit)
                {   // 신호 체크
        ...
        private void RequestThread()
        {
            while (m_runThread)
            {
                try
                {
                    if (this.IsConnected)
                    {
                        UInt16 nStartAddr = 23;  // 시작 주소(10024)
                        UInt16 nLength = RequestLength; // 데이터 읽을 갯수
                        int nSlaveID = 1;   // SlaveID

                        byte[] arrData = MakeRequestMsg(FC_Discrete, nSlaveID, m_nTransID, nStartAddr, nLength, IsReverse);
                        SendBytes(arrData);
