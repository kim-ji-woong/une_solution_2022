...
{
    public class SiemensProcessData
    {
        ...
        public SiemensProcessData(SiemensManager parentManager, ConnectionState state, int nServerSeqNo)
        {
        ...
        public bool OnReceive(byte[] bytes)
        {
            ...
            return ProcessSensorData(bytes);
        }
        ...
        private bool ProcessSensorData(byte[] bytes)
        {
            ...
            EventTypes eventType = GetEventType(opCode);
            if (eventType == EventTypes.ClearAll)
                ProcessAllClear();
            else if (eventType == EventTypes.DetectSensor)
                ProcessFire(tagNo);
            else if (eventType == EventTypes.ClearSensor)
                ProcessClear(tagNo);

            return true;
        }

        public void ProcessFire(int nTagNo)
        {
            SensorTag sensorTag = SensorManager.Instance.FindSensor(m_nServerSeqNo, nTagNo);
            m_parentManager.SendSensorData(sensorTag, true);
        }

        public void ProcessClear(int nTagNo)
        {
            SensorTag sensorTag = SensorManager.Instance.FindSensor(m_nServerSeqNo, nTagNo);
        ...
        private EventTypes GetEventType(byte opCode)
        {
            if (opCode == ClearAll)
                return EventTypes.ClearAll;
            else if (opCode == DetectFire)
                return EventTypes.DetectSensor;
            else if (opCode == ClearSensor)
                return EventTypes.ClearSensor;
            else if (opCode == DetectError)
                return EventTypes.DetectError;
            else if (opCode == ClearError)
                return EventTypes.ClearError;
            /* 테스트 (감시, 감시 복구)
             * else if (opCode == 0x96)
