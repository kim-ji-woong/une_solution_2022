...
namespace IntegrationServer.Servers.CCTV.S1.SVMS
{
    ...
    public class SVMSEventReceiver : IDisposable
    {
        ...
        private ServerTypes m_serverType = ServerTypes.CCTV_S1_SVMS;
        ...
        public SVMSEventReceiver(ISVMSEventOwner owner, int nSiteID, string ip, int port, string id, string pw)
        {
            m_owner = owner;
            m_nSiteID = nSiteID;
            m_strSvmsIP = ip;
        ...
        #region static
        public static List<SVMSEventReceiver> MakeInstances(ISVMSEventOwner owner, DataManager dataManager, int nSiteID, List<string> svmsIPs, List<int> svmsPorts, List<string> svmsIDs, List<string> svmsPWs)
        {
            ...
            {                
                SVMSEventReceiver receiver = new SVMSEventReceiver(owner, nSiteID, svmsIPs[i], svmsPorts[i], svmsIDs[i], svmsPWs[i]);
                receiver.m_dataManager = dataManager;
        ...
        public static List<SVMSEventReceiver> CloneInstances(List<SVMSEventReceiver> receivers)
        {
            ...
            {
                SVMSEventReceiver _receiver = new SVMSEventReceiver(receiver.m_owner, receiver.m_nSiteID, receiver.SvmsServerIP, receiver.SvmsPort, receiver.ID, receiver.Password);
        ...
        private static bool ReadConfig(List<string> svmsIPs, List<int> svmsPorts, List<string> svmsIDs, List<string> svmsPWs)
        {
            ...
            for (int i = 0; i < nCount; i++)
            {
                ...
                string strID_ = ids[i].Trim();
                string strPW_ = pws[i].Trim();
        ...
        private void OnModifiedCamera(string arg1, bool isSuccess, S1SVMSSDKv2.Model.Device.DeviceCamera modifyDeviceCamera, XmlNode originalActionStructure)
        {
            if (isSuccess == true)
            {
                ...
                if (m_dicCameras.TryGetValue(modifyDeviceCamera.CameraGUID, out cctv))
                {
                    ...
                    {
                        //m_parentManager.Logger.Write(LogTypes.Info, m_serverType, m_nServerSeqNo, "OnModifiedCamera, CCTV[" + cctv.ID + "], " + cctv.UniqueKey + ", Enabled : " + isEnabled);
                    ...
                    foreach (var deviceCameraItem in deviceCameras)
                    {
                        try
                        {
                            ...
                            if (deviceCameraGUID != null)
                            {
                                ...
                                //m_parentManager.Logger.Write(LogTypes.Info, m_serverType, m_nServerSeqNo, "SVMSEventCameraList, CCTV[" + cctv.CameraName + "], " + cctv.UniqueKey + ", Enabled : " + cctv.Enabled);
