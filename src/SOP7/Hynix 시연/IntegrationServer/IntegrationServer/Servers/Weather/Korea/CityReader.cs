...
namespace IntegrationServer.Servers.Weather.Korea
{
    public class CityReader
    {
        ...
        public CityReader(DataManager dataManager, WeatherManager manager)
        {
        ...
        private bool ReadCities()
        {
            ...
            for (int i = 0; i < nDataCount; i++)
            {
                ...
                int nIndex1 = weatherData.CityName.IndexOf('(');
                int nIndex2 = weatherData.CityName.IndexOf(')');
                
                if (nIndex1 < 0 || nIndex2 < nIndex1)
                    continue;

                string strTrgCity = weatherData.CityName.Substring(0, nIndex1).Trim();
                string strSrcCity = weatherData.CityName.Substring(nIndex1 + 1, nIndex2 - nIndex1 - 1).Trim();

                if (weatherData.RegionCode == null || weatherData.RegionCode.Length == 0)
                    return false;
                ...
                data.SetRegion(weatherData.RegionCode);
        ...
        private bool ReadData(Dictionary<int, bool> dicRead, int nIndex, int nCityCount)
        {
            ...
            try
            {
                ...
                using (HttpWebResponse resp = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    if (status != HttpStatusCode.OK)
                    {
                        ...
                        m_manager?.WriteLog("URL 실패 : " + data.URL, LogTypes.Error);
                        return false;
                    }
            ...
            catch (Exception e)
            {
                ...
                m_manager?.WriteLog("ReadData(Dictionary<int, bool>, int, int) " + e.Message, LogTypes.Error);
                success = false;
            }
        ...
        public void WriteCurrentData(DataManager dataManager, string strCityName, int state, float currentTemperature, float sensibleTemperature, float rain, float humidity, int windDirection, float windSpeed, float atm)
        {
            ...
            IEnumerable<Site> sites = dataManager.GetSelect().Select<Site>(string.Format("{0} = '{1}'", Site.Fields.Name.ToString(), strCityName), out strErrorMessage);
            ...
            if (sites == null)
            {
                if (strErrorMessage != null)
                {
                    ...
                    m_manager?.WriteLog("WriteCurrentData : " + strErrorMessage, LogTypes.Error);
                    return;
                }
            }
            ...
            if (sites == null)
            {
                ...
                m_manager?.WriteLog(strCityName + "에 해당하는 WeatherSite가 존재하지 않습니다.", LogTypes.Error);
                return;
            }

            Current current = dataManager.GetSelect().SelectFirst<Current>(string.Format("{0} = {1}", Current.Fields.WeatherSiteID.ToString(), site.ID), out strErrorMessage);
            ...
            if (current == null)
            {
                ...
                current.WeatherSiteID = site.ID;
                current.WindDirection = windDirection;
                current.WindSpeed = windSpeed;

                if (dataManager.GetCreate().Insert<Current>(current, out strErrorMessage) == false)
            ...
            else
            {
                ...
                if (dataManager.GetUpdate().Update<Current>(current, null, out strErrorMessage) == false)
        ...
        private bool ParseData(string strHTML, string strCity, out int state, out float currentTemperature, out float sensibleTemperature, out float rain, out float humidity, out int windDirection, out float windSpeed, out float atm)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                else if (nCurrentTemperatureIdx != -1 && i == nCurrentTemperatureIdx)
                {
                    ...
                    {
                        // 35 이상일 경우 <span> 태그 예외처리
                        int nIndex1 = strHTML.IndexOf("<span bgcolor=", nIndex);
                        int nIndex2 = strHTML.IndexOf("</span>", nIndex);

                        if (nIndex1 < 0 || nIndex2 < nIndex1)
                            return false;
                        else
                        {
                            strValue = strHTML.Substring(nIndex1 + 24, nIndex2 - nIndex1 - 24).Trim();
                ...
                else if (nSensibleTemperatureIdx != -1 && i == nSensibleTemperatureIdx)
                {
                    if (float.TryParse(strValue, out sensibleTemperature) == false)
                        return false;
                    else if (float.IsNaN(sensibleTemperature))
                        sensibleTemperature = 0;
                }
                ...
                else if (nWindDirectionIdx != -1 && i == nWindDirectionIdx)
                {
                    if (m_dicWindDirection.TryGetValue(strValue, out windDirection) == false)
                    {
                        ...
                        m_manager?.WriteLog("Unknown Wind Direction : " + strValue, LogTypes.Error);
                        return false;
                    }
                }
        ...
        public static bool ReadWeatherState(string strValue, ref int state)
        {
