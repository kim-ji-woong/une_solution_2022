                ...
                foreach (var data in m_dicSensors)
                {
                        ...
                        decimalSensorCode == nSensorCode)
                    {
                            ...
                            if (sensorZone != null)
                            {
                                if (AlarmLevel > 0)
                                {
                                    ...
                                    if (!AlarmManager.Instance.DicCurrentAlarm.TryGetValue(sensorZone.SensorZoneID, out AlarmInfo alarm)) // 알람 미존재
                                    {
                                        ...
                                        m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.PSM_Senko, AirPollutionManager.Instance.ServerSeqNo, "[Info] : 알람 발생 - SensorZoneID : " + sensorZone.ID.ToString());
                                    }
                                    else // 알람 존재
                                    {
                                        // 알람 단계 변경
                                        if (alarm.AlarmDepth != AlarmLevel)
                                        {
                                            if (AlarmLevel > 0)
                                            {
                                                ...
                                                SensorTag sensorTag = SensorManager.Instance.FindSensorByOrgSensorID(m_parentManager.ServerSeqNo, sensorZone.ID);
                                                m_parentManager.SendSensorData(sensorTag, dnsData.Sensor.Facility.FacilityType.PSM_SENSOR, true, AlarmLevel);
                                                m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.PSM_Senko, AirPollutionManager.Instance.ServerSeqNo, "[Info] : 알람 변경 - SensorZoneID : " + sensorZone.ID.ToString());
                                            }
                                        }
                                    }
                                }
                                else // AlarmLevel == 0
                                {
                                    if (AlarmManager.Instance.DicCurrentAlarm.TryGetValue(sensorZone.SensorZoneID, out AlarmInfo alarm)) // 알람 존재 
                                    {
                                        ...
                                        m_parentManager.Logger.Write(LogTypes.Info, ServerTypes.PSM_Senko, AirPollutionManager.Instance.ServerSeqNo, "[Info] : 알람 해제 - SensorZoneID : " + sensorZone.ID.ToString());
                                    }
                                }
                            }
                        }
                        else
                        {
                            ...
                            m_parentManager.Logger.Write(LogTypes.Error, ServerTypes.PSM_Senko, m_parentManager.ServerSeqNo, strErrorMessage);
                        }
                    }
                }
