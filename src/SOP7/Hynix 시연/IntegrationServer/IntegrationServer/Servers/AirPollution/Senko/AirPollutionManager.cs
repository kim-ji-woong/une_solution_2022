...
namespace IntegrationServer.Servers.AirPollution.Senko
{
    public partial class AirPollutionManager : IServer
    {
        ...
        {
            Logger.Write(Datas.LogTypes.Info, ServerTypes.PSM_Senko, ServerSeqNo, "[INFO] : State Connection " + state.IPAddress + " " + state.PortNo.ToString());
        }

        public void OnDropConnection(dnsTcpLib2.ConnectionState state)
        {
            Logger.Write(Datas.LogTypes.Info, ServerTypes.PSM_Senko, ServerSeqNo, "[INFO] : State Drop Connection " + state.IPAddress + " " + state.PortNo.ToString());
        }
