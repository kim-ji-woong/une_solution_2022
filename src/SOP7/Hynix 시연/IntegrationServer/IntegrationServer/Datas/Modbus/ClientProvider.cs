...
namespace IntegrationServer.Datas.Modbus
{
    ...
    {
        // Event Log만 남길 것인가?
        protected bool m_onlyEventLog = true;
        protected IServer m_parentManager = null;
        ...
        public ClientProvider(IServer mgr, string strServerIP, int port = 502)
        {
            m_parentManager = mgr;
        ...
        public override void OnReceiveData()
        {
            ...
            {
                WriteLog(LogTypes.Error, "ClientProvider OnReceiveData() : " + e.Message);
            }
        }
            ...
            while (m_runThread)
            {
                try
                {
                    if (!this.IsConnected)
                    {
                        lock (this)
                        {
                            if (m_parentManager != null && m_nPort > 0 && m_strServerIP != null && m_strServerIP != "")
                            {
                                this.Connect(m_strServerIP, m_nPort);

                                if (this.IsConnected)
                                {
                                    // 연결 성공
                                    WriteLog(LogTypes.Info, "ConnectionThread() : " + m_strServerIP + ":" + m_nPort.ToString() + " / " + this.IsConnected);
                                }
                                else
                                {
                                    // 연결 실패
                ...
                {
                    WriteLog(LogTypes.Error, "ConnectionThread() : " + e.Message);
                }
            }
        }
        ...
        private void RequestThread()
        {
            while (m_runThread)
            {
                ...
                {
                    WriteLog(LogTypes.Error, "RequestThread() : " + e.Message);
                }
            }
        }
        ...
        protected void SendBytes(byte[] bytes)
        {
            try
            {
                if (!this.IsClientDisposed && this.IsConnected)
                {
                    ...
                    {
                        if (m_onlyEventLog == false)
                            WriteBinaryLog(bytes, 0, bytes.Length, "[Send]");
                    }
                }
            }
            catch (Exception e)
            {
                WriteLog(LogTypes.Error, "SendBytes() : " + e.Message);
            }
        }

        protected virtual string WriteBinaryLog(byte[] bytes, int nIndex, int len, string strTag)
        {
            string strBytesLog = Logger.GetByteString(bytes, nIndex, len);

            if (m_parentManager.Logger != null)
                m_parentManager.Logger.Write(LogTypes.Info, m_parentManager.ServerType, m_parentManager.ServerSeqNo, strTag + " : " + strBytesLog);
            else
                Logger.Instance.Write(LogTypes.Info, m_parentManager.ServerType, m_parentManager.ServerSeqNo, strTag + " : " + strBytesLog);

            return strTag + " : " + strBytesLog;
        }

        protected virtual void WriteLog(LogTypes type, string strLog)
        {
            if (m_parentManager.Logger != null)
                m_parentManager.Logger.Write(type, m_parentManager.ServerType, m_parentManager.ServerSeqNo, strLog);
            else
                Logger.Instance.Write(type, m_parentManager.ServerType, m_parentManager.ServerSeqNo, strLog);
        }
    }
}
