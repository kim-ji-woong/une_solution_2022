...
namespace SOPWebServer.BLL.Models
{
    ...
    public class SensorZoneGroup
    {
        ...
        public static SensorZoneGroup GetSensorZoneGroup(IDataManager dataManager, SensorManager sensorManager, int nSensorZoneID, int nSensorType)
        {
            bool isNullable;

            string strCondition = string.Format("{0}.{1} = {2} and {0}.{3} = {4}",
                SensorZoneHistory.TableName,
                SensorZoneHistory.GetFieldName(SensorZoneHistory.Fields.SensorZoneID, out isNullable), nSensorZoneID,
                SensorZoneHistory.GetFieldName(SensorZoneHistory.Fields.SensorType, out isNullable), nSensorType);

            string strErrorMessage;
            ArrayList arrDatas = dataManager.GetSelectManager().JoinCurrentAlarmSensorZoneHistory(strCondition, out strErrorMessage);

            if (arrDatas == null)
                return MakeDefault(sensorManager, nSensorZoneID, nSensorType);

            int nDataCount = arrDatas.Count;

            for (int i=0;i<nDataCount-1;i+=2)
            {
                if (arrDatas[i] is SDMS.Model.Alarm.CurrentAlarm && arrDatas[i + 1] is SensorZoneHistory)
                {
                    var alarm = (SDMS.Model.Alarm.CurrentAlarm)arrDatas[i];
                    var sensorZoneHistory = (SensorZoneHistory)arrDatas[i + 1];

                    strCondition = string.Format("{0} = {1} and {2} = {3}",
                        SensorReactionHistory.GetFieldName(SensorReactionHistory.Fields.SensorZoneHistoryID, out isNullable), sensorZoneHistory.ID,
                        SensorReactionHistory.GetFieldName(SensorReactionHistory.Fields.ReactionType, out isNullable), (int)SensorReactionHistory.ReactionTypes.BEGIN_STATUS);

                    List<SensorReactionHistory> reactionHistories = dataManager.GetSelectManager().SelectSensorReactionHistories(null, strCondition, out strErrorMessage);

                    SensorZoneGroup sensorZoneGroup = MakeDefault(sensorManager, nSensorZoneID, nSensorType);
                    sensorZoneGroup.CurrentAlarm = FromCopy(alarm);

                    if (sensorZoneGroup.CurrentAlarm != null)
                    {
                        foreach (var reactionHistory in reactionHistories)
                        {
                            sensorZoneGroup.CurrentAlarm.ReactionHistoryParam4 = reactionHistory.Param4;
                            break;
                        }
                    }

                    return sensorZoneGroup;
                }
            }
        ...
        private static SensorZoneGroup MakeDefault(SensorManager sensorManager, int nSensorZoneID, int nSensorType)
        {
            SensorZoneGroup sensorZoneGroup = new SensorZoneGroup();
            sensorZoneGroup.SensorType = (Facility.FacilityType)nSensorType;

            SensorZone sensorZone = sensorManager.GetSensorZone(nSensorZoneID);

            if (sensorZone != null)
            {
                EquipmentZone equipZone = sensorManager.GetEquipmentZone(sensorZone.EquipZoneID);
                sensorZoneGroup.EquipmentZone = equipZone;
                sensorZoneGroup.SetID();
            }

            return sensorZoneGroup;
        }

        private static AlarmData FromCopy(SDMS.Model.Alarm.CurrentAlarm currentAlarm)
        {
            AlarmData alarm = new AlarmData();

            alarm.SensorZoneHistoryID = currentAlarm.SensorZoneHistoryID;
            alarm.AlarmDepth = currentAlarm.AlarmDepth;
            alarm.TimeStamp = currentAlarm.TimeStamp;

            if (currentAlarm.AlarmSensorZoneIDs.Count > 0)
                alarm.SensorZoneID = currentAlarm.AlarmSensorZoneIDs[0];
