...
namespace SOPWebServer.BLL
{
    ...
    public class SensorManager
    {
        ...
        public Result OnReceive(int _sensorType, int header, string strClientInfo, ArrayList arrDatas)
        {
            Facility.FacilityType sensorType = Facility.ToFacilityType(_sensorType);
            ...
            else if (_sensorType == (int)Facility.FacilityType.Event_ForcedDoorOpen)    // 강제 문열림
            {
                Server.ForcedDoorOpenSensor server = new Server.ForcedDoorOpenSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_CheatedTagging)    // 대리태깅
            {
                Server.CheatedTaggingSensor server = new Server.CheatedTaggingSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_Untagging)    // 꼬리물기
            {
                Server.UntaggingSensor server = new Server.UntaggingSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_StealCard)    // 사원증도용
            {
                Server.StealCardSensor server = new Server.StealCardSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_Stranger)    // 이상행위자
            {
                Server.StrangerSensor server = new Server.StrangerSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_EvasionItem)    // 무인 보안검색 우회
            {
                Server.EvasionItemSensor server = new Server.EvasionItemSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_NotPermittedPerson)    // 비인가 구역 출입
            {
                Server.NotPermittedPersonSensor server = new Server.NotPermittedPersonSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
            else if (_sensorType == (int)Facility.FacilityType.Event_NotPermittedItem)    // 비인가 구역 반입
            {
                Server.NotPermittedItemSensor server = new Server.NotPermittedItemSensor(m_mainManager, m_factory);
                return server.OnReceive(header, strClientInfo, arrDatas);
            }
