...
namespace SOPWebServer.BLL.Server
{
    ...
    class StrangerSensor : BaseServer
    {
        private class MovingLine
        {
            private DateTime m_timeStamp = new DateTime();
            private string m_strMoving = "";
            ...
            {
                get { return m_strMoving; }
                set { m_strMoving = value; }
            }
        }

        private const string EventTypeName = "이상행위자";
        private const string EventType = "Stranger";
        private const string EventTypeLv3 = "StrangerLv3";

        private MainManager m_mainManager = null;

        public StrangerSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Stranger);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                {
                    if (CheckAlarmTime(m_mainManager.CommonDataManager, EventTypeLv3) == false)
                    {
                ...
                for (int i=5;i<dataCount-2;i+=2)
                {
                    if (arrDatas[i] is DateTime && arrDatas[i + 1] is string)
                    {
                        MovingLine movingLine = new MovingLine();
                        movingLine.Timestamp = (DateTime)arrDatas[i];
                        movingLine.Moving = (string)arrDatas[i + 1];
                        movingLines.Add(movingLine);
                    }
                }

                Facility.FacilityType sensorType = Facility.ToFacilityType(nSensorType);
                SensorZoneGroup group = SensorZoneGroup.GetSensorZoneGroup(m_mainManager.SDMSDataManager, m_mainManager.SensorManager, nSensorZoneID, (int)Facility.FacilityType.Event_Stranger);
                ...
                if (nSensorData > 0)
                {
                    ...
                    // 알람 발생
                    AlarmData alarm;
                    int nResult = AddAlarm(group, nSensorTagID, sensorZone, isReal, nAlarmLevel, dtEvent, movingLines, out alarm);
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int? nAlarmLevel, DateTime dtEvent, List<MovingLine> movingLines, out AlarmData alarm)
        {
            alarm = null;
            ...
            else
            {
                ...
                alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).AddAlarm(sensorZone.ID, 1, nZoneID, (int)Facility.FacilityType.Event_Stranger, (int)detectionStatus, timeStamp, m_mainManager.SDMSDataManager, FacilityManager.DetectTypes.Detect, nSiteID);
                ...
                if (alarm != null)
                {
                    ...
                    string strParam3 = ((int)Facility.FacilityType.Event_Stranger).ToString();
                    ...
                    if (((Process.AlarmManager)m_mainManager.AlarmManager).AddReactionHistory(alarm, (int)reactionType, timeStamp, strMessage, strEquipZoneID, sensorZone.ID.ToString(), strParam3, strParam4, strParam5, m_mainManager.SDMSDataManager))
                    {
                        ...
                        AddSensorZoneHistoryInfo(sensorZone, alarm.SensorZoneHistoryID, alarm.TimeStamp, movingLines, out strErrorMessage);
        ...
        public MessageResult AddMovingPosition(int sensorZoneHistoryID, DateTime timestamp, string strPosition)
        {
            ...
            string strCondition = string.Format("{0} = (Select max({0}) from {1} where {2} = {3})",
                Hynix.Model.History.SensorZoneInfo.GetFieldName(Hynix.Model.History.SensorZoneInfo.Fields.OrderIndex, out isNullable),
                Hynix.Model.History.SensorZoneInfo.TableName,
                Hynix.Model.History.SensorZoneInfo.GetFieldName(Hynix.Model.History.SensorZoneInfo.Fields.SensorZoneHistoryID, out isNullable),
                sensorZoneHistoryID);

            string strErrorMessage;
            List<Hynix.Model.History.SensorZoneInfo> sensorZoneHistoryInfos = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSensorZoneHistoryInfos(null, strCondition, out strErrorMessage);

            if (sensorZoneHistoryInfos == null)
                return new MessageResult(false, strErrorMessage);

            int historyCount = sensorZoneHistoryInfos.Count;
            int orderIndex = historyCount == 0 ? 1 : sensorZoneHistoryInfos[historyCount - 1].OrderIndex + 1;
            int? workerID = GetWorkerID(sensorZoneHistoryID, out strErrorMessage);
            ...
            sensorZoneInfo.Param = string.Format("{0}, {1}", CheatedTaggingSensor.GetTimeString(timestamp), strPosition);
        ...
        private int? GetWorkerID(int sensorZoneHistoryID, out string strErrorMessage)
        {
            SensorZoneHistory sensorZoneHistory = m_mainManager.SDMSDataManager.GetSelectManager().SelectSensorZoneHistory(sensorZoneHistoryID, out strErrorMessage);

            if (sensorZoneHistory == null)
                return null;

            if (sensorZoneHistory.AllSensorZoneIDs.Count == 0)
                return null;

            int sensorZoneID = sensorZoneHistory.AllSensorZoneIDs[0];
            SensorZone sensorZone = m_mainManager.SDMSDataManager.GetSelectManager().SelectSensorZone(sensorZoneID, out strErrorMessage);

            if (sensorZone == null)
                return null;

            Worker worker = GetWorker(sensorZone, out strErrorMessage);
        ...
        private bool AddSensorZoneHistoryInfo(SensorZone sensorZone, int sensorZoneHistoryID, DateTime dtEvent, List<MovingLine> movingLines, out string strErrorMessage)
        {
            ...
            foreach (MovingLine movingLine in movingLines)
            {
                Hynix.Model.History.SensorZoneInfo sensorZoneInfo = new Hynix.Model.History.SensorZoneInfo();
                ...
                sensorZoneInfo.WorkerID = workerID;
                sensorZoneInfo.ItemID = null;
                sensorZoneInfo.Param = string.Format("{0}, {1}", CheatedTaggingSensor.GetTimeString(movingLine.Timestamp), movingLine.Moving);
        ...
        private Worker GetWorker(SensorZone sensorZone, out string strErrorMessage)
        {
