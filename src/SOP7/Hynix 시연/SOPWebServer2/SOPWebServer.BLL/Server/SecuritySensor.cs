    ...
    public class SecuritySensor : BaseServer
    {
        ...
        private Result ProcessSensorData(ArrayList arrDatas, bool isReal)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                int nAlarmDepthTemp = (arrDatas.Count >= 5 && arrDatas[4] is int) ? (int)arrDatas[4] : -1;
                ...
                //if (nSensorData > 0)
                {
                    ...
                    // 알람 발생
                    AlarmData alarm;
                    int nResult = AddAlarm(group, nSensorTagID, sensorZone, isReal, sensorType, nSensorData, svmsDevice, out alarm);
                    if (nAlarmDepthTemp == -1)
                    {
                ...
                else
                {
                    ...
                    foreach (AlarmData alarm in alarms)
                    {
                        ...
                        else if (alarm != null && group.CurrentAlarm != null)
                        {
                            ...
                            {
                                alarm.AlarmDepth = nAlarmDepthTemp;
                            }
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, Facility.FacilityType sensorType, int nSensorData, SVMSDevice svmsDevice, out AlarmData alarm)
        {
            alarm = null;
