...
{
    public class LaserSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public LaserSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Laser);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("LaserSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            else
            {
                ...
                if (zone != null)
                {
                    ...
                    strMessage = string.Format("[{0}]에서 레이저 감지 상황이 신고되었습니다", szLocationName);
                }
            }
        ...
        {
            string strMessage = "신고된 레이저 감지 상황이 종료되었습니다";
            int nZoneID;
            ...
            if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
            {
                ...
                if (zone != null)
                {
                    strMessage = string.Format("[{0}]에서 신고된 레이저 감지 상황이 종료되었습니다", zone.DisplayText);
                }
            }
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsLaserSensorType(alarm.SensorType))
                    continue;
