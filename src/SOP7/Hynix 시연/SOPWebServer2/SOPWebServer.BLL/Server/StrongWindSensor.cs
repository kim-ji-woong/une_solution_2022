...
{
    public class StrongWindSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public StrongWindSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.StrongWind);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("StrongWindSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
                ...
                else
                {
                    ...
                    if (alarm == null)
                        alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).FindAlarm(nSensorZoneID, Facility.ToFacilityType(nSensorType), m_mainManager.SDMSDataManager);
        ...
        private string GetDetectMessage(int nOrgSensorID, EquipmentZone equipZone, bool isReal)
        {
            ...
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.STRONG_WIND) + " 신호";
        ...
        private Result ProcessManualReport(ArrayList arrDatas)
        {
                                    ...
                                    && arrDatas[4] is int && arrDatas[5] is string && arrDatas[6] is string)
            {
                ...
                AlarmData alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).GetManualAlarm(nZoneID, Facility.FacilityType.STRONG_WIND, m_mainManager.SDMSDataManager);
                if (alarm != null)
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            else
            {
                ...
                if (zone != null)
                {
                    ...
                    strMessage = string.Format("[{0}]에서 강풍 상황이 신고되었습니다", szLocationName);
                }
            }
        ...
        {
            string strMessage = "신고된 강풍 상황이 종료되었습니다";
            int nZoneID;
            ...
            if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
            {
                ...
                if (zone != null)
                {
                    strMessage = string.Format("[{0}]에서 신고된 강풍 상황이 종료되었습니다", zone.DisplayText);
                }
            }
