...
{
    public class BatterySensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public BatterySensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.LowBattery);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("BatterySensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.LowBattery) + " 신호";
        ...
        private Result ProcessManualReport(ArrayList arrDatas)
        {
                                    ...
                                    && arrDatas[4] is int && arrDatas[5] is string && arrDatas[6] is string)
            {
                ...
                AlarmData alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).GetManualAlarm(nZoneID, Facility.FacilityType.LowBattery, m_mainManager.SDMSDataManager);
                if (alarm != null)
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (nZoneID < 0)
            {
                strMessage = "배터리 방전경고 상황이 신고되었습니다";
            }
            else
            {
                ...
                if (zone != null)
                {
                    ...
                    strMessage = string.Format("[{0}]에서 배터리 방전경고 상황이 신고되었습니다", szLocationName);
                }
            }
        ...
        {
            string strMessage = "신고된 배터리 방전경고 상황이 종료되었습니다";
            int nZoneID;
            ...
            if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
            {
                ...
                if (zone != null)
                {
                    strMessage = string.Format("[{0}]에서 신고된 배터리 방전경고 상황이 종료되었습니다", zone.DisplayText);
                }
            }
