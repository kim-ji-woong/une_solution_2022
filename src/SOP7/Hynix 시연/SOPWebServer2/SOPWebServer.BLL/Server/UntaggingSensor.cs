...
namespace SOPWebServer.BLL.Server
{
    ...
    class UntaggingSensor : BaseServer
    {
        private const string EventTypeName = "꼬리물기";
        private const string EventType = "Untagging";

        private MainManager m_mainManager = null;

        public UntaggingSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Untagging);
        }
        ...
        public static bool CheckEvent(MainManager mainManager, Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, SmartTag smartTag, out string strErrorMessage)
        {
            ...
            if (IsInWork(mainManager, cardTagHistories, out strErrorMessage) == false)
            {
                UntaggingSensor server = new UntaggingSensor(mainManager, mainManager.SensorManager.Factory);
                server.ProcessAlarm(tagHistory, sensorZone, tagHistory.SmartTagReaderID);
                return true;
            }
        ...
        public static bool IsInWork(MainManager mainManager, List<Hynix.Model.History.CardTag> cardTagHistories, out string strErrorMessage)
        {
            strErrorMessage = null;

            int historyCount = cardTagHistories.Count;

            for (int i=historyCount-1; i >= 0;i--)
            {
                var history = cardTagHistories[i];

                if (history.IsApprove && history.Type == (int)Hynix.Model.History.CardTag.TaggingTypes.Incoming)
                    return true;
                else if (history.IsApprove && history.Type == (int)Hynix.Model.History.CardTag.TaggingTypes.OutGoing)
                    return false;
            }
        ...
        private void ProcessAlarm(Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, int smartTagReaderID)
        {
            ...
            arrDatas.Add((int)Facility.FacilityType.Event_Untagging);
            ...
            ProcessSensorData(Header.SENSOR_DATA, arrDatas, true, smartTagReaderID);
        }

        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, int? smartTagReaderID = null, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                SensorZoneGroup group = SensorZoneGroup.GetSensorZoneGroup(m_mainManager.SDMSDataManager, m_mainManager.SensorManager, nSensorZoneID, (int)Facility.FacilityType.Event_Untagging);
                ...
                if (nSensorData > 0)
                {
                    ...
                    // 알람 발생
                    AlarmData alarm;
                    int nResult = AddAlarm(group, nSensorTagID, sensorZone, isReal, nAlarmLevel, dtEvent, smartTagReaderID, out alarm);
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int? nAlarmLevel, DateTime dtEvent, int? smartTagReaderID, out AlarmData alarm)
        {
            alarm = null;
            ...
            else
            {
                ...
                DateTime timeStamp = DateTime.Now;
                int nZoneID = -1;
                GetSmartTagReaderID(smartTagReaderID, out nZoneID);
                ...
                alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).AddAlarm(sensorZone.ID, 1, nZoneID, (int)Facility.FacilityType.Event_Untagging, (int)detectionStatus, timeStamp, m_mainManager.SDMSDataManager, FacilityManager.DetectTypes.Detect, nSiteID);
                ...
                if (alarm != null)
                {
                    ...
                    string strParam3 = ((int)Facility.FacilityType.Event_Untagging).ToString();
                    string strParam5 = alarm.AlarmDepth.ToString();

                    // param4는 TargetSensorID
                    string strParam4 = smartTagReaderID == null ? null : ((int)smartTagReaderID).ToString();
        ...
        private int? GetSmartTagReaderID(int? smartTagReaderID, out int zoneID)
        {
            zoneID = -1;
            ...
            SmartTagReader smartTagReader = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagReader((int)smartTagReaderID, out strErrorMessage);

            if (smartTagReader != null)
            {
                zoneID = smartTagReader.ZoneID;
                return smartTagReader.SmartTagReaderID;
            }
        ...
        private bool AddSensorZoneHistoryInfo(SensorZone sensorZone, int sensorZoneHistoryID, DateTime dtEvent, out string strErrorMessage)
        {
            ...
            bool isNullable;
            int smartTagID = (int)sensorZone.OrgSensorID;

            string strCondition = string.Format("{0} in (Select {1} from {2} where {3} = {4})",
                Worker.GetFieldName(Worker.Fields.WorkerID, out isNullable),
                SmartTag.GetFieldName(SmartTag.Fields.WorkerID, out isNullable),
                SmartTag.TableName,
                SmartTag.GetFieldName(SmartTag.Fields.SmartTagID, out isNullable),
                smartTagID);
            ...
            strCondition = string.Format("{0} = {1}", SmartTag.GetFieldName(SmartTag.Fields.WorkerID, out isNullable), worker.WorkerID);

            List<SmartTag> smartTags = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTags(null, strCondition, out strErrorMessage);

            if (smartTags == null || smartTags.Count == 0)
                return false;

            SmartTag smartTag = smartTags[0];

            strCondition = string.Format("{0} = (Select max({0}) from {1} where {2} = {3}) and {2} = {3}",
                ...
                Hynix.Model.History.SmartTag.GetFieldName(Hynix.Model.History.SmartTag.Fields.SmartTagID, out isNullable),
                smartTag.SmartTagID);

            List<Hynix.Model.History.SmartTag> tagHistories = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagHistorys(null, strCondition, out strErrorMessage);
            ...
            strCondition = string.Format("{0} = {1}", SmartTagReader.GetFieldName(SmartTagReader.Fields.SmartTagReaderID, out isNullable), tagHistory.SmartTagReaderID);
            List<SmartTagReader> tagReaders = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagReaders(null, strCondition, out strErrorMessage);

            if (tagReaders == null || tagReaders.Count == 0)
                return false;
            ...
            sensorZoneInfo.Param = string.Format("{0}, {1}", CheatedTaggingSensor.GetTimeString(dtEvent), tagReaders[0].UniqueKey);
        ...
        {
            string strEventName = "사원증 태깅을 하지않은 작업자가 발견되었습니다.";
            return strEventName;
        }
