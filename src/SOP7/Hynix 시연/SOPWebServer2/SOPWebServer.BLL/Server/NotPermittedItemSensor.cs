...
namespace SOPWebServer.BLL.Server
{
    ...
    class NotPermittedItemSensor : BaseServer
    {
        private const string EventTypeName = "꼬리물기";
        private const string EventType = "NotPermittedItem";

        private MainManager m_mainManager = null;

        public NotPermittedItemSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
        ...
        public static bool CheckEvent(MainManager mainManager, Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, SmartTag smartTag, out string strErrorMessage)
        {
            ...
            SmartTagReader smartTagReader = mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagReader(tagHistory.SmartTagReaderID, out strErrorMessage);

            if (smartTagReader == null)
                return false;

            Item item = mainManager.HynixDataManager.GetSelectManager().SelectHynixItem((int)smartTag.ItemID, out strErrorMessage);

            if (item == null)
                return false;

            bool isNullable;
            string strCondition = string.Format("{0} = {1}", ItemLinkZone.GetFieldName(ItemLinkZone.Fields.ItemID, out isNullable), item.ItemID);
            List<ItemLinkZone> linkedZones = mainManager.HynixDataManager.GetSelectManager().SelectHynixItemLinkZones(null, strCondition, out strErrorMessage);

            if (linkedZones == null)
                return false;

            foreach (ItemLinkZone zone in linkedZones)
            {
                if (zone.ZoneID == smartTagReader.ZoneID)
                    return false;
            }

            NotPermittedItemSensor server = new NotPermittedItemSensor(mainManager, mainManager.SensorManager.Factory);
            server.ProcessAlarm(tagHistory, sensorZone, smartTagReader);
            return true;
        }

        private void ProcessAlarm(Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, SmartTagReader smartTagReader)
        {
            ...
            arrDatas.Add((int)Facility.FacilityType.Event_NotPermittedItem);
            ...
            ProcessSensorData(Header.SENSOR_DATA, arrDatas, true, smartTagReader);
        }

        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, SmartTagReader smartTagReader = null, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                SensorZoneGroup group = SensorZoneGroup.GetSensorZoneGroup(m_mainManager.SDMSDataManager, m_mainManager.SensorManager, nSensorZoneID, (int)Facility.FacilityType.Event_NotPermittedItem);
                ...
                if (nSensorData > 0)
                {
                    ...
                    // 알람 발생
                    AlarmData alarm;
                    int nResult = AddAlarm(group, nSensorTagID, sensorZone, isReal, nAlarmLevel, dtEvent, smartTagReader, out alarm);
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int? nAlarmLevel, DateTime dtEvent, SmartTagReader smartTagReader, out AlarmData alarm)
        {
            alarm = null;
            ...
            else
            {
                ...
                DateTime timeStamp = DateTime.Now;
                int nZoneID = smartTagReader == null ? -1 : smartTagReader.ZoneID;
                ...
                alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).AddAlarm(sensorZone.ID, 1, nZoneID, (int)Facility.FacilityType.Event_NotPermittedItem, (int)detectionStatus, timeStamp, m_mainManager.SDMSDataManager, FacilityManager.DetectTypes.Detect, nSiteID);
                ...
                if (alarm != null)
                {
                    ...
                    string strParam3 = ((int)Facility.FacilityType.Event_NotPermittedItem).ToString();
                    string strParam5 = alarm.AlarmDepth.ToString();

                    // param4는 TargetSensorID
                    string strParam4 = smartTagReader == null ? null : smartTagReader.SmartTagReaderID.ToString();
        ...
        private bool AddSensorZoneHistoryInfo(SensorZone sensorZone, int sensorZoneHistoryID, DateTime dtEvent, out string strErrorMessage)
        {
            ...
            string strCondition = string.Format("{0} in (Select {1} from {2} where {3} = {4})",
                Item.GetFieldName(Item.Fields.ItemID, out isNullable),
                SmartTag.GetFieldName(SmartTag.Fields.ItemID, out isNullable),
                SmartTag.TableName,
                SmartTag.GetFieldName(SmartTag.Fields.SmartTagID, out isNullable),
                smartTagID);

            List<Item> items = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixItems(null, strCondition, out strErrorMessage);
            ...
            List<Hynix.Model.History.SmartTag> smartTagHistories = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagHistorys(null, strCondition, out strErrorMessage);

            if (smartTagHistories == null)
                return false;

            int historyCount = smartTagHistories.Count;

            if (historyCount == 0)
                return false;

            var tagHistory = smartTagHistories[historyCount - 1];
            SmartTagReader smartTagReader = m_mainManager.HynixDataManager.GetSelectManager().SelectHynixSmartTagReader(tagHistory.SmartTagReaderID, out strErrorMessage);
            ...
            sensorZoneInfo.WorkerID = null;
            sensorZoneInfo.ItemID = item.ItemID;
            sensorZoneInfo.Param = smartTagReader.UniqueKey;
        ...
        {
            string strEventName = "반입이 허용되지 않은 구역에서 물품이 발견되었습니다.";
            return strEventName;
        }
