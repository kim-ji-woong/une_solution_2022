...
namespace SOPWebServer.BLL.Server
{
    ...
    class NotPermittedPersonSensor : BaseServer
    {
        private const string EventTypeName = "비인가 구역 출입";
        private const string EventType = "NotPermittedPerson";

        private MainManager m_mainManager = null;

        public NotPermittedPersonSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
        ...
        public static bool CheckEvent(MainManager mainManager, Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, SmartTag smartTag, out string strErrorMessage)
        {
            ...
            Worker worker = mainManager.HynixDataManager.GetSelectManager().SelectHynixWorker((int)smartTag.WorkerID, out strErrorMessage);

            if (worker == null)
                return false;

            bool isNullable;
            string strCondition = string.Format("{0} = {1}", WokerLinkZone.GetFieldName(WokerLinkZone.Fields.WorkerID, out isNullable), worker.WorkerID);
            List<WokerLinkZone> linkedZones = mainManager.HynixDataManager.GetSelectManager().SelectHynixWokerLinkZones(null, strCondition, out strErrorMessage);

            if (linkedZones == null)
                return false;

            foreach (WokerLinkZone zone in linkedZones)
            {
                if (zone.ZoneID == smartTagReader.ZoneID)
                    return false;
            }

            NotPermittedPersonSensor server = new NotPermittedPersonSensor(mainManager, mainManager.SensorManager.Factory);
        ...
        private void ProcessAlarm(Hynix.Model.History.SmartTag tagHistory, SensorZone sensorZone, SmartTagReader smartTagReader)
        {
            ...
            arrDatas.Add((int)Facility.FacilityType.Event_NotPermittedPerson);
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, SmartTagReader smartTagReader = null, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                SensorZoneGroup group = SensorZoneGroup.GetSensorZoneGroup(m_mainManager.SDMSDataManager, m_mainManager.SensorManager, nSensorZoneID, (int)Facility.FacilityType.Event_NotPermittedPerson);
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int? nAlarmLevel, DateTime dtEvent, SmartTagReader smartTagReader, out AlarmData alarm)
        {
            ...
            else
            {
                ...
                alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).AddAlarm(sensorZone.ID, 1, nZoneID, (int)Facility.FacilityType.Event_NotPermittedPerson, (int)detectionStatus, timeStamp, m_mainManager.SDMSDataManager, FacilityManager.DetectTypes.Detect, nSiteID);
                ...
                if (alarm != null)
                {
                    ...
                    string strParam3 = ((int)Facility.FacilityType.Event_NotPermittedPerson).ToString();
        ...
        {
            string strEventName = "출입이 허용되지 않은 구역에서 작업자가 발견되었습니다.";
            return strEventName;
        }
