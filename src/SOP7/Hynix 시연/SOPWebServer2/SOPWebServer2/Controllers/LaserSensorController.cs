...
namespace SOPWebServer2.Controllers
{
    ...
    [ApiController]
    public class LaserSensorController : Controller
    {
        private SOPWebServer.BLL.MainManager m_mainManager = null;

        public LaserSensorController(SDMS.IDAL.IDataManager sdmsDataManager, Common.IDAL.IDataManager commonDataManager, TeamEditor.IDAL.IDataManager teamDataManager, Hynix.IDAL.IDataManager hynixDataManager)
        {
        ...
        public IActionResult Post(SensorParameter param)
        {
            ...
            Result result = m_mainManager.SensorManager.OnReceive((int)Facility.FacilityType.Laser, param.Header, param.ClientInfo, arrDatas);
            return Ok(result);
        }
    }
}
