...
namespace SOPWebServer2.Controllers
{
    ...
    public class FireSensorController : ControllerBase
    {
        ...
        public FireSensorController(SDMS.IDAL.IDataManager sdmsDataManager, Common.IDAL.IDataManager commonDataManager, TeamEditor.IDAL.IDataManager teamDataManager, Hynix.IDAL.IDataManager hynixDataManager)
        {
        ...
        public IActionResult Post(SensorParameter param)
        {
            ...
            if (arrDatas != null && arrDatas.Count > 0 && arrDatas[0] is int && param.Header != Header.CLEAR_DETECT_ALL)
            {
                ...
                Result result = m_mainManager.SensorManager.OnReceive((int)Facility.ToFacilityType(nSensorType), param.Header, param.ClientInfo, arrDatas);
                return Ok(result);
            }
