...
namespace WebSOPApp.Areas.SDMS.Controllers
{
    ...
    public class SensorSimulatorController : Controller
    {
        ...
        private IActionResult RequestSendSensorAlarm(RequestSendSensorAlarm data)
        {
            ...
            //else
            if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.H2)
                strUrl += "api/H2Sensor";
            else if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.Temp)
                strUrl += "api/TempSensor";
            else if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.Flow)
                strUrl += "api/FlowSensor";
            else if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.Conductivity)
                strUrl += "api/ConductivitySensor";
            else if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.PRESSURE_SENSOR)
                strUrl += "api/PressureSensor";
            else if (data.SensorType == (int)dnsData.Sensor.Facility.FacilityType.GAS)
                strUrl += "api/GasSensor";
            else
