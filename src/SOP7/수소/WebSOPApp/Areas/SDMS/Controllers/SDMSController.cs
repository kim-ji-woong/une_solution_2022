...
namespace WebSOPApp.Areas.SDMS.Controllers
{
    ...
    public class SDMSController : Controller
    {
        ...
        private IActionResult RequestElevators()
        {
            ResponseElevators response = m_processManager.GetLoadManager().GetElevators(0);
            return Ok(response);
        }
        ...
        {
            bool result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.H2);
            if (result == false)
                return Ok(result);

            result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.Temp);
            if (result == false)
                return Ok(result);

            result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.Flow);
            if (result == false)
                return Ok(result);

            result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.Conductivity);
            if (result == false)
                return Ok(result);

            result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.PRESSURE_SENSOR);
            if (result == false)
                return Ok(result);

            result = m_processManager.GetAlarmManager().AllClearReport((int)dnsData.Sensor.Facility.FacilityType.GAS);
            if (result == false)
                return Ok(result);
