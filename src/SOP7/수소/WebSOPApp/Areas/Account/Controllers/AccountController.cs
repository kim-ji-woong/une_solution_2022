...
namespace WebSOPApp.Areas.Account.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(global::SOPManager.IDAL.IDataManager sopDataManager, global::Common.IDAL.IDataManager commonDataManager, global::TeamEditor.IDAL.IDataManager teamDataManager, global::SDMS.IDAL.IDataManager sdmsDataManager, global::Hydrogen.IDAL.IDataManager hyDataManager)
        {
        ...
        public IActionResult RequestData([FromBody] AccountData data)
        {
            ...
            else if (data.RequestNewUsers != null)
                return RequestNewUsers(data.RequestNewUsers);

            return BadRequest();
        }

        private IActionResult RequestNewUsers(RequestNewUsers data)
        {
            ResponseNewUsers response = m_processManager.GetAccountManager().MakeNewUsers(data);
            return Ok(response);
        }

        private IActionResult RequestNewUser(RequestNewUser data)
        {
            ResponseNewUser response = m_processManager.GetAccountManager().MakeNewUser(data);
            return Ok(response);
        }
        ...
        {
            Hydrogen.BLL.Models.LoginResult result = null;
            ...
            if (data.Value != "" || data.Key != "")
            {
                try
                {
                    ...
                    if (nIndex > 0)
                    {
                        ...
                        result = m_procMgr.AccountManager.Login_Hydrogen(strID, strPW, data.Key, autoLogin, null);
                    ...
                    {
                        result = new Hydrogen.BLL.Models.LoginResult();
                        result.Success = false;
                    }
                }
                catch (Exception e)
        ...
        [HttpPost]
        public IActionResult RequestUpdateUserInfo([FromBody] Hydrogen.BLL.Models.ReqUpdateUserInfo req)
        {
            Hydrogen.BLL.Models.MessageResult res = m_procMgr.AccountManager.UpdateUserInfo(req);
            return Ok(res);
        }

        [HttpPost]
        public IActionResult RequestAddAccount([FromBody] Hydrogen.BLL.Models.ReqAddAccount req)
        {
            Hydrogen.BLL.Models.ResponseAddAccount res = m_procMgr.AccountManager.AddAccount(req);
            return Ok(res);
        }

        [HttpPost]
        public IActionResult RequestDoubleCheckID([FromBody] Hydrogen.BLL.Models.ReqDoubleCheckID req)
        {
            Hydrogen.BLL.Models.ResponseDoubleCheckID res = m_procMgr.AccountManager.DoubleCheckID(req);
            return Ok(res);
        }
    }
}
