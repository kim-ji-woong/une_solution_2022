...
namespace WebSOPApp.Areas.History.Controllers
{
    ...
    public class HistoryController : Controller
    {
        ...
        private global::Hydrogen.BLL.ProcessManager m_procMgr = null;
        public HistoryController(global::SOPManager.IDAL.IDataManager sopDataManager, global::Common.IDAL.IDataManager commonDataManager, global::TeamEditor.IDAL.IDataManager teamDataManager, global::SDMS.IDAL.IDataManager sdmsDataManager, global::Hydrogen.IDAL.IDataManager hyDataManager)
        {
            m_processManager = new global::History.BLL.ProcessManager(commonDataManager, sopDataManager, teamDataManager, sdmsDataManager);
            m_procMgr = new global::Hydrogen.BLL.ProcessManager(sdmsDataManager, sopDataManager, commonDataManager, hyDataManager);
        }
        ...
        public IActionResult RequestSensorDetectHistories_Hydrogen([FromBody] Hydrogen.BLL.Models.RequestSensorDetectHistories data)
        {
            DateTime beginTime = Convert.ToDateTime(data.BeginTime);
            DateTime endTime = Convert.ToDateTime(data.EndTime);

            Hydrogen.BLL.Models.ResponseSensorDetectHistories result = m_procMgr.LoadManager.DisplaySensorDetectHistories(
                beginTime, endTime, data.FacilityType, data.BuildingID, data.ZoneID, data.LastSensorZoneHistoryID, data.RowCount, data.IsDesc, data.SiteID);

            return Ok(result);
        }

        public IActionResult RequestSensorDetectHistories_Hydrogen2([FromBody] Hydrogen.BLL.Models.RequestSensorDetectHistories2 data)
        {
            DateTime beginTime = Convert.ToDateTime(data.BeginTime);
            DateTime endTime = Convert.ToDateTime(data.EndTime);

            Hydrogen.BLL.Models.ResponseSensorDetectHistories result = m_procMgr.LoadManager.DisplaySensorDetectHistories2(
                beginTime, endTime, data.FacilityType, data.BuildingID, data.ZoneID, data.LimitCount, data.RowCount, data.SiteID);
        ...
        [HttpPost]
        public IActionResult RequestGetMinMaxIndex_Hydrogen([FromBody] Hydrogen.BLL.Models.RequestGetMinMaxIndex data)
        {
            DateTime beginTime = Convert.ToDateTime(data.BeginTime);
            DateTime endTime = Convert.ToDateTime(data.EndTime);

            Hydrogen.BLL.Models.ResponseMinMaxIndex result = m_procMgr.LoadManager.GetMinMaxIndex(
                beginTime, endTime, data.FacilityType, data.BuildingID, data.ZoneID);

            return Ok(result);
        }

        [HttpPost]
        public IActionResult RequestGetCountIndex_Hydrogen([FromBody] Hydrogen.BLL.Models.RequestGetMinMaxIndex data)
        {
            DateTime beginTime = Convert.ToDateTime(data.BeginTime);
            DateTime endTime = Convert.ToDateTime(data.EndTime);

            Hydrogen.BLL.Models.ResponseCountIndex result = m_procMgr.LoadManager.GetCountIndex(
                beginTime, endTime, data.FacilityType, data.BuildingID, data.ZoneID);
