...
class NormalTab extends Component {
	...
	selectTurnStart = (mode) => {
		if (mode === null || mode === undefined || this.props.settings.turnStart === mode)
			return;
					...
					<ul className='contents'>
						<li className='item'>
							<div>
								...
								<select id={'monitoringSelect'} onClick={this.onClickSelect}>
									<option>15분</option>
									<option>30분</option>
									<option>1시간</option>
								</select>
							</div>
							<div className={'tooltipBox'} id='tooltip' data-tooltip="3D 회전 대기시간 및 자동회전을 설정합니다." >
						...
						<li className='item'>
							<div>
								...
								<div>
									...
									<label htmlFor='sensor_1'>Conductivity</label>
								</div>
								<div>
									<input type='checkbox' id='sensor_2' />
									<label htmlFor='sensor_2'>Flow</label>
								</div>
								<div>
									<input type='checkbox' id='sensor_3' />
									<label htmlFor='sensor_3'>Gas</label>
								</div>
								<div>
									<input type='checkbox' id='sensor_4' />
									<label htmlFor='sensor_4'>Hydrogen</label>
								</div>
								<div>
									<input type='checkbox' id='sensor_5' />
									<label htmlFor='sensor_5'>Pressure</label>
								</div>
								<div>
									<input type='checkbox' id='sensor_6' />
									<label htmlFor='sensor_6'>Temperature</label>
								</div>
							</div>
							<div className={'tooltipBox'} id='tooltip' data-tooltip="센서 유형별 이벤트 발생 표시를 설정합니다.">
								<img src={tooltip_icon} alt='도움말 아이콘' width={16} height={16} />
							</div>
						</li>
						<li className='item'>
							...
							<div className={'tooltipBox'} id='tooltip' data-tooltip="이벤트 시 자동 화면 전환 여부를 설정합니다.">
								<img src={tooltip_icon} alt='도움말 아이콘' width={16} height={16} />
							</div>
						</li>
						<li className='item'>
							<div>
								<p>이벤트 발생 시 알람 자동종료 설정</p>
								<div>
									<input type='radio' name='eventView' id='' />
									<label htmlFor=''>알람 유지</label>
								</div>
								<div>
									<input type='radio' name='eventView' id='' />
									<label htmlFor=''>24시간 이후 알람 자동종료</label>
								</div>
							</div>
							<div className={'tooltipBox'} id='tooltip' data-tooltip="이벤트 발생 시 알람 자동종료 여부를 설정합니다.">
								<img src={tooltip_icon} alt='도움말 아이콘' width={16} height={16} />
							</div>
						</li>
					</ul>
...
class DetectionTab extends Component {
	...
	initUseAlarmArea() {
		...
		let useAlarmArea = this.props.settings.useAlarmArea;
	...
	*/
	...
	initUseReceive() {
		...
		if (this.props.settings !== null && this.props.settings !== undefined) {
			...
			let earthquakeData = this.props.settings.useReceiveEarthquake;
			let strongWindData = this.props.settings.useReceiveStrongWind;
			let environmentData = this.props.settings.useReceiveEnvironment;
			let manufactureData = this.props.settings.useReceiveManufacture;
		...
		if (this.refUseReceiveFire.current)
			this.refUseReceiveFire.current.checked = this.props.useSensorTypes?.UseFire === true && useReceiveFire === "true" ? true : false;

		if (this.refUseReceivePSM.current)
			this.refUseReceivePSM.current.checked = this.props.useSensorTypes?.UsePSM === true && useReceivePSM === "true" ? true : false;

		if (this.refUseReceiveETC.current)
			this.refUseReceiveETC.current.checked = this.props.useSensorTypes?.UseETC === true && useReceiveETC === "true" ? true : false;

		if (this.refUseReceiveEnvironment.current)
			this.refUseReceiveEnvironment.current.checked = this.props.useSensorTypes?.UseEnvironment === true && useReceiveEnvironment === "true" ? true : false;
		if (this.refUseReceiveManufacture.current)
			this.refUseReceiveManufacture.current.checked = this.props.useSensorTypes?.UseManufacture === true && useReceiveManufacture === "true" ? true : false;

		if (this.refUseReceiveSVMS.current)
			this.refUseReceiveSVMS.current.checked = this.props.useSensorTypes?.UseSVMS === true && useReceiveSVMS === "true" ? true : false;

		if (this.refUseEarthquake.current)
			this.refUseEarthquake.current.checked = this.props.useSensorTypes?.UseEarthquake === true && useReceiveEarthquake === "true" ? true : false;

		if (this.refUseStrongWind.current)
			this.refUseStrongWind.current.checked = this.props.useSensorTypes?.UseStrongWind === true && useReceiveStrongWind === "true" ? true : false;			
	}

	onChangeReceiveFire = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveFire === value) {
			return;
        }
	...
	onChangeReceivePSM = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceivePSM === value) {
			return;
        }
	...
	onChangeReceiveETC = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveETC === value) {
			return;
        }
	...
	onChangeReceiveEnvironment = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveEnvironment === value) {
			return;
		}
		this.props.settings.useReceiveEnvironment = value;
		this.props.onChangeNeedToSave();
	}

	onChangeReceiveManufacture = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveManufacture === value) {
			return;
		}
		this.props.settings.useReceiveManufacture = value;
		this.props.onChangeNeedToSave();
	}

	onChangeReceiveSVMS = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveSVMS === value) {
			return;
        }
	...
	onChangeReceiveEarthquake = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveEarthquake === value) {
			return;
        }
		this.props.settings.useReceiveEarthquake = value;
		this.props.onChangeNeedToSave();
	}

	onChangeReceiveStrongWind = (e) => {
		const value = e.checked.toString();
		if (this.props.settings.useReceiveStrongWind === value) {
			return;
        }
		this.props.settings.useReceiveStrongWind = value;
		this.props.onChangeNeedToSave();
	}
	...
	*/
	...
	selectDisplayAlarm = (mode) => {
		if (mode === null || mode === undefined || this.props.settings.moveDisplayAlarm === mode)
			return;
	...
	selectUsePoiFocus = (use) => {
		if (use === null || use === undefined || this.props.settings.usePoiFocus === use)
			return;
	...
	selectUseAlarmArea = (use) => {
		if (use === null || use === undefined || this.props.settings.useAlarmArea === use)
			return;

		if (use === SettingsResource.useAlarmArea.on ||
			use === SettingsResource.useAlarmArea.off) {
			this.props.settings.useAlarmArea = use;
			this.props.onChangeNeedToSave();
        }
    }
	...
	getReceiveTypeUI = () => {
		...
		if (this.props.useSensorTypes?.UseFire === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_Fire"}><p>{i18n.t('facilityType.화재')}</p><input ref={this.refUseReceiveFire} type="checkbox" name="stgAlt" id="stgAlt01" onChange={(e) => this.onChangeReceiveFire(e.target)} /><label htmlFor="stgAlt01">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
		if (this.props.useSensorTypes?.UsePSM === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_PSM"}><p>{i18n.t('facilityType.누출')}</p><input ref={this.refUseReceivePSM} type="checkbox" name="stgAlt" id="stgAlt02" onChange={(e) => this.onChangeReceivePSM(e.target)} /><label htmlFor="stgAlt02">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseETC === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_ETC"}><p>{i18n.t('facilityType.기타')}</p><input ref={this.refUseReceiveETC} type="checkbox" name="stgAlt" id="stgAlt03" onChange={(e) => this.onChangeReceiveETC(e.target)} /><label htmlFor="stgAlt03">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}


		if (this.props.useSensorTypes?.UseEnvironment === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_Environment"}><p>{i18n.t('facilityType.환경설비')}</p><input ref={this.refUseReceiveEnvironment} type="checkbox" name="stgAlt" id="stgAlt07" onChange={(e) => this.onChangeReceiveEnvironment(e.target)} /><label htmlFor="stgAlt07">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseManufacture === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_Manufacture"}><p>{i18n.t('facilityType.제조설비')}</p><input ref={this.refUseReceiveManufacture} type="checkbox" name="stgAlt" id="stgAlt08" onChange={(e) => this.onChangeReceiveManufacture(e.target)} /><label htmlFor="stgAlt08">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}


		if (this.props.useSensorTypes?.UseSVMS === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_SVMS"}><p>{i18n.t('setting.monitoring3D.SVMS')}</p><input ref={this.refUseReceiveSVMS} type="checkbox" name="stgAlt" id="stgAlt04" onChange={(e) => this.onChangeReceiveSVMS(e.target)} /><label htmlFor="stgAlt04">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseEarthquake === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_Earthquake"}><p>{i18n.t('facilityType.지진')}</p><input ref={this.refUseEarthquake} type="checkbox" name="stgAlt" id="stgAlt05" onChange={(e) => this.onChangeReceiveEarthquake(e.target)} /><label htmlFor="stgAlt05">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
		if (this.props.useSensorTypes?.UseStrongWind === true) {
			receiveTypeUI.push(<li key={"receiveTypeUI_StrongWind"}><p>{i18n.t('facilityType.강풍')}</p><input ref={this.refUseStrongWind} type="checkbox" name="stgAlt" id="stgAlt06" onChange={(e) => this.onChangeReceiveStrongWind(e.target)} /><label htmlFor="stgAlt06">{i18n.t('setting.monitoring3D.수신')}</label></li>);
		}
	...
	initSoundOffTime() {
		...
		let alarmSoundOffTime = this.props.settings.alarmSoundOffTime;
	...
	onBlurSoundOffTime = (e) => {
		...
		this.props.settings.alarmSoundOffTime = value;
	...
	onChangeSoundOffTimeUse = () => {
		...
		this.props.settings.alarmSoundOffTime = soundOffTime;
