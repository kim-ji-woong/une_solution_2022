...
import { SectionGridCellComponent } from '../../SOPSimulator/styled/sectionGridStyled';
...
class SectionGridCell extends Component {
        ...
        if (event.ctrlKey) {
            if (!event.repeat) {
                if (event.key === 'c' || event.key === 'C') {
                    this.props.onProcessEdit(SopManagerResource.editMenu.복사, SopManagerResource.editMenu.복사);
                }
                else if (event.key === 'x' || event.key === 'X') {
                    this.props.onProcessEdit(SopManagerResource.editMenu.잘라내기, SopManagerResource.editMenu.잘라내기);
                }
                else if (event.key === 'v' || event.key === 'V') {
                    this.props.onProcessEdit(SopManagerResource.editMenu.붙여넣기, SopManagerResource.editMenu.붙여넣기);
                }
            }
        }
        else if (!event.repeat) {
            if (event.key === 'delete' || event.key === 'Delete') {
                this.props.onProcessEdit(SopManagerResource.editMenu.삭제, SopManagerResource.editMenu.삭제);
            }
        }
    }
    ...
    makeSectionData(menuType) {
        ...
        if (menuType === "process" || menuType === SopManagerResource.component.프로세스) {
            sectionData = Process.makeSectionData();
        }
        else if (menuType === "endpoint" || menuType === SopManagerResource.component.시작_끝) {
            sectionData = Endpoint.makeSectionData();
        }
        else if (menuType === "decision" || menuType === SopManagerResource.component.판단) {
            sectionData = Decision.makeSectionData();
        }
        else if (menuType === "annotation" || menuType === SopManagerResource.component.설명) {
            sectionData = Annotation.makeSectionData();
        }
        else if (menuType === "internal" || menuType === SopManagerResource.component.상황전파) {
    ...
    render() {
        ...
        const addClassName = this.props.isSelected && this.props.mode === "edit" ? " " + "selected" : "";
        
        return (
            <SectionGridCellComponent ref={this.refCell} tabIndex={this.props.columnIndex + "_" + this.props.index}
                className={"sectionGridCell" + addClassName + " " + "noDrag"}
