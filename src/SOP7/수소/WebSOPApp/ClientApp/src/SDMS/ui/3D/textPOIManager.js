...
export class TextPOIManager {
    ...
    buildingTextBorderColor_Hydrogen = { r: 0, g: 0, b: 0, a: 1 };
    
    borderTAlarmColor = { r: 246, g: 5, b: 0, a: 1.0 };
    borderSelectColor = { r: 0, g: 64, b: 255, a: 1.0 };
    textAlarmColor = { r: 255, g: 219, b: 218, a: 1.0 };

    buildingGroupFontSize = 12;
    buildingFontSize = 12;
    ...
    getBuildingLanguageList(buildings) {
        ...
        for (const buildingGroupName in buildings) {
            ...
            for (const buildingName in buildingGroup) {
                ...
                const buildingInfo = buildingGroup[buildingName];
                const objBuildingName = TextPOIManager.parseJson(buildingInfo[1]);

                if (typeof objBuildingName === 'object') {
                    for (const languageType in objBuildingName) {
                        languageTypes.push(languageType);
                    }
                }
    ...
    updateAlarmBuildingText(buildings, zones, sensorAlarms, siteID) {
        const language = i18n.language;
        const alarms = sensorAlarms.filter(x => x.isAlarm === true);
            ...
            for (const buildingName in buildingGroup) {
                ...
                if (x !== null && x !== undefined && y !== null && y !== undefined && z !== null && z !== undefined) {
                    ...
                    for (const alarm of alarms) {
                        const zoneData = zones[alarm.zoneID];
                    ...
                    // id 이용하여 알람 여부 체크하여 this.workerManagerNormal 넘겨서 처리
                    this.workerManagerNormal.updateAlarmBuildingText(id, buildingGroupName, buildingName, displayText, this.buildingFontSize, siteID, this, isAlarm);
                }
            }
        }
    }

    selectBuildingText(buildings, buildingText, siteID, isSelect) {
        const language = i18n.language;
        ...
        for (const buildingGroupName in buildings) {
            ...
            for (const buildingName in buildingGroup) {
                ...
                if (x !== null && x !== undefined && y !== null && y !== undefined && z !== null && z !== undefined) {                    
                    //this.workerManagerNormal.updateSelectBuildingText(id, buildingGroupName, displayText, this.buildingFontSize, siteID, this, isSelect);
                    this.workerManagerNormal.updateSelectBuildingText(id, buildingGroupName, buildingName, this.buildingFontSize, siteID, this, isSelect, displayText);
                }
            }
        }
    }
                ...
                for (const buildingGroupName in buildings) {
                    ...
                    for (let i = 0; i < buildingGroupCount; i++) {
                        ...
                        if (buildingGroupName === _buildingGroupName) {
                            ...
                            const _buildingGroupDisplayText = (typeof objBuildingGroupDisplayText === 'object' ? objBuildingGroupDisplayText[languageType] : objBuildingGroupDisplayText);
    ...
    makeEquipZoneText(textTag, text, zoneID, id, x, y, z, fontSize, siteID) {
        //const backgroundColor = { r: 0, g: 0, b: 0, a: 0.7 };
    ...
    setBuildingTextScale(value) {
        this.workerManagerNormal.setBuildingTextScale(value);
    }
    ...
    addText(tag, text, zoneID, id, x, y, z, fontSize, backgroundColor, borderColor, textColor, borderThickness, siteID, isAlarm = false) {
