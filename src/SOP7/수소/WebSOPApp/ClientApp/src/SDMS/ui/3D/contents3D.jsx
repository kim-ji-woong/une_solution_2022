...
// 수소
import { ModelDataManager } from './Model/ModelDataManager';
...
export class Contents3D extends Component {
    ...
    static FacilityHeadTag = "equipment*";  // .TODO: 매칭 안되록 수정
    ...
    componentDidMount() {
        ...
        this.props.menu3DTools.onClick3DTools = this.onClick3DTools;
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (this.props.sensorAlarms !== prevProps.sensorAlarms) {
            // POI 알람 상태 변화 업데이트
            this.poiManager.updateAlarmPoi(this.props.sensorAlarms, this.props.visiblePopups[SDMS.menu.eventInfoNew]);

            // 설비명 알람 상태에 따라 변화 업데이트
            this.textPOIManager.updateAlarmBuildingText(this.props._3dOptions.buildings, this.props._3dOptions.zones, this.props.sensorAlarms, this.props._3dOptions.siteID);
        }

        //if (this.props.visiblePopups[SDMS.menu.eventInfoNew] !== prevProps.visiblePopups[SDMS.menu.eventInfoNew]) {
            this.poiManager.setVisibleAlarmSensor(this.props.visibleSensorTypes, this.props.visiblePopups, this.props.visiblePopups[SDMS.menu.eventInfoNew], this.props._3dOptions.zones, this.selectEquipmentModel.id);
    ...
    getOutdoorModelFiles(_3dOptions) {
        ...
        for (const buildingGroupName in _3dOptions.indoorModels) {
            ...
            // 빌딩 건물 파일도 포함
            for (const key in buildingGroup) {
                const building = buildingGroup[key];

                if (building?.file) {
                    modelFiles.push(building.file);
                }
            }
        }
    ...
    static processMenu(props, state) {
        ...
        if (props.command) {
            if (props.command.menu == SDMSMainMenu.Menu_Save_BuildingGroup_Viewport) {
                ...
                const [zoneID, sensorType, sensorID, alarmLevel, alarmTime] = props.command.menuParameter;
                state.prevInstance.addAlarm(zoneID, sensorType, sensorID, alarmLevel, alarmTime);
            }
                ...
                let buildingName = props.command.menuParameter;
                //state.prevInstance.moveToBuilding(buildingName);
                if (buildingName) {
                    if (Array.isArray(buildingName) && buildingName.length > 0) {
                        buildingName = buildingName[0];
                    }
                    
                    const equipment = state.prevInstance.modelDataManager.searchEquipment(buildingName);
                    if (equipment) {
                        // 조회가 된 경우
                        state.prevInstance.showEquipment(equipment);
                        // .TODO: 카메라 이동 >> 설비가 밀집되어 이동 X - 수소
                        //state.prevInstance.moveToBuilding(buildingName);
                        // 빌딩(설비) 정보 띄우기
                        DataInfo.processBuildingData(buildingName, state.prevInstance.props.showInfo);
                    }
                ...
                if (sensorType === SDMSMainMenu.H2_Sensor) {
                    props.onSelectSensorPOI(sensorID, sensorType);
                }
    ...
    init() {
        ...
        // HDRI 설정
        new RGBELoader()
            .load(this.props._3dOptions.textureBaseURL + '/evening_road_01_2k.hdr', (texture) => {
                texture.mapping = THREE.EquirectangularReflectionMapping;
                this.scene.background = texture; // 3차원 배경으로 사용
                this.scene.environment = texture; // 광원으로 사용
                //texture.dispose();
            }
        );

        const hemiLight = new THREE.HemisphereLight(0xffffff, 0x444444, 0.1);   // 기본값
        
        hemiLight.position.set(0, 20, 0);   // 기본값
        this.scene.add(hemiLight);

        
        this.dirLight = new THREE.DirectionalLight(0xffffff/*, this.directionalLightPower*/);   // 기본값
        this.dirLight.position.set(-3, 10, -10);  // 기본값


        // .TODO: 광원 테스트
        //this.dirLight.target.position.set(0, 0, 0); // 빛이 향하는 방향 설정
        ...
        this.dirLight.shadow.mapSize.width = 2048;    // 기본값
        this.dirLight.shadow.mapSize.height = 2048;   // 기본값
        //this.dirLight.shadow.mapSize.width = 0;
        //this.dirLight.shadow.mapSize.height = 0;

        this.dirLight.shadow.camera.updateProjectionMatrix();
        this.scene.add(this.dirLight);
        this.scene.add(this.dirLight.target);

         // .TODO: 광원 테스트 (선택 사항) 그림자 헬퍼와 카메라 헬퍼 추가 (디버깅에 매우 유용)
        //const lightHelper = new THREE.DirectionalLightHelper(this.dirLight, 2);
        //this.scene.add(lightHelper);
        //const shadowCameraHelper = new THREE.CameraHelper(this.dirLight.shadow.camera);
        ...
        //const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.4, 0.3);
    ...
    static animate(_this) {
        ...
        //_this.dirLight.position.x = Math.sin(time * 0.5) * 20;
        //_this.dirLight.position.z = Math.cos(time * 0.5) * 20;
        //_this.dirLight.position.y = 20;

        //_this.lightHelper.update();
        //_this.shadowCameraHelper.update();
    }
    ...
    needCameraRotation() {
        ...
        if (this.camera === this.perspectiveCamera) {
            ...
            // 3D 조작툴 메뉴에서 회전 버튼 클릭 유무 조건 추가
            if (timeSpan >= idleTime || this.props.visibleSensorTypes[SDMSMainMenu.Use3DRotation]) {
    ...
    loadRootModel(contents, nextIndex, files, mode, visible, _3dOptions) {
        ...
        loader.load(fileName, function (object) {
            ...
            //const lightPos = new THREE.Vector3(sceneHalfMaxLen, sceneMaxLen, sceneHalfMaxLen);
            const lightPos = new THREE.Vector3(sceneHalfMaxLen * -1, sceneMaxLen, sceneHalfMaxLen * -1);
            _this.dirLight.position.add(lightPos);
            ...
            if (mode === Contents3D.Mode_Outdoor_All || mode === contents.Mode_Outdoor_Part) {
                ...
                // 그룹 텍스트 X, 건물 텍스트만 표시
                _this.textPOIManager.addBuildingText(_3dOptions.buildingGroups, _3dOptions.buildings, _3dOptions.siteID);

                // 외부 모델링 로딩 중에 바운딩 박스 하이드 시키는 부분
                Contents3D.hideBoundingBoxes(modelNode, _3dOptions.buildingGroups, _3dOptions.buildings);

                // 실내 모델링 감추기
                Contents3D.hideIndoorModels(modelNode);
            }
    ...
    loadFile(contents, visible, cameraOptions, mode, _3dOptions, postMethod = null, postMethodParam = null) {
        ...
        loader.load(fileName, function (object) {
            ...
            obj.traverse((child) => {
                ...
                if (child instanceof THREE.Mesh) {
                    ...
                    //child.material = new THREE.MeshStandardMaterial({
                        //roughness: 0.1, // 거칠기 조절
                        //metalness: 0.9, // 금속성 조절
                    //});
            ...
            if (_this.isIndoor() && mode !== Contents3D.Mode_Indoor) {
                ...
                if (visible === true) {
                    // 외부 모델링 로딩 중에 바운딩 박스 하이드 시키는 부분
                    Contents3D.hideBoundingBoxes(modelNode, null, null);
                }
            }
    ...
    onAfterLoadFile(contents, _3dOptions, mode, postMethod, postMethodParam) {
        ...
        this.setState({ loading: false });
        */
        // 모델이 로딩되지 않아서 표시하지 못했던 알람정보를 표현 - 수소
    ...
    static hideBoundingBoxes(obj, buildingGroups, buildings) {
        ...
        for (let i = 0; i < childCount; i++) {
            ...
            // 한단계 더 추가로 확인 (모델링 그룹 안으로 바운더리 박스를 포함시킴)
            let childCount2 = child.children?.length;

            if (childCount2 > 0) {
                for (let j = 0; j < childCount2; j++) {
                    const child2 = child.children[j];

                    if (child2.name.endsWith(SDMSDataManager.BoundingBoxTag)) {
    ...
    static hideIndoorModels(obj) {
        ...
        for (let i = 0; i < childCount; i++) {
            ...
            if (child.name.startsWith(ModelDataManager.IndoorModelTag)) {
                child.visible = false;
            }
        }
    }
    ...
    async saveOutdoorModelViewport(camera, target) {
        ...
        const result = this.props.isEditMode ? await SDMSController.requestSaveOrthoModelViewport(modelName, cameraData, this.getCurrentViewZoneID(this.props.currentView)) : await SDMSController.requestSaveIndoorModelViewport(modelName, cameraData, this.getCurrentViewZoneID(this.props.currentView));
    ...
    moveToBuildingGroupFromID(buildingGroupID) {
        ...
        for (let i = 0; i < buildingGroupCount; i++) {
            ...
            if (buildingGroup.length >= 7 && buildingGroup[6] === buildingGroupID) {
                DataInfo.processBuildingGroupData(buildingGroupID, this.props.showInfo);
    ...
    _moveToBuildingGroup(buildingGroupName) {
        ...
        if (buildingGroup && buildingGroup.camera) {
            DataInfo.processBuildingGroupData(buildingGroup.buildingGroupID, this.props.showInfo);
    ...
    moveToSensor(zoneID, sensorType, sensorID) {
        ...
        // 수소에서 zoneID 값은 장비ID 값으로 사용으로 인한 주석처리
        /*
        if (!zone) {
    ...
    moveToFacility(zoneID, facilityID) {
        ...
        if (facility) {
            DataInfo.processFacilityInfo(facility.name, this.props.showInfo);   // 설비 정보창 띄우기 - K.D.R
    ...
    getSensorCameraOption(sensor, modelCamera, isIndoor) {
        ...
        //const movePos = [-2.67737, 14.332775, 13.2244];
        //const rotation = [-0.821292, -0.136645, -0.145336];
        const movePos = [-6.5171, 8.2680, -3.7651];
        const rotation = [-1.9981, -0.6222, -2.2340];
    ...
    onClick = (event) => {
        ...
        // 선택된 설비 유지가 필요로 주석 처리 - 수소
        //this.setSelectedFacility(null);
        // .TODO: 선택된 건물(설비) 클릭 해제 구현 필요 - 수소
        ...
        */
        ...
        if (poi === null && isIndoor === false && this.props.editMode === Contents3D.Edit_Mode_None && eqPoiType !== SDMSMainMenu.EquipZoneNameText) {
            ...
            if (poiID !== null && poiType !== null) {
                ...
                if (poiType === SDMSMainMenu.BuildingNameText) {
                    // 빌딩(설비) 이름 클릭시 해당 설비 포커스 이동 - 수소
                    const building = this.props._3dOptions.buildingIDs[poiID];

                    if (building) {
                        const modelName = building[3];
                        const buildingName = modelName.substring(0, modelName.length - SDMSDataManager.BoundingBoxTag.length);

                        // 이미 선택된 설비이라면 무시 - 수소
                        if (this.selectEquipmentModel.outdoorName && buildingName === this.selectEquipmentModel.outdoorName) {
                            isReturn = false;
                        }
                        else if (buildingName) {
                            const equipment = this.modelDataManager.searchEquipment(buildingName);
                            if (equipment) {
                                // 조회가 된 경우
                            ...
                            //this.moveToBuilding(buildingName);
                            // 빌딩(설비) 정보 띄우기
                            DataInfo.processBuildingData(modelName, this.props.showInfo);

                            // 현황정보 설비 선택
                            this.props.onChangeStatusInfoID(equipment.id, null, null);
                        }
                    }
                }
                ...
                if (this.props.selectedAlarm !== null) {
                    // 선택된 알람 해제
                    this.hideAlarms();
                    this.props.onSelectedAlarm(null, false);
                }
            ...
            else if (this.props.selectedAlarm !== null) {
                // 선택된 알람 해제
                this.hideAlarms();
                this.props.onSelectedAlarm(null, false);
            }
        ...
        else if (poi !== null && poi?.object?.userData?.isAlarm === true) {
            // 알람 poi 클릭된 경우
            const [sensorType, zoneID, sensorID] = this.poiManager.getSensorInfo(poi.object.name);

            const alarm = this.props.sensorAlarms.find(x => x.orgSensorID === sensorID);
            if (alarm && this.props.selectedAlarm !== alarm) {
                const alarmLevel = alarm.alarmDepth;
                const alarmModels = this.getCurrentAlarmModels(alarmLevel);

                const zones = this.props._3dOptions.zones;
                if (zones && zones[zoneID]) {
                    const zoneData = zones[zoneID];
                    const buildingID = (zoneData?.length > 1) ? zoneData[1] : null;

                    const equipment = this.modelDataManager.searchEquipmentID(buildingID);
                    if (equipment) {
                        // 조회가 된 경우
                        this.showEquipment(equipment);
                    }
                ...
                if (alarmModels && alarmModels.length > 0) {
                    ...
                    const alarmModel = this.moveAlarmAnimation(poi.point.x, poi.point.y, poi.point.z, alarmLevel);
                    ...
                    this.props.onSelectedAlarm(alarm, false);
                }
            }
        ...
        if (this.boundingBoxModel && this.isIndoor() === false) {
            ...
            // 건물(설비) 클릭된 경우
            // 클릭된 설비 조회
            let equipmentName = modelName;
            equipmentName = equipmentName.substring(0, (equipmentName.length - SDMSDataManager.BoundingBoxTag.length));

            const equipment = this.modelDataManager.searchEquipment(equipmentName);
            if (equipment) {
                // 조회가 된 경우
                this.showEquipment(equipment);
            }
            // 카메라 이동 >> 설비가 밀집되어 이동 X  - 수소
            //this.moveToBuilding(equipmentName);
            // 빌딩(설비) 정보 띄우기
            DataInfo.processBuildingData(modelName, this.props.showInfo);
            ...
            if (facility) {
                ...
                // 장비 정보 띄우기
                DataInfo.processFacilityInfo(facility.object.name, this.props.showInfo);
                //this.processFacilityInfo(facility.object.name);

                // 현황정보 장비 선택
                //this.props.onChangeStatusInfoID(equipment.id, zone.id, null);
            }
            else {
                DataInfo.processFacilityInfo(null, this.props.showInfo);
            }
        }
    ...
    onClickPOI(poi, event) {
        ...
        if (type === SDMSMainMenu.CCTV_Type || type === SDMSMainMenu.CCTV_SafetyI_Type || type === SDMSMainMenu.CCTV_PTZ_Type ||
            ...
            this.props.onSelectSensorPOI(id, type);
        }
    ...
    showBuildingInfo(type, id, siteID) {
        ...
        if (arrInfo) {
            this.props.showInfo(arrInfo[0], arrInfo);
        }
    }
    ...
    showAlarm(zoneID, sensorType, sensorID, alarmLevel, isAlarm, selectedPOI) {
        ...
        let modelData = [outdoorModel.file, outdoorModel.modelDisplayText, outdoorModel.camera];
        ...
        if (sensor) {
            ...
            if (sensorType === SDMSMainMenu.H2_Sensor) {
                this.props.onSelectSensorPOI(sensorID, sensorType);
            }
        }
        ...
        // 로딩이 끝난 후에 알람 표시
        // ZoneID (장치ID) 이용하여 해당 BuildingID (설비ID) 찾기 
        let equipment = null;
        ...
        if (zones && zones[zoneID]) {
            ...
            equipment = this.modelDataManager.searchEquipmentID(buildingID);
        }

        if (this.state.loading === false && equipment) {
            // 조회가 된 경우
            this.showEquipment(equipment);
        ...
        if (model) {
            ...
            if (sensorPOI) {
                ...
                if (!selectedPOI || (selectedPOI && selectedPOI[0] === sensorType && selectedPOI[1] === sensorID && selectedPOI[2] === zoneID)) {
    ...
    addAlarm(zoneID, sensorType, sensorID, alarmLevel, alarmTime = null) {
    ...
    goBackOutdoor(model, cameraOption) {
        ...
        // 설비 선택 해제 - 수소
        this.showEquipment(null);

        // 현황정보 트리 닫기 - K.D.R
        //this.props.onChangeBuildingGroup(null, SDMS.SelectedStatusInfoType.none);
        this.props.onChangeStatusInfoID(null, null, null);
            ...
            if (intersectCount > 0) {
                ...
                if (nearestIntersect) {
                    ...
                    for (let i = 0; i < intersectCount; i++) {
                        ...
                        // 선택된 설비가 있을 경우 해당 건물 바운딩 박스 제외 처리 - 수소
                        if (this.selectEquipmentModel.outdoorName && model?.name.startsWith(this.selectEquipmentModel.outdoorName))
                            model = null;
    ...
    getAlarmElements() {
        ...
        //    );
        //}
        if (/*!this.props.alarmSound || this.state.alarmSoundOff*/this.props.isMute) {
            return (
                <></>
            );
        }
    ...
    async setInitialViewportWithIntersect(intersects) {
        ...
        if (success) {
            this.props.showConfirmDialog(i18n.t('common.확인'), [i18n.t('sdms.toolbar.초기화면으로 지정되었습니다')], null, null);
        }
    }
    ...
    showSolarPV = () => {
        ...
        const scenes = this.scene?.children;

        if (scenes?.length > 0) {
            if (visibleSensorTypes[SDMSMainMenu.PV_Panel] === false) {
                for (let i = 0; i < scenes.length; i++) {
                    const scene = scenes[i];

                    if (scene.name === "solar_PV.glb") {
                        scene.visible = false;
                        break;
                    }
                }
    ...
    showEquipment = (equipmentModelData) => {
        if (equipmentModelData?.id === this.selectEquipmentModel?.id)
            return;
        else if (this.state.loading === true)
            return;

        // 기존에 내부 설비 표시 중이라면
        if (this.selectEquipmentModel.isIndoor === true) {
            // 내부 설비 hide
            this.setEquipmentModel(ModelDataManager.IndoorModelTag + this.selectEquipmentModel.outdoorName, false);
            // 외부 설비 show
            this.setEquipmentModel(this.selectEquipmentModel.outdoorName, true);
            // 내부 센서 hide
            this.poiManager.setEquipmentSensors(this.selectEquipmentModel.outdoorName, this.props._3dOptions.allBuildings, this.props._3dOptions.zones, false, this.props.visibleSensorTypes, this.props.visiblePopups[SDMS.menu.eventInfoNew]);
        }

        // 설비명 선택 해제
        if (this.selectEquipmentModel.outdoorName?.length > 0) {
            this.textPOIManager.selectBuildingText(this.props._3dOptions.buildings, this.selectEquipmentModel.outdoorName, this.props._3dOptions.siteID, false);
        }

        // 설비 정보가 없다면
        if (!equipmentModelData) {
            this.selectEquipmentModel.id = null;
            this.selectEquipmentModel.outdoorName = null;
            this.selectEquipmentModel.isIndoor = null;
            return;
        }
            
        this.selectEquipmentModel.id = equipmentModelData.id;
        this.selectEquipmentModel.outdoorName = equipmentModelData.outdoorName;
        this.selectEquipmentModel.isIndoor = equipmentModelData.isIndoor;        

        // 내부 설비가 있을 경우
        if (equipmentModelData.isIndoor === true) {
            // 외부 설비 hide
            this.setEquipmentModel(this.selectEquipmentModel.outdoorName, false);
            // 내부 설비 show
            this.setEquipmentModel(ModelDataManager.IndoorModelTag + this.selectEquipmentModel.outdoorName, true);
            // 내부 설비 센서 show
            this.poiManager.setEquipmentSensors(this.selectEquipmentModel.outdoorName, this.props._3dOptions.allBuildings, this.props._3dOptions.zones, true, this.props.visibleSensorTypes, this.props.visiblePopups[SDMS.menu.eventInfoNew]);
        }

        // 설비명 선택
        this.textPOIManager.selectBuildingText(this.props._3dOptions.buildings, this.selectEquipmentModel.outdoorName, this.props._3dOptions.siteID, true);
    }

    setEquipmentModel = (equipmentName, visible) => {
        if (!equipmentName)
            return; 

        const scenesCount = this.scene?.children.length;
        let chk = false;
    ...
    onClick3DTools = (menu) => {
        if (menu === LnbPopup.InitViewport) {
            this.initViewport();
        }
        else if (menu === LnbPopup.SetInitialViewport) {
            this.setInitialViewport();
        }
        else if (menu === LnbPopup.ZoomIn) {
            this.zoom(true);
        }
        else if (menu === LnbPopup.ZoomOut) {
            this.zoom(false);
        }
        else if (menu === LnbPopup.StartAutoRotation) {
    ...
    render() {
        ...
        //            <li key={"liQuickKey_statusInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+1</span><a id={"dsBot_" + SdmsResource.popupLayer.statusInfo} className={this.getQuickButtonClassName(SDMS.menu.statusInfo) + " " + 'statusInfoIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.statusInfo)}><span><em>설비<br />리스트</em></span></a></li>
        ...
        //            <li key={"liQuickKey_eventInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+3</span><a id={"dsBot_" + SdmsResource.popupLayer.event} className={this.getQuickButtonClassName(SDMS.menu.eventInfo) + " " + 'eventIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.eventInfo)}><span><em>이벤트<br />정보</em></span></a></li>
        //            <li key={"liQuickKey_detectionInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+4</span><a id={"dsBot_" + SdmsResource.popupLayer.detectionInfo} className={this.getQuickButtonClassName(SDMS.menu.detectionInfo) + " " + 'detectionIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.detectionInfo)}><span><em>이상<br />탐지</em></span></a></li>
        //            <li key={"liQuickKey_simulationInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+5</span><a id={"dsBot_" + SdmsResource.popupLayer.simulationInfo} className={this.getQuickButtonClassName(SDMS.menu.simulationInfo) + " " + 'simulationIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.simulationInfo)}><span><em>시뮬<br />레이션</em></span></a></li>
        //            <li key={"liQuickKey_analysisInfo"}><span className={"shortcutKey" + " " + 'shortCut' + " " + 'hideKey'}>Ct+6</span><a id={"dsBot_" + SdmsResource.popupLayer.analysisInfo} className={this.getQuickButtonClassName(SDMS.menu.analysisInfo) + " " + 'analysisIcon'} onClick={() => this.setVisiblePopups(SDMS.menu.analysisInfo)}><span><em>위험도<br />분석</em></span></a></li>
