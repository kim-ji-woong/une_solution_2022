...
class PopupDraggable extends Component {
    ...
    popupDragMouseMove = (event) => {
        ...
        // 팝업이 화면밖으로 안나가도록 처리
        if (moveX > 80 && moveX + width < this.state.maxScreenWidth) {
            ...
            // 드래그 도중 좌측 끝지점이 80px 이하로 넘어가게 될 때
            // 수소 lnb 영역으로 인해 수정 (241011 한봄희)
            if (left > 80) {
                // 팝업을 좌측 60px에 고정
                let lim = (80 / this.state.maxScreenWidth) * 100;
                this.state.popup.style.left = lim + '%';
            } else if (this.state.preMousePosition.x < mousePosition.x) {
    ...
    popupDragMousePress(event) {
        if (this.props.usePopupDrag === false) {
            return;
        }
