...
export class WorkerManager {
    ...
    updateAlarmBuildingText(buildingID, buildingGroupName, buildingName, displayText, fontSize, siteID, textPOIManager, isAlarm, isReload = false) {
        let buildingSprites = this.buildingText[buildingGroupName];
        if (!buildingSprites && isReload === false) {
            setTimeout(() => this.updateAlarmBuildingText(buildingID, buildingGroupName, buildingName, displayText, fontSize, siteID, textPOIManager, isAlarm, true), 3000);
            return;
        }
        else if (!buildingSprites) {
            return;
        }
            

        let sprite = buildingSprites[buildingName];
        if (sprite && sprite?.userData?.isAlarm !== isAlarm) {
            // 기존 숨김처리
            sprite.visible = false;
            
            if (sprite.userData.isAlarm !== isAlarm) {
                // 생성하고
                const sprite_new = this.makeBuildingText(displayText, buildingID, buildingID, sprite.position.x, sprite.position.y, sprite.position.z, fontSize, siteID, textPOIManager, isAlarm);
                if (sprite_new) {
                    // 기존 삭제하고
                    buildingSprites[buildingName] = sprite_new;
                }                
            }           
        }
    }

    //updateSelectBuildingText(buildingID, buildingGroupName, buildingName, fontSize, siteID, textPOIManager, isSelect) {
    updateSelectBuildingText(buildingID, buildingGroupName, buildingName, fontSize, siteID, textPOIManager, isSelect, displayText) {
        let buildingSprites = this.buildingText[buildingGroupName];
        if (!buildingSprites) {
            return;
        }
        ...
        let sprite = buildingSprites[buildingName];
        if (sprite && sprite?.userData?.isAlarm !== true && sprite?.userData?.isSelect !== isSelect) {
            // 기존 숨김처리
            sprite.visible = false;

            // 생성하고
            const sprite_new = this.makeBuildingText(displayText, buildingID, buildingID, sprite.position.x, sprite.position.y, sprite.position.z, fontSize, siteID, textPOIManager, false, isSelect);
            if (sprite_new) {
                // 기존 삭제하고
                buildingSprites[buildingName] = sprite_new;
            }            
    ...
    addBuildingText(buildingGroupName, buildingName, displayText, id, x, y, z, fontSize, siteID, textPOIManager, isAlarm = false) {


        const sprite = this.makeBuildingText(displayText, -1, id, x, y, z, fontSize, siteID, textPOIManager, isAlarm);
    ...
    makeBuildingText(text, zoneID, id, x, y, z, fontSize, siteID, textPOIManager, isAlarm = false, isSelect = false) {
        let backgroundColor = null;
        let borderColor = null;
        let textColor = null;

        if (isAlarm) {
            backgroundColor = textPOIManager.borderTAlarmColor;
            borderColor = textPOIManager.borderTAlarmColor;
            textColor = textPOIManager.textColor;
        }
        else if (isSelect) {
            backgroundColor = textPOIManager.borderSelectColor;
            borderColor = textPOIManager.borderSelectColor;
            textColor = textPOIManager.textAlarmColor;
        }
        else {
            backgroundColor = { r: 0, g: 0, b: 0, a: 1 };
            borderColor = textPOIManager.buildingTextBorderColor_Hydrogen;
            textColor = textPOIManager.textColor;            
        }
       
        const borderThickness = textPOIManager.borderThickness;
        const sprite = this.addText(SDMSMainMenu.BuildingNameText, text, zoneID, id, x, y, z, fontSize, backgroundColor, borderColor, textColor, borderThickness, textPOIManager, this.getBuildingTextLayer(siteID), isAlarm);
    ...
    setBuildingTextScale(value) {
        ...
        for (const languageType in languageBuildingTextLayers) {
            ...
            for (const siteID in siteBuildingTextLayers) {
                ...
                if (buildingTextLayer?.children?.length > 0) {
                    for (let key in buildingTextLayer.children) {
                        let buildingText = buildingTextLayer.children[key];

                        if (buildingText?.scale) {
                            buildingText.scale.x = value;
                            buildingText.scale.y = value;
                            buildingText.scale.z = value;
                        }
