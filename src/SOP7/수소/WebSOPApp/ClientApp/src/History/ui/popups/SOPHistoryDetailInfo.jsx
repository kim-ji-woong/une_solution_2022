...
class SOPHistoryDetailInfo extends Component {
	...
	async onClickAllDownload() {		
		...
		worksheet.addRow([i18n.t('history.historyDetail.시간') + ' : ' + this.props.selectedData.beginTime]);
		worksheet.addRow([i18n.t('history.historyDetail.SOP유형') + ' : ' + this.props.selectedData.disasterName ]);
		worksheet.addRow([i18n.t('history.historyDetail.위기경보단계') + ' : ' + this.props.selectedData.actionStepName]);
		worksheet.addRow([i18n.t('history.historyDetail.SOP모드') + ' : ' + this.props.selectedData.realMode]);
		worksheet.addRow([]);
		...
		if (this.state.dataSource) {
			...
			for (let i = 0; i < dataLength; i++) {
				...
				const status = this.state.dataSource[i].strStatus === "확인" ? i18n.t('history.historyDetail.확인') : i18n.t('history.historyDetail.실행중');
				...
				if (missionCount > 0) {
					for (let j = 0; j < missionCount; j++) {
						...
						let completion = "";
						if (statusDetail === "완료")
							completion = i18n.t('history.historyDetail.완료');
						else if (statusDetail === "미완료")
							completion = i18n.t('history.historyDetail.미완료');
						else if (statusDetail === "부분완료")
							completion = i18n.t('history.historyDetail.부분완료');
						else if (statusDetail === "확인")
							completion = i18n.t('history.historyDetail.확인');
		...
		for (let j = 0; j < datacount; j++) {
			grid1.push(
				<tr onClick={() => this.onClickhsmDtl(j + 1, dataSource[j].componentHistoryID)} key={'grid1_' + j}>
					...
					<td>{(dataSource[j].strStatus === "확인" ? i18n.t('history.historyDetail.확인') : i18n.t('history.historyDetail.실행중'))}</td>
				</tr>
			);
			...
			if (this.state.selectComponentHistoryID === dataSource[j].componentHistoryID) {
				...
				for (let i = 0; i < missionCount; i++) {
					...
					let completion = "";
					if (missionDatas[i].completion === "완료")
						completion = i18n.t('history.historyDetail.완료');
					else if (missionDatas[i].completion === "미완료")
						completion = i18n.t('history.historyDetail.미완료');
					else if (missionDatas[i].completion === "부분완료")
						completion = i18n.t('history.historyDetail.부분완료');
					else if (missionDatas[i].completion === "확인")
						completion = i18n.t('history.historyDetail.확인');
					...
					<div>
						<div className={newStyles.hsmCont + " " + newStyles.sop}>
							<div className={newStyles.hsmTitle}>
								<h3>{i18n.t('history.historyDetail.SOP 상세정보')}</h3>
								<a onClick={this.onClickAllDownload} className={newStyles.hsmExl}>{i18n.t('history.historyDetail.엑셀 다운로드')}</a>
								<a onClick={this.onClose} className={newStyles.hsmCls}>{i18n.t('common.닫기')}</a>
							</div>
							<div className={"scroll-wrapper" + " " + newStyles.hsmPrc + " " + "scroll-bar"}>
								<table className={newStyles.hsmTb}>
									...
									<thead>
										<tr>
											<th>No.</th>
											<th>{i18n.t('history.historyDetail.프로세스 제목')}</th>
											<th>{i18n.t('history.historyDetail.전파대상자')}</th>
											<th>{i18n.t('history.historyDetail.시간')}</th>
											<th>{i18n.t('history.historyDetail.완료여부')}</th>
										</tr>
									</thead>
									<tbody>
										{grid1}
								...
								<table className={newStyles.hsmTb}>
									...
									<thead>
										<tr>
											<th>No.</th>
											<th>{i18n.t('history.historyDetail.프로세스 제목')}</th>
											<th>{i18n.t('history.historyDetail.세부 임무/전파 메세지')}</th>
											<th>{i18n.t('history.historyDetail.시간')}</th>
											<th>{i18n.t('history.historyDetail.완료여부')}</th>
										</tr>
									</thead>
...
export default withTranslation()(SOPHistoryDetailInfo);