...
class DecisionProperty extends Component {
	...
	saveSectionData(shouldUpdate) {
		...
		//const orgDescription = i18nUtil.convertTextByLanguage(this.state.curLanguage, sectionData.description);
		const orgDescription = sectionData.description;
		if (orgText === this.refTitle.current.value && orgDescription === this.refDescription.current.value) {
			return;
		}
		...
		//sectionData.description = i18nUtil.convertJsonByLanguage(this.state.curLanguage, sectionData.description, this.refDescription.current.value);
        ...
        return (
			<div className={DecisionProperty.cssStyles.sprCont + " " + DecisionProperty.cssStyles.pt150 + " " + bodyStyles.noDrag}>
				<div className={DecisionProperty.cssStyles.sprTop}>
					<h4 className={DecisionProperty.cssStyles.sprtTitle}>{i18n.t('sopManager.formText.판단문 작성')}</h4>
					<div className={"scroll-wrapper " + DecisionProperty.cssStyles.sprtTxtara + " " + DecisionProperty.cssStyles.ssk + " scrollbar-outer scroll-textarea"} id="pos_relative">
						<div className="scroll-content scroll-scrolly_visible" id="decision_scrollContent">
							<textarea ref={this.refTitle} name="" id="" cols="30" rows="10" className={DecisionProperty.cssStyles.sprtTxtara + " " + DecisionProperty.cssStyles.ssk + " scrollbar-outer"} defaultValue={i18nUtil.convertText(this.state.sectionData?.text)} onChange={this.onTextChange}></textarea>
						</div>
					...
					<div className={DecisionProperty.cssStyles.sskChk}>
						<label className={DecisionProperty.cssStyles.clickable}>
							...
							{i18n.t('sopManager.formText.수식 사용')}
						</label>
					</div>
				</div>
						...
						<div className="scrollbar-outer scroll-content" id="decision_scrollContent2">
							<div className={DecisionProperty.cssStyles.sprmCont}>
								<dl className={DecisionProperty.cssStyles.sprmAcdn}>
									<dt ref={this.refVariableDT} className={this.getVariableClassName()} onClick={this.onClickCascade}>{i18n.t('sopManager.formText.수식')}</dt>
									<dd ref={this.refVariableDD} className={this.getVariableClassName()}>
										...
										<h5>{i18n.t('sopManager.formText.기본 타입')}</h5>
										<div className={DecisionProperty.cssStyles.sopEdtTb}>
											<table>
												<caption>{i18n.t('sopManager.formText.변수, 타입, 설명으로 구성된 표')}</caption>
												<colgroup>
													<col id="width_25Pro" />
													<col id="width_25Pro" />
													<col id="width_50Pro" />
												...
												<thead>
													<tr>
														<th>{i18n.t('sopManager.formText.변수')}</th>
														<th>Type</th>
														<th>{i18n.t('sopManager.formText.설명')}</th>
													</tr>
												</thead>
												<tbody>
													<tr onClick={this.onClickToggle}>
														<td>time</td>
														<td>{i18n.t('sopManager.formText.문자열')}</td>
														<td className={commonStyles.tal}><p className={commonStyles.nwrp}>{i18n.t('sopManager.formText.재난 발생 시간')}</p></td>
													</tr>
													<tr onClick={this.onClickToggle}>
														<td>location</td>
														<td>{i18n.t('sopManager.formText.문자열')}</td>
														<td className={commonStyles.tal}><p className={commonStyles.nwrp}>{i18n.t('sopManager.formText.재난 발생 위치')}</p></td>
													</tr>
													<tr>
														<td>-</td>
														<td>-</td>
										...
										<h5>{i18n.t('sopManager.formText.사용자 정의 타입')}</h5>
										<div className={DecisionProperty.cssStyles.sopEdtTb}>
											<table>
												<caption>{i18n.t('sopManager.formText.사용자 정의 타입')}</caption>
												<colgroup>
													<col id="width_25Pro" />
													<col id="width_25Pro" />
													<col id="width_50Pro" />
									...
									<dt ref={this.refDescriptionDT} className={this.getDescriptionClassName()} onClick={this.onClickCascade}>{i18n.t('sopManager.formText.판단 기준 설명')}</dt>
				...
				<div className={DecisionProperty.cssStyles.sprBot}>
					<a className={DecisionProperty.cssStyles.clickable} onClick={() => this.onClickApply(true)}>{i18n.t('common.확인')}</a>
					<a className={DecisionProperty.cssStyles.clickable} onClick={() => this.onClickApply(false)}>{i18n.t('common.취소')}</a>
				</div>
			</div>
            );
    }
...
export default withTranslation()(DecisionProperty);