...
class SopManagerBodyMain extends Component {
	...
	static getActionStepName(sopData) {
		...
		return SopManagerResource.actionStep._2nd;
    }
	...
	onClickDt(cascading, show) {
		if (cascading === SopManagerResource.cascadingMenu.특수문자_입력_형식 && show) {
		...
		if (specialMessages) {
			...
			for (let i = 0; i < messageCount; i++) {
				if (prevSpecialMessage.currentID === specialMessages[i].id) {
					let currentMessage = i18nUtil.convertText(specialMessages[i].message);
					if (currentMessage) {
						currentMessage = currentMessage.replaceAll("<", "{");
						currentMessage = currentMessage.replaceAll(">", "}");
                    }

					prevSpecialMessage.currentMessage = currentMessage;
					findID = true;
					break;
                }
            }
			...
			if (findID === false) {
				if (messageCount > 0) {
					let currentMessage = i18nUtil.convertText(specialMessages[0].message);
					if (currentMessage) {
						currentMessage = currentMessage.replaceAll("<", "{");
						currentMessage = currentMessage.replaceAll(">", "}");
					}
				...
				<select onChange={this.onSelectSpecialMessage}>
					{
						specialMessage.messages.map((message, index) => {
							...
							const value = index;
							return <option key={key} value={value}>{i18nUtil.convertText(message.category)}</option>
                        })
					}
				</select>
				);
	...
	onSelectSpecialMessage = (event) => {
		...
		if (index !== null && index !== undefined) {
			...
			if (message) {
				let currentMessage = i18nUtil.convertText(message.message);
				if (currentMessage) {
					currentMessage = currentMessage.replaceAll("<", "{");
					currentMessage = currentMessage.replaceAll(">", "}");
				}
				...
				specialMessage.currentMessage = currentMessage;
	...
	render() {
		...
		return (
			<SopManagerBodyMainComponent ref={this.refSpWrap} id={'spWrap'}>
				...
				<div id={'spRht'}>
					<ComponentProperties sectionData={sectionData} arrowData={arrowData} actionStep={actionStep} sopData={this.state.sopData} onApplyComponentProperty={this.onApplyComponentProperty} onSelectComponent={this.onSelectComponent} />
				</div>
