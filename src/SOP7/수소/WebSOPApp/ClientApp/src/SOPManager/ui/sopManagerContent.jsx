...
class SopManagerContent extends Component {
	...
	render() {
		return (
			<SopManagerContentComponent className={'saLeft'} $disabled={this.props.sopData}>
				<div className={'aslWrap typeC'}>
					...
					<div className={'salMenu on'}>
						<a onClick={(e) => this.onClickMenu(e.target)} className={'salIco ico0201'}>{i18n.t('sopManager.menu.SOP 편집')}</a>
						<dl className={'salCont'}>
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('sopManager.menu.새 SOP')}</a></dd>
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('sopManager.menu.열기')}</a></dd>
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('common.저장')}</a></dd>
							{/* <dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('sopManager.menu.다른 이름 저장')}</a></dd> */}
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('common.삭제')}</a></dd>
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('sopManager.menu.파일 열기')}</a></dd>
							<dd><a className={'clickable'} onClick={(e) => this.onClickMenu(e.target)}>{i18n.t('sopManager.menu.파일 저장')}</a></dd>
						</dl>
					</div>
				</div>
