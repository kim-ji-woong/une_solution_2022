    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            if (m_configManager.Site.SiteID != null && m_configManager.Site.DBType != null)
            {
                ...
                services.AddTransient<global::Hydrogen.IDAL.IDataManager>(service => new global::Hydrogen.DAL.DataManager(nDBType, strDbHost, strDBName, strDbID, strDbPW, nSiteID));
            }

            services.AddSwaggerGen(c =>
            {
                c.CustomSchemaIds(type => type.ToString());
                c.SwaggerDoc("AnomalyDetection", new Microsoft.OpenApi.Models.OpenApiInfo
                {
                    Version = "AnomalyDetection",
                    Title = "이상탐지 데이터",
                    Description = "이상탐지 데이터를 위한 API"
                });
                c.SwaggerDoc("MERI", new Microsoft.OpenApi.Models.OpenApiInfo
                {
                    Version = "MERI",
                    Title = "시뮬레이션 데이터",
                    Description = "시뮬레이션 데이터를 위한 API"
                });
                c.SwaggerDoc("KGS", new Microsoft.OpenApi.Models.OpenApiInfo
                {
                    Version = "KGS",
                    Title = "위험성 평가 예측 데이터",
                    Description = "위험성 평가 예측 데이터를 위한 API"
                });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            // textures 폴더에도 provider 적용 (필요시)
            string strTexturePath = Path.Combine(Directory.GetCurrentDirectory(), "ClientApp\\build\\resource\\textures");
            if (Directory.Exists(strTexturePath))
            {
                app.UseStaticFiles(new StaticFileOptions
                {
                    FileProvider = new PhysicalFileProvider(strTexturePath),
                    RequestPath = "/resource/textures",
                    ContentTypeProvider = provider
                });
            }
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint("/swagger/AnomalyDetection/swagger.json", "이상탐지 데이터 API");
                c.SwaggerEndpoint("/swagger/MERI/swagger.json", "시뮬레이션 데이터 API");
                c.SwaggerEndpoint("/swagger/KGS/swagger.json", "위험성 평가 예측 데이터 API");
