...
namespace BusanTP.DAL
{
	public class SelectManager : QueryManager, ISelect
	{
		...
		{
			return string.Format("{0}-{1:00}-{2:00} {3:00}:{4:00}:{5:00}", time.Year, time.Month, time.Day, time.Hour,
				time.Minute, time.Second);
		}

		public List<Material> SelectBusanExternalMaterials(Dictionary<Material.Fields, object> dicConditions,
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<Material.Fields>(out nFieldCount),
				Material.TableName);

			string strCondition = "";

			if (SetCondition<Material.Fields>(ref strCondition, dicConditions, Material.GetFieldName,
				    Material.TableName, ref strErrorMessage) == false)
				return null;

			SetQuery(ref strSQL, strCondition, strAdditionalConditions);

			ArrayList arrResult = topNCount == null
				? m_dbManager.GetResultData(strSQL)
				: m_dbManager.GetResultData(strSQL, (int)topNCount);
		...
		public List<Sensor> SelectBusanExternalSensors(Dictionary<Sensor.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<Sensor.Fields>(out nFieldCount),
				Sensor.TableName);

			string strCondition = "";

			if (SetCondition<Sensor.Fields>(ref strCondition, dicConditions, Sensor.GetFieldName, Sensor.TableName,
				    ref strErrorMessage) == false)
				return null;
		...
		public List<SensorType> SelectBusanExternalSensorTypes(Dictionary<SensorType.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<SensorType.Fields>(out nFieldCount),
				SensorType.TableName);

			string strCondition = "";

			if (SetCondition<SensorType.Fields>(ref strCondition, dicConditions, SensorType.GetFieldName,
				    SensorType.TableName, ref strErrorMessage) == false)
				return null;
		...
		public List<KWeatherNodeInfo> SelectBusanKWeatherNodeInfos(
			Dictionary<KWeatherNodeInfo.Fields, object> dicConditions, string strAdditionalConditions,
		...
		public List<KWeatherNodeInfo> SelectBusanKWeatherNodeInfos(
			Dictionary<KWeatherNodeInfo.Fields, object> dicConditions, string strAdditionalConditions, int? topNCount,
			out string strErrorMessage)
		{
			strErrorMessage = null;
			int nFieldCount;

			string strSQL = string.Format("select {0} from {1}",
				GetFieldNames<KWeatherNodeInfo.Fields>(out nFieldCount), KWeatherNodeInfo.TableName);

			string strCondition = "";

			if (SetCondition<KWeatherNodeInfo.Fields>(ref strCondition, dicConditions, KWeatherNodeInfo.GetFieldName,
				    KWeatherNodeInfo.TableName, ref strErrorMessage) == false)
				return null;
		...
		private KWeatherNodeInfo ReadBusanKWeatherNodeInfo(ArrayList arrResult, int index, out string strErrorMessage)
		{
			...
			foreach (KWeatherNodeInfo.Fields field in
			         KWeatherNodeInfo.Fields.GetValues(typeof(KWeatherNodeInfo.Fields)))
			{
		...
		public List<SdmsOption> SelectBusanSdmsOptions(Dictionary<SdmsOption.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<SdmsOption.Fields>(out nFieldCount),
				SdmsOption.TableName);

			string strCondition = "";

			if (SetCondition<SdmsOption.Fields>(ref strCondition, dicConditions, SdmsOption.GetFieldName,
				    SdmsOption.TableName, ref strErrorMessage) == false)
				return null;
		...
		public List<SensorDataHistory> SelectBusanSensorDataHistorys(
			Dictionary<SensorDataHistory.Fields, object> dicConditions, string strAdditionalConditions,
			out string strErrorMessage)
		{
		...
		public List<SensorDataHistory> SelectBusanSensorDataHistorys(
			Dictionary<SensorDataHistory.Fields, object> dicConditions, string strAdditionalConditions, int? topNCount,
			out string strErrorMessage)
		{
			strErrorMessage = null;
			int nFieldCount;
			...
			string strSQL = string.Format("select {0} from {1}",
				GetFieldNames<SensorDataHistory.Fields>(out nFieldCount), SensorDataHistory.TableName);

			string strCondition = "";

			if (SetCondition<SensorDataHistory.Fields>(ref strCondition, dicConditions, SensorDataHistory.GetFieldName,
				    SensorDataHistory.TableName, ref strErrorMessage) == false)
				return null;
		...
		private SensorDataHistory ReadBusanSensorDataHistory(ArrayList arrResult, int index, out string strErrorMessage)
		{
			...
			foreach (SensorDataHistory.Fields field in SensorDataHistory.Fields.GetValues(
				         typeof(SensorDataHistory.Fields)))
			{
				string strFieldName = SensorDataHistory.GetFieldName(field, out isNullable);

				if (field == SensorDataHistory.Fields.ID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == SensorDataHistory.Fields.SensorID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
		...
		public List<SensorGIS> SelectBusanExternalSensorGISs(Dictionary<SensorGIS.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<SensorGIS.Fields>(out nFieldCount),
				SensorGIS.TableName);

			string strCondition = "";

			if (SetCondition<SensorGIS.Fields>(ref strCondition, dicConditions, SensorGIS.GetFieldName,
				    SensorGIS.TableName, ref strErrorMessage) == false)
				return null;
		...
		private SensorGIS ReadBusanExternalSensorGIS(ArrayList arrResult, int index, out string strErrorMessage)
		{
			...
			foreach (SensorGIS.Fields field in SensorGIS.Fields.GetValues(typeof(SensorGIS.Fields)))
			{
				...
				else if (field == SensorGIS.Fields.Acronym)
				{
					string data = WebDBManager.GetStringField(arrResult[index]);
		...
		public List<POIInfo> SelectBusanExternalPOIInfos(Dictionary<POIInfo.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<POIInfo.Fields>(out nFieldCount),
				POIInfo.TableName);

			string strCondition = "";

			if (SetCondition<POIInfo.Fields>(ref strCondition, dicConditions, POIInfo.GetFieldName, POIInfo.TableName,
				    ref strErrorMessage) == false)
				return null;
		...
		public List<UserMemo> SelectBusanUserMemos(Dictionary<UserMemo.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			return SelectBusanUserMemos(dicConditions, strAdditionalConditions, null, out strErrorMessage);
		}
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<UserMemo.Fields>(out nFieldCount),
				UserMemo.TableName);

			string strCondition = "";

			if (SetCondition<UserMemo.Fields>(ref strCondition, dicConditions, UserMemo.GetFieldName,
				    UserMemo.TableName, ref strErrorMessage) == false)
				return null;
			...
			int nResultCount = arrResult.Count;
			List<UserMemo> datas = new List<UserMemo>();

			for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
			{
				UserMemo model = ReadBusanUserMemo(arrResult, i, out strErrorMessage);
		...
		private UserMemo ReadBusanUserMemo(ArrayList arrResult, int index, out string strErrorMessage)
		{
			strErrorMessage = null;
			UserMemo model = new UserMemo();
			bool isNullable;

			foreach (UserMemo.Fields field in UserMemo.Fields.GetValues(typeof(UserMemo.Fields)))
			{
				string strFieldName = UserMemo.GetFieldName(field, out isNullable);

				if (field == UserMemo.Fields.ID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == UserMemo.Fields.UserID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == UserMemo.Fields.Memo)
				{
					string data = WebDBManager.GetStringField(arrResult[index]);
		...
		public List<TestOptions> SelectBusanTestOptions(Dictionary<TestOptions.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			return SelectBusanTestOptions(dicConditions, strAdditionalConditions, null, out strErrorMessage);
		}
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<TestOptions.Fields>(out nFieldCount),
				TestOptions.TableName);
			
			string strCondition = "";
			
			if (SetCondition<TestOptions.Fields>(ref strCondition, dicConditions, TestOptions.GetFieldName,
				    TestOptions.TableName, ref strErrorMessage) == false)
				return null;
			...
			int nResultCount = arrResult.Count;
			List<TestOptions> datas = new List<TestOptions>();
			
			for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
			{
				TestOptions model = ReadBusanTestOption(arrResult, i, out strErrorMessage);
		...
		private TestOptions ReadBusanTestOption(ArrayList arrResult, int index, out string strErrorMessage)
		{
			strErrorMessage = null;
			TestOptions model = new TestOptions();
			bool isNullable;
			
			foreach (TestOptions.Fields field in TestOptions.Fields.GetValues(typeof(TestOptions.Fields)))
			{
				string strFieldName = TestOptions.GetFieldName(field, out isNullable);
				
				if (field == TestOptions.Fields.ID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == TestOptions.Fields.PropertyName)
				{
					string data = WebDBManager.GetStringField(arrResult[index]);
				...
				else if (field == TestOptions.Fields.PropertyValue)
				{
					string data = WebDBManager.GetStringField(arrResult[index]);
		...
		public List<TestEvent> SelectBusanTestEvents(Dictionary<TestEvent.Fields, object> dicConditions,
			string strAdditionalConditions, out string strErrorMessage)
		{
			return SelectBusanTestEvents(dicConditions, strAdditionalConditions, null, out strErrorMessage);
		}
			...
			string strAdditionalConditions, int? topNCount, out string strErrorMessage)
		{
			...
			string strSQL = string.Format("select {0} from {1}", GetFieldNames<TestEvent.Fields>(out nFieldCount),
				TestEvent.TableName);
			
			string strCondition = "";
			
			if (SetCondition<TestEvent.Fields>(ref strCondition, dicConditions, TestEvent.GetFieldName,
				    TestEvent.TableName, ref strErrorMessage) == false)
				return null;
			...
			int nResultCount = arrResult.Count;
			List<TestEvent> datas = new List<TestEvent>();
			
			for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
			{
				TestEvent model = ReadBusanTestEvent(arrResult, i, out strErrorMessage);
		...
		private TestEvent ReadBusanTestEvent(ArrayList arrResult, int index, out string strErrorMessage)
		{
			strErrorMessage = null;
			TestEvent model = new TestEvent();
			bool isNullable;
			
			foreach (TestEvent.Fields field in TestEvent.Fields.GetValues(typeof(TestEvent.Fields)))
			{
				string strFieldName = TestEvent.GetFieldName(field, out isNullable);
				
				if (field == TestEvent.Fields.ID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == TestEvent.Fields.NodeID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == TestEvent.Fields.UniqueID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				} else if (field == TestEvent.Fields.Value)
				{
					VariousData<float> data = WebDBManager.GetFloatField(arrResult[index].ToString());
				...
				} else if (field == TestEvent.Fields.RegDate)
				{
					VariousData<DateTime> data = WebDBManager.GetDateTimeField(arrResult[index]);
		...
		public List<WeatherSensorDataHistory> SelectBusanWeatherSensorDataHistory(
			Dictionary<WeatherSensorDataHistory.Fields, object> dicConditions, string strAdditionalConditions,
			out string strErrorMessage)
		{
			return SelectBusanWeatherSensorDataHistory(dicConditions, strAdditionalConditions, null, out strErrorMessage);
		}

		public List<WeatherSensorDataHistory> SelectBusanWeatherSensorDataHistory(
			Dictionary<WeatherSensorDataHistory.Fields, object> dicConditions, string strAdditionalConditions, int? topNCount,
			out string strErrorMessage)
		{
			strErrorMessage = null;
			int nFieldCount;
			...
			string strSQL = string.Format("select {0} from {1}",
				GetFieldNames<WeatherSensorDataHistory.Fields>(out nFieldCount), WeatherSensorDataHistory.TableName);

			string strCondition = "";

			if (SetCondition<WeatherSensorDataHistory.Fields>(ref strCondition, dicConditions, WeatherSensorDataHistory.GetFieldName,
				    WeatherSensorDataHistory.TableName, ref strErrorMessage) == false)
				return null;
			...
			int nResultCount = arrResult.Count;
			List<WeatherSensorDataHistory> datas = new List<WeatherSensorDataHistory>();

			for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
			{
				WeatherSensorDataHistory model = ReadBusanWeatherSensorDataHistory(arrResult, i, out strErrorMessage);
		...
		private WeatherSensorDataHistory ReadBusanWeatherSensorDataHistory(ArrayList arrResult, int index, out string strErrorMessage)
		{
			strErrorMessage = null;
			WeatherSensorDataHistory model = new WeatherSensorDataHistory();
			bool isNullable;

			foreach (WeatherSensorDataHistory.Fields field in WeatherSensorDataHistory.Fields.GetValues(
				         typeof(WeatherSensorDataHistory.Fields)))
			{
				string strFieldName = WeatherSensorDataHistory.GetFieldName(field, out isNullable);

				if (field == WeatherSensorDataHistory.Fields.ID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == WeatherSensorDataHistory.Fields.SensorID)
				{
					VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
				...
				else if (field == WeatherSensorDataHistory.Fields.Value)
				{
					VariousData<float> data = WebDBManager.GetFloatField(arrResult[index].ToString());
				...
				else if (field == WeatherSensorDataHistory.Fields.OriginTimeStamp)
				{
					VariousData<DateTime> data = WebDBManager.GetDateTimeField(arrResult[index]);
				...
				else if (field == WeatherSensorDataHistory.Fields.TimeStamp)
				{
					VariousData<DateTime> data = WebDBManager.GetDateTimeField(arrResult[index].ToString());
