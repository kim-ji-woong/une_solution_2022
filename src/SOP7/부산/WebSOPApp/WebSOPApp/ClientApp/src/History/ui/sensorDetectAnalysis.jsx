...
class SensorDetectHistory extends Component {
	...
	constructor(props) {
		...
		this.state = {
			...
			saerchFacilityType: -1, // -1: 전체, 1: 대기(센코), 2: 대기(케이웨더), 3: 전류
	...
	async display() {
		...
		const result = await HistoryController.DisplaySensorDetectAnalysis(beginDate, endDate, 21, buildingGroupID, buildingID, zoneID, this.props.selectedSiteID);
		if (!result) {
			$("body").css("cursor", "default");
		...
		if (facilityType === -1) {
			...
			filteredDataSources = this.getFilteredDataSources(dataSource);
		}
		
		const datacount = filteredDataSources.length;
		...
		this.setState({ 
			...
			pageIndex: 1, 
			loadingIndicator: false,
			searchFacilityType: this.state.facilityType,
		});
	}
	
	getFilteredDataSources(dataSource) {
		const etcSensorList = this.props.etcSensorList;
		const externalSensorTypes = this.props.externalSensorTypes;
		...
		for (let i = 0; i < dataSource.length; i++) {
			...
			const targetSensor = etcSensorList.find(sensor => sensor.id === dataSource[i].sensorZoneID);
			const targetExternalSensor = externalSensors.find(sensor => sensor.zoneID === targetSensor.zoneID);
			const targetExternalSensorType = externalSensorTypes.find(sensor => sensor.id === targetExternalSensor.sensorType);
			if (facilityType === 1 && targetExternalSensorType.id === HistoryResource.ExternalSensorTypes.Atmosphere) // 대기(센코)
			{
				filteredDataSources.push(dataSource[i]);
			} else if (facilityType === 2 && targetExternalSensorType.id === HistoryResource.ExternalSensorTypes.KWeather) // 대기(케이웨더)
			{
				filteredDataSources.push(dataSource[i]);
			} else if (facilityType === 3 && targetExternalSensorType.id === HistoryResource.ExternalSensorTypes.Electricity) // 전류
			{
				filteredDataSources.push(dataSource[i]);
			}
		}
	...
	async onClickDownload(isCheckedDownload) {
		...
		let content1 = ('센서탐지분석 요약1', {
			selectedDate: this.state.selectedDate,
			searchZoneName: this.state.searchZoneName,
			allDetectCount: this.state.allDetectCount,
			allMalfunctionRate: this.state.allMalfunctionRate
		});
	...
	getGridData() {
		...
		for (let i = beginIndex; i < beginIndex + this.state.maxRowCount; i++) {
			...
			if (strSensorType.indexOf("대기센서") !== -1)
				strSensorCategory = "대기유해물질";
			else if (strSensorType.indexOf("전류CT센서") !== -1)
	...
	getSensorTypeUI = () => {
		...
		sensorTypeUI.push(
			<li><input type="radio" name="hscsType" id="hscsTypeAll" checked={this.state.facilityType === -1} onChange={() => this.onClickFacilityType(-1)}/> <label>전체</label></li>
		)
		...
		for (let i = 0; i < sensorTypes.length; i++) {
			...
			let element = (
				<li><input type="radio" name="hscsType" id="hscsTypeAll" checked={this.state.facilityType === sensorType.id}
