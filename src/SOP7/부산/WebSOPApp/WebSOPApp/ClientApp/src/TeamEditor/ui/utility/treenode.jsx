...
import { LabelInput } from '../../../SOPManager/styled/componentsStyled';
import { EditArea, TreeEdit, TreeMinus, TreePlus } from '../../../TeamEditor/styled/teamStyled';
...
class TreeNode extends Component {
	...
	onChecked = (event) => {
		...
		this.setCheck(this.props.teamTreeData, this.props.teamTreeData.checked);
	...
	// 하위노드들의 checked 상태를 모두 바꾼다.
	setCheck(node, checked) {
		for (const child of node.Children) {
			child.checked = checked;
			this.setCheck(child, checked);
        }
    }
	...
	getCheckBox() {
		if (this.props.useCheckBox === TreeNode.CheckBox_NormalUse) {
			if (this.props.teamTreeData.checked === TreeNode.CHECKED_ALL) {
				return <LabelInput><input type="checkbox" checked={true} onChange={this.onChecked} style={{ verticalAlign: 'top' }} /></LabelInput>;
			}
			else {
				return <LabelInput><input type="checkbox" checked={false} onChange={this.onChecked} style={{ verticalAlign: 'top' }} /></LabelInput>;
			}
		}
			...
			<>
				{
					(this.props.editNodeID === this.props.teamTreeData.ID || this.state.nodeEdit) ?
						...
						<a className={selectNodeClass} id={team.wordIength} onClick={(e) => this.onTreeNodeChanged(e)} onContextMenu={(e) => this.onTreeNodeChanged(e)}>
							...
							{/* <EditArea>
								<TreeEdit onClick={this.props.editTeam}></TreeEdit>
								<TreeMinus onClick={this.props.removeTeam}></TreeMinus>
								<TreePlus onClick={this.props.addTeam}></TreePlus>
							</EditArea> */}
						</a>
				}
			</>
