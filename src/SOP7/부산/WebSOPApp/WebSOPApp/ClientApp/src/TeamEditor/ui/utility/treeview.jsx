...
class TreeView extends Component {
	...
	componentDidMount() {
		$('.regularNameTag').click(function () {
			console.log("regularNameTag 제이쿼리");
	...
	render() {
		...
		if(this.props.treeViewID === 'teamTree') {
			treeHeight = this.props.treeViewHeight;
		}
		else if (treeHeight === null || treeHeight === undefined || isNaN(treeHeight) === true) {
			// 트리뷰 높이 Props값  설정이 없을 경우 or 값이 잘못 들어가 있을 경우
			if (target[0] != null) {
				treeHeight = target[0].clientHeight - 30;
		...
		console.log(this.props.treeViewID);
		//console.log('teamTree' + "Area");

		if (this.props.teamTreeData && this.props.teamTreeData.length > 0) {
			return (
				<div id={this.props.treeViewID} className="scrollbar-outer">
					<ul id={this.props.treeViewID + "Area"} className={this.props.treeViewID === 'temporaryPopupTree' ? 'sarTreeTeam treeviewTeam' : 'sarTree treeview'} style={{ "height": treeHeight }}  >
						{
			...
			return (<div id={this.props.treeViewID} className="scrollbar-outer" > </div>);
        }
	}
}
