...
export class AccountController {
    ...
    */
    ...
    static async deleteUser(id) {
        const jsonData = JsonManager.makeDeleteUser(id);
    ...
    static async requestAddUser(user) {
        const jsonData = JsonManager.makeRequestAddUser(user);
    ...
    static async requestFindPw(name, phone, mode) {
        const loginKeyResult = await AccountController.requestLoginKey2(name, phone, mode);
        
        if (loginKeyResult === null) {
            const result = {};
            result.success = false;
            result.message = "임시 비밀번호를 발급할 수 없습니다.";
        ...
        try {
            ...
            const jsonData = JsonManager.makeRequestSendPassword(name, phone, strEnc, key, mode);
        ...
        return [false, "임시 비밀번호 발급에 실패하였습니다."];
    }

    static async getBusanTPAccountUsers() {
        try {
            const jsonData = JsonManager.makeRequestAccountUsers();
