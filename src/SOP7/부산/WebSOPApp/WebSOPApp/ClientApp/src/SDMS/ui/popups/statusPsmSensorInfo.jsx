...
class StatusPsmSensorInfo extends Component {
    ...
    getSensorDatas = (sensors, zoneID) => {
        ...
        for (const sensor of normalSensors) {
            ...
            materialValue = parseFloat(sensor.value);
            
            let element = { 
                index: normalIdx, 
                category: sensor.sensorTypeName, 
                //value: parseFloat(sensor.value).toFixed(2),
                value: materialValue === null ? parseFloat(sensor.value).toFixed(2) : materialValue.toFixed(2),
                status: status 
            };
            normalIdx++;
        ...
        temp = parseFloat(temp).toFixed(2) === "NaN" ? "N/A" : parseFloat(temp).toFixed(2);
        humi = parseFloat(humi).toFixed(2) === "NaN" ? "N/A" : parseFloat(humi).toFixed(2);
        windSpeed = parseFloat(windSpeed).toFixed(2) === "NaN" ? "N/A" : parseFloat(windSpeed).toFixed(2);
    ...
    getMaterialIdFromUniqueID = (uniqueID) => {
        const materials = this.props.externalMaterials;
    ...
    getContentsUI = () => {
        ...
        let [sensorUI, weatherUI] = this.getSensorDatas(sensors, selectedSensor.zoneID);
        
        let title = selectedSensorType.id === SdmsResource.SensorType.atmosphere ? '센코 대기오염측정기' : '면오염원 측정기';
