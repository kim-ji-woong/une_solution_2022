...
class NavigationBar extends Component {
    ...
    onClickHome = () => {
        this.props.moveToInitialScreen();
    }
    ...
    onClickSimulation = (visible) => {
        if (this.props.isIndoor) {
            return;
        }
        this.props.setSimulationMode(visible);
    }
    
    getNavigationUI = () => {
        const simulationMode = this.props.visiblePopups[SDMS.menu.simulation];
        if (simulationMode) {
            return (
                <ul>
                    <li>
                        <ul className='navList'>
                            <li data-title="센서현황">
                                <button id={"dsBot_" + SdmsResource.popupLayer.statusInfo}
                                        className={"off" + " " + 'statusInfoIcon'}/>
                            </li>
                            <li data-title="이벤트현황">
                                <button id={"dsBot_" + SdmsResource.popupLayer.event}
                                        className={"off" + " " + 'eventIcon'}/>
                            </li>
                        </ul>
                    </li>
                    <li className='navHomeBtn' data-title="종료">
                        <button onClick={() => this.onClickSimulation(false)}><img src={nav_back} alt={""}></img></button>
                    </li>
                    <li>
                        <ul className='navList'>
                            <li data-title="미니맵">
                                <button id={"dsBot_" + SdmsResource.popupLayer.miniMap}
                                        className={"off" + " " + 'miniMapIcon'} />
                            </li>
                            <li data-title="확산시뮬레이션">
                                <button id={"dsBot_" + SdmsResource.popupLayer.simulation}
                                        className={this.getQuickButtonClassName(SDMS.menu.simulation) + " " + 'simulationIcon'}
                                        onClick={() => this.onClickSimulation(false)}/>
                            </li>
                        </ul>
                    </li>
                </ul>
                        ...
                        <ul className='navList'>
                            <li data-title="센서현황">
                                <button id={"dsBot_" + SdmsResource.popupLayer.statusInfo}
                                        ...
                                        onClick={() => this.props.setVisiblePopups(SDMS.menu.statusInfo)}/>
                            </li>
                            <li data-title="이벤트현황">
                                <button id={"dsBot_" + SdmsResource.popupLayer.event}
                                        className={this.getQuickButtonClassName(SDMS.menu.event) + " " + 'eventIcon'}
                                        onClick={() => this.onClickEventIcon()}/>
                            </li>
                        </ul>
                    </li>
                    <li className='navHomeBtn' data-title="초기화면">
                        <button onClick={() => this.onClickHome()}><img src={nav_home} alt={""}></img></button>
                    </li>
                    <li>
                        <ul className='navList'>
                            <li data-title="미니맵">
                                <button id={"dsBot_" + SdmsResource.popupLayer.miniMap}
                                        className={this.getQuickButtonClassName(SDMS.menu.miniMap) + " " + 'miniMapIcon'}
                                        onClick={() => this.props.setVisiblePopups(SDMS.menu.miniMap)}/>
                            </li>
                            <li data-title="확산시뮬레이션">
                                <button id={"dsBot_" + SdmsResource.popupLayer.simulation}
                                        className={this.getQuickButtonClassName(SDMS.menu.simulation) + " " + 'simulationIcon'}
                                        onClick={() => this.onClickSimulation(true)}/>
                            </li>
                        </ul>
                    </li>
                </ul>
    ...
    render() {
        ...
        const navigationUI = this.getNavigationUI();
