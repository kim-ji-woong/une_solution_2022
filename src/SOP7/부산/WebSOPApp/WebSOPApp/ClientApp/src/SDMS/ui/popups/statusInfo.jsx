...
class StatusInfo extends Component {
    ...
    getSensorUI = () => {
        ...
        for (let i = 0; i < buildings.length; i++) {
            ...
            for (let j = 0; j < zones.length; j++) {
                ...
                let alarmIcon = alarm_off;
                const sensorAlarms = this.props.sensorAlarms;
                const alarm = sensorAlarms.find(alarm => alarm.zoneID === zone.id);
                if (alarm && alarm.isAlarm) {
                    alarmIcon = alarm_on;
                }
                ...
                if (sensorType.id === SdmsResource.SensorType.atmosphere || sensorType.id === SdmsResource.SensorType.kWeather) {
            ...
            let isSelectedTypeElectricity = false;
            ...
            if (selectedSensor !== null && selectedSensor !== undefined) {
                ...
                isSelectedTypeElectricity = selectedSensorType === SdmsResource.SensorType.electricity;
            ...
            isSelectedTypeElectricity = selectedSensorType && selectedSensorType.id === SdmsResource.SensorType.electricity ? " on" : "";
             
            // 임시로 기상센서 UI 삭제
            if (building.id === 301 || building.id === 302 || building.id === 601) {
                continue;
            }
                ...
                <li key={'first-li' + building.id}>
                    ...
                    <ul id={'tree-1depth_' + building.id} className={'tree-1depth' + isSelectedBuilding}>
                        <li key={'second-li' + building.id}>
                            ...
                            <div onClick={(e) => this.onClickSensorType(e, building.id, SdmsResource.SensorType.electricity)}>
                                <p>전류CT센서</p>
                                <p>{electricityCount}</p>
                            </div>
                            {/* 트리 닫힌 상태 : className='tree-2depth' */}
                            {/* 트리 열린 상태 : className='tree-2depth on' */}
                            <ul id={'tree-2depth_' + building.id + "_" + SdmsResource.SensorType.electricity} className={'tree-2depth' + isSelectedTypeElectricity}>
                                {/* 센서 기본 상태 */}
                                {electricitySensors}
                            </ul>
            ...
            <li key={'first-li' + StatusInfo.outdoor}>
                ...
                <ul id={'tree-1depth_' + StatusInfo.outdoor} className={'tree-1depth' + isSelectedBuilding}>
                    <li key={'second-li' + StatusInfo.outdoor}>
                        ...
                        <div
                            onClick={(e) => this.onClickSensorType(e, StatusInfo.outdoor, SdmsResource.SensorType.weather)}>
                            <p>기상센서</p>
                            <p>{weathersCount}</p>
                        </div>
                        <ul id={'tree-2depth_' + StatusInfo.outdoor + "_" + SdmsResource.SensorType.weather}
                            className={'tree-2depth' + isSelectedTypeWeather}>
                            {weatherSensors}
                        </ul>
                    </li>
                </ul>
    ...
    onClickSensor = (zoneID, sensorType) => {
        ...
        if (sensorType.id === SdmsResource.SensorType.atmosphere || sensorType.id === SdmsResource.SensorType.kWeather || sensorType.id === SdmsResource.SensorType.weather) {
            poi = poiInfo.find(poi => poi.id === zoneID);
        } else if (sensorType.id === SdmsResource.SensorType.electricity) {
            poi = poiInfo.find(poi => poi.spaceID === zoneID)
        }
        
        if (poi === null || poi === undefined)
            return;
        
        this.props.sendSelectPOI(poi.spaceID, poi.id);
    ...
    render() {
        ...
        let visibleElectricCTPOI = !!this.props.visibleSensorTypes[SDMSMainMenu.ElectricCT_Sensor];
        ...
        let visibleElectricCTClassName = (visibleElectricCTPOI) ? 'visibleElectricCT' : 'disableElectricCT';
            ...
            <StatusInfoComponent id={this.props.popupType} className='UI_Section statusInfo' $opacity={opacity}
                ...
                >
                    <div className='dslTop'>
                        ...
                        />
                        <button className='dslX' onClick={() => this.props.setVisiblePopups(SdmsResource.ID.menu.statusInfo, false)}>닫기</button>
                    </div>
                    ...
                    <div className={'content'}>
                        <div className="contentBox flex">
                            ...
                            <ul>
                                ...
                                */}
                                <li><label className={visibleElectricCTClassName} data-title="전류CT"><input type="checkbox" checked={visibleElectricCTPOI} onChange={() => this.onChangeVisible(SDMSMainMenu.ElectricCT_Sensor)} /></label></li>
