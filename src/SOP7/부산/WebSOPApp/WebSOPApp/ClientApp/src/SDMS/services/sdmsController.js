...
import {JsonManager} from "../../Settings/services/jsonManager";
...
export class SDMSController {
    ...
    // static async WatchTestOptions() {
    //     let result = await SDMSController.requestTestOptions();
    //     result = result === null || result.success === false ? [] : result;
    //     const testInfo = {
    //         isTest: result.testMode,
    //         events: result.testEvents
    //     }
    //     actionStore.dispatch({ type: 'SET_TEST_INFO', testInfo: testInfo });
    // }
    ...
    static async requestAllSensors() {
        try {
            ...
            const res = await fetch(ProjectResource.baseUrl + '/BusanTP/BusanTP/RequestData', {
                method: 'post',
                headers: {
    ...
    static StartWatchTimer() {
        ...
        let timerAlarm = setTimeout(async function tick() {
            await SDMSController.WatchSensorAlarm();
            timerAlarm = setTimeout(tick, 1000);
        }, 1000);
        ...
        // 30초에 한번 실행 - 센서 리스트 업데이트
        SDMSController.WatchSensorList();
        let timerSensorList = setTimeout(async function tick() {
            await SDMSController.WatchSensorList();
            timerSensorList = setTimeout(tick, 1000 * 1.5);
        }, 1000 * 1.5);
        //
        // SDMSController.WatchTestOptions();
        // let timerTestOptions = setTimeout(async function tick() {
        //     await SDMSController.WatchTestOptions();
        //     timerTestOptions = setTimeout(tick, 1000);
        // }, 1000);
    }

    static async requestTestOptions() {
        try {
            const res = await fetch('BusanTP/BusanTP/RequestData', {
                method: 'post',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ "requestTestOptions": true })
            });
    ...
    static async requestExternalSensors() {
        try {
            const jsonData = SdmsJsonManager.makeRequestExternalSensors();
    ...
    static async requestExternalSensorTypes() {
        try {
            const jsonData = SdmsJsonManager.makeRequestExternalSensorTypes();
    ...
    static async requestExternalMaterials() {
        try {
            const jsonData = SdmsJsonManager.makeRequestExternalMaterials()
            ...
            if (res.ok) {
                ...
                if (result.success)
                    return result.externalMaterialJoinSensorMaterialName;
    ...
    static async requestGetAlarmMemo() {
        try {
            const jsonData = SdmsJsonManager.makeRequestAlarmMemo()
    ...
    static async requestBusanSdmsOptions() {
        try {
            const jsonData = SdmsJsonManager.makeRequestBusanSdmsOption();
    ...
    static async requestResetPopup(userID, popupState) {
        try {
            const jsonData = JsonManager.makeRequestResetPopup(userID, popupState);
    ...
    static async requestUpdateUseReceives(useReceives) {
        try {
            const jsonData = JsonManager.makeRequestUpdateUseReceives(useReceives);
    ...
    static async requestExternalSensorGIS() {
        try {
            const jsonData = JsonManager.makeRequestExternalSensorGIS();
    ...
    static async requestExternalPOIInfo() {
        try {
            const jsonData = SdmsJsonManager.makeRequestBusanExternalPOIInfo();
    ...
    static async requestViewport() {
        try {
            const jsonData = SdmsJsonManager.makeRequestViewport();
    ...
    static async requestSaveBusanViewport(data) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSaveBusanViewport(data);
    ...
    static async requestUserMemos() {
        try {
            const jsonData = SdmsJsonManager.makeRequestUserMemos();
    ...
    static async requestSaveUser(userID, userMemo, accountLevelID) {
        try {
            const jsonData = SdmsJsonManager.makeRequestSaveUser(userID, userMemo, accountLevelID);
    ...
    static async RequestSensorDataHistories(beginTime, endTime, buildingIDs, materialIDs, periodType /* true: 5분, false: 1시간 */) {
        try {
            const jsonData = JsonManager.makeRequestSensorDataHistories(beginTime, endTime, buildingIDs, materialIDs, periodType);
    ...
    static async RequestWeatherHistory(zoneID) {
        try {
            const jsonData = JsonManager.makeRequestWeatherHistory(zoneID);
