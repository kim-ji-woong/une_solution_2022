...
class InitialSituationManagement extends Component {
    ...
    onClickSave = async () => {
        ...
        if (success) {
            ...
            this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["저장에 실패하였습니다."], ['확인'], () => this.props.onCloseConfirmDialog());
        }
    }
                ...
                <section>
                    ...
                    <div className='contentWrap'>
                        <div className='header'>
                            <p>문자내용 작성</p>
                            <div id='tooltip' data-tooltip="{location} : 재난발생위치 / {date} : 재난발생시간 / {material} : 발생유형 특수문자 사용가능" >
...
class EditReceiver extends Component {
    ...
    onClickSave = () => {
        this.props.showConfirmDialog(ProjectResource.dialogTypes.QUESTION, ["수신자 설정을 적용하시겠습니까?"], ['확인'], this.onClickSaveConfirm);
    }
    onClickSaveConfirm = () => {
        this.props.onClickSave(this.tempRegularMemberID);
        this.props.showConfirmDialog(ProjectResource.dialogTypes.SUCCESS, ["저장되었습니다."], ['확인'], this.props.handlePopup(false));
    }
    
    onClickReset = () => {
        this.props.showConfirmDialog(ProjectResource.dialogTypes.QUESTION, ["수신자 설정을 초기화하시겠습니까?"], ['확인'], this.onClickResetConfirm);
    }
    
    onClickResetConfirm = () => {
        this.tempRegularMemberID = this.props.receivers;
        this.props.showConfirmDialog(ProjectResource.dialogTypes.SUCCESS, ["초기화 되었습니다."], ['확인'], () => this.props.onCloseConfirmDialog());
    }
    ...
    render() {
        ...
        return (
            <ModalBackground>
                <EditReceiverComponent className="UI_Section">
            ...
            <div className='btnWrap'>
                <button className='cancle' onClick={() => this.onClickReset()}>초기화</button>
                <button className='submit' onClick={() => this.onClickSave()}>적용</button>
            </div>
