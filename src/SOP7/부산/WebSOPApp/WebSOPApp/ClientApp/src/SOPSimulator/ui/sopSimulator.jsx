...
class SopSimulator extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            selectedAlarm: null, // SDMS에서 넘어온 알람 정보 ( 이벤트 창에서 SOP실행 클릭시 넘어오는 경로 )
        }
        ...
        store.subscribe(function () {
            ...
            this.setRunSopDatas(store.getState());
        }.bind(this));
    ...
    componentDidMount() {
        ...
        this.isEmptySopCommonSettings();
    }
    
    setCurrentAlarm = (selectedAlarm) => {
    ...
    isEmptySopCommonSettings = () => {
        if (this.state.commonSettings.constructor === Object && Object.keys(this.state.commonSettings).length === 0) {
            return SettingsController.reloadSopCommonSettings(true);
        }
    }
    ...
    async loadSOP(storeValue, isFirst) {
        ...
        if (sopHistories === null || sopHistories.length === 0) {
            this.setState({ sopDatas: [], viewSopDatas: [], content: SopSimulatorResource.ID.menu.callSOP, sopTabIndex: -1, currentSopTabKey: '' });
        }
        else {
            ...
            for (let i = 0; i < historyDumpLength; i++) {
                ...
                if (optionIndex === 1 && this.state.currentSopTabKey.length > 0 && sopHistory.key === this.state.currentSopTabKey) {                    
                    ...
                    break;
                     ...
                     // lastAccessActionStepHistoryID 일치하는 SOP 찾기
                    sopTabIndex = i;
                    currentSopTabKey = this.state.currentSopTabKey;
                    currentSopData = sopHistory.sopData;
                    break;
            ...
            if (closeSOPs.length > 0) {
                ...
                if (this.confirmDialogData) {
                    ...
                    this.showConfirmDialog(ProjectResource.dialogTypes.INFO, message, ['확인'], this.confirmDialogCloseSOP);
                }
            }
    ...
    */
    ...
    getCurrentActionStepID(sopKey) {
        const orgSopDatas = this.state.viewSopDatas;
        if (!orgSopDatas) {
            return -1;
        }
    ...
    onClickLogo = () => {
        ...
        this.onSelectMenu(SopSimulatorResource.ID.menu.callSOP);
    }
    onSelectMenu = (menu, param) => {
        ...
        if (menu === SopSimulatorResource.ID.menu.callSOP) {
