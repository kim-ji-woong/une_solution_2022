...
class BeginOption extends Component {
    ...
    componentDidMount() {
        ...
        $('#sqpRdo01').click(function () {
            $('.' + uis.sqpSelectBox).css({ background: '#0D121A' });
            $('.' + uis.sqpSelectBox).addClass(uis.pointerEventsOff);
            $('.' + uis.sqpSelectBox).removeClass(uis.pointerEventsOn);
        });

        $('#sqpRdo02').click(function () {
            $('.' + uis.sqpSelectBox).css({ background: '#fff' });
            $('.' + uis.sqpSelectBox).addClass(uis.pointerEventsOn);
            $('.' + uis.sqpSelectBox).removeClass(uis.pointerEventsOff);
        });
    ...
    onClickBegin = () => {
        ...
        if (!position || position.length === 0) {
            this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ['재난 발생 위치를 입력하세요.'], null, null);
            return;
        }
        else if (position.length > 50) {
            this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ['재난 발생 위치 길이는 50자를 초과할 수 없습니다. (현재:' + position.length + ')'], null, null);
            return;
        }
        else {
            let beginTime = '';
            if (this.state.isCurrent) {
                ...
                beginTime = this.getMakeDateTime();//new Date(year, month, day, hour, min, sec);
            }
            else {
    ...
    render() {
        ...
        return (
            <div id={uis.sopPop} className='UI_Section'>
                <div>
                    <div>
                        <div className={uis.sqPop}>
                            ...
                            <div className={uis.sqpCont}>
                                ...
                                <div className={uis.sqpDown}>
                                    ...
                                    {/* <li><input type="radio" name="sqpRdo" id="sqpRdo01" onChange={() => this.onChangeTimeMode(false)} checked={this.state.isCurrent} /><label htmlFor="sqpRdo01">현재 시간을 재난발생시간으로 설정</label></li>
                                        <li><input type="radio" name="sqpRdo" id="sqpRdo02" onChange={() => this.onChangeTimeMode(true)} /><label htmlFor="sqpRdo02">재난발생 시간 입력</label></li> */}
                                    ...
                                    <ul className={uis.sqpTime + " " + uneStyles.sqpTime}>
                                        <li>
                                            <select name="" id="" value={this.state.year||''} className={uis.sqpSelectBox} onChange={(e) => this.onChangeDate('year', e.target)}>
                                                {yearTag}
                                            </select>
                                        </li>
                                        <li>년</li>
                                        <li>
                                            <select name="" id="" value={this.state.month || ''} className={uis.sqpSelectBox} onChange={(e) => this.onChangeDate('month', e.target)}>
                                                {monthTag}
                                            </select>
                                        </li>
                                        <li>월</li>
                                        <li>
                                            <select name="" id="" value={this.state.day || ''} className={uis.sqpSelectBox} onChange={(e) => this.onChangeDate('day', e.target)}>
                                                {dayTag}
                                            </select>
                                        </li>
                                        <li>일</li>
                                        <li>
                                            <select name="" id="" value={this.state.hour || ''} className={uis.sqpSelectBox} onChange={(e) => this.onChangeDate('hour', e.target)}>
                                                {hourTag}
                                            </select>
                                        </li>
                                        <li>:</li>
                                        <li>
                                            <select name="" id="" value={this.state.min || ''} className={uis.sqpSelectBox} onChange={(e) => this.onChangeDate('min', e.target)}>
                                                {minTag}
                                            </select>
                                        </li>
                                    </ul> 
