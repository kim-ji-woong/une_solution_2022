...
/*        $('.' + uneStyles.rightt).click(function () {
    ...
    async requestSensorName(sensorZoneHistoryID) {
        ...
        if (sensorZoneHistoryID && sensorZoneHistoryID > 0) {
            ...
            sensorMaterials = await SopSimulatorController.getSensorMaterials(sensorZoneHistoryID);
        }
        this.setState({ sensorNames: sensorName, sensorMaterials: sensorMaterials });
    }
    ...
    async beginNextStep() {
        ...
        if (!userAuth || userAuth === null || userAuth.levelID === AccountResource.accountLevelID.user) {
            this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ['해당 로그인 사용자는 권한이 없습니다'], null, null);
            return;
        }
    ...
    beginSopData() {
        if (this.state.currentSection?.length > 0) {
            if (this.state.currentSection[0].componentType === 3 && this.state.currentSection[0].isBegin) {
                // 권한 (사용자 등급은 실행 권한 없음)
                const userAuth = ProjectResource.getUserInfo();
                if (!userAuth || userAuth.levelID === AccountResource.accountLevelID.user) {
    ...
    getSopTabUI() {
        ...
        for (let i = beginIndex; i < endIndex; i++) {
            ...
            if (this.props.sopDatas[i].position) {
                sopTabUI.push(<li className={className} key={i}><a style={{ cursor: "pointer" }} onClick={() => this.onChangeTab(index)}>{disasterName}({stepName})</a><img className={uis.onGoingSopImg} src={onGoingSopImg} /></li>);
            }
            else {
                sopTabUI.push(<li className={className} key={i}><a style={{ cursor: "pointer" }} onClick={() => this.onChangeTab(index)}>{disasterName}({stepName})</a></li>);
            }            
        }

        sopTabUI.push(<li className={uneStyles.posiRelative} key={'tabPlus'}><a className={uneStyles.plus + " " + uneStyles.posiAbsolute} onClick={() => this.props.changeContent(SopSimulatorResource.ID.menu.callSOP)}></a></li>);
    ...
    getPopup() {
        if (this.state.content === SopSimulatorResource.ID.menu.beginSOPOption) {
            ...
            return <BeginOption changeContent={this.changeContent} beginSOP={this.beginSOP} title={title} showConfirmDialog={this.props.showConfirmDialog} />
        }
        ...
        return (
            <>                
                {/* <section className={uis.appContainerWrapSop + " " + uis.clfix + " " + uneCommon.paddingTop60}> */}
                 <section className={uis.appContainerWrapSop + " " + uneCommon.paddingTop60 + " " + 'UI_Section'}>
                    <div className={uis.appContainer + " " + uis.pgProgress}>
                        ...
                        <ProcessListSB sopRunData={this.props.sopDatas[this.props.sopTabIndex]} sensorNames={this.state.sensorNames} sensorMaterials={this.state.sensorMaterials} />
