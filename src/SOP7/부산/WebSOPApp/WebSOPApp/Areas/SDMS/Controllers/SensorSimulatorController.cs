...
using BusanModel = BusanTP.Model;
using BusanIDAL = BusanTP.IDAL;
using dnsDBUtil;
using SDMS.IDAL;
...
namespace WebSOPApp.Areas.SDMS.Controllers
{
    ...
    public class SensorSimulatorController : Controller
    {
        ...
        private BusanIDAL.IDataManager m_sensorDataManager = null;

        public SensorSimulatorController(IDataManager dataManager, BusanIDAL.IDataManager serverDataManager)
        {
        ...
        private IActionResult RequestSendSensorAlarm(RequestSendSensorAlarm data)
        {
            ...
            List<BusanModel.SdmsOption> availableOptions = m_sensorDataManager.GetSelectManager().SelectBusanSdmsOptions(null, null, out strErrorMessage);
            ...
            foreach (BusanModel.SdmsOption option in availableOptions)
            {
                string strUseReceive = "UseReceive";

                string sensorCategory = option.PropertyName.Replace(strUseReceive, "");

                isAvailableSensor[sensorCategory] = option.PropertyValue;
            }
