...
namespace Weather.BLL
{
    ...
    public class LoadManager
    {
        ...
        // WeatherCurrent2 테이블
        public ResponseWeatherInfo GetWeatherInfo2()
        {
            string strErrorMessage;
            List<Site> sites = m_dataManager.GetSelectManager().SelectSites(null, null, out strErrorMessage);
            ...
            bool isNullable;
            string strCondition = string.Format("{0} in ({1})", Current2.GetFieldName(Current2.Fields.WeatherSiteID, out isNullable), strSiteIDs);
            List<Current2> current2s = m_dataManager.GetSelectManager().SelectCurrent2s(null, strCondition, out strErrorMessage);
            ...
            return MakeWeatherInfo2(dicSites, current2s);
        }
        ...
        // WeatherCurrent2 테이블
        private ResponseWeatherInfo MakeWeatherInfo2(Dictionary<int, Site> dicSites, List<Current2> current2s)
        {
            Site site;
            WeatherData data;
            Dictionary<int, WeatherData> dicWeatherDatas = new Dictionary<int, WeatherData>();

            foreach (Current2 current2 in current2s)
            {
                if (dicSites.TryGetValue(current2.WeatherSiteID, out site))
                {
                    if (dicWeatherDatas.TryGetValue(site.ID, out data) == false)
                    {
                        data = new WeatherData();
        ...
        private ResponseWeatherInfo MakeWeatherInfo(Dictionary<int, Site> dicSites, List<Current> currents)
        {
            Site site;
            WeatherData data;
            Dictionary<int, WeatherData> dicWeatherDatas = new Dictionary<int, WeatherData>();
