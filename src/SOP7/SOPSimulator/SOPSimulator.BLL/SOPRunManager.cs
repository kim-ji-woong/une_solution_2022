...
namespace SOPSimulator.BLL
{
    ...
    public class SOPRunManager
    {
        ...
        private static bool m_bChanged = false; // 이력 변경 여부
        public bool Changed
        {
        ...
        public Dictionary<string, SOPRunData> DicSopRunDatas
        {
        ...
        private Dictionary<int, string> m_dicCategoryIDs = new Dictionary<int, string>();
        private int m_nFireCategoryID = 0;
        private int m_nPSMCategoryID = 0;
        private int m_nCollapseCategoryID = 0;
        private int m_nSosCategoryID = 0;
        private int m_nConfinedCategoryID = 0;
        private int m_nVirtualFenceCategoryID = 0;
        ...
        private void InitCategoryIDs()
        {
            ...
            if (categories != null)
            {
                foreach (var category in categories)
                {
                    ...
                    else if (category.CategoryName.ToLower().Replace(" ", "").Contains("sos"))
                        m_nSosCategoryID = category.ID;
                    else if (category.CategoryName.Replace(" ", "").Contains("쓰러짐"))
                        m_nCollapseCategoryID = category.ID;
                    else if (category.CategoryName.Replace(" ", "").Contains("협착"))
                        m_nConfinedCategoryID = category.ID;
                    else if (category.CategoryName.Replace(" ", "").Contains("가상펜스"))
                        m_nVirtualFenceCategoryID = category.ID;

                    m_dicCategoryIDs[category.ID] = category.CategoryName;
                }
            }
        }
        ...
        private bool GetEndTimeSettingOptions(Dictionary<int, int> dicCategoryTimeoutSeconds, Dictionary<int, int> dicCategoryConfirmTimeoutSeconds, Dictionary<int, int> dicDetectCategoryTimeoutSeconds, Dictionary<int, int> dicDetectCategoryConfirmTimeoutSeconds)
        {
            string strErrorMessage;
            ...
            foreach (Options option in options)
            {
                ...
                {
                    case "FireSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nFireCategoryID, option.PropertyValue);
                        break;
                    case "PSMSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nPSMCategoryID, option.PropertyValue);
                        break;
                    case "CollapseSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nCollapseCategoryID, option.PropertyValue);
                        break;
                    case "SosSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nSosCategoryID, option.PropertyValue);
                        break;
                    case "ConfinedSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nConfinedCategoryID, option.PropertyValue);
                        break;
                    case "VirtualFenceSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, m_nVirtualFenceCategoryID, option.PropertyValue);
                        break;
                    case "ETCSOPWaitEndTime":
                        SetTimeoutSeconds(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, -1, option.PropertyValue);
                        break;
                    case "FireSOPRecoverEndTime":
        ...
        private int m_nAllCategoryTimeoutSecond = -1;
        private int m_nAllCategoryConfirmTimeoutSecond = -1;
        private bool CheckTimeoutHistory(List<ActionStepHistory> histories)
        {
            ...
            GetEndTimeSettingOptions(dicCategoryTimeoutSeconds, dicCategoryConfirmTimeoutSeconds, dicDetectCategoryTimeoutSeconds, dicDetectCategoryConfirmTimeoutSeconds);
            ...
            foreach (KeyValuePair<string, SOPRunData> datas in m_sopRunDatas)
            {
                ...
                foreach (ActionStepData actionStep in data.SOPData.ActionStepDatas)
                {
                    ...
                    int nCategoryID = data.SOPData.DisasterCategory.ID == m_nFireCategoryID || 
                        data.SOPData.DisasterCategory.ID == m_nPSMCategoryID || 
                        data.SOPData.DisasterCategory.ID  == m_nSosCategoryID ||
                        data.SOPData.DisasterCategory.ID == m_nCollapseCategoryID ||
                        data.SOPData.DisasterCategory.ID == m_nConfinedCategoryID ||
                        data.SOPData.DisasterCategory.ID == m_nVirtualFenceCategoryID
                        ? data.SOPData.DisasterCategory.ID : -1;
                    int nSeconds;
                    ...
                    if (data.SensorZoneHistoryID == null)
                    {
                        ...
                        else if (m_nAllCategoryTimeoutSecond > 0)
                        {
                            if (CheckTimeoutSeconds(dtNow, m_nAllCategoryTimeoutSecond, history))
                                CloseSOP(history);
                        }
                        else if (m_nAllCategoryConfirmTimeoutSecond > 0)
                        {
                            if (CheckTimeoutSeconds(dtNow, m_nAllCategoryConfirmTimeoutSecond, history))
                            {
                                if (!m_confirmTimeoutCloseSOPs.Contains(history.ID))
                                {
                                    //System.Diagnostics.Trace.WriteLine("확인후 종료할 것 : " + history.ID);                            
                    ...
                    else
                    {
                        ...
                        if (alarms != null && alarms.Count == 0)
                        {
                            ...
                            string strConditions = string.Format("ReactionType in ({0},{1},{2},{3})"
                                , (int)SensorReactionHistory.ReactionTypes.MALFUNCTION
                                , (int)SensorReactionHistory.ReactionTypes.END_STATUS
                                , (int)SensorReactionHistory.ReactionTypes.USER_RESET
                                , (int)SensorReactionHistory.ReactionTypes.TIME_OUT);

                            List<SensorReactionHistory> reactionHistories = m_processManager.SdmsManager.GetSelectManager().SelectSensorReactionHistories(dicCondition2, strConditions, out strErrorMessage);
        ...
        private void SetTimeoutSeconds(string strValue)
        {
            if (strValue == null)
                return;
            ...
            if (int.TryParse(tokens[0].Trim(), out nTime) && int.TryParse(tokens[1].Trim(), out nType) && int.TryParse(tokens[2].Trim(), out nMode))
            {
                ...
                {
                    // 자동종료
                    m_nAllCategoryTimeoutSecond = nSeconds;
                }
                else if (nMode == 1)
                {
                    // 확인후 종료
                    m_nAllCategoryConfirmTimeoutSecond = nSeconds;
                }
                else
                {
                    m_nAllCategoryTimeoutSecond = -1;
                    m_nAllCategoryConfirmTimeoutSecond = -1;
                }
            }
        }
        ...
        private List<ActionStepHistory> DisplayHistory()
        {
            ...
            foreach (ActionStepHistory history in histories)
            {
                ...
                if (!dicTemp.ContainsKey(strKey))
                {
                    ...
                    data.SiteID = res.SOPData.DisasterCategory.SiteID;
                    ...
                    foreach (ActionStepData actionStepData in res.SOPData.ActionStepDatas)
                    {
                        ...
                        if (actionStepData.StepMemberDatas != null && actionStepData.StepMemberDatas.Count > 0)
                        {
                            foreach (SectionData item in actionStepData.StepMemberDatas[0].Sections)
                            {
                                ...
                                item.Text = ReplaceMessage(item.Text, history.Position, history.BeginTime.ToString(), sopLoadManager);

                                if (item.Missions != null)
                                {
                                    foreach (ProcessMissionData mission in item.Missions)
                                    {
                                        mission.MissionText = ReplaceMessage(mission.MissionText, history.Position, history.BeginTime.ToString(), sopLoadManager);
                                    }
                                }
                            }
                        }
                        ...
                        if (actionStepData.ActionStep.ID == history.ActionStepID)
                        {
                            ...
                            if (componentHistories != null && componentHistories.Count == 0)
                            {
                                ...
                                CheckAutoSection(actionStepData.StepMemberDatas[0].Arrows, actionStepData.StepMemberDatas[0].Sections, fristSection.ComponentType, fristSection.ID, (int)SectionStatus.DONE, fristSection.Text, null, history, strKey);
                            }
                            break;
                        }
                    }
                            ...
                            if (actionStepData._ActionStepHistory.ID == componentHistory.ActionStepHistoryID)
                            {
                                ...
                                if (componentHistory.Status == 2)
                                {
                                    foreach (SectionData sectionData in actionStepData.StepMemberDatas[0].Sections)
                                    {
                                        ...
                                        {
                                            if (actionStepData.CurrentSection == null)
                                                actionStepData.CurrentSection = new List<SectionData>();

                                            if (!actionStepData.CurrentSection.Contains(sectionData))
                                                actionStepData.CurrentSection.Add(sectionData);
                                ...
                                {
                                    // 실행중 상태가 아니기때문에 CurrentSection[] 에서 삭제한다
                                    if (actionStepData.CurrentSection != null)
                                    {
                                        int nIndex = -1;
                                        for (int i = 0; i < actionStepData.CurrentSection.Count; i++)
                                        {
                                            SectionData currentSection = actionStepData.CurrentSection[i];
                                            if (currentSection.ComponentType == componentHistory.ComponentType && currentSection.ID == componentHistory.ComponentID)
                                            {
                                                nIndex = i;                                                
                                                break;
                                            }
                                        ...
                                        if (nIndex >= 0)
                                            actionStepData.CurrentSection.RemoveAt(nIndex);
                                    }
                                }
            ...
            if (deleteCount > 0 || chgComponentHistory || bCloseSOP) // 종료된 SOP가 있거나 진행이력이 있다면
            {
                ...
                Console.WriteLine($"{DateTime.Now.ToString("HH:mm:ss")} : Backend sop update");
            }
            else
                m_bChanged = false;
        ...
        private Dictionary<string, SOPRunData> CopySOPRunDatas()
        {
            ...
            foreach (KeyValuePair<string, SOPRunData> item in m_sopRunDatas)
            {
                ...
                dicTemp[item.Key].SiteID = item.Value.SiteID;
        ...
        {
            if (m_processManager.SdmsManager == null)
                return;
            ...
            for (int i = 0; i < alarms.Count; i++)
            {
                ...
                if (alarm.SopStatus == 0)
                {
                    ...
                    if (sopData != null)
                        ExcuteActionStep(sopData, alarm, zone.ZoneName);
                    else
                    {
                        // 연결된 SOP 없음 > SOP 종료로 상태 업데이트
                        select.GetResultData($"update {CurrentAlarm.TableName} set {CurrentAlarm.Fields.SopStatus}=2 where {CurrentAlarm.Fields.SensorZoneHistoryID}={alarm.SensorZoneHistoryID}", out strErrorMessage);
                    }
                }
            }
        }
        private List<SectionData> GetNextSection(List<ArrowData> arrowDatas, List<SectionData> sectionDatas, int componentType, int componentID)
        {
            return GetNextSection(arrowDatas, sectionDatas, componentType, componentID, null);
        }
        private List<SectionData> GetNextSection(List<ArrowData> arrowDatas, List<SectionData> sectionDatas, int componentType, int componentID, DecisionValue decisionValue)
        {
            ...
            for (var i = 0; i < arrowCount; i++)
            {
                ...
                if (curComponentID == beginComponentID)
                {
                    ...
                    if (componentType == 1)
                    {
                        if (decisionValue == null || decisionValue.ArrowID != arrowData.ID)
                            continue;                        
                    }
        ...
        {
            // 대표 Type
            int nSensorType = alarm.SensorType;
            if (dnsData.Sensor.Facility.IsFireSensorType(dnsData.Sensor.Facility.ToFacilityType(alarm.SensorType)))
                nSensorType = (int)FacilityType.FIRE_SENSOR;
            //else if (dnsData.Sensor.Facility.IsPSMSensorType(dnsData.Sensor.Facility.ToFacilityType(alarm.SensorType)))
            ...
            else if (dnsData.Sensor.Facility.IsStrongWindSensorType(dnsData.Sensor.Facility.ToFacilityType(alarm.SensorType)))
                nSensorType = (int)FacilityType.STRONG_WIND;
            //else if (dnsData.Sensor.Facility.IsBlackOutSensorType(dnsData.Sensor.Facility.ToFacilityType(alarm.SensorType)))
            //    nSensorType = (int)FacilityType.BLACKOUT;
            ...
            {
                strAdditionalConditions = string.Format("({0} = {1} or {2} = {3})",
                    LinkedSop.GetFieldName(LinkedSop.Fields.LinkedBuildingID, out isNullable), (int)zone.BuildingID,
                    LinkedSop.GetFieldName(LinkedSop.Fields.LinkedZoneID, out isNullable), zone.ID);
                //dicCondition.Add(LinkedSop.Fields.LinkedBuildingID, zone.BuildingID);
            }
            else
            {
                strAdditionalConditions = string.Format("{0} = {1}", LinkedSop.GetFieldName(LinkedSop.Fields.LinkedZoneID, out isNullable), zone.ID);
            }

            //dicCondition.Add(LinkedSop.Fields.LinkedZoneID, zone.ID);

            string strErrorMessage = null;
            List<LinkedSop> sops = m_processManager.SopDataManager.GetSelectManager().SelectLinkedSops(dicCondition, strAdditionalConditions, null, out strErrorMessage);
            if (sops == null)
                return null;
            ...
            if (sops.Count == 0)
            {
                ...
                if (m_processManager.CommonDataManager.SiteID == 20) // 여수
                {
                    return null;
                }
            ...
            if (sop != null)
            {
                sopData = GetLinkedSOP(sop.DisasterCategoryID, sop.SubDisasterCategoryID, sop.DisasterName, alarm);
            }
        ...
        private SOPManager.BLL.Models.SOP.SOPData GetLinkedSOP(int disasterCategoryID, int subDisasterCategoryID, string disasterName, CurrentAlarm alarm)
        {
            ...
            string strCondition = string.Format("({0}.OwnerID = {1}.ID or {0}.OwnerID is null) AND {0}.ID = {2}.VersionID AND {2}.{3}={4} AND {2}.{5}='{6}' AND {3} in (Select {7} From {8} Where {9}={10})",
        ...
        public void ExcuteActionStep(SOPManager.BLL.Models.SOP.SOPData sopData, CurrentAlarm alarm, string strPosition)
        {
            ...
            if (actionStepData != null)
            {
                ...
                if (!m_sopRunDatas.ContainsKey(key))
                    BeginSOP(alarm.TimeStamp, actionStepData.ActionStep.ID, actionStepData, strPosition, alarm.SensorZoneHistoryID);

                SetAlarmSopStatus(alarm.SensorZoneHistoryID, 1);
            }
        }
        public ActionStepHistory ExcuteActionStep(SOPManager.BLL.Models.SOP.SOPData sopData, string strPosition, int? nAlarmDepth)
        {
            SOPManager.BLL.Models.SOP.ActionStepData actionStepData = GetExcuteActionStep(sopData, nAlarmDepth);
            if (actionStepData != null)
            {
                string key = sopData.DisasterCategory.ID + "/" + sopData.SubDisasterCategory.ID + "/" + sopData.Disaster.ID;
                if (!m_sopRunDatas.ContainsKey(key))
                    return BeginSOP(DateTime.Now, actionStepData.ActionStep.ID, actionStepData, strPosition, null);
            }
        ...
        private List<int> GetContainsStepIndex(SOPManager.BLL.Models.SOP.SOPData sopData)
        {
            ...
            for (int i = 0; i < actionStepsCount; i++)
            {
                ...
                for (int j = 0; j < m_strActionStepNames.Count; j++)
                {
        ...
        private SOPManager.BLL.Models.SOP.ActionStepData GetActionStep(SOPManager.BLL.Models.SOP.SOPData sopData, int index)
        {
            ...
            for (int i = 0; i < actionStepsCount; i++)
            {
                ...
                if (m_strActionStepNames.Count > index && actionStepData.StepName == m_strActionStepNames[index])
                {
        ...
        /// 시작
        /// </summary>
        public ActionStepHistory BeginSOP(DateTime beginTime, int actionStepID, ActionStepData actionStepData, string position, int? sensorZoneHistoryID)
        {
            ActionStepHistory newHistory = ExcuteSOP(beginTime, actionStepID, position, null, sensorZoneHistoryID);
            if (newHistory != null)
            {
                if (actionStepData != null)
                {
                    ...
                    CheckAutoSection(arrowDatas, sectionDatas, currentSectionData.ComponentType, currentSectionData.ID, (int)SectionStatus.DONE, currentSectionData.Text, null, newHistory, ""); 
                }
                else
                {
        ...
        public int BeginSOP(string strBeginTime, int versionID, int actionStepID, string position, int? sensorZoneHistoryID)
        {
            SOPManager.BLL.ProcessManager processMgr =
                new SOPManager.BLL.ProcessManager(m_processManager.CommonDataManager, m_processManager.SopDataManager, m_processManager.TeamDataManager, m_processManager.SdmsManager);

            SOPManager.BLL.Models.Response.ResponseOpen res = processMgr.GetLoadManager().OpenDB(versionID);
            if (!res.Success)
                return -1;

            foreach (ActionStepData item in res.SOPData.ActionStepDatas)
            {
                if (item.ActionStep == null)
                    continue;

                if (item.ActionStep.ID == actionStepID)
                {
                    DateTime dtBeginDate = DateTime.Now;
                    if (strBeginTime?.Length > 0)
                        dtBeginDate = Convert.ToDateTime(strBeginTime);

                    ActionStepHistory history = BeginSOP(dtBeginDate, actionStepID, item, position, sensorZoneHistoryID);
                    if (history == null)
                        return -1;
        ...
        private void CheckAutoSection(List<ArrowData> arrowDatas, List<SectionData> sectionDatas, int componentType, int componentID, int status, string sectionText, DecisionValue decisionValue, ActionStepHistory history, string strSopKey)
        {
            // 시작
            ProgressSOP(history, componentID, componentType, /*(int)SectionStatus.DONE*/status, null, sectionText);


            // 수정 
            bool bWhile = true;
            AutoRunSection(strSopKey, history, arrowDatas, sectionDatas, componentType, componentID, decisionValue, ref bWhile);
            ...
            //            {
            //                if (nextSection.IsSMS == true)
            //                {
            //                    SendSMS(nextSection.Receivers, nextSection.Message);
            //                }
            //            }
        ...
        private void AutoRunSection(string strSopKey, ActionStepHistory history, List<ArrowData> arrowDatas, List<SectionData> sectionDatas, int componentType, int componentID, DecisionValue decisionValue, ref bool bWhile)
        {            
            List<SectionData> nextSections = GetNextSection(arrowDatas, sectionDatas, componentType, componentID, decisionValue);
                ...
                foreach (SectionData nextSection in nextSections)
                {
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            if (nextSection.ComponentType == 0 && nextSection.Missions?.Count > 0)
                            {
                                for (int i = 0; i < nextSection.Missions.Count; i++)
                                {
                                    if (strSopKey.Length > 0)
                                    {
                                        ProgressMission(strSopKey, history.ID, componentType, componentID, i, (int)SectionStatus.DONE, null, true, false); 
                                    }
                                    else
                                    {
                                        ...
                                        ComponentHistory history1 = ProgressSOP(history, componentID, componentType, (int)SectionStatus.DONE, null, nextSection.Text, addDescription);
                                        if (history1 == null)
                                            continue;
                                        ...
                                        ComponentHistoryDetail detail = m_processManager.CommonDataManager.GetCreateManager().CreateComponentHistoryDetail(
                                            history1.ID, i, nData, null, null, DateTime.Now);
                            ...
                            else
                            {
                                if (strSopKey.Length>0)
                                {
                                    ProgressMission(strSopKey, history.ID, componentType, componentID, -1, (int)SectionStatus.DONE, null, true, false /*이미 다음 Section 찾고 있음*/); 
                                }
                                else
                                {
                                    // 시작 컴포넌트 다음이 바로 자동컴포넌트인 경우 SOP가 m_sopRunDatas 쌓이기 전이기때문에 ComponentHistory 정보를 별도로 넣어줌
                                    ...
                                    ComponentHistoryDetail detail = m_processManager.CommonDataManager.GetCreateManager().CreateComponentHistoryDetail(
                                        history1.ID, -1, nData, null, null, DateTime.Now);
        ...
        /// 단계 격상
        /// </summary>
        public void NextActionStep(RequestNextActionStep req)
        {
            try
            {
                m_bOnGoing = true;
                if (!m_sopRunDatas.ContainsKey(req.SopKey))
                {
                    m_bOnGoing = false;
                    return;
                }
                ...
                DateTime dtNow = DateTime.Now;
                SOPRunData datas = m_sopRunDatas[req.SopKey];
                foreach (ActionStepData runData in datas.SOPData.ActionStepDatas)
                {
                    ...
                    {
                        if (runData.ActionStep.ID == req.NextActionStepID)
                        {
                            BeginSOP(dtNow, runData.ActionStep.ID, runData, datas.Position, datas.SensorZoneHistoryID);
                        }
                    }
                    else if (runData._ActionStepHistory != null)
                    {
                        if (runData.ActionStep.ID == req.PrevActionStepID)
                        {
                            CloseSOPByUser(runData._ActionStepHistory.ID, dtNow, req.AccessedUserID);
                        }
                    }
                }
                ...
                foreach (ActionStepData runData in datas.SOPData.ActionStepDatas)
                {
                    ...
                    if (runData._ActionStepHistory == null)
                    {
                        if (data.ActionStepHistoryID == -1)
                        {
                            ...
                            {
                                BeginSOP(DateTime.Now, runData.ActionStep.ID, runData, datas.Position, datas.SensorZoneHistoryID);
                                break;
                            }
                        }
                        continue;
                    ...
                    SectionData matchCurrentSection = null;
                    foreach (var item in runData.CurrentSection)
                    {
                        if (item.ID == data.ComponentID && item.ComponentType == data.ComponentType)
                        {
                            matchCurrentSection = item;
                            bInCurrentSection = true;
                            break;
                        }
                    }

                    if (bInCurrentSection)
                    {
                        // 현재 임무의 다음 버튼을 눌렀거나

                        bool bAllChecked = GetAllMissionsChecked(matchCurrentSection);
                        // 다음버튼이 눌러진 임무 종료 처리 후 다음 임무를 찾아서 스킵 처리한다
                        StepMemberData stepMemberData = runData.StepMemberDatas[0];
                        int nStatus = bAllChecked ? (int)SectionStatus.DONE : (int)SectionStatus.SKIP;
                        //if (data.ComponentType == 1) // 판단문은 실행중이 없다 => 판단문 재 실행 할수 없어서 일단 보류
                        //    nStatus = (int)SectionStatus.DONE;
                        CheckAutoSection(stepMemberData.Arrows, stepMemberData.Sections, data.ComponentType, data.ComponentID, nStatus, data.Text, data.DecisionValue, runData._ActionStepHistory, data.SopKey);
                    }
                    else if (data.Skip && data.ComponentType != 3)
                    {
                        // 다른 임무의 다음 버튼을 누른 경우 (건너뛰기) > 현재 진행중인 임무 모두 완료(스킵) 처리
                        foreach (var item in runData.CurrentSection)
                        {
                            bool bAllChecked = GetAllMissionsChecked(item);
                            // 다음버튼이 눌러진 임무 종료 처리 후 다음 임무를 찾아서 스킵 처리한다
                            //StepMemberData stepMemberData = runData.StepMemberDatas[0];
                            int nStatus = bAllChecked ? (int)SectionStatus.DONE : (int)SectionStatus.SKIP;
                            ProgressSOP(runData._ActionStepHistory, data.ComponentID, data.ComponentType, nStatus, data.AccessedUserID, data.Text);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            foreach (var currentSection in runData.CurrentSection)
                            {
                                bool bAllChecked = GetAllMissionsChecked(currentSection);
                                // 다음버튼이 눌러진 임무 종료 처리 후 다음 임무를 찾아서 스킵 처리한다
                                //StepMemberData stepMemberData = runData.StepMemberDatas[0];
                                int nStatus = bAllChecked ? (int)SectionStatus.DONE : (int)SectionStatus.SKIP;
                                ProgressSOP(runData._ActionStepHistory, currentSection.ID, currentSection.ComponentType, nStatus, data.AccessedUserID, currentSection.Text);
                            }
                    ...
                    //    bool bAllChecked = GetAllMissionsChecked(runData.CurrentSection);
                    //    // 다음버튼이 눌러진 임무 종료 처리 후 다음 임무를 찾아서 실행 처리한다
                    //    StepMemberData stepMemberData = runData.StepMemberDatas[0];
                    //    int nStatus = bAllChecked ? (int)SectionStatus.DONE : (int)SectionStatus.RUN;
                    //    //if (data.ComponentType == 1) // 판단문은 실행중이 없다 => 판단문 재 실행 할수 없어서 일단 보류
                    //    //    nStatus = (int)SectionStatus.DONE;
                    //    CheckAutoSection(stepMemberData.Arrows, stepMemberData.Sections, data.ComponentType, data.ComponentID, nStatus, data.Text, data.DecisionValue, runData._ActionStepHistory);
                    //}
                    //else
                    //{
                    //    // 현재 임무와 다른 임무인가 ? (건너뛰기) Y: 선택한 임무 진행중 처리
        ...
        private bool GetAllMissionsChecked(SectionData section)
        {            
            if (section.ComponentType == 6)
            {
                return section.Checked;
            }
            else if (section.ComponentType == 0)
            {
                // 임무가 하나도 없는 경우 클라이언트에서 행동요령에 체크했는지 유무에 따른다
                if (section.Missions == null || section.Missions.Count == 0)
                    return section.Checked;
        ...
        private ComponentHistory ProgressSOP(ActionStepHistory actionStepHistory, int componentID, int componentType, int status, int? userID, string text, string addDescription = "")
        {
            ...
            Console.WriteLine($"ProgressSOP] status={status}, 설명: {strDescription}");
        ...
        public bool ProgressMission(string sopKey, int actionStepHistoryID, int componentType, int componentID, int dataIndex, int componentStatus, int? userID, bool isChecked, bool bFindNextAutoSection = true)
        {
            try
            {
                m_bOnGoing = true;
                ...
                foreach (SectionData section in stepMemberData.Sections)
                {
                    if (section.ID == componentID && section.ComponentType == componentType)
                    {
                        ...
                        if (allCheck && isChecked && bFindNextAutoSection)
                        {
                            CheckAutoSection(stepMemberData.Arrows, stepMemberData.Sections, componentType, componentID, (int)SectionStatus.DONE, section.Text, null, actionStepData._ActionStepHistory, sopKey);
                        }
        ...
        public bool ExcuteExternalProgram(string sopKey, int actionStepHistoryID, int componentType, int componentID, int dataIndex, int componentStatus, int? userID)
        {
            try
            {
                m_bOnGoing = true;
                ...
                foreach (SectionData section in actionStepData.StepMemberDatas[0].Sections)
                {
                    if (section.ID == componentID && section.ComponentType == componentType)
                    {
                        ...
                        ProcessMissionData data = section.Missions[dataIndex];
                        if (data.ProgramID == null)
                            return false;

                        string strErrorMessage = null;
                        ExternalProgram program = m_processManager.SopDataManager.GetSelectManager().SelectExternalProgram((int)data.ProgramID, out strErrorMessage);
                        if (program == null)
                            return false;

                        string strInstallPath = ""; 
                        if (program.InstallPath?.Length > 0)
                            strInstallPath = program.InstallPath;
                        else
                        {
                            strInstallPath = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location);
                        }

                        // 외부 프로그램마다 파라미터 추가 인자가 있을 수 있음 
                        ...
                        // 외부 프로그램 파라미터 추가 확인
                        GetExternalProgramParameters(actionStepData, program, parameters, out strErrorMessage);

                        string addDescription = data.ProgramName + " 실행";
                        if (ExcuteExternalProgram(strInstallPath, program.ExeName, parameters))
                        {
                            ComponentHistory history = ProgressSOP(actionStepData._ActionStepHistory, componentID, componentType, componentStatus, userID, section.Text, addDescription);
                            if (history == null)
                                return false;
        ...
        private bool GetExternalProgramParameters(ActionStepData actionStepData, ExternalProgram program, List<string> parameters, out string strErrorMessage)
        {
            strErrorMessage = "";

            if (actionStepData == null || program == null || parameters == null)
            {
                strErrorMessage = "데이터가 올바르지 않습니다.";
                return false;
            }
                
            // 수자인 전관 순차 피난 메시지 전파 파라미터
            if (program.ExeName == "SujainEmergencySend.exe")
            {
                int? nSensorZoneHistoryID = actionStepData._ActionStepHistory?.SensorZoneHistoryID;
                if (nSensorZoneHistoryID.HasValue)
                {
                    string strAdditionalConditions = string.Format("{0}.{1} = {2}", SensorZoneHistory.TableName, SensorZoneHistory.Fields.ID, nSensorZoneHistoryID);
                    ArrayList arrResult = m_processManager.SdmsManager.GetSelectManager().JoinEquipmentZoneSensorReactionHistorySensorZoneSensorZoneHistory2(null, null, null, null,  strAdditionalConditions, out strErrorMessage);
                    if (arrResult == null)
                        return false;
                    ...
                    if (resultCount == 0)
                    {
                        strErrorMessage = "SensorZoneHistoryID 해당 데이터가 존재하지 않습니다. SensorZoneHistoryID: " + nSensorZoneHistoryID;
                        return false;
                    }

                    for (int i = 0; i < resultCount; i += 5)
                    {
                        if (arrResult[i + 2] is SDMS.Model.Sensor.SensorZone &&
                            arrResult[i + 4] is SDMS.Model.Spatial.Zone)
                        {
                            SDMS.Model.Sensor.SensorZone sensorZone = arrResult[i + 2] as SDMS.Model.Sensor.SensorZone;
                            SDMS.Model.Spatial.Zone zone = arrResult[i + 4] as SDMS.Model.Spatial.Zone;

                            if (sensorZone.SensorType == 0)
                                parameters.Insert(0, zone.ID.ToString());
                        }
                    }
                }
            }
        ...
        private bool ExcuteExternalProgram(string strInstallPath, string strExeName, List<string> parammeters)
        {
            try
            {
                ...
                if (parammeters != null && parammeters.Count > 0)
                    args = string.Join(" ", parammeters);

                System.Diagnostics.Process.Start(strInstallPath + "\\" + strExeName, args);
        ...
        public bool ProgressSpread(string sopKey, int actionStepHistoryID, int componentType, int componentID, int dataIndex, int componentStatus, int? userID, bool isSMS, bool isEmail, bool isBroadcast, bool isSiren, string strMessage, List<Receiver> teams = null)
        {
            try
            {
                Logger.Instance.Write("in SOPRunManager");
                m_bOnGoing = true;
                ...
                foreach (SectionData section in actionStepData.StepMemberDatas[0].Sections)
                {
                    if (section.ID == componentID && section.ComponentType == componentType)
                    {
                        List<string> messages = new List<string>(); // 문자,메일,방송 전파용
                        List<ProcessMissionData> missionDatas = new List<ProcessMissionData>(); // 외부프로그램 실행용
                        if (componentType == 0)
                        {
                            ...
                            {                                
                                ...
                                {
                                    if (section.Missions[i].MissionType == ProcessMissionData.MissionDataType.Normal)
                                    {
                                        messages.Add(section.Missions[i].MissionText);
                                    }
                                    else if (section.Missions[i].MissionType == ProcessMissionData.MissionDataType.External)
                                    {
                                        missionDatas.Add(section.Missions[i]);
                                    }                                    
                                }
                            ...
                            else
                            {
                                ...
                                if (section.Missions[dataIndex].MissionType == ProcessMissionData.MissionDataType.Normal)
                                    messages.Add(section.Missions[dataIndex].MissionText);
                                else if (section.Missions[dataIndex].MissionType == ProcessMissionData.MissionDataType.External)
                                    missionDatas.Add(section.Missions[dataIndex]);
                            } 
                        ...
                        for (int i = 0; i < messages.Count; i++)
                        {
                            ...
                            {
                                int dataI = 2; // 0: 체크해제, 1: 체크, 10: 문자메시지전파, 20: 메일전파, 30: 방송전파, 40: 외부프로그램실행
                                int index = dataIndex;
                                if (index == -1)
                                    index = i;
                                ...
                                if (isSMS)
                                {
                                    ...
                                    //{
                                    //    if (SendSMS(phoneNumbers, message))
                                    //    {
                                    //        ComponentHistory history = ProgressSOP(actionStepData._ActionStepHistory, componentID, componentType, componentStatus, userID, section.Text, addDescription);
                                    //        if (history == null)
                                    //            return false;
                                    ...
                                    if (IsSopAlone)
                                    {
                                        if (m_processManager.SopSimulatorDataManager.SiteID == 1)
                                        {
                                            string strSQL = $@"  
                                              Select SubCategoryName
                                                from SopCategoryDisasterCategory dc
                                               inner join SopCategorySubDisasterCategory sdc on dc.ID=sdc.DisasterCategoryID
                                               inner join SopCategoryDisaster d on sdc.ID=d.SubDisasterCategoryID
                                               where d.ID={actionStepData.ActionStep.ID}";

                                            string strError = null;
                                            ArrayList arrResult = m_processManager.SopDataManager.GetSelectManager().GetResultData(strSQL, out strError);
                                            if (arrResult == null || arrResult.Count == 0)
                                                return false;

                                            string strDisaster = dnsDBUtil.WebDBManager.GetStringField(arrResult[0]);
                                            string strMessageTemp = 
                                                $@"[깨끗한나라 SOP 대응 요령]\r\n{actionStepData._ActionStepHistory.Position}에서 {strDisaster}상황이 감지되었습니다.\r\n담당자는 아래 행동요령을 확인하세요\r\n\r\n{strMessage}\r\n\r\n-깨끗한나라-";

                                            message = strMessageTemp;
                                        }
                                    }

                                    List<Receiver> receivers = teams == null ? section.Receivers : teams;

                                        if (SendSMS(receivers, message))
                                        {
                                            ComponentHistory history = ProgressSOP(actionStepData._ActionStepHistory, componentID, componentType, componentStatus, userID, section.Text, addDescription);
                                            if (history == null)
                                                return false;
                                ...
                                if (isBroadcast)
                                {
                                    ...
                                    // 현재 실행중인 방송이 있으면 방송을 진행하지 않는다.
                                    if (SendBroadcast(actionStepData._ActionStepHistory.ID, componentID, message, out strErrorMessage))
                                    {
                                        ComponentHistory history = ProgressSOP(actionStepData._ActionStepHistory, componentID, componentType, componentStatus, userID, section.Text, addDescription);
                                        if (history == null)
                                            return false;
                        ...
                        {
                            ProcessMissionData data = missionDatas[i];
                            if (data.ProgramID == null)
                                return false;
                            ...
                            string addDescription = data.ProgramName + " 실행";
                            if (ExcuteExternalProgram(strInstallPath, program.ExeName, data.Parameters))
                            {
                                ComponentHistory history = ProgressSOP(actionStepData._ActionStepHistory, componentID, componentType, componentStatus, userID, section.Text, addDescription);
                                if (history == null)
                                    return false;
        ...
        {
            bool result = m_processManager.GetSMSManager().SendSMS("", phoneNumbers, message);
            return result;
        }
        ...
        private bool SendBroadcast(int actionStepHistoryID, int componentID, string message, out string strErrorMessage)
        {
            strErrorMessage = null;
            ResponseBroadcastRunning result = CheckBroadcastRunning();

            if (result.Success == false)
                return false;

            if (result.RunBroadcast == false)
            {
                BroadcastHistory broadcastHistory = m_processManager.CommonDataManager.GetSelectManager().SelectBroadcastHistory(actionStepHistoryID, componentID, out strErrorMessage);

                ArrayList arrDatas = new ArrayList();
                arrDatas.Add(actionStepHistoryID);
                arrDatas.Add(componentID);

                BaseMessageClient messageClient = MessageClientFactory.CreateMessageClient();
                messageClient.Tag = arrDatas;//new BroadcastHistoryData(actionStepHistoryID, componentID);
                messageClient.OnCompleteBroadcast = this.OnCompleteBroadcast;

                messageClient.Run(false, message);

                if (broadcastHistory == null)
                    return CreateBroadcastHistory(actionStepHistoryID, componentID, (int)BroadcastHistory.BroadcastStatus.Run, out strErrorMessage);

                return UpdateBroadcastHistory(actionStepHistoryID, componentID, (int)BroadcastHistory.BroadcastStatus.Run, out strErrorMessage);
            }
        ...
        {
            if (sender != null && sender.Tag != null && sender.Tag is ArrayList/*BroadcastHistoryData*/)
            {
                ArrayList arrDatas = (ArrayList)sender.Tag;
                ...
                if (arrDatas.Count == 2 && arrDatas[0] is int && arrDatas[1] is int)
                {
                    ...
                    BroadcastHistory history = new BroadcastHistory();
                    history.ActionStepHistoryID = actionStepHistoryID/*data.ActionStepHistoryID*/;
                    history.ComponentID = componentID/*data.ComponentID*/;
                    history.EventTime = DateTime.Now;

                    if (status == BaseMessageClient.Status.Completed)
                        history.Status = (int)BroadcastHistory.BroadcastStatus.Complete;
                    else if (status == BaseMessageClient.Status.Stopped)
                        history.Status = (int)BroadcastHistory.BroadcastStatus.Stop;
                    else
                        history.Status = (int)BroadcastHistory.BroadcastStatus.TimeOut;

                    string strErrorMessage;
                    m_processManager.CommonDataManager.GetUpdateManager().UpdateBroadcastHistory(history, out strErrorMessage);
                }
            }
        }

        private bool CreateBroadcastHistory(int actionStepHistoryID, int componentID, int status, out string strErrorMessage)
        {
            BroadcastHistory history = m_processManager.CommonDataManager.GetCreateManager().CreateBroadcastHistory(actionStepHistoryID, componentID, DateTime.Now, status, out strErrorMessage);
            return history != null;
        }

        private bool UpdateBroadcastHistory(int actionStepHistoryID, int componentID, int status, out string strErrorMessage)
        {
            BroadcastHistory history = new BroadcastHistory();
            history.ActionStepHistoryID = actionStepHistoryID;
            history.ComponentID = componentID;
            history.EventTime = DateTime.Now;
            history.Status = status;

            return m_processManager.CommonDataManager.GetUpdateManager().UpdateBroadcastHistory(history, out strErrorMessage);
        }
        ...
        private string GetStringStatus(int componentType, int nStatus, string text)
        {
            ...
            if (nStatus == (int)SectionStatus.NORMAL)
                ...
                strStatus = "완료";
            else if (nStatus == (int)SectionStatus.SKIP)
                strStatus = "스킵";
        ...
        public string ReplaceMessage(string message, string position, string time, SOPManager.BLL.LoadManager loadManager)
        {
            ...
            {                
                ...
                req.Time = time;
                req.Variables = new List<string>();
                req.AddVariable("location", position);
                req.AddVariable("time", time);
        ...
        // 현재 방송이 실행중인가?
        public ResponseBroadcastRunning CheckBroadcastRunning()
        {
            string strErrorMessage = null;

            Dictionary<BroadcastHistory.Fields, object> dicConditions = new Dictionary<BroadcastHistory.Fields, object>();
            dicConditions[BroadcastHistory.Fields.Status] = (int)BroadcastHistory.BroadcastStatus.Run;

            // 현재 실행중인 방송이 있는가?
            List<BroadcastHistory> broadcastHistories = m_processManager.CommonDataManager.GetSelectManager().SelectBroadcastHistories(dicConditions, null, out strErrorMessage);

            if (broadcastHistories == null)
                return new ResponseBroadcastRunning(false, strErrorMessage, false);

            DateTime dtNow = DateTime.Now;

            foreach (BroadcastHistory history in broadcastHistories)
            {
                TimeSpan span = dtNow - history.EventTime;

                // 방송시작후 5분이상 경과하면 방송 종료로 간주한다.
                if (span.TotalMinutes > 5.0)
                {
                    history.EventTime = dtNow;
                    history.Status = (int)BroadcastHistory.BroadcastStatus.TimeOut;

                    if (m_processManager.CommonDataManager.GetUpdateManager().UpdateBroadcastHistory(history, out strErrorMessage) == false)
                        return new ResponseBroadcastRunning(false, strErrorMessage, false);
                }
                else
                    return new ResponseBroadcastRunning(true, "", true);
            }

            return new ResponseBroadcastRunning(true, "", false);
        }
    }
}
