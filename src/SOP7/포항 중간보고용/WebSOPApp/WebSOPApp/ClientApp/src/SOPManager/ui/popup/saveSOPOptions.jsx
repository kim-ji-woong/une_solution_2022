...
import { SopPop, SpPop, SppTop, SppSel, SppCont, SppLft, SppRht, SpprCont, SpprCont2, ScTb, SpprBotSave } from '../../../SOPManager/styled/popupStyled';
...
class SaveSOPOptions extends Component {
	...
	async getVersions(sopData) {
		...
		if (disasterVersions === null) {
			this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [message], ['확인'], this.onClickCancle);
		...
		if (ok) {
			if (this.refCheckNewVersion.current.checked) {
				...
				if (newVersionName.length === 0) {
					this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [SopManagerResource.ID.messages.inputSOPVersionName], null, null);
					return;
				}
			...
			if (this.state.selectedVersion) {
				...
				this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [SopManagerResource.ID.messages.selectSOPVersion], null, null);
            }
		}
		else {
			this.props.content(SopManager.menu.editSOP, this.state.sopData);
						...
						<SpPop>
							...
							<SpprCont2>
												...
												<tbody>
													{
														this.state.currentVersion && (
															<tr onClick={(event) => this.tbRdo(event, this.state.currentVersion)}>
																...
																<td>{this.state.currentVersion.description}</td>
															</tr>
                                                        )
                                                    }
								...
								<SpprBotSave>
									...
									<a className="cancel" /* className={SaveSOPOptions.cssStyles.clickable} */ onClick={() => this.onClickApply(false)}>취소</a>
									<a className="save" /* className={SaveSOPOptions.cssStyles.blu} */ onClick={() => this.onClickApply(true)}>저장</a>
								</SpprBotSave>
							</SpprCont2>
						</SpPop>
					</div>
