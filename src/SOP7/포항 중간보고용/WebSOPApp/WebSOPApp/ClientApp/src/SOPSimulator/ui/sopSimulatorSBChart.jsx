...
/*    watermarkplay() {
    ...
    showConfirmDialog() {
        ...
        if(userInfo && userInfo !== null && (parseInt(userInfo.levelID) === AccountResource.accountLevelID.master || parseInt(userInfo.levelID) === AccountResource.accountLevelID.generalAdmin)) {
            this.props.showConfirmDialog(ProjectResource.dialogTypes.QUESTION, ['실행중인 SOP는 종료 후 수정 할 수 있습니다.', '종료할까요 ?'], ['종료 후 열기', '종료하지 않고 열기', '취소'], this.onClickEditSOP);
        } 
        else {
            this.props.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ['권한이 없습니다.'], null, null);
        }
    }
    ...
    onClickEditSOP(index) {        
        ...
        if (index <= 1) {
            this.props.history.push("/sop-manager?sop=" + this.props.sopData.version.id, '_blank');
                ...
                <section className={uis.subSection + " " + uis.progressViewWrap} >
                    <div className={uis.tit + " " + uis.clfix}>
                        ...
                        <div className={uis.btnArea}>
                            ...
                            <button className={uis.btnEnd} onClick={() => this.props.closeSOP(false)}><i className={uis.iconEnd}></i></button>
                        </div>
                    </div>
                    <div className={uis.chartWrap}>
                        ...
                        <div className={apps.chartArea + " " + uis.scrollbar}>
                            <section className={sectionStyles.panelAreas + " " + uis.scrollbar}>
                                ...
                                {/* 실행모드일 경우 Cell hover 테두리 제거를 위해서 apps.sectionPanels 클래스 네임 선언 */}
                                    <div className={apps.sectionPanels + " " + uis.scrollbar}>
                                        <SectionPanel
