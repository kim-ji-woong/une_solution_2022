...
import { TeamSubAside } from '../../TeamEditor/styled/teamStyled';
...
class TeamEditor extends Component {
	...
	componentDidMount() {
		...
		$('#' + styles.subPage).click(function (e) {
	...
	removeTeam = () => {
		// 경고 메시지 띄우기
		this.showConfirmDialog(ProjectResource.dialogTypes.QUESTION, ["조직을 삭제하시겠습니까?", "(하위 조직,조직원 및 하위 조직원에 부여된 계정권한도 함께 삭제됩니다.)"], ["취소", "확인"], this.doRemoveTeam);
	}
	...
	async makeRemoveRegularTeam(selectedTeam, data) {
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (!success) {
				...
				//alert(message);
				this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
		}
		else {
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["비상조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
        }
	...
	editTeam = async (team, chgName) => {
		...
		if (chgName === null || chgName === undefined || chgName.replace(/\s/gi, "") === "") {
			this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["변경할 팀 이름을 다시 확인해주세요."], null, null);
			return;
		} 

		const curTeamTreeData = [...this.state.teamTreeData];
		
		const nodeData = { ID: team.ID, TeamName: chgName, ParentTeamID: team.ParentTeamID, SiteID: ProjectResource.campusID};
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {			
			const [success, message] = await TeamEditController.UpdateRegularTeam(nodeData);
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
            }
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["비상조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
	...
	checkEmail = (id, email, target) => {
		...
		if (chk === true) {
			this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [email + " 이메일 주소가 이미 사용 중입니다."], null, null);
			target.value = "";
        }
	...
	checkMemberID = (id, memberID, target) => {
		...
		if (chk === true) {
			// 사번이 중복됨.
			this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [memberID + " 사번이 이미 사용 중입니다."], null, null);
			target.value = "";
		}
	...
	checkPhoneNumber = (id, phoneNumber, target) => {
		...
		if (chk === true) {
			// 휴대전화번호이 중복됨.
			this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, [phoneNumber + " 휴대전화번호가 이미 사용 중입니다."], null, null);
			target.value = "";
		}
	...
	onChangeMemberEditMode = (member, editType, isEditMode) => {
		...
		if (userInfo && userInfo !== null && userInfo.levelID !== AccountResource.accountLevelID.master && userInfo.levelID !== AccountResource.accountLevelID.generalAdmin) {
			isEditMode = false;
		}
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (isUpdate) {
				...
				if (!success && message.length > 0) {
					...
					//alert(message);
					this.showConfirmDialog(ProjectResource.dialogTypes.ERROR, ["조직원 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
					return;
				}
	...
	render() {
		return (
			<div id={styles.subPage} className='UI_Section'>
				<TeamSubAside className="pageMenu">
					{/* 조직, 근무표 메뉴 버튼 컴포넌트 */}
					{						
						//<TeamEditorContent
