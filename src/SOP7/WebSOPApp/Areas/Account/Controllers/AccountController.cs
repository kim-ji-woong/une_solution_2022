...
namespace WebSOPApp.Areas.Account.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult CheckSSOLogin([FromBody] CheckSSOLogin chk)
        {
            if (chk == null)
                return BadRequest();

            string strLoginOption = Startup.ConfigManager.LoginOption;

            ResponseSSOLogin res = m_processManager.GetAccountManager().CheckSSOLogin(chk.Code, string.Concat(Request.Scheme, "://", Request.Host.Value), strLoginOption);
            return Ok(res);
        }

        [HttpPost]
        public IActionResult RequestSSOUrl()
        {
            string strLoginOption = Startup.ConfigManager.LoginOption;

            ResponseSSOUrl res = m_processManager.GetAccountManager().RequestSSOUrl(string.Concat(Request.Scheme, "://", Request.Host.Value), strLoginOption);
            return Ok(res);
        }
    }
}
