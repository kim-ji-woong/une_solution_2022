...
class SpeedDetectionHistory extends Component {
	static PLACE_1 = 19000;
	static PLACE_2 = 19001;
	...
	async onClickDownload(isCheckedDownload, begin, end, name, count, timeData, speed) {
		const title = '차량과속 이력';
		...
		const message = begin + ' ~ ' + end + ' 동안 ' + name + '의 차량과속 횟수는 ' + count + '회 이며 가장 많은 위반 시간대는 ' + timeData + '시, 속도 초과는 ' + speed + ' 입니다.';		
		worksheet.addRow([message]);
		worksheet.addRow([]);

		const chart1 = document.getElementById('chart1');
		let img_chart1 = chart1.toDataURL(1.0);
		let img2_chart1 = workbook.addImage({ base64: img_chart1, extension: 'png' });
		worksheet.addImage(img2_chart1, 'A7:B12');

		const chart2 = document.getElementById('chart2');
		let img_chart2 = chart2.toDataURL(1.0);
		let img2_chart2 = workbook.addImage({ base64: img_chart2, extension: 'png' });
		worksheet.addImage(img2_chart2, 'C7:D12');
		...
		worksheet.columns = [
			{ key: "no", width: 5 },
			{ key: "detectionTime", width: 50 },
			{ key: "sensorName", width: 50 },
			{ key: "speed", width: 15 }
		];
	...
	getBarCharts = (timeGroupLabels, speedGroupLabels, place1, place2) => {

		const chartData = {
			labels: [
				"06~10시", "10~17시", "17~24시", "이후 시간",
				"",
				"06~10시", "10~17시", "17~24시", "이후 시간",
			],
			datasets: [
				{
					label: "속도측정장비",
					data: [
						place1.time_6_10, place1.time_10_17, place1.time_17_24, place1.time_0_6,
						null,
						place2.time_6_10, place2.time_10_17, place2.time_17_24, place2.time_0_6,
					],
					yAxisID: 'A',
					backgroundColor: [
						'#ff3b3b',
			...
			datasets: [
				{
					label: "속도측정장비",
					data: [
						place1.speed_25_30, place1.speed_30_40, place1.speed_40_50, place1.speed_50,
						null,
						place2.speed_25_30, place2.speed_30_40, place2.speed_40_50, place2.speed_50,
					],
					yAxisID: 'A',
					backgroundColor: [
						'#ff3b3b',
		...
		const centerDividerPlugin = {
			afterDraw: function (chart) {
				...
				const dividerBar = meta.data[dividerIndex];
				if (!dividerBar || !dividerBar._model) return;

				const x = dividerBar._model.x;
				ctx.save();
				ctx.strokeStyle = '#ccc';
				ctx.beginPath();
		...
		const createGroupLabelPlugin = (leftLabel, rightLabel) => ({
			afterDraw: function (chart) {
				const ctx = chart.ctx;
				...
				const leftCenter =
					(xScale.getPixelForTick(0) + xScale.getPixelForTick(3)) / 2;
				const rightCenter =
					(xScale.getPixelForTick(5) + xScale.getPixelForTick(8)) / 2;

				const y = chartArea.bottom + 30;
				ctx.fillText(leftLabel, leftCenter, y);
				ctx.fillText(rightLabel, rightCenter, y);
				ctx.restore();
			},
		});
		...
		return (
			<div style={{ width: '100%', height: '150px', display: 'flex', gap: '10px' }}>
				
				<div style={{ flex: 1 }}>
					<p style={{ fontWeight: 600, color: '#004BB9' }}>위반 시간대</p>
					<Bar
						id='chart1'
						data={chartData}
						...
						plugins={[
							...
							createGroupLabelPlugin(
								timeGroupLabels.left,
								timeGroupLabels.right
							),
						]}
					/>
				</div>
				...
				<div style={{ flex: 1 }}>
					<p style={{ fontWeight: 600, color: '#004BB9' }}>속도 초과</p>
					<Bar
						id='chart2'
						data={chartData2}
						options={chartOptions}
						plugins={[
							...
							centerDividerPlugin,
							createGroupLabelPlugin(
								speedGroupLabels.left,
								speedGroupLabels.right
							),
						]}
					/>
				</div>
	...
	getChartData = () => {
		...
		if (dataSource?.length > 0) {
			for (let i = 0; i < dataSource.length; i++) {
				...
				if (data.sensorID === SpeedDetectionHistory.PLACE_1) {
					let time = new Date(data.detectionTime);
				...
				else if (data.sensorID === SpeedDetectionHistory.PLACE_2) {
					let time = new Date(data.detectionTime);
					let speed = data.speed;
	...
	getMessage = (place1, place2) => {
		...
		if (selectedSensor === SpeedDetectionHistory.PLACE_1) {
			name = "보안실A-속도측정장비";
		}
		else if (selectedSensor === SpeedDetectionHistory.PLACE_1) {
			name = "S1동B-속도측정장비";
		}
		else {
			name = "전체";
		...
		if (max < place1.time_6_10 || max < place2.time_6_10) {
			time = "06 ~ 10 ";

			max = place1.time_6_10;
			if (place1.time_6_10 < place2.time_6_10)
				max = place2.time_6_10;
		}
		else if (max < place1.time_10_17 || max < place2.time_10_17) {
			time = "10 ~ 17 ";

			max = place1.time_10_17;
			if (place1.time_10_17 < place2.time_10_17)
				max = place2.time_10_17;
		}
		else if (max < place1.time_17_24 || max < place2.time_17_24) {
			time = "17 ~ 24 ";

			max = place1.time_17_24;
			if (place1.time_17_24 < place2.time_17_24)
				max = place2.time_17_24;
		}
		else if (max < place1.time_0_6 || max < place2.time_0_6) {
			time = "이후 ";
		}

		max = 0;

		if (max < place1.speed_25_30 || max < place2.speed_25_30) {
			speed = "25 ~ 30 ";

			max = place1.speed_25_30;
			if (place1.speed_25_30 < place2.speed_25_30)
				max = place2.speed_25_30;
		}
		else if (max < place1.speed_30_40 || max < place2.speed_30_40) {
			speed = "30 ~ 40 ";

			max = place1.speed_30_40;
			if (place1.speed_30_40 < place2.speed_30_40)
				max = place2.speed_30_40;
		}
		else if (max < place1.speed_40_50 || max < place2.speed_40_50) {
			speed = "40 ~ 50 ";

			max = place1.speed_40_50;
			if (place1.speed_40_50 < place2.speed_40_50)
				max = place2.speed_40_50;
		}
		else if (max < place1.speed_50 || max < place2.speed_50) {
			speed = "50초과 ";
		}

		return [beginDate, endDate, name, count, time, speed];
    }
	...
	render() {
		...
		const [place1, place2] = this.getChartData();
		const [beginDate, endDate, name, count, time, speed] = this.getMessage(place1, place2);
						...
						<div id={'hsCont'}>
...

							{
								this.state.loadingIndicator === true ?
									...
									:
									<ul className={'hscExl'}>
										<li><a onClick={() => this.onClickDownload(false, beginDate, endDate, name, count, time, speed)} className={'all'}>{i18n.t('history.formText.전체 다운로드')}</a></li>
										<li><a onClick={() => this.onClickDownload(true, beginDate, endDate, name, count, time, speed)} className={'exl'}>{i18n.t('history.formText.선택 다운로드')}</a></li>
									</ul>
							}

							<p className={'hscWng'}>
								<span>{beginDate} ~ {endDate}</span> 동안 <span>{name}</span>의 차량과속 횟수는 <span>{count}</span>회 이며 가장 많은 위반 시간대는 <span>{time}</span>시, 속도 초과는 <span>{speed}</span> 입니다.
							</p>
