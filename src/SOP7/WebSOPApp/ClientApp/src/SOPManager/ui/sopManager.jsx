...
class SopManager extends Component {    
        ...
        if (content === SopManagerResource.menu.SOP_편집) {
            if (menuDatas) {
                ...
                if (userInfo?.siteID === ProjectResource.Site.GG_A) {
                    this.setState({ content: content, sopData: menuDatas, menuDatas: menuDatas, selectedSiteID: ProjectResource.Site.GG_A });
                }
                else {
    ...
    getPopup() {
        if (this.state.content === SopManagerResource.menu.열기) {
            return <OpenSOPOptions sopData={this.state.sopData} content={this.changeContent} selectedSiteID={this.state.selectedSiteID} onChangeSite={this.onChangeSite} updateSelectedSiteID={this.updateSelectedSiteID} />;
        }
        else if (this.state.content === SopManagerResource.menu.삭제) {
            return <DeleteSOPOptions sopData={this.state.sopData} content={this.changeContent} showConfirmDialog={this.showConfirmDialog} onCloseConfirmDialog={this.onCloseConfirmDialog} selectedSiteID={this.state.selectedSiteID} onChangeSite={this.onChangeSite} updateSelectedSiteID={this.updateSelectedSiteID} />;
        }
    ...
    onChangeSite = (siteID) => {
        ...
        if (siteID >= ProjectResource.Site.GG_A && siteID <= ProjectResource.Site.GG_H) {
            this.setState({ selectedSiteID: siteID, sopData: null, menuDatas: null });
        }
        else {
            ...
            <div id="subPage">
                {
                    ...
                    <div className={styles.teamEditorName}><span>{i18n.t('sopManager.menu.SOP 편집')}</span>{/* <span className={styles.teamEditorNameIcon}></span> */}</div>
				}
                ...
                <div id={styles.subAside} className={styles.sop}>
                    {/*siteList*/}
                    <SopManagerContent sopData={this.state.sopData} content={this.changeContent} loginUser={this.state.loginUser} onChangeSite={this.onChangeSite} menu={this.state.content}  />
                </div>
                <SopManagerBody
