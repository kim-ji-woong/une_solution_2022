...
import MoveMembers from './popups/moveMembers';
...
class TeamEditor extends Component {
	constructor(props) {
		...
		this.state = {
			...
			showMoveMembersPopup: false,
			selectedMoveTeam: null,
	...
	async initTeamData(selectedSiteID) {
		...
		if (regularDatas.length > 0) {
			...
			let newMembers = this.setMemberDatas(members, regularDatas);
			let displayMembers = this.getViewRegularMember(regularDatas[0], newMembers);
	...
	async displayRegular() {
		...
		let teamDatas = await TeamEditController.DisplayRegular(siteID);
		let members = [];
		let displayMembers = [];
		
		if (teamDatas.length > 0) {
			members = await TeamEditController.DisplayRegularMember(siteID); // 해당 팀원 불러오기
	...
	async displayTemporary() {
		...
		let teamDatas = await TeamEditController.DisplayTemporary(true, siteID);
		let temporaryMembers = [];
		let message = null;
		let displayMembers = [];
	...
	async displayTemporaryEmergency() {
		...
		let teamDatas = await TeamEditController.DisplayTemporary(false, siteID);
		let temporaryMembers = [];
		let message = null;
		let displayMembers = [];
	...
	setMemberDatas = (members, regularDatas) => {
		const newMembers = [];
		const { jobLevels, jobPositions } = this.state;

		if (members && regularDatas && jobLevels && jobPositions && members.length > 0) {
			for (const member of members) {
				const regular = regularDatas.find((item) => item.ID === member.RegularID);

				if (!regular) {
					continue;
                }

				const jobLevel = jobLevels.find((item) => item.value === member.JobLevelID);
				const jopPosition = jobPositions.find((item) => item.value === member.JobPositionID);

				member.regularName = regular?.TeamName;
				member.jobLevelName = jobLevel?.name;
				member.jopPositionName = jopPosition?.name;
	...
	sortMembers = (sortType, sortOrder) => {
		const { memberGridData } = this.state;
	
		let sortedMembers = [...memberGridData];
		...
		switch (sortType) {
			case TeamEditorResource.sortType.memberName:
				...
				break;
			case TeamEditorResource.sortType.jobLevelName:
				sortedMembers.sort((a, b) => 
					sortOrder === 'desc' 
						? b.jobLevelName.localeCompare(a.jobLevelName) 
						: a.jobLevelName.localeCompare(b.jobLevelName)
				);
				break;
			case TeamEditorResource.sortType.jopPositionName:
				sortedMembers.sort((a, b) => 
					sortOrder === 'desc' 
						? b.jopPositionName.localeCompare(a.jopPositionName) 
						: a.jopPositionName.localeCompare(b.jopPositionName)
				);
				break;
			case TeamEditorResource.sortType.phoneNumber:
				sortedMembers.sort((a, b) => 
					sortOrder === 'desc' 
				...
				break;
			case TeamEditorResource.sortType.memberID:
				sortedMembers.sort((a, b) => sortOrder === 'desc' ? a.MemberID - b.MemberID : b.MemberID - a.MemberID);
                break;
			case TeamEditorResource.sortType.officePhoneNumber:
				sortedMembers.sort((a, b) => 
					sortOrder === 'desc' 
				...
				break;
			case TeamEditorResource.sortType.email:
				sortedMembers.sort((a, b) => 
					sortOrder === 'desc' 
		...
		this.setState({ memberGridData: sortedMembers });
	};

	handleMoveMembersPopup = (value) => {
		this.setState({ 
			showMoveMembersPopup: value, 
			selectedMoveTeam: value ? this.state.selectedMoveTeam : null 
		});
	};

	setSelectedMoveTeam = (team) => {
		this.setState({ selectedMoveTeam: team });
	}

	doMoveMembers = async (teamType, teamID, memberIDs) => {
		if (teamType === TeamEditorResource.menu.조직) {
			const [success, message] = await TeamEditController.moveRegularMembers(teamID, memberIDs);

			if (success) {
				this.showConfirmDialog("확인", ["조직이동이 완료되었습니다."], null, null);
				this.handleMoveMembersPopup(false);
				this.displayRegular(this.state.selectedSiteID);
			}
			else {
				this.showConfirmDialog("오류", [message], null, null);
			}
		}
		else if (teamType === TeamEditorResource.menu.평일_비상조직 || teamType === TeamEditorResource.menu.휴일_비상조직) {
			const [success, message] = await TeamEditController.moveTemporaryMembers(teamID, memberIDs);

			if (success) {
				this.showConfirmDialog("확인", ["조직이동이 완료되었습니다."], null, null);
				this.handleMoveMembersPopup(false);

				if (teamType === TeamEditorResource.menu.평일_비상조직) {
					this.displayTemporary();
				}
				else if (teamType === TeamEditorResource.menu.휴일_비상조직) {
					this.displayTemporaryEmergency();
				}
			}
			else {
			...
			<TeamEditorComponent>
				{
					...
					<div className={'teamEditorName'}><span>{i18n.t('menu.조직관리')}</span>{/* <span className={'teamEditorNameIcon'}></span> */}</div>
				}
				<div id="subPage">
					...
					<DisplayContent
						...
						handleMoveMembersPopup={this.handleMoveMembersPopup}
					/>
					{
						/* alert창 대신 사용 */
				...
				{
					this.state.showMoveMembersPopup &&
					<MoveMembers 
						showMoveMembersPopup={this.state.showMoveMembersPopup}
						handleMoveMembersPopup={this.handleMoveMembersPopup}
						memberGridData={this.state.memberGridData}
						teamTreeData={this.state.teamTreeData}
						selectedMoveTeam={this.state.selectedMoveTeam}
						setSelectedMoveTeam={this.setSelectedMoveTeam}
						showConfirmDialog={this.showConfirmDialog}
						teamType={this.state.teamType}
						doMoveMembers={this.doMoveMembers}
					/>
				}
			</TeamEditorComponent>
		);
		...
		if (this.props.teamType === TeamEditorResource.menu.조직) {
			return (
				<RegularMemberPage
					...
					sortMembers={this.props.sortMembers}
					handleMoveMembersPopup={this.props.handleMoveMembersPopup}
				/>
			);
