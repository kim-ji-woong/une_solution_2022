...
export class TeamEditController {
    ...
    // bReturnOrgDatas : true > 원본 데이터도 리턴
    // bReturnOrgDatas : false > tree 형태로 가공
    static async DisplayRegular(selectedSiteID, bReturnOrgDatas) {
        try {
            ...
            const orgDatas = await response.json();            
            let treeDatas = this.convertToTree(orgDatas);
            if (bReturnOrgDatas) {
                return [treeDatas, orgDatas];
            }
            else {
                return treeDatas;
            }
    ...
    static async DisplayBasicRegular(selectedSiteID) {
        try {
            const response = await fetch('TeamEditor/TeamEdit/DisplayRegular', {
                method: 'POST',
                headers: {
	...
	static async DisplayTemporary(isNormal, selectedSiteID) {
        try {
            const response = await fetch('TeamEditor/TeamEdit/DisplayTemporary', {
                ...
                body: JSON.stringify({IsNormal: isNormal, SiteID: selectedSiteID })
            });
            const datas = await response.json();
    ...
    static convertToTree(datas) {
        // 데이터가 부모부터 자식 순으로 순서대로 구성되어 있지 않을 경우가 있어 로직 수정     2021.12.01 K.D.R
    ...
    static getChildNode(parentNode, datas) {
        ...
        for (const data of datas) {
            ...
            if (parentNode.ID === data.parentTeamID) {
                ...
                const nodeData = {
                    ID: data.id, TeamName: data.teamName, ParentTeam: parentNode, ParentTeamID: data.parentTeamID, SiteID: data.siteID
                };
    ...
    static getRootNode(datas) {
        ...
        for (const data of datas) {
            ...
            if (data.parentTeamID === null) {
                const nodeData = {
                    ID: data.id, TeamName: data.teamName, ParentTeam: null, ParentTeamID: data.parentTeamID, SiteID: data.siteID
                };
                rootNode = nodeData;
    ...
    */
    ...
    static async DisplayRegularMember(selectedSiteID) {
    ...
    static async moveRegularMembers(targetTeamID, regularMemberIDs) {
        try {
            const jsonData = JsonManager.makeMoveRegularMembers(targetTeamID, regularMemberIDs);

            const response = await fetch('TeamEditor/TeamEdit/MoveMembers', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json;charset=utf-8'
                },
        ...
        return [false, 'moveRegularMembers 호출 실패'];
    }

    static async moveTemporaryMembers(targetTeamID, temporaryMemberIDs) {
        try {
            const jsonData = JsonManager.makeMoveTemporaryMembers(targetTeamID, temporaryMemberIDs);
        ...
        return [false, 'moveTemporaryMembers 호출 실패'];
    }

    static async moveRegularTeams(targetParentTeamID, regularTeamIDs) {
        try {
            const jsonData = JsonManager.makeMoveRegularTeams(targetParentTeamID, regularTeamIDs);

            const response = await fetch('TeamEditor/TeamEdit/MoveTeams', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json;charset=utf-8'
                },
        ...
        return [false, 'moveRegularTeams 호출 실패'];
    }

    static async moveTemporaryTeams(targetParentTeamID, temporaryTeamIDs) {
        try {
            const jsonData = JsonManager.makeMoveTemporaryTeams(targetParentTeamID, temporaryTeamIDs);
        ...
        return [false, 'moveTemporaryTeams 호출 실패'];
    }
