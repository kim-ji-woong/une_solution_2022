...
class LayoutSetting extends Component {
	...
	async init(paramSiteID) {
		...
		this.setState({ selectedSiteID, settings: transformedSettings, isLoading: false, buildingGroupList, teamTreeDatas, teams, members, disasterCategories, spreadMessages, linkedSOPs, sensorTypes });
	}
		...
		if (this.state.isLoading === true) {
			...
			menuUI.push(
				<Monitoring3D key="Monitoring3D" settings={this.state.settings} buildingGroupList={this.state.buildingGroupList} spreadMessages={this.state.spreadMessages}
					...
					onChangeNeedToSave={this.onChangeNeedToSave} selectedSiteID={this.state.selectedSiteID} />
			);
	...
	onClickSave = () => {
		...
		if (userInfo.siteID === ProjectResource.Site.GG_A) {
			const siteMap = [
				[ProjectResource.Site.GG_B, "41"],
				[ProjectResource.Site.GG_D, "43"],
				[ProjectResource.Site.GG_E, "44"],
				[ProjectResource.Site.GG_F, "45"],
				[ProjectResource.Site.GG_G, "46"],
				[ProjectResource.Site.GG_H, "47"],
			];
		
			for (const [siteID, suffix] of siteMap) {
				if (settings.useReceiveSensorTypesGroups[siteID]) {
					eval(`useReceiveFire_${suffix} = {
						name: "useReceiveFire",
						subName: null,
						value: settings.useReceiveSensorTypesGroups[siteID].UseReceiveFire.toLowerCase() === "true",
						properTypeSDMS,
						siteID
					};`);
		
					eval(`useReceiveEmergencyBell_${suffix} = {
						name: "useReceiveEmergencyBell",
						subName: null,
						value: settings.useReceiveSensorTypesGroups[siteID].UseReceiveEmergencyBell.toLowerCase() === "true",
						properTypeSDMS,
						siteID
					};`);
		
					eval(`useReceiveBlackOut_${suffix} = {
						name: "useReceiveBlackOut",
						subName: null,
						value: settings.useReceiveSensorTypesGroups[siteID].UseReceiveBlackOut.toLowerCase() === "true",
						properTypeSDMS,
						siteID
					};`);
					
					if (siteID === ProjectResource.Site.GG_B || siteID === ProjectResource.Site.GG_D || siteID === ProjectResource.Site.GG_F) {
						eval(`useReceiveSubmerge_${suffix} = {
							name: "useReceiveSubmerge",
							subName: null,
							value: settings.useReceiveSensorTypesGroups[siteID].UseReceiveSubmerge.toLowerCase() === "true",
							properTypeSDMS,
							siteID
						};`);

						eval(`useReceiveEarthquake_${suffix} = {
							name: "useReceiveEarthquake",
							subName: null,
							value: settings.useReceiveSensorTypesGroups[siteID].UseReceiveEarthquake.toLowerCase() === "true",
							properTypeSDMS,
							siteID
						};`);
					}
		...
		if (userInfo.siteID === ProjectResource.Site.GG_A) {
			usePropertyDatas.push(useReceiveFire_41);
			usePropertyDatas.push(useReceiveFire_43);
			usePropertyDatas.push(useReceiveFire_44);
			usePropertyDatas.push(useReceiveFire_45);
			usePropertyDatas.push(useReceiveFire_46);
			usePropertyDatas.push(useReceiveFire_47);
			usePropertyDatas.push(useReceiveEmergencyBell_41);
			usePropertyDatas.push(useReceiveEmergencyBell_43);
			usePropertyDatas.push(useReceiveEmergencyBell_44);
			usePropertyDatas.push(useReceiveEmergencyBell_45);
			usePropertyDatas.push(useReceiveEmergencyBell_46);
			usePropertyDatas.push(useReceiveEmergencyBell_47);
			usePropertyDatas.push(useReceiveBlackOut_41);
			usePropertyDatas.push(useReceiveBlackOut_43);
			usePropertyDatas.push(useReceiveBlackOut_44);
			usePropertyDatas.push(useReceiveBlackOut_45);
			usePropertyDatas.push(useReceiveBlackOut_46);
			usePropertyDatas.push(useReceiveBlackOut_47);
			usePropertyDatas.push(useReceiveSubmerge_41);
			usePropertyDatas.push(useReceiveSubmerge_43);
			usePropertyDatas.push(useReceiveSubmerge_45);
			usePropertyDatas.push(useReceiveEarthquake_41);
			usePropertyDatas.push(useReceiveEarthquake_43);
			usePropertyDatas.push(useReceiveEarthquake_45);
		}
		else {
