...
export class SDMSDataManager {
    ...
    static async get3DOptions(buildingGroupList, outdoorZones, errorMessage, userID, siteIDs) {
        ...
        if (!buildingGroupList && errorMessage && errorMessage.length > 0) {
            ...
            for (let i = 0; i < buildingGroupCount; i++) {
                ...
                if (!buildingGroups) {
                    ...
                    siteOutdoorZones[buildingGroup.siteID] = [];
                }
            ...
            for (let i = 0; i < outdoorZoneCount; i++) {
                ...
                if (!_outdoorZones) {
                    ...
                    siteBuildingGroups[outdoorZone.siteID] = [];
                    siteBuildings[outdoorZone.siteID] = {};
                    siteZones[outdoorZone.siteID] = {};
                }
            ...
            const [models, option, message] = await SDMSController.requestGltfModelList(userID, siteIDs);
            ...
            if (!models && message && message.length > 0) {
                ...
                for (const siteID in siteBuildingGroups) {
                    const _siteID = Number(siteID);

                    const buildingGroups = _siteID === ProjectResource.Site.GG_A ? SDMSDataManager.getAllBuildingGroups(siteBuildingGroups) : siteBuildingGroups[siteID];
                    const _outdoorZones = siteOutdoorZones[siteID];
                    const buildings = _siteID === ProjectResource.Site.GG_A ? SDMSDataManager.getAllBuildings(siteBuildings) : siteBuildings[siteID];
                    ...
                    const _3DOptions = this.make3DOptions(buildingGroups, _outdoorZones, buildings, zones, option, siteModels, Number(siteID));
    ...
    static getAllBuildings(siteBuildings) {
        const buildings = {};

        for (const siteID in siteBuildings) {
            const _buildings = siteBuildings[siteID];

            for (const buildingGroupName in _buildings) {
                buildings[buildingGroupName] = { ..._buildings[buildingGroupName] };
            }
        }
    ...
    static getAllBuildingGroups(siteBuildingGroups) {
        const buildingGroups = [];

        for (const siteID in siteBuildingGroups) {
            const _buildingGroups = siteBuildingGroups[siteID];

            for (const buildingGroup of _buildingGroups) {
    ...
    static async getGltfModelList(userID, siteIDs) {
        const [models, /* option, */ message] = await SDMSController.requestGltfModelList(userID, siteIDs);
        if (!models && message && message.length > 0) {
            alert(message);
            return {};
    ...
    static getSiteModels(models, siteID) {
        ...
        for (let i = 0; i < modelCount; i++) {
            ...
            if (siteID >= ProjectResource.Site.GG_A.toString() && siteID <= ProjectResource.Site.GG_H.toString()) {
            ...
            for (const zoneID in buildingZones) {
                ...
                if (equipmentZoneDatas) {
                    ...
                    for (let j = 0; j < equipZoneCount; j++) {
                        ...
                        equipZoneData.push(equipmentZoneData.type);
    ...
    static make3DOptions(buildingGroups, outdoorZones, buildings, zones, option, models, siteID) {
        const _3DOptions = {};
        const modelCount = models.length;

        for (let i = 0; i < modelCount; i++) {
            const model = models[i];
            const json = SDMSDataManager.addModel(model, siteID);
    ...
    static addModel(model, siteID) {        
        const data = {};
        const childModelCount = model.childModels.length;

        for (let i = 0; i < childModelCount; i++) {
            const childModel = model.childModels[i];
            if (siteID && Number(siteID) >= ProjectResource.Site.GG_A && Number(siteID) <= ProjectResource.Site.GG_H) {
                // 경기도청
                // 통합방재실 > 전체 모델
                // 입주기관 > 지하층, 각 기관 모델
                if (Number(siteID) === ProjectResource.Site.GG_A) {
                    const json = SDMSDataManager.addModel(childModel, siteID);
                    data[childModel.modelName] = json;
                }
                else {
                    if (childModel.siteID === ProjectResource.Site.GG_A || Number(siteID) === childModel.siteID) {
                        const json = SDMSDataManager.addModel(childModel, siteID);
                        data[childModel.modelName] = json;
                    }
                }

                //if (siteID === ProjectResource.Site.GG_A || childModel.siteID === ProjectResource.Site.GG_A || Number(siteID) === childModel.siteID) {
