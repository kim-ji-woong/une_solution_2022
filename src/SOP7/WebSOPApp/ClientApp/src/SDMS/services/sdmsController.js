...
import { initResourceVersion } from './resourceVersion';
...
export class SDMSController {
    ...
    static StartWatchTimer() {
        ...
        let timerId = setTimeout(async function tick() {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.Magog) {
                SDMSController.WatchDoorStatus();
            }
            if (ProjectResource.SiteID === ProjectResource.Site.Wonik) {
                SDMSController.WatchSpeedDetection();
            }
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.Soulbrain ||
                ...
                SDMSController.WatchWorkerInfos(this.watchCount);
            }
        ...
        // 3초에 한번씩 실행 - sensorCount
        let timerSensorCount = setTimeout(async function tick() {
            SDMSController.watchSensorCount();
            timerSensorCount = setTimeout(tick, 3000);
        }, 3000);
    }
    ...
    static async _requestGltfModelList(userID, siteIDs) {
        try {
            // 1. 모델 목록을 받아오기 전에 리소스 버전을 먼저 확보하고, 이후 모든 GLB/텍스처
            //    요청에 캐시 버스팅용 버전 쿼리(?v=...)가 붙도록 URL 모디파이어를 등록한다.
            await initResourceVersion();
    ...
    static async requestWonikRemainerSMS(phoneNumbers, message) {
        try {
