...
export class WorkerManagerWorker extends WorkerManager {
    ...
    addText(tag/*: string*/, text/*: string*/, zoneID/*: number*/, id/*: number*/, x/*: number*/, y/*: number*/, z/*: number*/, fontSize/*: number*/, backgroundColor/*: rgbaColor*/, borderColor/*: rgbaColor*/, textColor/*: rgbaColor*/, borderThickness/*: number*/, textPOIManager, textLayer, workerCount, visitorCount)/*: THREE.Sprite | null*/ {
        if (workerCount !== 0 && !workerCount) {
            if (tag === SDMSMainMenu.BuildingGroupNameText) {
                this.buildingGroupWorkerCount[id] = workerCount;
            }
            else if (tag === SDMSMainMenu.BuildingNameText) {
                this.buildingWorkerCount[id] = workerCount;
            }
        ...
        return WorkerManagerWorker._addText(tag, text, zoneID, id, x, y, z, fontSize, backgroundColor, borderColor, textColor, borderThickness, textPOIManager, textLayer, workerCount, visitorCount, this.activeImage, this.passiveImage, this.buildingGroupWorkerCount, this.buildingWorkerCount);
    }

    static _addText(tag, text, zoneID, id, x, y, z, fontSize, backgroundColor, borderColor, textColor, borderThickness, textPOIManager, textLayer, workerCount, visitorCount, activeImage, passiveImage, buildingGroupWorkerCount, buildingWorkerCount) {
        ...
        if (tag === SDMSMainMenu.BuildingGroupNameText) {
            ...
            if (buildingGroupWorkerCount) {
                buildingGroupWorkerCount[id] = workerCount;
            }
            ...
            if (buildingWorkerCount) {
                buildingWorkerCount[id] = workerCount;
            }
        ...
        //    context.drawImage(activeImage, rectX + 5 + (w - originWidth) / 2 + originWidth * xRatio/* + margin*/, (rectY + fontSize) * yRatio - iconDown, imageSize * 1.0, imageSize * 1.0);
        //}
        //else if (workerCount === 0 && passiveImage) {
        //    context.drawImage(passiveImage, rectX + 5 + (w - originWidth) / 2 + originWidth * xRatio/* + margin*/, (rectY + fontSize) * yRatio - iconDown, imageSize * 1.0, imageSize * 1.0);
        //}
    ...
    checkWorkerCount(buildingGroupID, workerCount) {
        const _prevCount = this.buildingGroupWorkerCount[buildingGroupID];
        const prevCount = _prevCount ? _prevCount : 0;
        return prevCount === workerCount;
    }

    checkBuildingWorkerCount(buildingID, workerCount) {
        const _prevCount = this.buildingWorkerCount[buildingID];
        const prevCount = _prevCount ? _prevCount : 0;
