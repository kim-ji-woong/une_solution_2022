...
export class TextPOIManager {
    ...
    selectedPOI = null;
    // NameTag 선택 직전의 스케일. 선택 해제 시 이 값으로 원래 크기를 복원한다.
    selectedOriginScale = null;
    ...
    static getFontScale(fontSize, text) {
        // 폰트 길이에 따라 스케일 변화가 있어 기존 사이트 예외처리
        if (!text || text.length >= 11 ||
            ...
            ProjectResource.siteID === ProjectResource.Site.SUJAIN ||
    ...
    _addEquipZoneText(equipZoneName, equipZone, zoneID, equipZoneID, siteID, equipZoneSprites) {
        ...
        if (sprite) {
            // 긴 구역명으로 캔버스가 넓어진 경우 가로 왜곡을 막기 위해 종횡비를 반영한다.
            sprite.scale.set(6.25 * (sprite.userData.canvasAspect || 2), 6.25, 1.0);            
            if (ProjectResource.siteID === ProjectResource.Site.Wonik ||
                ProjectResource.siteID === ProjectResource.Site.Magog) {
                // 원익/마곡 구역명 스케일 업
                sprite.scale.set(12.5 * (sprite.userData.canvasAspect || 2), 12.5, 1.0);
            }
            if (zoneID >= 20000)
                sprite.scale.set(20 * (sprite.userData.canvasAspect || 2), 20, 1.0);
        ...
        for (const equipZoneID in equipZoneSprites) {
            ...
            if (sprite.userData.rank !== rank) {
                ...
                if (rankSprite && equipZoneSprites) {
                    ...
                    // 긴 구역명으로 캔버스가 넓어진 경우 가로 왜곡을 막기 위해 종횡비를 반영한다.
                    rankSprite.scale.set(6.25 * (rankSprite.userData.canvasAspect || 2), 6.25, 1.0);
                    if (zoneID >= 20000)
                        rankSprite.scale.set(20 * (rankSprite.userData.canvasAspect || 2), 20, 1.0);

                    rankSprite.visible = true;

                    // 등급 배지를 별도 스프라이트로 라벨에 부착한다.
                    // 자식으로 붙이므로 부모(라벨)의 visible/제거가 자동 전파되어
                    // 기존 표시·정리 로직을 추가로 수정할 필요가 없다.
                    if (rank)
                        this.attachRankBadge(rankSprite, rank);
    ...
    // 안전구역 등급 배지를 라벨 스프라이트의 자식으로 부착한다.
    // - 라벨 텍스트 렌더링(addText)은 건드리지 않아 다른 라벨 로직에 영향 없음
    // - 자식 스프라이트라 부모의 visible/제거가 그대로 전파됨(별도 정리 불필요)
    attachRankBadge(parentSprite, rank) {
        if (!parentSprite || !rank)
            return;

        // 등급 이미지 키 (원익 사이트의 B/C는 전용 이미지 사용) - 기존 addText 로직과 동일
        let imgKey = rank;
        if (ProjectResource.SiteID === ProjectResource.Site.Wonik && (rank === "B" || rank === "C")) {
            imgKey = rank + "_wonik";
        }

        const badgeImg = this.assessImgURL[imgKey];
        if (!badgeImg)
            return;

        const texture = new THREE.Texture(badgeImg);
        texture.needsUpdate = true;
        // 배지 이미지가 아직 로드되지 않았다면 로드 완료 후 텍스처를 갱신한다.
        if (!badgeImg.complete || badgeImg.naturalWidth === 0) {
            badgeImg.addEventListener('load', () => { texture.needsUpdate = true; }, { once: true });
        }
        ...
        badge.name = (parentSprite.name || 'equipZone') + '_rankBadge';
        badge.userData.isRankBadge = true;

        // 부모 라벨 scale(실내 12.5x6.25 / 외곽 40x20, 모두 2:1)에 대한 상대 크기.
        // 원래 의도 크기(라벨 캔버스 300x150 위의 44x75)를 그대로 재현 → 로컬 스케일 = (44/300, 75/150).
        // 부모 scale이 무엇이든 2:1이므로 배지 종횡비(44:75)가 일정하게 유지된다.
        badge.scale.set(44 / 300, 75 / 150, 1.0);
        // 라벨 텍스트 위쪽에 배치(로컬 y = 부모 위 방향). 잘리지 않도록 캔버스 경계 개념 없이 자유 배치.
        badge.position.set(0, 0.43, 0);

        parentSprite.add(badge);
        parentSprite.userData.badgeSprite = badge;
    }
    ...
    addText(tag/*: string*/, text/*: string*/, zoneID/*: number*/, id/*: number*/, x/*: number*/, y/*: number*/, z/*: number*/, fontSize/*: number*/, backgroundColor/*: rgbaColor*/, borderColor/*: rgbaColor*/, textColor/*: rgbaColor*/, borderThickness/*: number*/, siteID, type = null, rank = null)/*: THREE.Sprite | null*/ {
        ...
        // EquipZone: 텍스트 사각형이 완전히 담기도록 캔버스 폭 확장(rectX >= 0 보장).
        // canvas.width 재할당은 2D 컨텍스트를 초기화하므로 상태를 다시 적용한다.
        if (tag === SDMSMainMenu.EquipZoneNameText) {
            canvas.width = Math.max(300, Math.ceil(w + borderThickness + padding * 2));
            context.font = fontSize + "px " + fontFace;
            context.fillStyle = "rgba(" + backgroundColor.r + "," + backgroundColor.g + "," + backgroundColor.b + "," + backgroundColor.a + ")";
            context.strokeStyle = "rgba(" + borderColor.r + "," + borderColor.g + "," + borderColor.b + "," + borderColor.a + ")";
            context.lineWidth = borderThickness;
        ...
        // 등급이 존재한다면 표시        
        // 등급 배지는 라벨 캔버스에 함께 그리지 않는다.
        // (캔버스 상단 밖 음수 좌표로 그려져 윗부분이 잘리던 문제)
        // → 등급이 적용되는 showAssessment에서 attachRankBadge()로 별도 스프라이트를 부착한다.
        ...
        if (tag === SDMSMainMenu.EquipZoneNameText) {
            // 캔버스 전체가 UV[0,1]에 매핑됨. 캔버스 y는 top-down, UV v는 bottom-up.
            // 사각형 픽셀 범위: x∈[rectX, rectX+w], y∈[rectY, rectY+h].
            sprite.userData.uvArea = {
                top: 1 - rectY / canvas.height,
                bottom: 1 - (rectY + h) / canvas.height,
                left: rectX / canvas.width,
                right: (rectX + w) / canvas.width
            };
        }
        else {
            // 건물/건물그룹 라벨(poiManager.js) 판정용 기존 공식 유지.
            sprite.userData.uvArea = {
        ...
        // 호출부(scale override)에서 가로 왜곡 보정을 위해 캔버스 종횡비를 저장한다.
        sprite.userData.canvasAspect = canvas.width / canvas.height;
    ...
    selectEquipZonePOI(poi, editMode, editModeParam, currentSiteID) {
        ...
        if (!_poi) {
            if (!this.selectedPOI) {
                ...
                // 편집모드 종료 등으로 선택이 해제된 경우.
                // currentSiteID를 넘기지 않으면 복원 스프라이트가 엉뚱한 textLayer로 가므로 반드시 전달한다.
        ...
        if (poi) {
            ...
            if (sensorType && sensorType === SDMSMainMenu.EquipZoneNameText &&
                ...
                if (sprite && equipZoneSprites) {
                    ...
                    if (oldData) {
                        // 선택 시점의 "선택 전 크기"를 기억해 둔다.
                        // 스케일 공식이 생성 경로마다 달라(showAssessment는 원익/마곡 스케일업 분기가 없음)
                        // 공식을 다시 계산하면 선택 전 크기로 되돌아가지 않기 때문에 실제 값을 보관한다.
                        if (isSelected) {
                            this.selectedOriginScale = {
                                x: oldData.scale.x,
                                y: oldData.scale.y,
                                z: oldData.scale.z
                            };
                    ...
                    if (isSelected === false && this.selectedOriginScale) {
                        // 선택 해제(다른 NameTag 선택 / 편집모드 종료) 시 선택 전 크기로 복원한다.
                        const originScale = this.selectedOriginScale;
                        sprite.scale.set(originScale.x, originScale.y, originScale.z);
                        this.selectedOriginScale = null;
                    }
                    else {
                        // 긴 구역명으로 캔버스가 넓어진 경우 가로 왜곡을 막기 위해 종횡비를 반영한다.
                        sprite.scale.set(6.25 * (sprite.userData.canvasAspect || 2), 6.25, 1.0);
