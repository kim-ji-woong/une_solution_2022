...
export class POIManager extends IOptionType {
        ...
        if (poi) {
            ...
            if (sensorType) {
                ...
                if (!sensor) {
                    ...
                    else if (sensorType === SDMSMainMenu.Door_Sensor ||
                        sensorType === SDMSMainMenu.Exit) {
                        sensor = this.getZoneIcon(zoneID, sensorType, sensorID);
                    }
                }
        ...
        if (result && result.success) {
            ...
            for (let i = 0; i < sensorCount; i++) {
                ...
                if (id === sensorID) {
                    ...
                    if (isSelected && ProjectResource.SiteID !== ProjectResource.Site.Hydrogen) {                        
    ...
    addOutdoorSensors(outdoorZones, zones, visibleSensorTypes) {
        ...
        //this.addTempPOI(SDMSMainMenu.CCTV_Type);
        //this.addTempCCTVPOI();
    }
    ...
    addZoneSensors(zoneID, scale, outdoorZones, zones, visibleSensorTypes) {
        ...
        if (zone) {
            for (const sensorType in visibleSensorTypes) {
                ...
                let isZoneIcons = false;

                if (sensorType === SDMSMainMenu.Door_Sensor ||
                    sensorType === SDMSMainMenu.Exit) {
                    const zoneIcons = this._getZoneIcons(zoneID, sensorType);

                    if (zoneIcons && zoneIcons.length > 0) {
                        sensors = zoneIcons;
                        isZoneIcons = true;
                    }
                }
                ...
                if (sensors) {
                    ...
                    if (isZoneIcons) {
                        const closedDoorDatas = this.contents3D?.props?.closedDoorDatas;
                        const doorDatas = closedDoorDatas ? closedDoorDatas[zoneID] : null;
                        ...
                        for (const poi of pois) {
                            ...
                            if (sensorType === SDMSMainMenu.Door_Sensor) {
                                // 출입문 Icon은 화재시에만 보이도록 한다.
                                poi.visible = false;

                                if (doorDatas?.closedDoors) {
                                    const [_sensorType, _zoneID, sensorID] = SDMS.getSensorInfo(poi);
                                    const isClosed = this.isClosedDoors(sensorID, doorDatas.closedDoors);
    ...
    getSensorImageURL(sensorType, sensor) {
        ...
        else if (ProjectResource.SiteID >= ProjectResource.Site.GG_A && ProjectResource.SiteID <= ProjectResource.Site.GG_H) {
