...
import RemainerSMSPopup from './RemainerSMSPopup';
...
class WorkerStatusPopup extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            showSMSDialog: false,      // 메시지 작성 팝업 표시 여부
        }
    ...
    onSendRemainerSMS = (message) => {
        // 1. 빈 메시지 차단
        if (!message || message.trim() === '') {
            this.props.showConfirmDialog(i18n.t('sdms.worker.잔류자 상황 전파'), [i18n.t('sdms.worker.메시지를 입력해주세요')], [i18n.t('common.확인')], null);
            return;
        }
        ...
        // 4. 전송 전 최종 확인 — 작성 팝업은 그대로 열린 상태에서 확인 다이얼로그가 위에 겹침
        this.props.showConfirmDialog(
            i18n.t('sdms.worker.잔류자 상황 전파'),
            [i18n.t('sdms.worker.상황 전파를 하시겠습니까?')],
            ...
            async (index) => {
                ...
                const [success, errMsg] = await SDMSController.requestWonikRemainerSMS(phoneNumbers, message);
                this.setState({ showSMSDialog: false });
                if (success === true) {
                    this.props.showConfirmDialog(i18n.t('sdms.worker.잔류자 상황 전파'), [i18n.t('sdms.worker.상황 전파 하였습니다')], [i18n.t('common.확인')], null);
                } else {
                    const txt = i18n.t('sdms.worker.상황 전파 실패하였습니다') + " " + errMsg;
        ...
        return (
            <>
                {this.state.showSMSDialog && (
                    <RemainerSMSPopup
                        defaultMessage={i18n.t('sdms.worker.기본 전파 메시지 템플릿')}
                        onSend={this.onSendRemainerSMS}
                        onCancel={() => this.setState({ showSMSDialog: false })}
                    />
                )}
