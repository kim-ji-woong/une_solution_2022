...
class Event extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedSite: props.selectSiteID,   // 표시할 입주기관 ID (경기)
    ...
    keysPressed(e, target) {
        if (this.state.popupMemoHistoryID !== null && this.state.popupMemoHistoryID > 0) {
            return ;
        }
    ...
    onArrow(isUp) {
        const { currentAlarms } = this.state;
        const { selectedAlarm } = this.props;
    
        // 선택된 알람이 없거나 알람이 1개 미만이면 return
        if (!selectedAlarm || currentAlarms.length < 2) {
            return;
        }
    
        // selectedAlarm 인덱스 찾기
        const currentIndex = currentAlarms.findIndex(alarm => alarm.sensorZoneHistoryID === selectedAlarm.sensorZoneHistoryID);

        // 선택된 siteID의 알람 목록에 selectedAlarm이 포함되지 않은 경우 currentAlarms의 첫번째 알람을 선택
        if (currentIndex === -1) {
            this.onSelectedAlarm(currentAlarms[0]);
            return;
        }

        let sensorZoneHistoryID = null;
    
        // 위 방향키
        if (isUp) {
            // 이전 항목이 있다면 selectAlarm에 할당
            if (currentIndex > 0) {
                const prevAlarm = currentAlarms[currentIndex - 1];
                sensorZoneHistoryID = prevAlarm.sensorZoneHistoryID;
                this.onSelectedAlarm(prevAlarm);
            }
        }
        // 아래 방향키
        else {
            // 다음 항목이 있다면 selectAlarm에 할당
            if (currentIndex < currentAlarms.length - 1) {
                const nextAlarm = currentAlarms[currentIndex + 1];
                sensorZoneHistoryID = nextAlarm.sensorZoneHistoryID;
                this.onSelectedAlarm(nextAlarm);
            }
        }

        // 선택된 알람으로 스크롤 이동
        if (sensorZoneHistoryID) {
            const element = document.getElementById('alarm_' + sensorZoneHistoryID);
            if (element) {
    ...
    componentDidUpdate(prevProps, prevState) {
        if ((this.props.selectSiteID !== prevProps.selectSiteID || this.props.sensorAlarms !== prevProps.sensorAlarms) && this.props.sensorAlarms && this.props.sensorAlarms.length > 0) {
            this.setCurrentAlarms();
        }
    ...
    setCurrentAlarms = () => {
        ...
        for (const alarm of this.props.sensorAlarms) {
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
                const siteID = alarm.siteID;
                const facilityType = alarm.facilityType;

                // 지진 알람은 모든 입주기관에서 표출되어야 함
                if (!(facilityType === SDMSResource.facilityType.Earthquake && SDMSResource.isEarthquakeSensorType(facilityType)) &&
                    (this.props.selectSiteID !== ProjectResource.Site.GG_A && this.props.selectSiteID !== siteID)){
                    continue;
                }
            }
    ...
    setGridUI() {
        ...
        const selectedSite = this.props.selectSiteID;   // 경기

        if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
            // 경기도청 알람 리스트는 화재센서가 최상단에 위치해야 함.
            alarms.sort((a, b) => {
                // isAlarm true가 최상위로 오도록 정렬
                if (a.isAlarm && !b.isAlarm) return -1;
                if (!a.isAlarm && b.isAlarm) return 1;
            
                // 둘 다 isAlarm === true인 경우
                if (a.isAlarm && b.isAlarm) {
                    // 화재 알람이 상위로 오도록 정렬
                    if (a.facilityType === 0 && b.facilityType !== 0) return -1;
                    if (a.facilityType !== 0 && b.facilityType === 0) return 1;
            
                    // 둘 다 화재 알람인 경우 dtTime 최신순 정렬
                    if (a.facilityType === 0 && b.facilityType === 0) {
        ...
        for (let i = 0; i < alarms.length; i++) {
            ...
            // (경기) 선택된 입주기관 확인
            if (selectedSite !== null && ProjectResource.SiteID === ProjectResource.Site.GG_A) {
                const siteID = alarm.siteID;
                const facilityType = alarm.facilityType;

                // 지진 알람은 모든 입주기관에서 표출되어야 함
                if (!(facilityType === SDMSResource.facilityType.Earthquake && SDMSResource.isEarthquakeSensorType(facilityType)) &&
                    (selectedSite !== ProjectResource.Site.GG_A && selectedSite !== siteID)){
                    continue;
                }
            }
            ...
            const depth = alarm.isAlarm ? alarm.alarmDepth : alarm.maxAlarmDepth;

            // 긴급 센서존(EmergencySensorZoneIDs)에 해당하는 알람은 단계 대신 '긴급'으로 표시
            const isEmergency = emergencyZoneIDs.has(alarm.sensorZoneID);

            const alarmDepth = isEmergency ? '긴급' :
                depth === 1 ? SopManagerResource.actionStep._1st :
                depth === 2 ? SopManagerResource.actionStep._2nd :
                depth === 3 ? SopManagerResource.actionStep._3rd :
                depth === 4 ? SopManagerResource.actionStep._4th :
                '';
            ...
            // 긴급 센서존 알람은 종료 여부와 무관하게 빨강 배경으로 강조
            if (isEmergency) {
                rowClassName = rowClassName ? rowClassName + ' emergencyAlarm' : 'emergencyAlarm';
            }

            // 발생 중인 긴급 알람은 빨강 배경 위 빨강 '발생' 문구가 묻히므로 클래스를 비워 기본 흰색으로 표시
            if (isEmergency && alarm.isAlarm) {
                statusClassName = '';
            }
            ...
            if (alarm.facilityType === SDMSResource.facilityType.FIRE) {
                ...
                typeString = i18n.t('facilityType.정전');
            } else if (alarm.facilityType === SDMSResource.facilityType.STRONG_WIND) {
                typeString = i18n.t('facilityType.강풍');
            }
            ...
            if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
                ...
                positionName = !alarm.zoneName || alarm.zoneName.length === 0
            ...
            grid.push(
                <React.Fragment key={"eventGrid_" + i}>
                    <tr className={rowClassName} id={`alarm_${alarm.sensorZoneHistoryID}`} onClick={() => this.onSelectedAlarm(alarm)} onDoubleClick={() => this.onMoveSelectedAlarm()}>
        ...
        this.props.setCurrentAlarms(currentAlarms);
        return grid;
    }
    ...
    onClickAllClear = async () => {
        ...
        this.props.showConfirmDialog(i18n.t('sdms.formText.알람 종료'), [i18n.t('sdms.formText.탐지된 신호를 종료할까요?')],
    ...
    onAllClear = async (index) => {
        ...
        if (index === 0) {
            ...
            //    }
            //}
            await SDMSController.requestAllClearReport(0); // 화재 전체복구
        }
    ...
    getAlarmInfo() {
        ...
        if (
            ...
            this.state.selectedSite !== ProjectResource.Site.GG_A &&
            selectedAlarm?.siteID !== this.state.selectedSite)
        ) {
            return <></>;
        }
        ...
        if (ProjectResource.SiteID === ProjectResource.Site.GG_A) {
            ...
            if (selectedAlarm.facilityType >= 0) {
                ...
                } else if (SDMSResource.isEarthquakeSensorType(selectedAlarm.facilityType)) {
                    evtClassName = 'evtEarthquake';
                } else if (SDMSResource.isWaterLevelSensorType(selectedAlarm.facilityType)) {
                    evtClassName = 'evtWater';
                } else if (SDMSResource.isTerrorSensorType(selectedAlarm.facilityType)) {
                    evtClassName = 'evtTerror';
                } else if (SDMSResource.isBlackOutSensorType(selectedAlarm.facilityType)) {
                    evtClassName = 'evtElectric';
                } else if (SDMSResource.isStrongWindSensorType(selectedAlarm.facilityType)) {
                    evtClassName = 'evtStrongWind';
                }
            }
        }
            ...
            <ul className={'noMemoIconBox'}>
                {
                    ProjectResource.SiteID !== ProjectResource.Site.CheongSim &&
                        <li onClick={this.onSituationNotice}><span className={'eventShortCut' + " " + 'hideKey'}>Sh+Q</span><span className={'eIcon1'}></span><a>S O P</a></li>
                }

                {
                    ProjectResource.SiteID !== ProjectResource.Site.CheongSim &&
                    <li onClick={() => this.onMoveSelectedAlarm()}><span className={'eventShortCut' + " " + 'hideKey'}>Sh+W</span><span className={'eIcon2'}></span><a>{i18n.t('sdms.event.화면전환')}</a></li>
                }

                {
                    (this.props.alarmSound)
                        ? <li onClick={this.onSound}><span className={'eventShortCut' + " " + 'hideKey'}>Sh+E</span><span className={'eIcon3'}></span><a>{i18n.t('sdms.event.소리끄기')}</a></li>
                        : <li onClick={this.onSound}><span className={'eventShortCut' + " " + 'hideKey'}>Sh+E</span><span className={'eIcon4'}></span><a>{i18n.t('sdms.event.소리켜기')}</a></li>
                }
                {
                    (useAlarmMemo)
                ...
                <li onClick={this.onMalfunction}><span className={'eventShortCut' + " " + 'hideKey'}>Sh+R</span><span className={'eIcon6'}></span><a>{i18n.t('sdms.event.종료')}</a></li>
                {
                    ProjectResource.SiteID === ProjectResource.Site.Magog &&
                    <li onClick={this.onClickAllClear}><span className={'eventShortCut' + " " + 'hideKey'}></span><span className={'eIcon7'}></span><a>{i18n.t('sdms.event.전체종료')}</a></li>
                }
            </ul>; 
        ...
        if(userInfo?.siteID === ProjectResource.Site.GG_A) {
            for (let i = 0; i < sites?.length; i++) {
                ...
                if (siteData.id === ProjectResource.Site.GG_A) {
                    selectSiteUI_GG.push(<option key={"selectSiteUI_" + siteData.id} value={siteData.id}>전체기관</option>);
                }
                else {
                        ...
                        <div className={'dslContEvent'}>
                            <div ref={this.refAlarmList} className={'alarmList'}>
                                <div className='alarmListTop'>
                                    ...
                                    {
                                        (userInfo?.siteID === ProjectResource.Site.GG_A && (!this.state.selectSiteID || this.state.selectSiteID === ProjectResource.Site.GG_A)) &&
                                            <select className={'eventSel long'} onChange={this.onChangeSite}>
                                                {selectSiteUI_GG}
                                            </select>
                                    }
