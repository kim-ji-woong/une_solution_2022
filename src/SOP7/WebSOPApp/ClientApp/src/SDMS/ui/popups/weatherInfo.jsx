...
class WeatherInfo extends Component {
    ...
    getSiteOption(data, index, isSelected) {
        ...
        return <button key={"weatherSite_" + index} className={isSelected ? 'weatherSiteButton' + " " + 'active' : 'weatherSiteButton'} value={index} onClick={this.onSelectSite}>
            {siteName}
        </button>
    }
    ...
    getSpecialReportURL(weatherDatas, dataCount, selectedIndex) {
        ...
        if (ProjectResource.SiteID === ProjectResource.Site.Wonik) {
            if (selectedIndex === 1)
                url = "https://www.weather.go.kr/w/weather/warning/list.do?prevStn=133&prevKind=&prevCmtCd=&stn=109&kind=&date=";
            else
                url = "https://www.weather.go.kr/w/weather/warning/list.do?prevStn=109&prevKind=&prevCmtCd=&stn=143&kind=&date=";
        }
            ...
            if (selectedIndex < dataCount) {
                return (
                    <>
                        ...
                        <span className={'weatherButton' + " " + isActive} onClick={() => WeatherInfo.onClickSpecialReport(specialReportURL)}>기상특보</span>
                    </>
                );
            }
        }
    ...
    static onClickSpecialReport = (url) => {
        ...
        if (ProjectResource.SiteID === ProjectResource.Site.Wonik) {
            specialReportURL = url;
        }

        var child = window.open(specialReportURL, i18n.t('sdms.weatherInfo.기상특보'), "width=800, height=700, location=no, toolbar=no, menubar=no, scrollbars=no, status=no");
        child.scrollTo(0, 40);
    }
    ...
    getCurrentData() {
        ...
        if (selectedIndex >= dataCount || !weatherDatas) {
    ...
    render() {
        ...
        return (
            <WeatherInfoComponent id={this.props.popupType} className={'viewDashboardBoxD viewDashboardWeather'} $show={this.state.weatherShow}>
                <PopupDraggable
                ...
                >
                    ...
                    <div className={'dslTop dslGrd'}>
                        <h5 className={'dslTitle'} >
                            {i18n.t('sdms.weatherInfo.기상정보')}
                            <span >{updateTime}</span>
                        </h5>
                        <a className={'dslX'} onClick={() => this.props.setVisiblePopups(SDMS.menu.weatherInfo, false)}></a>
                    </div>
                    <div className={'dslCont'}>
                        <div>
                            ...
                            <dl className={'dslInfo'}>
                                <dt>
                                    <img className={'weatherImage'} src={imgIcon} />
                                    <div>
                                        <h5><span className='weatherArea'></span></h5>
                                        <h5><span>{temp}</span><span>℃</span><em>℃</em></h5>
                                    </div>
					            </dt>
                                <dd>
                                    <p>{i18n.t('sdms.weatherInfo.바람')}{windSpeed} m/s</p>
                                    <p>{i18n.t('sdms.weatherInfo.강수량')}{rain} mm</p>
                                    <p>{i18n.t('sdms.weatherInfo.습도')}{humidity} %</p>
                                </dd>
				            </dl>
			            </div>
                    </div>
...
export default withTranslation()(WeatherInfo);