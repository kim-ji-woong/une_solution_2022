                ...
                <ToolbarComponent id={'dsNav'}>
                    {
                        ...
                        ? <>
                                <span style={{ display: 'none' }} className={'balloonHome'}>{i18n.t('sdms.toolbar.초기화면')}</span>
                                <span style={{ display: 'none' }} className={'balloonDefault'}>{i18n.t('sdms.toolbar.기본뷰로 설정')}</span>
                                <span style={{ display: 'none' }} className={'balloonIn'}>{i18n.t('sdms.toolbar.확대')}</span>
                                <span style={{ display: 'none' }} className={'balloonOut'}>{i18n.t('sdms.toolbar.축소')}</span>
                                <span style={{ display: 'none' }} className={'balloonRotate'}>{i18n.t('sdms.toolbar.즉시회전')}</span>
                                <span style={{ display: 'none' }} className={'balloonAutoRotate'}>{autoRotationText}</span>
                            <button onClick={this.onClickNavigator}>{i18n.t('sdms.toolbar.지도옵션 열기')}</button>
                                <div className={'dsnMenuDiv'}>
                                    <ul className={'dsnMenuHydrogen'}>
                                        <li className={'liHome'}><a onClick={() => this.props.initViewport()}></a></li>
                                        <li className={'liDefault'}><a onClick={() => this.props.setInitialViewport()}></a></li>
                                        <li className={'liIn'}><a onClick={() => this.props.zoom(true)}></a></li>
                                        <li className={'liOut'}><a onClick={() => this.props.zoom(false)}></a></li>
                                        <li className={'liRotate'}><a onClick={() => this.props.startAutoRotation()}></a></li>
                                        <li className={'liAutoRotate'}><a className={autoRotationClassName} onClick={() => this.props.setUseIdleTime(!this.props.useIdleTime)}></a></li>
                                    </ul>
                                    {
