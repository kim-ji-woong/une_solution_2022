...
class SensorSimulator extends Component {
    ...
    setSensorList(sensors, _3dOptions, sensorList, facilityType) {
        for (const sensor of sensors) {
            ...
            */
            let zoneData = null;
            let outdoorZoneData = null;
            let _3dOption = null;

            for (const siteID in _3dOptions) {
                const opt = _3dOptions[siteID];

                // 실내 zone 우선 조회
                const data = opt.zones ? opt.zones[sensor.zoneID] : null;
                if (data?.length > 3) {
                    _3dOption = opt;
                    zoneData = data;
                    break;
                ...
                // 실내에 없으면 외곽(외부) zone 조회
                const outdoor = opt.outdoorZones ? opt.outdoorZones[sensor.zoneID] : null;
                if (outdoor) {
                    _3dOption = opt;
                    outdoorZoneData = outdoor;
                    break;
                }
            }

            // 실내/외곽 어디에도 없으면 스킵
            if (!zoneData && !outdoorZoneData)
                continue;
            
            // 배치할 그룹/건물/zone 이름 결정
            ...
            if (zoneData) {
                ...
                buildingGroupName = buildingData[1];
                buildingName = buildingData[2];
                zoneName = zoneData[3];
            }
            else {
                // 외곽(외부): "외곽" 그룹 아래에 외곽 zone 이름으로 배치
                buildingGroupName = "외곽";
                buildingName = "외곽";
                zoneName = outdoorZoneData.name || ("외곽 " + sensor.zoneID);
            }
