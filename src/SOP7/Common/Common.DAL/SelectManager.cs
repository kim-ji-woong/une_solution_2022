...
namespace Common.DAL
{
    ...
    public class SelectManager : QueryManager, ISelect
    {
        ...
        public List<Options> SelectOption(Options.OptionTarget eTargetName, string strPropertyName, int nSiteID, out string strErrorMessage)
        {
            bool isNullable;
            string tableName = string.Format("Option{0}", eTargetName.ToString());
            string strCondition = strPropertyName != null && strPropertyName.Length > 0 ? string.Format("{0} = '{1}'", Options.GetFieldName(Options.Fields.PropertyName, out isNullable), strPropertyName) : null;
        ...
        public BroadcastHistory SelectBroadcastHistory(int actionStepHistoryID, int componentID, out string strErrorMessage)
        {
            strErrorMessage = null;
            int nFieldCount;

            string strSQL = string.Format("select {0} from {1} where ActionStepHistoryID = {2} and ComponentID = {3} ",
                GetFieldNames<BroadcastHistory.Fields>(out nFieldCount), BroadcastHistory.TableName
                , actionStepHistoryID
                , componentID);
            ...
            {
                BroadcastHistory model = ReadBroadcastHistory(arrResult, 0, out strErrorMessage);
        ...
        public List<BroadcastHistory> SelectBroadcastHistories(Dictionary<BroadcastHistory.Fields, object> dicConditions, string strAdditionalConditions, out string strErrorMessage)
        {
            return SelectBroadcastHistories(dicConditions, strAdditionalConditions, null, out strErrorMessage);
        }

        public List<BroadcastHistory> SelectBroadcastHistories(Dictionary<BroadcastHistory.Fields, object> dicConditions, string strAdditionalConditions, int? topNCount, out string strErrorMessage)
        {
            strErrorMessage = null;
            int nFieldCount;

            string strSQL = string.Format("select {0} from {1}", GetFieldNames<BroadcastHistory.Fields>(out nFieldCount), BroadcastHistory.TableName);
            ...
            int nResultCount = arrResult.Count;
            List<BroadcastHistory> datas = new List<BroadcastHistory>();

            for (int i = 0; i < nResultCount - (nFieldCount - 1); i += nFieldCount)
            {
                BroadcastHistory model = ReadBroadcastHistory(arrResult, i, out strErrorMessage);
        ...
        private BroadcastHistory ReadBroadcastHistory(ArrayList arrResult, int index, out string strErrorMessage)
        {
            strErrorMessage = null;
            BroadcastHistory model = new BroadcastHistory();
            bool isNullable;

            foreach (BroadcastHistory.Fields field in BroadcastHistory.Fields.GetValues(typeof(BroadcastHistory.Fields)))
            {
                string strFieldName = BroadcastHistory.GetFieldName(field, out isNullable);

                if (field == BroadcastHistory.Fields.ActionStepHistoryID)
                {
                ...
                else if (field == BroadcastHistory.Fields.ComponentID)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
                ...
                else if (field == BroadcastHistory.Fields.EventTime)
                {
                ...
                else if (field == BroadcastHistory.Fields.Status)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
        ...
        public ArrayList JoinActionStepHistoryActionStepDisaster(Dictionary<ActionStepHistory.Fields, object> dicConditions1, Dictionary<ActionStep.Fields, object> dicConditions2, Dictionary<Disaster.Fields, object> dicConditions3, string strAdditionalConditions, out string strErrorMessage)
        {
            return JoinActionStepHistoryActionStepDisaster(dicConditions1, dicConditions2, dicConditions3, strAdditionalConditions, null, out strErrorMessage);
        }

        public ArrayList JoinActionStepHistoryActionStepDisaster(Dictionary<ActionStepHistory.Fields, object> dicConditions1, Dictionary<ActionStep.Fields, object> dicConditions2, Dictionary<Disaster.Fields, object> dicConditions3, string strAdditionalConditions, int? topNCount, out string strErrorMessage)
        {
            strErrorMessage = null;
            ...
            int nHistoryFieldCount, nActionStepFieldCount, nDisasterFieldCount;

            string strHistoryFields = GetFieldNames<ActionStepHistory.Fields>(strHistoryTableName, out nHistoryFieldCount);
            string strActionStepFields = GetFieldNames<ActionStep.Fields>(strActionStepTableName, out nActionStepFieldCount);
            string strDisasterFields = GetFieldNames<Disaster.Fields>(strDisasterTableName, out nDisasterFieldCount);

            int nFieldsCount = nHistoryFieldCount + nActionStepFieldCount + nDisasterFieldCount;

            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("Select {0}, {1}, {2} ", strHistoryFields, strActionStepFields, strDisasterFields);
            sb.AppendFormat("  From {0}, {1}, {2} ", strHistoryTableName, strActionStepTableName, strDisasterTableName);
            sb.AppendFormat(" Where {0}.{1} = {2}.{3} and {2}.{4} = {5}.{6} ", strHistoryTableName, ActionStepHistory.Fields.ActionStepID
                                                       , strActionStepTableName, ActionStep.Fields.ID
                                                       , ActionStep.Fields.DisasterID
                                                       , strDisasterTableName, Disaster.Fields.ID);
            ...
            if (SetCondition<Disaster.Fields>(ref strCondition3, dicConditions3, Disaster.GetFieldName, strDisasterTableName, ref strErrorMessage) == false)
                return null;
            ...
            for (int i = 0; i < nResultCount - (nFieldsCount - 1); i += nFieldsCount)
            {
                ActionStepHistory actionStepHistory = ReadActionStepHistory(arrResult, i, out strErrorMessage);
                if (actionStepHistory == null)
                    return null;
                else
                    arrDatas.Add(actionStepHistory);

                ActionStep actionStep = ReadActionStep(arrResult, i + nHistoryFieldCount, out strErrorMessage);
                ...
                Disaster disaster = ReadDisaster(arrResult, i + nHistoryFieldCount + nActionStepFieldCount, out strErrorMessage);
        ...
        private Disaster ReadDisaster(ArrayList arrResult, int index, out string strErrorMessage)
        {
            strErrorMessage = null;
            Disaster model = new Disaster();
            bool isNullable;

            foreach (Disaster.Fields field in Disaster.Fields.GetValues(typeof(Disaster.Fields)))
            {
                string strFieldName = Disaster.GetFieldName(field, out isNullable);

                if (field == Disaster.Fields.ID)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
                ...
                else if (field == Disaster.Fields.DisasterName)
                {
                    string data = WebDBManager.GetStringField(arrResult[index]);
                ...
                else if (field == Disaster.Fields.SubDisasterCategoryID)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
                ...
                else if (field == Disaster.Fields.VersionID)
                {
                    VariousData<int> data = WebDBManager.GetIntField(arrResult[index].ToString());
                    ...
                    {
                        model.VersionID = data.Data;
                    }
                }
                else if (field == Disaster.Fields.UserLevelIDs)
                {
                    string data = WebDBManager.GetStringField(arrResult[index]);
                    ...
                    {
                        List<int> userLevelIDs = new List<int>();
                        ...
                        foreach (string strToken in tokens)
                        {
                            ...
                            if (int.TryParse(strToken.Trim(), out levelID))
                                userLevelIDs.Add(levelID);
                        }

                        model.UserLevelIDs = userLevelIDs;
                    }
                }
                else if (field == Disaster.Fields.Description)
                {
                    string data = WebDBManager.GetStringField(arrResult[index]);
