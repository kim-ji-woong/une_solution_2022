namespace Common.DAL
{
    ...
    public class CreateManager : QueryManager, ICreate
    {
        ...
        {
            if (EqualsValue(history.ActionStepID, nActionStepID) &&
                EqualsValue(history.BeginTime, dtBegin) &&
                EqualsValue(history.RealMode, realMode) &&
                EqualsValue(history.EndTime, dtEnd) &&
                EqualsValue(history.LastAccessedTime, dtLastAccessed) &&
                EqualsValue(history.DetectEndTime, dtDetectEnd) &&
                EqualsValue(history.DetectTime, dtDetect) &&
                EqualsValue(history.Position, strPosition) &&
                EqualsValue(history.LastAccessedUserID, nLastAccesseduserID) &&
                EqualsValue(history.StartOption, nStartOption) &&
                EqualsValue(history.DisasterOption, strDisasterOption) &&
                EqualsValue(history.SensorZoneHistoryID, nSensorZoneHistoryID) &&
                EqualsValue(history.Description, strDescription))
                return true;
        ...
        {
            if (EqualsValue(history.ActionStepHistoryID, nActionStepHistoryID) &&
                EqualsValue(history.ComponentID, nComponentID) &&
                EqualsValue(history.ComponentType, nComponentType) &&
                IsSameDateTime2(history.Time, dtTime) &&
                EqualsValue(history.Status, nStatus) &&
                EqualsValue(history.Task, strTask) &&
                EqualsValue(history.CompleteCount, nCompleteCount) &&
                EqualsValue(history.ShowBoard, showBoard) &&
                EqualsValue(history.AccessedUserID, nAccessedUserID) &&
                EqualsValue(history.CheckedNotify1, nCheckedNotify1) &&
                EqualsValue(history.CheckedNotify2, nCheckedNotify2) &&
                EqualsValue(history.CheckedRun, nCheckedRun) &&
                EqualsValue(history.CheckedComplete, nCheckedComplete) &&
                EqualsValue(history.Description, strDescription))
                return true;
        ...
        public BroadcastHistory CreateBroadcastHistory(int nActionStepHistoryID, int nComponentID, DateTime eventTime, int status, out string strErrorMessage)
        {
            strErrorMessage = null;
            Dictionary<BroadcastHistory.Fields, object> dicFieldDatas = new Dictionary<BroadcastHistory.Fields, object>();
            dicFieldDatas[BroadcastHistory.Fields.ActionStepHistoryID] = nActionStepHistoryID;
            dicFieldDatas[BroadcastHistory.Fields.ComponentID] = nComponentID;
            dicFieldDatas[BroadcastHistory.Fields.EventTime] = eventTime;
            dicFieldDatas[BroadcastHistory.Fields.Status] = status;

            string strSQL = string.Format("Insert into {0} ({1}) values ({2})",
                BroadcastHistory.TableName,
                GetFieldNames<BroadcastHistory.Fields>(),
                GetFieldValues(dicFieldDatas));
            ...
            if (arrResult != null)
            {
                ...
                history.ActionStepHistoryID = nActionStepHistoryID;
                history.ComponentID = nComponentID;
                history.EventTime = eventTime;
