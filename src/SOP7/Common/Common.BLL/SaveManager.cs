...
namespace Common.BLL
{
    public class SaveManager
    {
        ...
        /// </summary>
        public enum TargetType { POI = 0, EquipzoneName, FakeWall, SOP, StatusInfo, UserAuth, BuildingGroupName, BuildingName, EquipZoneArea, SensorEquipZone }
        ...
        {            
            ...
            if (targetType == TargetType.POI)
                ...
                returnStr = "빌딩 명칭";
            else if (targetType == TargetType.EquipZoneArea)
                returnStr = "영역";
            else if (targetType == TargetType.SensorEquipZone)
                returnStr = "센서 구역정보";
        ...
        /// <param name="nEquipZoneID"></param>
        /// <returns></returns>
        public bool SaveUserHistory_UpdateEquipZoneArea(int nUserID, ActionType actionType, int nZoneID, int nEquipZoneID)
        {
            TargetType targetType = TargetType.EquipZoneArea;

            string strHistorycontent = MakeHistoryContentEquipZoneArea(targetType, actionType, nZoneID, nEquipZoneID);

            bool result = m_dataManager.GetCreateManager().CreateUserHistory(nUserID, (int)targetType, (int)actionType, strHistorycontent);
            return result;
        }

        private string MakeHistoryContentEquipZoneArea(TargetType targetType, ActionType actionType, int nZoneID, int nEquipZoneID)
        {
            SDMS.IDAL.ISelect sdmsSelect = m_processManager.SdmsDataManager.GetSelectManager();

            string strErrorMessage = null;
            string strEquipmentZoneName = "";

            string position = GetTargetPosition(sdmsSelect, nZoneID);

            EquipmentZone ez = sdmsSelect.SelectEquipmentZone(nEquipZoneID, out strErrorMessage);
            if (ez != null)
                strEquipmentZoneName = ez.DisplayText;

            string strType = GetTypeString(targetType, actionType);

            string returnStr = string.Format("[{0}] [{1}] {2}", position, strEquipmentZoneName, strType);
