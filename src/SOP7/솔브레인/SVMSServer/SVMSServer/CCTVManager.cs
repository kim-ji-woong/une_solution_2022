...
namespace SVMSServer
{
    public class CCTVManager
    {
        ...
        private string m_strRTSPtoWebRTC_URL = "http://127.0.0.1:8083/stream/player/";
        private string m_strGo2rtc_URL = "http://127.0.0.1:1984/stream.html?src=";
        ...
        public CCTVManager(DataManager dataManager, Common.DAL.DataManager commonDataManager)
        {
           ...
           string strRTSPtoWebRTC_URL = ConfigurationManager.AppSettings.Get("RTSPtoWebRTC_URL");
           if (strRTSPtoWebRTC_URL?.Length > 0)
                m_strRTSPtoWebRTC_URL = strRTSPtoWebRTC_URL;

            string strGo2rtc_URL = ConfigurationManager.AppSettings.Get("go2rtc_URL");
            if (strGo2rtc_URL?.Length > 0)
                m_strGo2rtc_URL = strGo2rtc_URL;
        }
        ...
        public void Update(ICollection<CCTV> svmsCCTVs)
        {
            ...
            if (changedCCTVs.Count > 0 || /*deletedCCTVs.Count > 0 || */newCCTVs.Count > 0)
            {
                ...
                if (KillProcess(strExeName))
                {
                    ...
                    UpdateCCTV(cctvs, strJsonFile);
                    RunProcess(strRunModule);
                }

                if (m_dataManager.SiteID == 10)
                {
                    string strExeName_Sub = ConfigurationManager.AppSettings.Get("rtspServerName_Sub");
                    string strRunModule_Sub = ConfigurationManager.AppSettings.Get("runRTSPServer_Sub");
                    string strJsonFile_Sub = ConfigurationManager.AppSettings.Get("cctvJson_Sub");

                    if (strExeName_Sub == null || strRunModule_Sub == null || strJsonFile_Sub == null)
                        return;

                    if (KillProcess(strExeName_Sub))
                    {
                        ICollection<CCTV> cctvs = m_dicCCTVs.Values;

                        UpdateCCTV_RTSPtoWebRTC(cctvs, strJsonFile_Sub);
                        RunProcess(strRunModule_Sub);
                    }
                }
            }
        }
        ...
        private void ChangeCCTVs(List<CCTV> changedCCTVs, List<string> safetyI_IP_List)
        {
            ...
            foreach (CCTV cctv in changedCCTVs)
            {
                ...
                {   // 솔브레인 RTSP URL 관련 정보 수정
                    _cctv = CloneCCTV(cctv);

                    string strURL = cctv.SmallURL != null ? cctv.SmallURL : cctv.URL;
                    _cctv.URL = UpdateRTSPURL(strURL, cctv.ID);
                }

                if (m_dataManager.GetUpdateManager().UpdateCCTV(_cctv, out strErrorMessage) == false)
                {
                    Logger.Instance.Write("CCTVManager.ChangeCCTVs, CCTV[" + _cctv.ID + "], " + _cctv.UniqueKey + ", Enabled : " + _cctv.Enabled);
                }
            }
        }
        ...
        private bool UpdateCCTV(int nID, string strCameraName, string strURL, string strBigURL, string strSmallURL, bool? enabled)
        {
            ...
            if (m_dataManager.SiteID == 10)
            {   // 솔브레인 RTSP URL 관련 정보 수정
                strURL = strSmallURL != null ? strSmallURL : strURL;
                strURL = UpdateRTSPURL(strURL, nID);
            }
        ...
        public bool UpdateCCTV(CCTV cctv)
        {
            ...
            if (m_dicCCTVs.TryGetValue(cctv.UniqueKey, out _cctv))
            {
                ...
                if (m_dataManager.SiteID == 10)
                {   // 솔브레인 RTSP URL 관련 정보 수정
                    cctvData = CloneCCTV(cctv);

                    string strURL = cctv.SmallURL != null ? cctv.SmallURL : cctv.URL;
                    cctvData.URL = UpdateRTSPURL(strURL, _cctv.ID);
                }

                bool success = m_dataManager.GetUpdateManager().UpdateCCTV(cctvData, out strErrorMessage);
            ...
            {
                _cctv = m_dataManager.GetCreateManager().CreateCCTV(cctv.CameraName, "", cctv.UniqueKey, null, null, null, null, false, "RTSP", null, cctv.UserID, cctv.Password, cctv.URL, null, null, cctv.Enabled, cctv.CameraIP, cctv.CameraCompanyName, cctv.CameraModelName, null, null);
        ...
        public static void UpdateCCTV(ICollection<CCTV> cctvs, string strJsonFile)
        {
            string strJson = "";
            ...
            // go2rtc 방식
            if (strJson.Length > 0 && strJson.StartsWith("streams:"))
            {
                strJson = "streams:\r\n";
        ...
        public static void UpdateCCTV_RTSPtoWebRTC(ICollection<CCTV> cctvs, string strJsonFile)
        {
            string strJson = "";
            StreamReader reader = new StreamReader(strJsonFile, Encoding.UTF8);
        ...
        public CCTV CloneCCTV(CCTV cctv)
        {
            ...
            _cctv.Y = cctv.Y;
            _cctv.Z = cctv.Z;
            _cctv.ZoneID = cctv.ZoneID;
            _cctv.IsIndoor = cctv.IsIndoor;
            _cctv.Type = cctv.Type;
            _cctv.Channel = cctv.Channel;
            _cctv.UserID = cctv.UserID;
            _cctv.Password = cctv.Password;
            _cctv.URL = cctv.URL;
            _cctv.BigURL = cctv.BigURL;
            ...
            _cctv.CameraCompanyName = cctv.CameraCompanyName;
            _cctv.CameraModelName = cctv.CameraModelName;
            _cctv.Description = cctv.Description;
            _cctv.SiteID = cctv.SiteID;
