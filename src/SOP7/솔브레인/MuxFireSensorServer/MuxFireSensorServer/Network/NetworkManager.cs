...
namespace MuxFireSensorServer.Network
{
    public class NetworkManager
    {
        ...
        private bool m_bConnState = false;
        ...
        public NetworkManager(string strIP, string strPort, string strMuxType, WebDBManager dbMgr)
        {
            m_instance = this;
            m_dbMgr = dbMgr;
        ...
        private void ConnectionThread()
        {
            ...
            while (m_runThread)
            {
                try
                {
                    ...
                    {
                        if (m_bConnState == false)
                        {   // 연결 상태 업데이트   
                            if (UpdateConnState(true))
                                m_bConnState = true;
                        }
                    ...
                    if (!m_provider1.IsConnected)
                    {
                        lock (m_provider1)
                        {
                            if (m_nPort > 0)
                            {
                                ...
                                if (m_provider1.IsConnected == false && m_bConnState == true)
                                {   // 연결 상태 업데이트   
                                    if (UpdateConnState(false))
                                        m_bConnState = false;
                                }
                            }
                        }
        ...
        private bool UpdateConnState(bool bConnState)
        {
            bool bResult = true;

            string strSQL = $@"Update OptionSDMS 
                                Set PropertyValue = '{bConnState}'  
                                Where PropertyName = 'ServerConnState_Fire'";

            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);

            if (arrResult == null)
            {
                Logger.Instance.Write("[UpdateConnState Error] " + m_dbMgr.LastErrorMessage);
                bResult = false;
            }
