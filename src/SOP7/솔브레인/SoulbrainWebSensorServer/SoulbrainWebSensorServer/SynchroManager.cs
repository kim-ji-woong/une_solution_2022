...
namespace SoulbrainWebSensorServer
{
    public class SynchroManager
    {
        ...
        private int m_nThreadSleep = 1000 * 60 * 10;                          // 쓰레드 슬립타임
        ...
        public void StopThread()
        {
            ...
            if (m_SynchroThread != null)
                m_SynchroThread.Abort();
        }

        public SynchroManager(dnsDBUtil.DirectDBManager hrDBManager, TeamEditor.DAL.DataManager dataManager, TeamEditor.BLL.ProcessManager processManager)
        {
            ...
            while (!m_shutdownThread)
            {
                if (m_startThread)
                {
                    ...
                    {   // 하루에 최초 한번 동작
                        ...
                        if (m_processManager.GetSaveManager().UpdateRegularMemberData(dicPathRegularMember, out strErrorMessage, m_dataManager.SiteID, dicRegulars_Current, dicRegularMembers_Current) == false)
                        {
        ...
        private bool SetCurrentRegularMemberData(List<HrRegular> rootRegulars, out Dictionary<int, Regular> dicRegulars, out Dictionary<int, RegularMember> dicRegularMembers, out string strErrorMessage)
        {
            ...
            foreach (Regular regular in regulars)
            {
                ...
                if (bChk == true)
                {
                    ...
                    team.SiteID = m_dataManager.SiteID;
        ...
        private void GetChildRegularTeam(Regular parentTeam, List<Regular> regulars, ref Dictionary<int, Regular> dicRegulars)
        {
            ...
            foreach (Regular regular in regulars)
            {
                if (parentTeam.ID == regular.ParentTeamID)
                {
                    ...
                    child.SiteID = m_dataManager.SiteID;
        ...
        private int GetBelongID (List<Regular> regulars, string strBelongName)
        {
            ...
            {   // 새로 추가된 소속
                ...
                regularBelong.SiteID = m_dataManager.SiteID;
