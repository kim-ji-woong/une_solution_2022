...
namespace SoulbrainWebSensorServer
{
    public partial class FormMain : Form
    {
        ...
        private const int m_nThreadClearSleep = 1000 * 60 * 3;              // 1분(1초 * 60) * 1 = 1분
        ...
        public FormMain()
        {
            ...
            // 데이터 추가 작업 용도
            m_webServiceMgr.RequestLogin();
            m_webServiceMgr.RequestDeviceList();

            //// 추가 작업 시에 한번에 센서 데이터 받아오기
            m_webServiceMgr.RequestAllSensorData();
            Dictionary<string, DataDevice> dicDevices = m_webServiceMgr.DicDevices;

            //// ETC Sensor 추가 작업 용도 
            m_wsopDataMgr.AddETCSensor(dicDevices);
        }
        ...
        private void WatchReloadThread()
        {
            while (!m_shutdownThread)
            {
                ...
                if (m_bFirstServer || m_bDataServer)
                {
                    ...
                    // 디바이스 센서 데이터 조회 쓰레드 생성 >> ReloadSensorThread3 쓰레드로 이동
                    ...
                    //    continue;
                    //}
                    //if (m_watchSensor == null || m_watchSensor.ThreadState == ThreadState.Stopped)
                    //{
        ...
        /*private bool ReloadSensorThread2()
        {
            ...
            while (!m_shutdownSensor)
            {
                ...
                Console.WriteLine("ReloadSensorThread2 End");
            }
        ...
        /*private void WatchDeviceThread(int nNum, int nShutdownThread)
        {
            ...
            while (m_nShutdownThread == nShutdownThread)
            {
                for (int i = nNum; i < nEndNum; i++)
                {
                    ...
                    // .TODO: 디버깅 테스트 디바이스 검색
                    //if (data.DeviceId == "HF___GT06010100H06")
            ...
            if (m_nShutdownThread != nShutdownThread)
            {
        ...
        {
            Logger.Instance.Write("WatchWishThread 실행");
