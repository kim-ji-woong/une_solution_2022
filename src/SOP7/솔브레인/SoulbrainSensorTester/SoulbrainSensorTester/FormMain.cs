...
namespace SoulbrainSensorTester
{
    public partial class FormMain : Form
    {
        ...
        private void InitDBSet()
        {
            ...
            string strHost = ConfigurationManager.AppSettings.Get("DbHost");
            string strDbPW = ConfigurationManager.AppSettings.Get("DbPw");

            if (strDbID != null && strDbID.Trim().Length > 0 && strHost != null && strHost.Trim().Length > 0 && strDbPW != null && strDbPW.Trim().Length > 0)
            {
                string key = new string(new char[] { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '1', '2', '3', '4', '5', '6' });

                strHost = dnsDBUtil.AES256Cipher.AES_decrypt(strHost.Trim(), key);
                strDbID = dnsDBUtil.AES256Cipher.AES_decrypt(strDbID.Trim(), key);
                strDbPW = dnsDBUtil.AES256Cipher.AES_decrypt(strDbPW.Trim(), key);
                
                m_nSiteID = nSiteID;

                //m_dbManager = new WebDBManager(strDBName, nDBType, nSiteID, strWebServerURL);
                m_dataMgr = new SDMS.DAL.DataManager(nDBType, strHost, strDBName, strDbID, strDbPW, nSiteID);
            }
        }
        ...
        {
            m_SopQueryMgr.SendAllClearQuery(CommonString.ALARM_METHOD, m_nSiteID, m_dataManager.StrAlarm_Fire_RUL);
        }
