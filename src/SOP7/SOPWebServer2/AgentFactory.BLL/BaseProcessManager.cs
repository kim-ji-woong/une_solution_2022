...
namespace AgentFactory.BLL
{
    public abstract class BaseProcessManager
    {
        ...
        public BaseProcessManager(Factory factory)
        {
            ...
            {
                var dicReactionType = new Dictionary<int, SensorReactionHistory.ReactionTypes>();

                foreach (SensorReactionHistory.ReactionTypes type in Enum.GetValues(typeof(SensorReactionHistory.ReactionTypes)))
                {
                    dicReactionType[(int)type] = type;
                }

                m_dicReactionType = dicReactionType;
            }

            if (m_dicDetectionStatus == null)
            {
                var dicDetectionStatus = new Dictionary<int, SensorZoneHistory.DetectionType>();

                foreach (SensorZoneHistory.DetectionType status in Enum.GetValues(typeof(SensorZoneHistory.DetectionType)))
                {
                    dicDetectionStatus[(int)status] = status;
                }

                m_dicDetectionStatus = dicDetectionStatus;
            }
        }
        ...
        public abstract void UpdateAlarm(AlarmData alarm, List<int> alarmSensorZoneIDs, object sensorZoneGroup);
        // 탐지된 알람이 복구되었다.
