...
{
    public class FlowSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public FlowSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Flow);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("FlowSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Flow) + " 신호";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"유량 상황이 신고되었습니다\", \"en\":\"Flow situation has been reported\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        //strMessage = string.Format("[{0}]에서 유량 상황이 신고되었습니다", szLocationName);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 유량 상황이 신고되었습니다\", \"en\":\"Flow situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
        ...
        {
            string strMessage = "신고된 유량 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 유량 상황이 종료되었습니다\", \"en\":\"The reported Flow situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 유량 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 유량 상황이 종료되었습니다\", \"en\":\"The Flow situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsFlowSensorType(alarm.SensorType))
                    continue;
