...
{
    public class O2JAGSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public O2JAGSensor(MainManager mainManager, Factory factory)
           : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.O2JAG);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("O2JAGSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.O2JAG) + " 신호";
            string strErrorMessage = null;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {   // 다국어 설정
                strEventName = @"{""ko"":""산소 가스"", ""en"":""O2 GAS""}";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"산소 가스 상황이 신고되었습니다\", \"en\":\"O2 GAS situation has been reported\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        //strMessage = string.Format("[{0}]에서 산소 상황이 신고되었습니다", szLocationName);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 산소 가스 상황이 신고되었습니다\", \"en\":\"O2 GAS situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string szLocationName = zone.DisplayText;
                        strMessage = string.Format("[{0}]에서 산소 가스 상황이 신고되었습니다", szLocationName);
                    }
                }
            }
        ...
        {
            string strMessage = "신고된 산소 가스 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 산소 가스 상황이 종료되었습니다\", \"en\":\"The reported O2 GAS situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 산소 가스 상황이 종료되었습니다\", \"en\":\"The O2 GAS situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
            {
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        strMessage = string.Format("[{0}]에서 신고된 산소 가스 상황이 종료되었습니다", zone.DisplayText);
                    }
                }
            }
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsO2JAGSensorType(alarm.SensorType))
                    continue;
