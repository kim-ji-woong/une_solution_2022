...
{
    public class H2LowSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public H2LowSensor(MainManager mainManager, Factory factory)
           : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.H2Low);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("H2LowSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.H2Low_Senko) + " 신호";
            string strErrorMessage = null;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {   // 다국어 설정
                strEventName = @"{""ko"":""수소 저농도"", ""en"":""Hydrogen Low""}";
                ...
                if (isReal)
                {
                    ...
                    if (equipZone == null)
                    {
                        ...
                        strDetectMessage = $"{{\"ko\":\"{jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"e{jEventName["en"].ToString()} 신호가 탐지되었습니다\"}}";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"수소 저농도 상황이 신고되었습니다\", \"en\":\"Hydrogen Low situation has been reported\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        //strMessage = string.Format("[{0}]에서 수소 상황이 신고되었습니다", szLocationName);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 수소 저농도 상황이 신고되었습니다\", \"en\":\"Hydrogen Low situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string szLocationName = zone.DisplayText;
                        strMessage = string.Format("[{0}]에서 수소 저농도 상황이 신고되었습니다", szLocationName);
                    }
                }
            }
        ...
        {
            string strMessage = "신고된 수소 저농도 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 수소 저농도 상황이 종료되었습니다\", \"en\":\"The reported Hydrogen Low situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 수소 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 수소 저농도 상황이 종료되었습니다\", \"en\":\"The Hydrogen Low situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
            {
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        strMessage = string.Format("[{0}]에서 신고된 수소 저농도 상황이 종료되었습니다", zone.DisplayText);
                    }
                }
            }
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsH2LowSensorType(alarm.SensorType))
                    continue;
