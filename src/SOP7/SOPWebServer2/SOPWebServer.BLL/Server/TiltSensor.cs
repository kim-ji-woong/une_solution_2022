...
{
    public class TiltSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public TiltSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("TiltSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Tilt) + " 신호";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            else
            {
                ...
                if (zone != null)
                {
                    ...
                    strMessage = string.Format("[{0}]에서 기울기 상황이 신고되었습니다", szLocationName);
                }
            }
        ...
        {
            string strMessage = "신고된 기울기 상황이 종료되었습니다";
            int nZoneID;
            ...
            if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
            {
                ...
                if (zone != null)
                {
                    strMessage = string.Format("[{0}]에서 신고된 기울기 상황이 종료되었습니다", zone.DisplayText);
                }
            }
