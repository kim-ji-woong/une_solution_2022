...
{
    public class PressureSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public PressureSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Pressure);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                // HistoryRiskID 추가
                int? nHistoryRiskID = null;
                if (arrDatas.Count > 5 && arrDatas[5] is int)
                    nHistoryRiskID = (int)arrDatas[5];

                WriteLog("PressureSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
                ...
                if (nSensorData > 0)
                {
                    ...
                    AlarmData alarm;
                    // HistoryRiskID 추가
                    int nResult = AddAlarm(group, nSensorTagID, sensorZone, isReal, nAlarmLevel.Value, nHistoryRiskID, out alarm);
                ...
                else
                {
                    ...
                    foreach (AlarmData alarm in alarms)
                    {
                        ...
                        else if (alarm != null && group.CurrentAlarm != null)
                        {
                            ...
                            // HistoryRiskID 추가
                            ChangeAlarm(m_mainManager, group.CurrentAlarm, alarmPrev, group, sensorZone, null);
                        }
        ...
        // HistoryRiskID 추가
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int nAlarmLevel, int? nHistoryRiskID, out AlarmData alarm)
        {
            alarm = null;
            ...
            if (currentAlarm != null && nSensorDataCount > 0)
            {
                ...
                return ChangeAlarm(m_mainManager, currentAlarm, group, sensorZone, nAlarmLevel, nHistoryRiskID);
            }
...

            {
                ...
                if (alarm != null)
                {
                    ...
                    // HistoryRiskID 추가
                    string strParam4 = null;
                    if (nHistoryRiskID.HasValue)
                        strParam4 = nHistoryRiskID.Value.ToString();

                    // HistoryRiskID 추가
                    if (((Process.AlarmManager)m_mainManager.AlarmManager).AddReactionHistory(alarm, (int)reactionType, timeStamp, strMessage, strEquipZoneID, sensorZone.ID.ToString(), strParam3, strParam4, strParam5, txDataManager))
                    {
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.PRESSURE_SENSOR) + " 신호";

            string strErrorMessage = null;
            string strDetectMessage = "";

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {   // 다국어 설정
                strEventName = @"{""ko"":""압력"", ""en"":""Pressure""}";
                ...
                if (nOrgSensorID.HasValue)
                {
                    ...
                    if (etc != null && etc.MaterialType != null)
                    {
                        ...
                        if (material != null)
                        {
                            ...
                            strEventName = material.MaterialName;
                        }
                    }
                }

                JObject jEventName = JObject.Parse(strEventName);
                ...
                if (isReal)
                {
                    if (equipZone == null)
                    {
                        ...
                        strDetectMessage = $"{{\"ko\":\"{jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"{jEventName["en"].ToString()} signal was detected\"}}";
                    }
                    else
                    {
                        //return string.Format("{0}[{1}]에서 {2}가 탐지되었습니다", strTag, equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strDetectMessage = $"{{\"ko\":\"{jEquipZone["ko"].ToString()}에서 {jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"{jEventName["en"].ToString()} signal was detected in the {jEquipZone["en"].ToString()}\"}}";
                    }
                ...
                else
                {
                    if (equipZone == null)
                    {
                        ...
                        strDetectMessage = $"{{\"ko\":\"[테스트]{jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"[TEST]{jEventName["en"].ToString()} signal was detected\"}}";
                    }
                    else
                    {
                        //return string.Format("[테스트][{0}]에서 {1}가 탐지되었습니다", equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strDetectMessage = $"{{\"ko\":\"[테스트]{jEquipZone["ko"].ToString()}에서 {jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"[TEST]{jEventName["en"].ToString()} signal was detected in the {jEquipZone["en"].ToString()}\"}}";
                    }
                }
            }
            else
            {
                ...
                if (isReal)
                {
                    ...
                    if (equipZone == null)
                        strDetectMessage = strTag + strEventName + "가 탐지되었습니다";
                    else
                        strDetectMessage = string.Format("{0}[{1}]에서 {2}가 탐지되었습니다", strTag, equipZone.DisplayText, strEventName);
                }
                else
                {
                    if (equipZone == null)
                        strDetectMessage = string.Format("[테스트]{0}가 탐지되었습니다", strEventName);
                    else
                        strDetectMessage = string.Format("[테스트][{0}]에서 {1}가 탐지되었습니다", equipZone.DisplayText, strEventName);
                }
            }
                
            return strDetectMessage;
        }
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                ...
                if (isReal)
                {
                    ...
                    if (equipZone == null)
                    {
                        ...
                        strMessage = $"{{\"ko\":\"{jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"{jEventName["en"].ToString()} signal has been restored\"}}";
                    }
                    else
                    {
                        //strMessage = string.Format("{0}[{1}]에서 탐지된 {2}가 복구되었습니다", strTag, equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jEquipZone["ko"].ToString()}에서 탐지된 {jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"{jEventName["en"].ToString()} signal detected in the {jEquipZone["en"].ToString()} has been restored\"}}";
                    }
                }
                else
                {
                    if (equipZone == null)
                    {
                        ...
                        strMessage = $"{{\"ko\":[테스트]\"{jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":[TEST]\"{jEventName["en"].ToString()} signal has been restored\"}}";
                    }
                    else
                    {
                        //strMessage = string.Format("[테스트][{0}]에서 탐지된 {1}가 복구되었습니다", equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strMessage = $"{{\"ko\":\"[테스트]{jEquipZone["ko"].ToString()}에서 탐지된 {jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"[TEST]{jEventName["en"].ToString()} signal detected in the {jEquipZone["en"].ToString()} has been restored\"}}";
                    }
                }
            }
            else
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"압력 상황이 신고되었습니다\", \"en\":\"e압력 상황이 신고되었습니다\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        //strMessage = string.Format("[{0}]에서 온도 상황이 신고되었습니다", szLocationName);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 압력 상황이 신고되었습니다\", \"en\":\"Pressure situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string szLocationName = zone.DisplayText;
                        strMessage = string.Format("[{0}]에서 압력 상황이 신고되었습니다", szLocationName);
                    }
                }
            }
        ...
        {
            string strMessage = "신고된 압력 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 압력 상황이 종료되었습니다\", \"en\":\"The reported Pressure situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 압력 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 압력 상황이 종료되었습니다\", \"en\":\"The Pressure situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
        ...
        // HistoryRiskID 추가
        private int ChangeAlarm(MainManager mainManager, AlarmData currentAlarm, SensorZoneGroup group, SensorZone sensorZone, int nAlarmLevel, int? nHistoryRiskID)
        {
            AlarmData alarmPrev = currentAlarm.Clone();

            currentAlarm.AlarmDepth = nAlarmLevel;

            ChangeAlarm(mainManager, currentAlarm, alarmPrev, group, sensorZone, nHistoryRiskID);

            return ErrorMessageType.SUCCESS;
        }

        // HistoryRiskID 추가
        private void ChangeAlarm(MainManager mainManager, AlarmData alarmCurrent, AlarmData alarmPrev, SensorZoneGroup group, SensorZone sensorZone, int? nHistoryRiskID)
        {
        ...
        private string GetChangeAlarmDepthString(int nOrgSensorID, int nAlarmDepth, int nPrevAlarmDepth, bool isReal, EquipmentZone equipZone)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strTag = isReal ? $"{{\"ko\":\"\", \"en\":\"e\"}}" : $"{{\"ko\":\"[테스트]\", \"en\":\"[TEST]\"}}";
                strEventName = @"{""ko"":""압력"", ""en"":""Pressure""}";
                ...
                if (equipZone != null)
                {
                    JObject jTag = JObject.Parse(strTag);
                    JObject jDisplayText = JObject.Parse(equipZone.DisplayText);
                    JObject jEventName = JObject.Parse(strEventName);
                    ...
                    strMessage = $"{{\"ko\":\"{jTag["ko"].ToString()}[{jDisplayText["ko"].ToString()}]에서 탐지된 {jEventName["ko"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\", " +
                        $"\"en\":\"The alarm level of the {jEventName["en"].ToString()} detected in the [{jTag["en"].ToString()}[{jDisplayText["en"].ToString()}] has been changed from Level {nPrevAlarmDepth} to Level {nAlarmDepth}\"}}";
                }
                else
                {
                    JObject jEventName = JObject.Parse(strEventName);
                    //strMessage = strTag + string.Format("탐지된 {0}의 알람 단계가 {1}단계에서 {2}단계로 변경되었습니다.", strEventName, nPrevAlarmDepth, nAlarmDepth);
                    strMessage = $"{{\"ko\":\"탐지된 {jEventName["ko"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\", " +
                        $"\"en\":\"The alarm level of the {jEventName["en"].ToString()} detected has been changed from Level {nPrevAlarmDepth} to Level {nAlarmDepth}\"}}";
                }
            }
            else
            {
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsPressureSensorType(alarm.SensorType))
                    continue;
