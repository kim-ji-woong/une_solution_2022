...
{
    public class ManufactureSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public ManufactureSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Manufacture);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("ManufactureSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        private AlarmData FindActiveZoneAlarm(int nSensorZoneID)
        {
            ...
            return ((Process.AlarmManager)m_mainManager.AlarmManager).FindAlarm(nSensorZoneID, Facility.FacilityType.Manufacture, m_mainManager.SDMSDataManager);
        }
        ...
        private string GetDetectMessage(int nOrgSensorID, EquipmentZone equipZone, bool isReal)
        {
            ...
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Manufacture) + " 신호";
        ...
        private Result ProcessManualReport(ArrayList arrDatas)
        {
                                    ...
                                    && arrDatas[4] is int && arrDatas[5] is string && arrDatas[6] is string)
            {
                ...
                AlarmData alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).GetManualAlarm(nZoneID, Facility.FacilityType.Manufacture, m_mainManager.SDMSDataManager);
                if (alarm != null)
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            else
            {
                ...
                if (zone != null)
                {
                    ...
                    strMessage = string.Format("[{0}]에서 제조설비 상황이 신고되었습니다", szLocationName);
                }
            }
        ...
        {
            string strMessage = "신고된 제조설비 상황이 종료되었습니다";
            int nZoneID;
            ...
            if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
            {
                ...
                if (zone != null)
                {
                    strMessage = string.Format("[{0}]에서 신고된 제조설비 상황이 종료되었습니다", zone.DisplayText);
                }
            }
