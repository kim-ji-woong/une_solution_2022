...
{
    public class GasSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public GasSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Gas);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("GasSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.GAS) + " 신호";
            string strErrorMessage = null;
            string strDetectMessage = "";

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strEventName = @"{""ko"":""가스"", ""en"":""GAS""}";
                ...
                if (isReal)
                {
                    ...
                    else
                    {
                        ...
                        strDetectMessage = $"{{\"ko\":\"{jEquipZone["ko"].ToString()}에서 {jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"e{jEquipZone["en"].ToString()}에서 {jEventName["en"].ToString()} 신호가 탐지되었습니다\"}}";
                    }
                ...
                else
                {
                    if (equipZone == null)
                    {
                        ...
                        strDetectMessage = $"{{\"ko\":\"[테스트]{jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"[TEST]{jEventName["en"].ToString()} 신호가 탐지되었습니다\"}}";
                    }
                    else
                    {
                        //return string.Format("[테스트][{0}]에서 {1}가 탐지되었습니다", equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strDetectMessage = $"{{\"ko\":\"[테스트]{jEquipZone["ko"].ToString()}에서 {jEventName["ko"].ToString()} 신호가 탐지되었습니다\", \"en\":\"[TEST]{jEquipZone["en"].ToString()}에서 {jEventName["en"].ToString()} 신호가 탐지되었습니다\"}}";
                    }
                }
            }
            else
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                ...
                if (isReal)
                {
                    ...
                    if (equipZone == null)
                    {
                        ...
                        strMessage = $"{{\"ko\":\"{jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"e{jEventName["en"].ToString()} 신호가 복구되었습니다\"}}";
                    }
                    else
                    {
                        //strMessage = string.Format("{0}[{1}]에서 탐지된 {2}가 복구되었습니다", strTag, equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jEquipZone["ko"].ToString()}에서 탐지된 {jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"e{jEquipZone["en"].ToString()}에서 탐지된 {jEventName["en"].ToString()} 신호가 복구되었습니다\"}}";
                    }
                }
                else
                {
                    if (equipZone == null)
                    {
                        ...
                        strMessage = $"{{\"ko\":[테스트]\"{jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":[TEST]\"{jEventName["en"].ToString()} 신호가 복구되었습니다\"}}";
                    }
                    else
                    {
                        //strMessage = string.Format("[테스트][{0}]에서 탐지된 {1}가 복구되었습니다", equipZone.DisplayText, strEventName);
                        JObject jEquipZone = JObject.Parse(equipZone.DisplayText);
                        strMessage = $"{{\"ko\":\"[테스트]{jEquipZone["ko"].ToString()}에서 탐지된 {jEventName["ko"].ToString()} 신호가 복구되었습니다\", \"en\":\"[TEST]{jEquipZone["en"].ToString()}에서 탐지된 {jEventName["en"].ToString()} 신호가 복구되었습니다\"}}";
                    }
                }
            }
            else
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"가스 상황이 신고되었습니다\", \"en\":\"e가스 상황이 신고되었습니다\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 가스 상황이 신고되었습니다\", \"en\":\"e{jDisplayText["en"].ToString()}에서 가스 상황이 신고되었습니다\"}}";
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string szLocationName = zone.DisplayText;
                        strMessage = string.Format("[{0}]에서 가스 상황이 신고되었습니다", szLocationName);
                    }
                }
            }                
        ...
        {
            string strMessage = "신고된 가스 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 가스 상황이 종료되었습니다\", \"en\":\"e신고된 가스 상황이 종료되었습니다\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 가스 상황이 종료되었습니다\", \"en\":\"e{jDisplayText["en"].ToString()}에서 신고된 가스 상황이 종료되었습니다\"}}";
                    }
                }
            }
            else
            {
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        strMessage = string.Format("[{0}]에서 신고된 가스 상황이 종료되었습니다", zone.DisplayText);
                    }
                }
            }
        ...
        private string GetChangeAlarmDepthString(int nOrgSensorID, int nAlarmDepth, int nPrevAlarmDepth, bool isReal, EquipmentZone equipZone)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                ...
                if (equipZone != null)
                {
                    ...
                    strMessage = $"{{\"ko\":\"{jTag["ko"].ToString()}[{jDisplayText["ko"].ToString()}]에서 탐지된 {jEventName["ko"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\", \"en\":\"e{jTag["en"].ToString()}[{jDisplayText["en"].ToString()}]에서 탐지된 {jEventName["en"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\"}}";
                }
                else
                {
                    JObject jEventName = JObject.Parse(strEventName);
                    //strMessage = strTag + string.Format("탐지된 {0}의 알람 단계가 {1}단계에서 {2}단계로 변경되었습니다.", strEventName, nPrevAlarmDepth, nAlarmDepth);
                    strMessage = $"{{\"ko\":\"탐지된 {jEventName["ko"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\", \"en\":\"e탐지된 {jEventName["en"].ToString()}의 알람 단계가 {nPrevAlarmDepth}단계에서 {nAlarmDepth}단계로 변경되었습니다\"}}";
                }
            }
            else
            {
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsGasSensorType(alarm.SensorType))
                    continue;
