...
{
    public class TempSensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public TempSensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Temp);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("TempSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Temp) + " 신호";
            string strErrorMessage = null;
            string strDetectMessage = "";

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {   // 다국어 설정
                strEventName = @"{""ko"":""온도"", ""en"":""Temperature""}";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"온도 상황이 신고되었습니다\", \"en\":\"Temperature situation has been reported\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 온도 상황이 신고되었습니다\", \"en\":\"Temperature situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
        ...
        {
            string strMessage = "신고된 온도 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 온도 상황이 종료되었습니다\", \"en\":\"The reported Temperature situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 온도 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 온도 상황이 종료되었습니다\", \"en\":\"The Temperature situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsTempSensorType(alarm.SensorType))
                    continue;
