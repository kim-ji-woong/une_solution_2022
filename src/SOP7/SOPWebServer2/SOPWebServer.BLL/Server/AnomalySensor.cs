...
{
    public class AnomalySensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public AnomalySensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Anomaly);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("AnomalySensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Anomaly) + " 신호";
            string strErrorMessage = null;
            string strDetectMessage = "";

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {   // 다국어 설정
                strEventName = @"{""ko"":""이상탐지"", ""en"":""Anomaly""}";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"이상탐지 상황이 신고되었습니다\", \"en\":\"Anomaly situation has been reported\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 이상탐지 상황이 신고되었습니다\", \"en\":\"Anomaly situation has been reported in the {jDisplayText["en"].ToString()}\"}}";
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        string szLocationName = zone.DisplayText;
                        strMessage = string.Format("[{0}]에서 이상탐지 상황이 신고되었습니다", szLocationName);
                    }
                }
            }
        ...
        {
            string strMessage = "신고된 이상탐지 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 이상탐지 상황이 종료되었습니다\", \"en\":\"The reported Anomaly situation has been resolved\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 이상탐지 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 이상탐지 상황이 종료되었습니다\", \"en\":\"The Anomaly situation reported in the {jDisplayText["en"].ToString()} has been resolved\"}}";
                    }
                }
            }
            else
