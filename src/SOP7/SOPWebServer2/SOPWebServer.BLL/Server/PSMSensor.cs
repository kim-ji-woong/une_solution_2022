...
namespace SOPWebServer.BLL.Server
{
    ...
    public class PSMSensor : BaseServer
    {
        ...
        protected override Result OnReceiveEvent(int header, string strClientInfo, ArrayList arrDatas)
        {
            ...
            else if (header == Header.SENSOR_USER_RESET || header == Header.SENSOR_MALFUNCTION || header == Header.TIMEOUT)
                return ProcessUserReset(header, arrDatas);
        ...
        private Result ProcessUserReset(int header, ArrayList arrDatas)
        {
            if (arrDatas.Count >= 2 && arrDatas[0] is int && arrDatas[1] is int)
            {
                ...
                foreach (KeyValuePair<SensorZone, int> pair in group.GetSensors())
                {
                    ...
                    int result = RemoveAlarm_UserReset(group, pair.Key, sensor, nSOPGenUserID, header);
                ...
                {
                    if (header == Header.SENSOR_USER_RESET)
                        ...
                        alarm.Status = (int)SensorReactionHistory.ReactionTypes.TIME_OUT;
        ...
        // 탐지신호 사용자 복구
        private int RemoveAlarm_UserReset(SensorZoneGroup group, SensorZone sensorZone, PSM sensor, int nSOPGenUserID, int header)
        {
            ...
            SensorReactionHistory.ReactionTypes reactionType = SensorReactionHistory.ReactionTypes.NONE;
            if (header == Header.SENSOR_USER_RESET)
        ...
        private Result ProcessSensorData(ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("PSMSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
                ...
                if (nSensorData > 0)
                {
                    ...
                    {
                        m_mainManager.ProcessManager.UpdateAlarm(alarm, group.GetAlarmSensorZoneIDs(), group);
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, PSM sensor, int nSensorData, bool isReal, int nAlarmLevel, out AlarmData alarm, out AlarmData prevAlarm)
        {
...

            {
                ...
                group.SetSensorData(sensorZone, nSensorData, true, txDataManager);
                ...
                if (group.EquipmentZone != null && group.EquipmentZone.LinkedZoneIDs != null)
                {
                    ...
                    else if (group.EquipmentZone.LinkedZoneIDs.Count > 1)
                    {
                        ...
                        else
                        {
                            ...
                            PSM psm = txDataManager.GetSelectManager().SelectPSMSensor((int)sensorZone.OrgSensorID, out strErrorMessage);
                            if (psm == null)
                                nZoneID = -1;
                            else
                                nZoneID = psm.ZoneID;
                ...
                alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).AddAlarm(sensorZone.ID, nSensorData, nZoneID, sensorZone.SensorType, (int)detectionStatus, timeStamp, txDataManager, FacilityManager.DetectTypes.Detect, nSiteID);
                ...
                if (alarm != null)
                {
                    ...
                    if (((Process.AlarmManager)m_mainManager.AlarmManager).CheckAlarmDuplication(alarm, group, m_mainManager.SensorManager))
                    {
                        ...
                        // 방금 만든 이력은 롤백으로 되돌린다. (트랜잭션을 못 연 경우에만 수동으로 지운다)
                        if (bInTransaction)
                    ...
                    if (((Process.AlarmManager)m_mainManager.AlarmManager).AddReactionHistory(alarm, (int)reactionType, timeStamp, strMessage, strEquipZoneID, sensorZone.ID.ToString(), sensor.ID.ToString(), strEquipZoneName, alarm.AlarmDepth.ToString(), txDataManager))
                    {
        ...
        public int RemoveAlarm(SensorZoneGroup group, SensorZone sensorZone, bool isReal)
        {
            ...
            if (group.GetSensors().Length > 0 && group.CurrentAlarm != null)
            {
                m_mainManager.ProcessManager.UpdateAlarm(group.CurrentAlarm, group.GetAlarmSensorZoneIDs(), group);
        ...
        private string GetDetectPSMMessage(int? nMaterialType, EquipmentZone equipZone, bool isReal, string strLocationName)
        {
            ...
            if (isReal)
            {
                ...
                if (nMaterialType == null || nMaterialType < 0 || strMaterialName == null)
                {
                    ...
                    strMessage = string.Format("{0}[{1}]에서 유해화학물질 누출이 탐지되었습니다", strTag, strLocationName);
                }
                else
                {
            ...
            else
            {
                ...
                if (nMaterialType == null || nMaterialType < 0 || strMaterialName == null)
                {
                    ...
                    strMessage = string.Format("[테스트][{0}]에서 유해화학물질 누출이 탐지되었습니다", strLocationName);
                }
                else
                {
