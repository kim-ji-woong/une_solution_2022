...
{
    public class ConductivitySensor : BaseServer
    {
        private MainManager m_mainManager = null;

        public ConductivitySensor(MainManager mainManager, Factory factory)
            : base(factory)
        {
            m_mainManager = mainManager;
            m_agent = factory.MakeAgent(Factory.AgentType.Conductivity);
        }
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("ConductivitySensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        {
            string strEventName = Facility.GetFacilityTypeString(Facility.FacilityType.Conductivity) + " 신호";
            string strErrorMessage = null;
            ...
            {
                // 다국어 설정
                strEventName = @"{""ko"":""전도도"", ""en"":""Conductivity""}";
        ...
        private string GetManualReportString(int nZoneID)
        {
            ...
            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                if (nZoneID < 0)
                {
                    strMessage = $"{{\"ko\":\"전도도 상황이 신고되었습니다\", \"en\":\"e전도도 상황이 신고되었습니다\"}}";
                }
                else
                {
                    ...
                    if (zone != null)
                    {
                        ...
                        //strMessage = string.Format("[{0}]에서 전도도 상황이 신고되었습니다", szLocationName);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 전도도 상황이 신고되었습니다\", \"en\":\"e{jDisplayText["en"].ToString()}에서 전도도 상황이 신고되었습니다\"}}";
        ...
        {
            string strMessage = "신고된 전도도 상황이 종료되었습니다";
            int nZoneID;

            if (m_mainManager.SDMSDataManager.SiteID == 15)
            {
                strMessage = $"{{\"ko\":\"신고된 전도도 상황이 종료되었습니다\", \"en\":\"e신고된 전도도 상황이 종료되었습니다\"}}";
                ...
                if (int.TryParse(alarm.ReactionHistoryParam1, out nZoneID))
                {
                    ...
                    if (zone != null)
                    {
                        //strMessage = string.Format("[{0}]에서 신고된 전도도 상황이 종료되었습니다", zone.DisplayText);
                        JObject jDisplayText = JObject.Parse(zone.DisplayText);
                        strMessage = $"{{\"ko\":\"{jDisplayText["ko"].ToString()}에서 신고된 전도도 상황이 종료되었습니다\", \"en\":\"e{jDisplayText["en"].ToString()}에서 신고된 전도도 상황이 종료되었습니다\"}}";
                    }
                }
            }
            else
        ...
        private Result ProcessAllClear(ArrayList arrDatas)
        {
            ...
            foreach (AlarmData alarm in alarms)
            {
                if (!Facility.IsConductivitySensorType(alarm.SensorType))
                    continue;
