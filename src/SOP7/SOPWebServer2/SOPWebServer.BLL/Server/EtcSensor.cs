...
namespace SOPWebServer.BLL.Server
{
    ...
    {
        protected MainManager m_mainManager = null;
        ...
        protected override Result OnReceiveEvent(int header, string strClientInfo, ArrayList arrDatas)
        {
            ...
            else if (header == Header.SENSOR_MALFUNCTION || header == Header.SENSOR_USER_RESET || header == Header.TIMEOUT)
        ...
        private Result ProcessSensorData(int header, ArrayList arrDatas, bool isReal, bool clearAlarm = false)
        {
            if (arrDatas.Count >= 4 && arrDatas[0] is int && arrDatas[1] is int && arrDatas[2] is int && arrDatas[3] is int)
            {
                ...
                WriteLog("EtcSensor ProcessSensorData 수신 (SensorType: " + nSensorType.ToString() + ", SensorTagID: " + nSensorTagID.ToString() + ", SensorZoneID: " + nSensorZoneID.ToString() +
        ...
        private Result ProcessManualReport(ArrayList arrDatas)
        {
                                    ...
                                    && arrDatas[4] is int && arrDatas[5] is string && arrDatas[6] is string)
            {
                ...
                AlarmData alarm = ((Process.AlarmManager)m_mainManager.AlarmManager).GetManualAlarm(nZoneID, Facility.FacilityType.ETC, m_mainManager.SDMSDataManager);
                if (alarm != null)
        ...
        private int AddAlarm(SensorZoneGroup group, int nSensorTagID, SensorZone sensorZone, bool isReal, int? nAlarmLevel, out AlarmData alarm)
        {
...

            {
                ...
                if (alarm != null)
                {
                    ...
                    string strMessage = GetDetectMessage(sensorZone, group.EquipmentZone, isReal);
        ...
        protected virtual string GetDetectMessage(SensorZone sensorZone, EquipmentZone equipZone, bool isReal)
        {
            return GetDetectEtcMessage((int)sensorZone.OrgSensorID, equipZone, isReal);
        }
        ...
        protected virtual string GetManualReportString(int nZoneID)
        {
            string strMessage = "";
        ...
        protected string GetTrainingModeString()
        {
