...
namespace SOPWebServer.BLL.Process
{
    ...
    public class ProcessManager : BaseProcessManager
    {
        ...
        public override void NewAlarm(AlarmData alarm, List<int> alarmSensorZoneIDs)
        {
            ...
            int nAlarmType = alarm.Status == (int)SensorReactionHistory.ReactionTypes.NOTIFY_SIGNAL ? (int)CurrentAlarm.AlarmTypes.Report : (int)CurrentAlarm.AlarmTypes.Detect;

            int nSopStatus = -1; //(-1: SOP 시작 하기전, 0: SOP 실행 요청, 1: SOP 실행중)
            if (alarm.SiteID == 14)
            {
                // TLB -> 알람이 뜨면 SOP를 실행한다
                nSopStatus = 0;
            }
            else
            {
        ...
        public override void UpdateAlarm(AlarmData alarm, List<int> alarmSensorZoneIDs, object group)
        {
            if (alarmSensorZoneIDs.Count == 0)
            {
                // CurrentAlarm에 SensorZoneID가 하나도 남지 않게 되면 알람이 종료되었다는 것을 의미한다.
                RemoveAlarm(alarm, (SensorZoneGroup)group);
                return;
            }
            ...
            /*System.Diagnostics.Trace.WriteLine("UpdateCurrentAlarm : " + alarmSensorZoneIDs.Count.ToString());*/
        }

        private void RemoveAlarm(AlarmData alarm, SensorZoneGroup group)
        {
            Dictionary<CurrentAlarm.Fields, object> dicCondition = new Dictionary<CurrentAlarm.Fields, object>();
            dicCondition[CurrentAlarm.Fields.SensorZoneHistoryID] = alarm.SensorZoneHistoryID;

            string strErrorMessage;

            if (m_mainManager.SDMSDataManager.GetDeleteManager().DeleteCurrentAlarm(dicCondition, null, out strErrorMessage))
            {
                Dictionary<SensorZoneHistory.Fields, object> dicConditions = new Dictionary<SensorZoneHistory.Fields, object>();
                dicConditions[SensorZoneHistory.Fields.ID] = alarm.SensorZoneHistoryID;

                ArrayList arrDatas = m_mainManager.SDMSDataManager.GetSelectManager().JoinEquipmentZoneSensorReactionHistorySensorZoneSensorZoneHistory(null, null, null, dicConditions, null, out strErrorMessage);
                ...
                if (arrDatas != null)
                {
                    ...
                    {
                        if (arrDatas[i] is EquipmentZone && arrDatas[i + 3] is SensorZoneHistory)
                        {
                            EquipmentZone equipZone = (EquipmentZone)arrDatas[i];
                            SensorZoneHistory sensorZoneHistory = (SensorZoneHistory)arrDatas[i + 3];

                            if (((Process.AlarmManager)m_mainManager.AlarmManager).RemoveAlarm(alarm, DateTime.Now, (int)SensorReactionHistory.ReactionTypes.END_STATUS, "", equipZone.ID.ToString(), sensorZoneHistory.SensorZoneID.ToString(), null, null, null, (int)SensorZoneHistory.DetectionType.Real, m_mainManager.SDMSDataManager))
                            {
        ...
        {
            string strErrorMessage;
            if (alarm.Status == (int)SensorReactionHistory.ReactionTypes.BEGIN_STATUS || alarm.Status == (int)SensorReactionHistory.ReactionTypes.CHANGE_ALARM_DEPTH)
            {
                Dictionary<CurrentAlarm.Fields, object> dicSets = new Dictionary<CurrentAlarm.Fields, object>();
            ...
            messageType = Process.SMSManager.ReactionTypeToMessageType((SensorReactionHistory.ReactionTypes)alarm.Status, alarm.SensorType);
            ...
            if (messageType != SMS.SMSMessageTypes.UNKNOWN)
            {
                ...
                else
                {
                    ...
                    if (sz != null)
                    {
                        equipZone = m_mainManager.SDMSDataManager.GetSelectManager().SelectEquipmentZone(sz.EquipZoneID, out strErrorMessage);

                        if (equipZone != null && equipZone.LinkedZoneIDs.Count > 0)
                    ...
                    dicConditions[Zone.Fields.ID] = nZoneID;

                    ArrayList arrDatas = m_mainManager.SDMSDataManager.GetSelectManager().JoinBuildingGroupBuildingZone(null, null, dicConditions, null, out strErrorMessage);
                    ...
                    else if (arrDatas.Count < 3)
                    {
                        zone = m_mainManager.SDMSDataManager.GetSelectManager().SelectZone(nZoneID, out strErrorMessage);

                        if (zone == null)
                        {
                            System.Diagnostics.Trace.WriteLine("ProcessAlarm Error : Zone을 확인할 수 없습니다.");
                            return SMS.SMSMessageTypes.UNKNOWN;
                        }
                    ...
                    else
                    {
                        sensorZone = sz;
                        equipmentZone = equipZone;
                        zone = (Zone)arrDatas[2];
                        building = (Building)arrDatas[1];
                        group = (BuildingGroup)arrDatas[0];
                    }
                ...
                if (strCaller != null && regularMembers.Count > 0 && nFacilityType != (int)Facility.FacilityType.NONE)
                {
                    ...
                    if (spreadMessages.Count > 0)
                    {
                        ...
                        if (alarm.Status != (int)SensorReactionHistory.ReactionTypes.END_STATUS)
                        {
                        ...
                        {   // 발생 해제 문자
                            string strMessage = $"{strDate}에 {strLocation}에서 발생한 {Facility.GetFacilityTypeString((Facility.FacilityType)nSensorType)} 알람이 해제 되었습니다.";
        ...
        {
            situationType = BaseBroadcastManager.ReactionTypeToSituationType((SensorReactionHistory.ReactionTypes)alarm.Status, alarm.SensorType);
