...
namespace SOPWebServer.BLL.Process
{
    ...
    public class AlarmManager : IAlarmManager
    {
        ...
        public AlarmManager(MainManager mainManager, SensorManager sensorManager)
        {
            ...
            // 기동 시 미해제 알람 복원.
            // 이제 신호 처리 시점마다 DB에서 그룹 상태를 다시 읽으므로 이 복원이 실패해도 알람 처리에는 지장이 없다.
            // 반면 여기서 예외가 나면 MainManager 생성이 통째로 실패해 첫 요청이 500으로 죽으므로 삼킨다.
            try
            {
            ...
            {
                Logger.Instance.Write("AlarmManager 기동 시 알람 복원 실패(신호 처리는 계속 가능) : " + ex.ToString());
            }
        }
        ...
        public bool RemoveAlarm(AlarmData alarm, DateTime timeStamp, int nReactionType, string strMessage, string strParam1, string strParam2, string strParam3, string strParam4, string strParam5, int? detectionStatus, IDataManager dataManager)
        {
            ...
            // 메모리 캐시에 항목이 없더라도 DB 반영(반응이력 기록)은 끝났으므로 성공으로 본다.
            // 예전에는 TryRemove 실패 시 false를 돌려주어, 그룹의 센서를 순회하며 여러 번 호출될 때
            // 두 번째부터 "RemoveAlarm 실패" 오탐 로그가 남았다.
            // 이제 호출부가 이 반환값으로 트랜잭션을 롤백하므로, 오탐이면 정상 해제까지 취소된다.
            /*System.Diagnostics.Trace.WriteLine("Remove CloseReactionHistory : " + alarm.SensorZoneHistoryID + ", " + nReactionType);*/
            return true;
        }
        ...
        public void RemoveCurrentAlarm(int nSensorZoneHistoryID)
        {
            ...
            if (m_mainManager.SDMSDataManager.GetDeleteManager().DeleteCurrentAlarm(dicConditions, null, out strErrorMessage))
            {
                ...
                /*System.Diagnostics.Trace.WriteLine("DeleteCurrentAlarm : " + nSensorZoneHistoryID);*/
            }
        }
        ...
        public AlarmData GetManualAlarm(int nZoneID, Facility.FacilityType facility, IDataManager dataManager)
        {
            ...
            {
                if (alarm.SensorZoneID == 0 && alarm.SensorType == facility && alarm.Status == (int)SensorReactionHistory.ReactionTypes.NOTIFY_SIGNAL)
                {
        ...
        public AlarmData AddAlarm(int nSensorZoneID, int nSensorData, int nZoneID, int nSensorType, int? detectionStatus, DateTime timeStamp, IDataManager dataManager, FacilityManager.DetectTypes detectType, int nSiteID)
        {
            List<int> allSensorZoneIDs = new List<int>();
            allSensorZoneIDs.Add(nSensorZoneID);
            SensorZoneHistory szh = dataManager.GetCreateManager().CreateSensorZoneHistory(nSensorZoneID, nSensorData.ToString(), timeStamp, nZoneID, nSensorType, detectionStatus, nSiteID, null, allSensorZoneIDs);
        ...
        public AlarmData FindAlarm(int sensorZoneID, Facility.FacilityType sensorType, IDataManager dataManager)
        {
            string strErrorMessage;
            ArrayList arrDatas = dataManager.GetSelectManager().JoinCurrentAlarmSensorZoneHistory(null, out strErrorMessage);
            ...
            {
                CurrentAlarm alarm = (CurrentAlarm)arrDatas[i];
                SensorZoneHistory sensorZoneHistory = (SensorZoneHistory)arrDatas[i + 1];

                if (sensorZoneHistory.AllSensorZoneIDs != null)
                {
                    if (sensorZoneHistory.AllSensorZoneIDs.Contains(sensorZoneID))
                    {
                        AlarmData alarmData;

                        if (m_dicSensorZoneHistoryIDAlarms.TryGetValue(sensorZoneHistory.ID, out alarmData))
                            return alarmData;
                        else
                        {
                            ...
                            alarmData.TimeStamp = sensorZoneHistory.Time;
                            alarmData.SiteID = sensorZoneHistory.SiteID;
                            alarmData.SensorType = sensorType;
        ...
        /// <summary>
        /// 여러 SensorZone 중 하나라도 물고 있는 미해제 알람을 DB에서 찾는다.
        /// FindAlarm()을 SensorZone 수만큼 호출하면 조회가 반복되므로 한 번만 조회한다.
        ///
        /// SensorZoneGroup의 CurrentAlarm을 DB 기준으로 갱신하기 위해 사용한다.
        /// </summary>
        public AlarmData FindAlarm(ICollection<int> sensorZoneIDs, Facility.FacilityType sensorType, IDataManager dataManager)
        {
            if (sensorZoneIDs == null || sensorZoneIDs.Count == 0 || dataManager == null)
                return null;
            ...
            // DB에 실제로 살아있는 알람의 SensorZoneHistoryID 집합
            Dictionary<int, int> dicAliveIDs = new Dictionary<int, int>();
            ...
            for (int i = 0; i < nDataCount - 1; i += 2)
            {
                ...
                dicAliveIDs[sensorZoneHistory.ID] = sensorZoneHistory.ID;

                if (result != null || sensorZoneHistory.AllSensorZoneIDs == null)
                    continue;

                foreach (int nSensorZoneID in sensorZoneIDs)
                {
                    if (sensorZoneHistory.AllSensorZoneIDs.Contains(nSensorZoneID) == false)
                        continue;

                    AlarmData alarmData;

                    // 이미 메모리에 있는 알람 객체가 있으면 그대로 쓴다.
                    // (같은 요청 안에서 알람 객체를 이어서 갱신하는 코드들이 있기 때문)
                    if (m_dicSensorZoneHistoryIDAlarms.TryGetValue(sensorZoneHistory.ID, out alarmData) == false)
                    {
                        alarmData = new AlarmData();
                        alarmData.SensorZoneID = nSensorZoneID;
                        alarmData.SensorZoneHistoryID = sensorZoneHistory.ID;
                        alarmData.TimeStamp = sensorZoneHistory.Time;
                        alarmData.SiteID = sensorZoneHistory.SiteID;
                        alarmData.SensorType = sensorType;
                        alarmData.AlarmDepth = alarm.AlarmDepth;
                        alarmData.IsReal = sensorZoneHistory.DetectionStatus == SensorZoneHistory.DetectionType.Real;

                        m_dicSensorZoneHistoryIDAlarms[sensorZoneHistory.ID] = alarmData;
                    }
            ...
            // DB(SdmsAlarmCurrent)에 더 이상 없는 알람은 메모리에서도 제거한다.
            // 이 캐시(CurrentAlarms)는 CheckAlarmDuplication 등에서 그대로 쓰이기 때문에,
            // 남겨두면 이미 해제된 알람 때문에 신규 알람이 "중복"으로 판정되어 사라진다.
            List<int> removeIDs = null;

            foreach (KeyValuePair<int, AlarmData> pair in m_dicSensorZoneHistoryIDAlarms)
            {
                if (dicAliveIDs.ContainsKey(pair.Key))
                    continue;
            ...
            if (removeIDs != null)
            {
                foreach (int nID in removeIDs)
                {
                    AlarmData removed;
                    m_dicSensorZoneHistoryIDAlarms.TryRemove(nID, out removed);
                }
            }
        ...
        public void ReloadAlarms(IDataManager dataManager)
        {
            m_dicSensorZoneHistoryIDAlarms.Clear();
            CheckAlarmSOPState(dataManager);
        }
        ...
        private void ReadCurrentAlarms()
        {
            ...
            foreach (SensorReactionHistory srh in sensorReactionHistories)
            {
                ...
                if (m_dicSensorZoneHistoryIDAlarms.TryGetValue(srh.SensorZoneHistoryID, out alarm) == false)
                {
                    ...
                    alarm.Status = (int)srh.ReactionType;
        ...
        private void TimeoutAlarm(IDataManager dataManager, int nSensorZoneHistoryID, DateTime time)
        {
            ...
            string strErrorMessage;
            dataManager.GetDeleteManager().DeleteCurrentAlarm(dicConditions, null, out strErrorMessage);
        }
        ...
        private AlarmData AddAlarm(IDataManager dataManager, SensorReactionHistory.ReactionTypes type, int nSensorZoneHistoryID, int nSensorReactionHistoryID, DateTime timeStamp, string strMessage, int nSensorZoneID, SensorZoneHistory.DetectionType detectionStatus)
        {
            ...
            else
            {
                ...
                alarm.SensorZoneID = nSensorZoneID;
                alarm.Status = (int)type;
                alarm.SensorType = (Facility.FacilityType)sensorZone.SensorType;
                alarm.IsReal = detectionStatus == SensorZoneHistory.DetectionType.Real;

                if (group.EquipmentZone?.SiteID > 0)
                    alarm.SiteID = group.EquipmentZone.SiteID;
