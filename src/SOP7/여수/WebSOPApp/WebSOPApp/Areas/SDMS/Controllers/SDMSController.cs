...
namespace WebSOPApp.Areas.SDMS.Controllers
{
    ...
    public class SDMSController : Controller
    {
        ...
        public IActionResult RequestData([FromBody] RequestData data)
        {
            ...
            else if (data.RequestSensorZoneHistories != null)
                return RequestHistorySensorReaction();

            return null;
        }

        private IActionResult RequestHistorySensorReaction()
        {
            List<SensorReactionHistory> response = m_processManager.GetLoadManager().RequestSensorReactionHistories();
            return Ok(response);
        }
