...
import { YeosuLogoBox, YeosuLogoBlackBox } from '../styled/TeamStyled';
...
class TeamEditor extends Component {
	...
	async makeRemoveRegularTeam(selectedTeam, data) {
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (!success) {
				...
				//alert(message);
				this.showConfirmDialog("에러", ["조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
		}
		else {
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog("에러", ["비상조직 삭제 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
        }
	...
	editTeam = async (team, chgName) => {
		...
		if (chgName === null || chgName === undefined || chgName.replace(/\s/gi, "") === "") {
			this.showConfirmDialog("에러", ["변경할 팀 이름을 다시 확인해주세요."], null, null);
			return;
		} 
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {			
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog("에러", ["조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
            }
			...
			if (!success) {
				// 사용자에게 불필요한 메시지 수정 - K.D.R
				//alert(message);
				this.showConfirmDialog("에러", ["비상조직 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
				return;
			}
	...
	checkEmail = (id, email, target) => {
		...
		if (chk === true) {
			this.showConfirmDialog("에러", [email + " 이메일 주소가 이미 사용 중입니다."], null, null);
			target.value = "";
        }
	...
	checkPhoneNumber = (id, phoneNumber, target) => {
		...
		if (chk === true) {
			// 휴대전화번호이 중복됨.
			this.showConfirmDialog("에러", [phoneNumber + " 휴대전화번호가 이미 사용 중입니다."], null, this.onCloseConfirmDialog());
			target.value = "";
		}
	...
	onChangeMemberEditMode = (member, editType, isEditMode) => {
		...
		if (userInfo && userInfo !== null && userInfo.levelID !== AccountResource.AccountLevel.Admin) {
			isEditMode = false;
		}
		...
		if (this.state.teamType === TeamEditorResource.ID.textRegular) {
			...
			if (isUpdate) {
				...
				if (!success && message.length > 0) {
					...
					//alert(message);
					this.showConfirmDialog("에러", ["조직원 정보 수정 실패하였습니다.", "새로고침 후에 다시 시도하거나 시스템 관리자에게 문의해주세요."], null, null);
					return;
				}
	...
	render() {
		...
		return (
			<div id="subPage" className={SdmsResource.UISection}>
				<Link to="/sdms"><YeosuLogoBlackBox className={SdmsResource.UISection}></YeosuLogoBlackBox></Link>
				<div id={styles.subAsideTeam} className="pageMenu">
					{/* 조직, 근무표 메뉴 버튼 컴포넌트 */}
					{						
						<TeamEditorContent
					...
					<DisplayMenu
						...
						temporaryTreeData={this.state.temporaryTreeData}
						temporaryEmergencyTreeData={this.state.temporaryEmergencyTreeData}
					/>
				</div>
	...
	render() {
		if (this.props.teamType === TeamEditorResource.ID.textRegular ||
			...
			return <TeamMenu
				...
				temporaryTreeData={this.props.temporaryTreeData}
				temporaryEmergencyTreeData={this.props.temporaryEmergencyTreeData}
			/>;
		}
