...
import { SettingCont } from '../../styled/SettingStyled';
...
class LayoutSetting extends Component {
    constructor(props) {
		...
		this.sendShutDownSystem = this.sendShutDownSystem.bind(this);
	...
	async init() {
		...
		const [yeosuSettings, yeosuMessage] = await SettingController.requestYeosuSettings();

		if (result === null || result === undefined)
			return;

		if (yeosuSettings === null || yeosuSettings === undefined)
			return;
		...
		SettingsStore.dispatch({ type: 'SETTINGS', idleTime, moveDisplayAlarm, turnStart, useAlarmTurn, weatherState, weatherSoundState, yeosuSettings });
		...
		this.setState({ settings: result, yeosuSettings: yeosuSettings, isLoading: false, buildingGroupList, teamTreeDatas, teams, members, disasterCategories, spreadMessages, linkedSOPs , siteID: siteID});
	...
	setMenuUI = () => {
		...
		if (this.state.isLoading === true) {
			menuUI.push(
				<React.Fragment key="Loading"><p className={'white'}>설정을 불러오고 있는 중입니다...</p></React.Fragment>
			);
			...
			menuUI.push(
				<>
					<Monitoring3D key="Monitoring3D" settings={this.state.settings} buildingGroupList={this.state.buildingGroupList} spreadMessages={this.state.spreadMessages}
						teamTreeDatas={this.state.teamTreeDatas} teams={this.state.teams} members={this.state.members} showConfirmDialog={this.showConfirmDialog} yeosuSettings={this.state.yeosuSettings} siteID={this.state.siteID}
					/>
				    <ul className={'dspBtn'}>
						<li><a className={'pointCursor'} onClick={() => this.onClickReset()}>기본값 복원</a></li>
					  <li><a className={'pointCursor'} onClick={this.onClickSave}>저장</a></li>
					</ul>
			    </>
			);
			...
			menuUI.push(
				<>
					<SystemInfo key="SystemInfo" settings={this.state.settings} showConfirmDialog2={this.showConfirmDialog2} onClickShutDown={this.onClickShutDown} />
					{/*<ul className={'dspBtn'}>
						<span className={'systemEndBtn'} onClick={() => this.onClickShutDown()}>시스템 종료</span>
					</ul>*/}
				</>
	         );
		}
	...
	onClickShutDown = () => {
		if (this.wsMgr) {
			this.showConfirmDialog("[INFO]", "시스템을 종료합니다.", ["확인", "취소"], this.sendShutDownSystem);
		} else {
			this.showConfirmDialog("[ERROR]", "WebSocket 연결을 확인해주세요.", null, null);
		}
	}

	sendShutDownSystem(index) {
	...
	onClickSave = () => {
		...
		const yeosuSettings = this.state.yeosuSettings;
		...
		let yeosuSaveData = {
			useReceiveAtmosphere: yeosuSettings.useReceiveAtmosphere,
			useReceiveWater: yeosuSettings.useReceiveWater,
			useReceiveVOC: yeosuSettings.useReceiveVOC,
			useReceiveOU: yeosuSettings.useReceiveOU
		}

		this.doSave(saveData, buildingFile, groupFile, facilityFile, regularTeamFile, spreadMessages, linkedSOPs, yeosuSaveData);

	}

	async doSave(saveData, buildingFile, groupFile, facilityFile, regularTeamFile, spreadMessages, linkedSOPs, yeosuSaveData) {
		// 설정 값이 이미 저장 중 일 경우
		...
		let [successYeosu, messageYeosu] = await SettingController.requestSaveSettingsYeosu(yeosuSaveData);
		...
		if (success === true) {
			...
			if (this.wsMgr) {
				...
				let bWeatherState = true;
				let bWeatherSoundState = true;

				if (saveData.weatherState === "0") {
					bWeatherState = false;
				}

				if (saveData.weatherSoundState === "0") {
					bWeatherSoundState = false;
				}

				this.wsMgr.setWeatherOption(bWeatherState, bWeatherSoundState);
			} // 설정 저장 완료시 날씨상태효과 여부 웹소켓 
		...
		if (linkedSOPData !== null && linkedSOPData !== undefined) {
			for (let i = 0; i < linkedSOPData.length; i++) {
				...
				if (chk === false) {
					// 여수는 BuildingGroup = Building = Zone 형식으로 관리
					linkedSOP.siteID = ProjectResource.SiteID;
		...
		if (regularTeamFile !== null && regularTeamFile !== undefined) {
			[success, message] = await SettingController.requestUploadRegularTeamFile(regularTeamFile, ProjectResource.Site.Yeosu);
			if (success !== true) {
				// 저장이 끝남 상태변화
				this.state.isSaving = false;
		...
		if (this.state.menu === SettingResource.ID.menu.systemInfo) {
			this.showConfirmDialog("시스템 종료", ["시스템을 종료하시겠습니까?"], ["확인"], this.onClickSaveClose);
		} else {
			this.showConfirmDialog("확인", ["설정이 저장되었습니다."], ["확인"], null);
		}
	}

	sendIdleTime = (idleTime) => {
	...
	getAuthorMenu() {
		...
		authorMenuUI.push(<React.Fragment key={"memuCommon"}>
			<li><a onClick={(e) => this.onClickMenu(SettingResource.ID.menu.monitoring3D, e.target)} className={'on'}>3D 관제 시스템</a></li>
		</React.Fragment>);
		...
		if (ProjectResource.isGSMode !== true) 
			menuBtn = <li><a onClick={(e) => this.onClickMenu(SettingResource.ID.menu.sopSet, e.target)}>SOP</a></li>;
		...
		authorMenuUI.push(<React.Fragment key={"memuInfo"}>
		    <li><a onClick={(e) => this.onClickMenu(SettingResource.ID.menu.systemInfo, e.target)}>시스템 정보</a></li>
		</React.Fragment>);
	...
	render() {
        ...
        return (
			<>
				<SettingCont id={'stgPop'} className={SDMSResource.UISection}>
					<div>
						<div>
							<div className={'stgCont'}>
								<div className={'stgLft'}>
									<div style={{ display: 'flex', padding: '30px 24px', alignItems : 'center' }}>
									  <span className={'settingIcon'}></span>
									  <h4 className={'stgTitle'}>환경설정</h4>
								   </div>
								...
								<div className={'stgRht'}>
									<a onClick={() => this.onClickClose(SettingResource.closeMode.cancle)} className={'stgClose'}>닫기</a>
									  {menuUI}
									...
									{/* <ul className={newStyles.dspBtn}>
									   <span className={newStyles.systemEndBtn}>시스템 종료</span>
								    </ul> */}
								</div>
							</div>
						</div>
