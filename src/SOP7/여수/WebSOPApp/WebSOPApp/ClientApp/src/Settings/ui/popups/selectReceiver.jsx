...
import { SelectCont } from '../../styled/SettingStyled';
...
class SelectReceiver extends Component {
	...
	getParentNode = (node) => {
		...
		return (<li key={"parentNode_" + id}>
			<h5 id="spreadBtn01"><span id={id} onClick={() => this.selectRegular(id)}> {name} </span></h5>
			<ul style={{ display: 'none', /* display: 'inline-block' */ marginRight: '20px', marginLeft: '8px' }}>{childTree}</ul>
		</li>);
	}
					...
					<div>
						<div>
							<div className={'dspCont'}>
								<h4 className={'dspTitle'}>수신자편집</h4>
								<div style={{ padding: '20px', backgroundColor: '#2b2b2b' }}>
								...
								<div className={'stguWrap'}>
									<div>
								        <div className={'stguTree stguWrapScroll'}>
										    <ul className={'dsiTree'} style={{ fontSize: "11px" }}>
									...
									<div>
										<div className={'stguTeam scrollbar'} style={{ overflow: "scroll" , overflowX: "scroll"}}>
