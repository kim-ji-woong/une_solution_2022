...
import { SubSectionProgressViewWrap, ChartWrap, ChartArea, PanelAreas, SectionPanels } from '../styled/ChartStyled';
...
/*    watermarkplay() {
    ...
    getActionStepArea() {
        ...
        let _actionStepHistory = null;

        // 현재 시작된 Sop중 가장 첫번째 요소
        if (stepDatas && stepDatas.length !== 0) {
            for (let i = 0; i < stepDatas.length; i++) {
                if (stepDatas[i]._ActionStepHistory !== null && stepDatas[i]._ActionStepHistory !== undefined) {
                    _actionStepHistory = stepDatas[i];
                    break;
                }
            }
        }
    ...
    onClickEditSOP(index) {        
        if (index === 0) {
            this.props.closeSOP(true);
        }

        if (index <= 1) {
            //window.open("/sop-manager?sop=" + this.props.sopData.version.id, '_blank');
            this.props.history.push('/sop-manager?sop=' + this.props.sopData.version.id);
        }
    ...
    render() {
        ...
        return (
          <>
                ...
                <SubSectionProgressViewWrap className={'subSection progressViewWrap'}> 
                    <div className={'tit clfix'}>
                      <div className={'chartTitle'}>
                        <strong>{sopName}</strong>
                        <button id={uneStyles.refresh} onClick={this.toOriginalLocation}>새로고침</button>
                      </div>
                        {
                            /*
                                ...
                                {/* 실행모드일 경우 Cell hover 테두리 제거를 위해서 apps.sectionPanels 클래스 네임 선언 */}
                                    <SectionPanels className={apps.sectionPanels}>
                                        <SectionPanel
                                            ...
                                            className={'chartScrollbar'}
                                         />
                                    {/*<div id={apps.watermark}>Training mode</div>*/}
                                    {/*<div id={apps.watermark}>훈련모드</div>*/}
                                    <span className={uis.editIconB} onClick={this.showConfirmDialog}></span>
                                    </SectionPanels>
                            </PanelAreas>
                        </ChartArea>
...
export default withRouter(SopSimulatorSBChart);