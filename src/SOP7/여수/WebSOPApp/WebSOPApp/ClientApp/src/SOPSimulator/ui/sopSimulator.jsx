...
import SettingsController from '../../Settings/services/settingController';
...
import { SettingCommon } from '../../Settings/styled/SettingStyled';
...
class SopSimulator extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            // 초기에 불러올때 리덕스에서 ActionType SopHistory가 들어오지 않기때문에 상태값 저장
            storeValue: null,
            isFirst: true,
            isOpenedEndPopup: false,
        ...
        store.subscribe(function () {
            ...
            this.loadSOP(store.getState(), false); 

            // 초기 SOP 로드시 ActionType을 못받는 현상 때문에 상태값 저장
            this.setRunSopDatas(data);
    ...
    componentDidMount() {
        ...
        const selectedAlarm = this.props.location.state;

        if (selectedAlarm) {
            this.setCurrentAlarm(selectedAlarm);
        }

        if (this.state.commonSettings === null || this.state.commonSettings === undefined) {
            this.getSettings();
        }

        if (this.isEmptySettings(this.state.commonSettings)) {
            this.getSettings();
        }
    ...
    isEmptySettings = (settings) => {
        if (settings.constructor === Object && Object.keys(settings).length === 0) {
            return true;
        }
    ...
    getSettings = async () => {
        const result = await SettingsController.reloadSopCommonSettings(true);
    ...
    setRunSopDatas = (storeValue) => {

        if (storeValue.actionType === 'SOP_HISTORY' || this.state.isFirst === true) {
            if (storeValue.sopHistory) {

                this.loadSOP(storeValue, this.state.isFirst);

                this.setState({ storeValue: storeValue, isFirst: false });
    ...
    async loadSOP(storeValue, isFirst) {
        if (storeValue && storeValue.actionType !== 'SOP_HISTORY' && isFirst === false)
            return;
        ...
        if (sopHistories === null || sopHistories.length === 0) {
            if (!this.state.isOpenedEndPopup) {
            ...
            if (this.props.location.state && isFirst) {
                for (let k = 0; k < sopHistories.length; k++) {
                    if (sopHistories[k].sensorZoneHistoryID === this.state.selectedAlarm.sensorZoneHistoryID) {
                        sopTabIndex = k;
                    }
                }
            }

            // 알람을 통해 들어오면 Sop Key 값을 강제로 할당해줘야 한다.
            // 처음 접속시에 보고있던 Sop를 유지해주는 기능때문
            if (isFirst && this.state.selectedAlarm) {
                for (let i = 0; i < sopHistories.length; i++) {
                    const sopHistory = sopHistories[i];

                    if (sopHistory.sensorZoneHistoryID === this.state.selectedAlarm.sensorZoneHistoryID) {
                        currentSopTabKey = sopHistory.key;
                        break;
                    }
                }
            ...
            // ex) 현재실행중인 SOP의 단계가 무조건 심각으로 나오는 현상 수정
            if (isFirst && this.state.selectedAlarm !== null && this.state.selectedAlarm !== undefined) {
                for (let i = 0; i < sopHistories.length; i++) {
                    const sopHistory = sopHistories[i];
                    const sopData = sopHistory.sopData;
                    const actionStepDatas = sopData.actionStepDatas;
                    if (sopHistory.sensorZoneHistoryID === this.state.selectedAlarm.sensorZoneHistoryID) {
                        for (let k = 0; k < actionStepDatas.length; k++) {
                            const actionStepData = actionStepDatas[k];
                            if (actionStepData._ActionStepHistory) {
    ...
    */
    ...
    setCurrentActionStep(sopData, actionStepID, lastActionStepHistoryID) {
        ...
        //    if (sopData.actionStepDatas[i].currentSection) {
        //        currentActionData = sopData.actionStepDatas[i];
        //    }
        //}
        ...
        //            else {
        //                //sopData.currentActionStep = actionStepData;
        //                sopData.currentActionStep = currentActionData;
        //            }
        //        }
        //    }
        //}
    ...
    onSelectMenu = (menu, param) => {
        ...
        if (menu === SopSimulator.resource.ID.menu.callSOP) {
            ...
            if (this.props.location.state !== null && this.props.location.state !== undefined) {
                if (this.props.location.state.alarm !== null && this.props.location.state.alarm !== undefined) {
                    selectedAlarm = this.props.location.state.alarm;
                }
            }
    ...
    // EndPopup이 열려있으면 Content변경을 막기 위함 
    // 열려있는 SOP가 없으면 SOP목록으로 이동하기 때문
    setEndPopupState = (isOpened) => {
        this.setState({ isOpenedEndPopup: isOpened , isFirst: true });
    }
        ...
        if (this.state.content === SopSimulatorResource.ID.menu.callSOP) {
            ...
            ui = <>
                <SopSimulatorBody
                    ...
                    commonSettings={this.state.commonSettings}
                    setEndPopupState={this.setEndPopupState}
                    selectedAlarm={this.state.selectedAlarm}
                    isFirs={this.state.isFirst}
                />
            </>
        }

        return (
            <div className={SdmsResource.UISection}>
                {ui}
                {
                    /* alert창 대신 사용 */
...
export default withRouter(SopSimulator);