...
class SopSimulatorSBcall extends Component {
    ...
    onChangeNormal = (e) => {
        ...
        if (value === SopSimulatorSBcall.isNormal) {
            isEmergency = false;
        }

        if (value === SopSimulatorSBcall.isEmergency) {
            isNormal = false;
        }
        
        this.setState({ isNormal: isNormal , isEmergency: isEmergency });
    }
                        ...
                        for (var k = 0; k < sdc.disasterDatas.length; k++) {
                            ...
                            if (lastAccessVersion !== null) {
                                ...
                                disaster.push(
                                    <>
                                        <tbody>
                                            <tr key={'disaster/' + lastAccessVersion.id} onClick={() => this.onSelectVersion(lastAccessVersion.id)}>
                                                <td><a key={d}></a></td>
                                                <td><p>{dc.disasterCategory.categoryName}&nbsp;&gt;&nbsp;{sdc.subDisasterCategory.subCategoryName}&nbsp;&gt;&nbsp;{d.disasterName}</p></td>
                                                <td>
                                                    {
                                                        (lastAccessVersion.isNormal)
                                                            ? <span className={uis.noti}>평일/주간</span>
                                                            : <span className={uis.noti + " " + uis.cGreen}>휴일/야간</span>
                                                    }
                                                </td>
                                                <td><span className={uis.date}>{lastAccessTime}</span></td>
                                            </tr>
                                        </tbody>
                                    </>
                                    ...
                                    /* <li key={'disaster/' + lastAccessVersion.id}>
                                        <a key={d} onClick={() => this.onSelectVersion(lastAccessVersion.id)}>
                                            <p>{dc.disasterCategory.categoryName}&nbsp;&gt;&nbsp;{sdc.subDisasterCategory.subCategoryName}&nbsp;&gt;&nbsp;{d.disasterName}나옴?</p>
                                            {
                                                (lastAccessVersion.isNormal)
                                                    ? <span className={uis.noti}>평일/주간</span>
                                                    : <span className={uis.noti + " " + uis.cGreen}>휴일/야간</span>
                    ...
                    if (this.state.selectedDisasterCategory === null || this.state.selectedDisasterCategory.id === sdc.subDisasterCategory.disasterCategoryID) {
                        ...
                        for (var k = 0; k < sdc.disasterDatas.length; k++) {
                            ...
                            if (lastAccessVersion !== null) {
                                const lastAccessTime = lastAccessVersion.lastAccessTime.replace('T', ' ');//this.getMakeDateTime(lastAccessVersion.lastAccessTime);
                                disaster.push(
                                    <li className={uis.sopList} key={'disaster/' + lastAccessVersion.id}>
                                        <a key={d} onClick={() => this.onSelectVersion(lastAccessVersion.id)}>
                                            <p>{dc.disasterCategory.categoryName}&nbsp;&gt;&nbsp;{sdc.subDisasterCategory.subCategoryName}&nbsp;&gt;&nbsp;{d.disasterName}뭐 보임?</p>
                                            {
                                                (lastAccessVersion.isNormal)
                                                    ? <span className={uis.noti}>평일/주간</span>
                                                    : <span className={uis.noti + " " + uis.cGreen}>휴일/야간</span>
                ...
                <Link to="/sdms"><span className={uis.yeosuLogoBox}></span></Link>
                    <div className={uis.appContainer}>
                       <section className={uis.subSection + " " + uis.menualListWrap}>
                         <div style={{ display: 'block', height: 'calc(100% - 126px)' }}>
                            <dl>
                                <div className={uis.list + ' category'}>
                                <dt><span className={uis.leftBorder}>재난분야</span><em className={uis.menualArrow}></em></dt>
                                    <dd>
                                ...
                                <dt><span className={uis.leftBorder}>재난종류</span><em className={uis.menualArrow}>{/* {subCategoryName} */}</em></dt>
                                    <dd>
                                        <ul className={uis.bullet}>
                                            {subDisasterCategory}
                                        </ul>
                        ...
                        {/* <div className={uis.returnArea} onClick={() => this.props.onSelectMenu(SopSimulatorResource.ID.menu.callSOP)}><span className={uis.returnBtn}></span>이전페이지</div> */}
                        </section>
                        <section className={uis.subSectionn + " " + uis.boardListWrap}>
                            <div className={uis.tit + " " + uis.clfix}>
                                <strong>SOP[SOP 임무 목록]</strong>
                                <span className={uis.returnSOPBtn} onClick={() => this.props.onSelectMenu(SopSimulatorResource.ID.menu.callSOP)}></span>
                                ...
                                {/* <div className={uis.filterArea + " " + uis.clfix}>
                                    ...
                                    <div className={uis.checkBox + " " + uis.cGreen}>
                                        <input type="checkbox" id="filter02" checked={this.state.isEmergency} onChange={this.onChangeEmergency}/>
                                        <label>휴일/야간</label>
                                    </div>
                                </div> */}
                            </div>
                            <div className={uis.innerSectionSOP + " " + uis.scrollbar}>
                                <ol className={uis.numList + " " + uis.list}>
                                    <table className={uis.sopListTable}>
                                        <thead>
                                        <tr>
                                            <th style={{ width: '10%', borderTopLeftRadius: '5px', borderBottomLeftRadius: '5px' }}>번호</th>
                                            <th style={{ width: '60%' }}>목록명</th>
                                            <th style={{ width: '10%'}}>
                                                <select className={uis.sopSelectBox} onChange={(e) => this.onChangeNormal(e)}>
                                                    <option value={SopSimulatorSBcall.isBoth}>전체</option>
                                                    <option id="filter01" checked={this.state.isNormal} value={SopSimulatorSBcall.isNormal}>평일/주간</option>
                                                    <option id="filter02" checked={this.state.isEmergency} value={SopSimulatorSBcall.isEmergency}>휴일/야간</option>
                                                </select>
                                            </th>
                                            <th style={{ width: '20%', borderTopRightRadius: '5px', borderBottomRightRadius: '5px' }}>날짜</th>
                                        </tr>
                                        </thead>
                        ...
                        {/*<div className={uis.pageNum}>*/}
                        {/*    <span className={uis.active}>1</span>*/}
                        {/*    <span>2</span>*/}
                        {/*    <span>3</span>*/}
                        {/*</div>*/}
                      </section>
