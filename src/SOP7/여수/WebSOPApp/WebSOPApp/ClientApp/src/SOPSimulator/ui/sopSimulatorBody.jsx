...
import { AppContainerWrapSop, YeosuLogoBox, AppContainerPgProgress, TabArea } from '../styled/SopStyled';
...
class SopSimulatorBody extends Component {
    ...
    constructor(props) {
        ...
        this.closeSOP2 = this.closeSOP2.bind(this);

        this.tabBoxRef = React.createRef();
        this.tabScrollRef = React.createRef();
    }
...
/*        $('.' + uneStyles.rightt).click(function () {
    ...
    async requestSensorName(sensorZoneHistoryID) {
        ...
        if (sensorZoneHistoryID && sensorZoneHistoryID > 0) {
            ...
            sensorMaterials = await SopSimulatorController.requestSensorMaterials(sensorZoneHistoryID);
        }

        let strArray = sensorMaterials.split(',');
        let uniqueStrArray = [...new Set(strArray)];
        let uniqueSensorName = uniqueStrArray.join(',');

        //this.setState({ sensorNames: sensorName });
        // 여수
        this.setState({ sensorNames: uniqueSensorName });
    }
    ...
    async runSection(section, decisionValue, fromChart) {        
        ...
        if (this.props.sopDatas[this.props.sopTabIndex].position === null && this.state.currentActionStepHistoryID === -1) {
            ...
            if (section.isBegin === false) {
                this.closeSOP2(this.props.sopTabIndex, false);
            }
    ...
    async closeSOP(ignoreSummary) {
        ...
        if (actionStepHistory !== null) {
            ...
            if (this.props.commonSettings.UseResultSummary === 'true' && !ignoreSummary) {
                this.setState({ content: SopSimulatorResource.ID.menu.summarySOP, summarySOPKey: sopRunData.key });
            }
            else {
    ...
    async closeSOP2(sopTabIndex, ignoreSummary) { // ProcessList 컴포넌트의 CheckBox이용시 CheckBox에 연동된 sopTabIndex가 없어서 인덱스 만들어 줘야함
        const sopRunData = this.props.sopDatas[sopTabIndex];
        const actionStepHistory = sopRunData.sopData.currentActionStep?._ActionStepHistory;
        if (actionStepHistory && actionStepHistory !== null) {
            ...
            else if (this.props.commonSettings.UseResultSummary === undefined || this.props.commonSettings.UseResultSummary === null) {
                if (!ignoreSummary) {
                    this.setState({ content: SopSimulatorResource.ID.menu.summarySOP, summarySOPKey: sopRunData.key });
                }
            } else  {
                await this.props.closeSOP(sopTabIndex, actionStepHistory.id, actionStepHistory.endTime);
            }
        }
        else {
            // 시작 전 SOP는 탭만 없애준다
            // await this.props.closeSOP(this.props.sopTabIndex, null, null, null);
            await this.props.closeSOP(sopTabIndex, null, null, null);
        }
    }
    ...
    getPopup() {
        ...
        if (this.state.content === SopSimulatorResource.ID.menu.summarySOP) {
            const sopRunData = this.props.sopDatas[this.props.sopTabIndex];

            if (this.state.summarySOPKey === sopRunData.key) {
                return <SummarySOP changeContent={this.changeContent} closeSOP={this.props.closeSOP} closeSOP2={this.state.closeSOP2} sopRunData={sopRunData} loginUser={this.state.loginUser} setEndPopupState={this.setEndPopupState} />
            }
        }
    ...
    setEndPopupState = (isOpened) => {
        this.props.setEndPopupState(isOpened);
    }

    onWheelTabBox = (e) => {
        const { deltaY } = e;
        const el = this.tabBoxRef.current;
    ...
    render() {
        ...
        return (
            <>                
                ...
                <AppContainerWrapSop className={'appContainerWrapSop paddingTop100'}>
                    <Link to="/sdms"><YeosuLogoBox className={SdmsResource.UISection}></YeosuLogoBox></Link>
                    <AppContainerPgProgress className={'appContainer pgProgress ' + SdmsResource.UISection}>

                        <TabArea className={'tabArea tabArea'}>
                            ...
                            <span className={'tabTitle'} onClick={() => this.props.changeContent(SopSimulatorResource.ID.menu.callSOP)}>SOP[SOP 임무목록]<p className={'menualArrowRight'}></p></span> {/* 임시 */}
                            <ul className={'tabBox'} ref={this.tabBoxRef} onWheel={(e) => this.onWheelTabBox(e)}>
                                {tabUI}
                            </ul>
                            <div className={'squaree'}>
                                <div className={'leftt'} onClick={this.onClickPrevTab}></div>
                                <div className={'rightt'} onClick={this.onClickNextTab}></div>
                            </div>
                        </TabArea>

                        <ProcessListSB
                            sopRunData={this.props.sopDatas[this.props.sopTabIndex]}
