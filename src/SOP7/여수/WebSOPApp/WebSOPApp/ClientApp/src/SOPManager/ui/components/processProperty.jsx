...
class ProcessProperty extends Component {
	...
	saveSectionData(shouldUpdate) {
		...
		const teamTreeData = this.state.teamTreeData;
		...
		if (teamTreeData) {
			...
			sectionData.receivers = TreeData.teamTreeDataToReceivers(teamTreeData, Receiver.RegularTeam);
	...
	onClickAddExternalProgram(add) {
		...
		if (add) {
			...
			if (selectedIndex < 0 || selectedIndex >= programDatas.length) {
				...
				this.showConfirmDialog("에러", [SopManagerResource.ID.messages.selectAddProgram], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
				return;
			}
			...
			for (let i = 0; i < parameterCount; i++) {
				...
				if (success === false) {
					...
					this.showConfirmDialog("에러", [SopManagerResource.format(SopManagerResource.ID.messageFormat.checkNthParameters, i)], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
					return;
				}
				else {
					arrDatas.push(data);
										...
										<dd className={this.getReceiversClassName()}>
											<div className={ProcessProperty.cssStyles.sprmTeam}>
												<ul className={ProcessProperty.cssStyles.sprmRdo}>
													<li>
														<label className={ProcessProperty.cssStyles.clickable}>
															<input type="radio" name="sprmRdo" className={bodyStyles.labelInputS} id={ProcessProperty.cssStyles.sprmRdo01} checked={this.state.teamType === Receiver.RegularTeam} onChange={() => this.onChangeTeamMode(Receiver.RegularTeam)} />
														정규조직
													</label>
													</li>
													<li>
														<label className={ProcessProperty.cssStyles.clickable}>
															<input type="radio" name="sprmRdo" className={bodyStyles.labelInputS} id={ProcessProperty.cssStyles.sprmRdo02} checked={this.state.teamType === Receiver.TemporaryNormalTeam} onChange={() => this.onChangeTeamMode(Receiver.TemporaryNormalTeam)} />
														평일 비상조직
													</label>
													</li>
													<li>
														<label className={ProcessProperty.cssStyles.clickable}>
															<input type="radio" name="sprmRdo" className={bodyStyles.labelInputS} id={ProcessProperty.cssStyles.sprmRdo03} checked={this.state.teamType === Receiver.TemporaryEmergencyTeam} onChange={() => this.onChangeTeamMode(Receiver.TemporaryEmergencyTeam)} />
														야간/휴일 비상조직
													</label>
													</li>
												</ul>
