...
class DeleteSOPOptions extends Component {
	...
	onClickDelete = (event) => {
		...
		if (versionIDs.length === 0) {
			this.props.showConfirmDialog("ERROR", "삭제할 버전을 선택하세요.", "확인", null);
		}
		if (versionIDs.length > 0) {
		...
		if (sopData?.disaster) {
			...
			for (const versionID of versionIDs) {
				if (versionID === disaster.versionID) {
					...
					this.props.showConfirmDialog('INFO', ["현재 화면에서 편집중인 버전을 삭제하려고 합니다.", "계속 진행할까요? 이 작업은 돌이킬수 없습니다."], ["예", "아니오"], this.onConfirmDelete);
					return false;
				}
			}
		}
							...
							<div className={DeleteSOPOptions.cssStyles.sppCont}>
								<div className={DeleteSOPOptions.cssStyles.sppLft}>
									<div className={DeleteSOPOptions.cssStyles.scrollbarOuter}>
										<ul className={'sarTreePop treeviewPop'}>
										{
