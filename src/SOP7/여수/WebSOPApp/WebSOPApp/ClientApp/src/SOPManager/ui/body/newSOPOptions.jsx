...
class NewSOPOptions extends Component {
	...
	getSOPData() {
		if (this.state.selectedSubDisasterCategory === this.state.newSubDisasterCategory) {
			...
			if (sdcName.length === 0) {
				...
				this.showConfirmDialog("에러", [SopManagerResource.ID.messages.inputDisasterType], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
				return null;
			}
		...
		if (disasterData === this.state.newDisaster) {
			...
			if (disasterName.length === 0) {
				...
				this.showConfirmDialog("에러", [SopManagerResource.ID.messages.inputSOPName], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
				return null;
			}
		...
		if (disaster === null) {
			...
			this.showConfirmDialog("에러", [SopManagerResource.ID.messages.unknownSOPName], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
			return;
		}
	...
	onClickApply() {
		if (!this.state.selectedDisasterCategory) {
			...
			this.showConfirmDialog("에러", [SopManagerResource.ID.messages.selectDisasterCategory], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
			return;
		}
		else if (!this.state.selectedSubDisasterCategory) {
			//alert(SopManagerResource.ID.messages.selectSubDisasterCategory);
			this.showConfirmDialog("에러", [SopManagerResource.ID.messages.selectSubDisasterCategory], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
			return;
		}
		else if (!this.state.selectedDisaster) {
			//alert(SopManagerResource.ID.messages.selectSOPName);
			this.showConfirmDialog("에러", [SopManagerResource.ID.messages.selectSOPName], this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
			return;
		}
