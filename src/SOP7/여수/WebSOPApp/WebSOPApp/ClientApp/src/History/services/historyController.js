...
export default class HistoryController {
    static async DisplayUserHistories(beginTime, endTime) {
        try {
            ...
            const res = await fetch(ProjectResource.baseUrl + '/History/History/RequestData', {
                method: 'post',
                headers: {
    ...
    static async DisplaySensorDetectHistories(beginTime, endTime, facilityType, buildingGroupID, buildingID, zoneID, lastSensorZoneHistoryID, rowCount, isDesc) {
        try {
            ...
            if (res.ok) {
                const result = await res.json();
                return [result.sensorDetectHistoryDatas, result.lastSensorReactionHistoryID, result.sensorZoneHistories];
            }
        } catch (e) {
            console.log(e);
        }
    ...
    // SOP 요약 팝업창 다운로드 기능에서 사용할 정보
    static async DisplaySOPHistories2(sensorZoneHistoryID) {
        try {
            const jsonData = JsonManager.makeRequestSOPHistories2(sensorZoneHistoryID);
