...
class accountFindPwd extends Component {
    ...
    onClick = () => {
        if (this.state.mode === AcoountResource.findMode.email) {
            ...
            if (!emailValid) {
                this.showConfirmDialog("에러", ["이메일 주소 형식이 아닙니다. 다시 한번 확인해주세요."], null, null);
                return;
            }
            ...
            if (!phoneValid) {
                this.showConfirmDialog("에러", ["핸드폰 번호 형식이 아닙니다. 다시 한번 확인해주세요."], null, null);
                return;
            }
    ...
    async changePassword(name, value, mode) {
        ...
        if (result === null) {
            ...
            this.setState({ showMessage: message });
            this.showConfirmDialog("성공", [message], null, null);
        }
        else {
            this.setState({ showMessage: message });
            this.showConfirmDialog("실패", [message], null, null);
        }
    }
                ...
                <>
                    ...
                    <tr key={"userEmail"} id="rowEmail">
                        ...
                        <td>
                            ...
                            ref={this.refEmail}
                            className={contents.DblueInput + " " + contents.w100p}
                            placeholder={AcoountResource.ID.textPlaceEmail} />
                        </td>
                    </tr>
                </>);
        }
    ...
    onClickSetPwd = (e) => {
        ...
        if (this.state.inputName === "") {
            return this.showConfirmDialog("에러", ["이름을 입력하세요."], null, null);
        }

        if (this.state.inputPhoneNum === "") {
            return this.showConfirmDialog("에러", ["핸드폰 번호를 입력하세요."], null, null);
        }

        let phone = this.state.inputPhoneNum;
        let regPhone = /^01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})$/;

        if (!regPhone.test(phone)) {
            return this.showConfirmDialog("에러", ["잘못된 핸드폰 번호 형식입니다, 다시 확인해주세요."], null, null);
        }
        ...
        // 등록된 사용자 이름 검증
        if (users.length === 0) {
            this.showConfirmDialog("에러", ["등록된 사용자 정보가 없습니다."], null, null);
            return
        }
        ...
        for (let u of this.state.users) {
            if (u.memberName === this.state.inputName) {
                user = u;
            }
        }
        ...
        if (user !== null && user !== undefined) {
            ...
            this.showConfirmDialog("[ERROR]", ["일치하는 사용자 정보가 없습니다."], null, null);
            return;
        }
        

        if (phone !== this.state.inputPhoneNum) {
            this.showConfirmDialog("에러", ["해당 사용자에 등록된 전화번호와 일치하지 않습니다. 전화번호를 확인해주세요."], null, null);
            return;
        }

        // 비밀번호 업데이트 , 문자발송
        this.changePassword(user.memberName, user.phoneNumber, AcoountResource.findMode.sms);
    ...
    getAccountUsers = async () => {
        const users = await AccountController.getAccountUsers(ProjectResource.Site.Yeosu);

        this.setState({ users: users });

    }

    onKeyPressOnName = (value) => {
        this.setState({ inputName: value });
    }

    onKeyPressOnPhone = (value) => {
        this.setState({ inputPhoneNum: value });
    }
    ...
    render() {
		...
		return (
            <>
                <div className={contents.accountBackground}>
                <div id={contents.popupConts} className={contents.loginPopup}>
                    ...
                    <div className={contents.indexLoginTitle}><img className={accounts.logoSizeYeosu} src={Ylogo} alt="여수" /></div>
                    <span className={contents.loginTitleYeosu}>여수산단 환경 모니터링 시스템</span>

                    <div className={contents.indexLoginBoxY}>
                        {/* yeosu */}
                            <ul>
                                <li><input type="text" className={contents.indexLoginYeosu} onChange={(e) => this.onKeyPressOnName(e.target.value)} placeholder="이름을 입력하세요." /></li>
                                <li><input type="password" className={contents.indexLoginPwYeosu} onChange={(e) => this.onKeyPressOnPhone(e.target.value)} placeholder="핸드폰 번호를 입력하세요." /></li>
                            </ul>

                            <div className={uis.btnArea}>
                                <a className={contents.btnLoginYeosu} href="/#" onClick={this.onClickSetPwd}>임시 비밀번호 발급</a>
                                <a className={contents.loginPageBack} href="/#" onClick={this.props.onCloseChangePW}>로그인 페이지로 돌아가기</a>
                            </div>
                    </div>
                </div>
