...
class SDMSMainMenu extends Component {
    ...
    constructor(props) {
        ...
        SDMSMainMenu.Test_Alarm_Level = SDMSMainMenu.ALARM_2;
    }
    ...
    moveBuildingName() {
        ...
        if (x !== null && x !== undefined && y !== null && y !== undefined && z !== null && z !== undefined) {
            ...
            //alert("숫자만 입력가능합니다.");
            this.showConfirmDialog("에러", "숫자만 입력 가능합니다.", this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
        }
    }
    ...
    moveSensor() {
        ...
        if (isNaN(x) === false && isNaN(z) === false && sensor) {
            ...
            if (!sensor) {
                //alert("선택된 센서가 없습니다.");
                this.showConfirmDialog("에러", "선택된 센서가 없습니다.", this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
            }
            else {
                //alert("숫자만 입력가능합니다.");
                this.showConfirmDialog("에러", "숫자만 입력 가능합니다.", this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
    ...
    movePOI() {
        ...
        if (isNaN(x) === false && isNaN(y) === false && isNaN(z) === false && this.props.selectedPOI) {
            ...
            if (!this.props.selectedPOI) {
                //alert("선택된 POI가 없습니다.");
                this.showConfirmDialog("에러", "선택된 POI가 없습니다.", this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
            }
            else {
                //alert("숫자만 입력가능합니다.");
                this.showConfirmDialog("에러", "숫자만 입력 가능합니다.", this.state.confirmMessage.buttons, () => this.onCloseConfirmDialog());
    ...
    showAlarm() {
        ...
        if (SDMSMainMenu.Test_Alarm_Level > SDMSMainMenu.ALARM_4) {
            SDMSMainMenu.Test_Alarm_Level = SDMSMainMenu.ALARM_2;
        }
    }
