...
class SopSet extends Component {
	...
	initData() {
		...
		if (this.refUseBroadcast.current && useBroadcast === "true")
			this.refUseBroadcast.current.click();

		if (this.refUseSMS.current && useSMS === "true")
			this.refUseSMS.current.click();

		if (this.refUseEmail.current && useEmail === "true")
			this.refUseEmail.current.click();

		if (this.refUseConfirm.current && useConfirm === "true")
			this.refUseConfirm.current.click();

		if (this.refUseResultSummary.current && useResultSummary === "true")
	...
	initWaitEndTime() {
		...
		this.refWaitEndTime.current.value = this.props.sensorTypes && this.props.sensorTypes.length > 0 ? this.props.sensorTypes[0].facilityTypeID : "";//SettingResource.facilityType.Fire.toString();
	...
	onChangeWaitEndTime = () => {
		...
		} else if (type === SettingResource.facilityType.SOS.toString()) {
			data = this.props.settings.sosSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.SVMS.toString()) {
			data = this.props.settings.svmsSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.Earthquake.toString()) {
			data = this.props.settings.earthquakeSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.StrongWind.toString()) {
			data = this.props.settings.strongWindSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.BlackOut.toString()) {
			data = this.props.settings.blackoutSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.Collapse.toString()) {
			data = this.props.settings.collapseSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.Confined.toString()) {
			data = this.props.settings.confinedSOPWaitEndTime;
		} else if (type === SettingResource.facilityType.VirtualFence.toString()) {
			data = this.props.settings.virtualFenceSOPWaitEndTime;
		}
	...
	onChangeWait = (e) => {
		...
		if (type === SettingResource.facilityType.Fire.toString()) {
			...
			this.props.settings.sosSOPWaitEndTime = value;
		}
		else if (type === SettingResource.facilityType.SVMS.toString()) {
			this.props.settings.svmsSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.Earthquake.toString()) {
			this.props.settings.earthquakeSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.StrongWind.toString()) {
			this.props.settings.strongWindSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.BlackOut.toString()) {
			this.props.settings.blackoutSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.Collapse.toString()) {
			this.props.settings.collapseSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.Confined.toString()) {
			this.props.settings.confinedSOPWaitEndTime = value;
		} else if (type === SettingResource.facilityType.VirtualFence.toString()) {
			this.props.settings.virtualFenceSOPWaitEndTime = value;
		}
	}
	...
	getSensorTypeUI = () => {
		return (
			<>
				{
					this.props.sensorTypes && this.props.sensorTypes.map((type, index) => (
						<option key={'sensorType_' + type.facilityTypeID} value={type.facilityTypeID}>{type.displayText}</option>
				))
			}
			</>
		);
				...
				<div className={newStyles.stgList}>
					...
					<div className={newStyles.stgHalf}>
						...
						<div>
							...
							<ul className={newStyles.stgMode}>
									<li><input ref={this.refUseSMS} type="checkbox" name="stgStat" id="stgStat02" onChange={(e) => this.onChangeUseSMS(e.target)} />
										<label htmlFor="stgStat02">
											{(ProjectResource.SiteID === ProjectResource.Site.Cleannara) ? '카톡 전파 사용' : '문자 전파 사용'}
										</label></li>
									{
