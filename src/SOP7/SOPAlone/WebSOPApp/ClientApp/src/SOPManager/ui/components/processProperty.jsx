...
import { SprContProcess, SprTop, SprtTitle, LabelInput, LabelInputRadio } from '../../../SOPManager/styled/componentsStyled';
import { SprtIpt, SprtIptRecipients, ScrollWrapper, ScrollWrapperReci, ScrollWrapperProcess, ScrollContentReci, ScrollContent, ScrollContentMission, SprMid, SprmContProcess, SprmAcdn, SprmTeam, SprmRdo } from '../../../SOPManager/styled/componentsStyled';
import { SprmAcdnDt, SprmAcdnDD, SprmUdn, SprmUdnUp, SprmUdnDown, SprmUdnDel, SprmAdd, SprmTb, SprmDsc, SprmIport } from '../../../SOPManager/styled/componentsStyled';
import { Tal, SprmBtn ,SprBot } from '../../../SOPManager/styled/componentsStyled';
			...
			if (index % 2 === 0) {
				return (
					<tr key={"missoin_" + index}>
						...
						<Tal>
							<ScrollWrapper /* className={"scroll-wrapper " + ProcessProperty.cssStyles.sprtTxt + " scrollbar-outer scroll-textarea"} */ id="pos_relative">
								<ScrollContentMission /* className="scroll-content" id="mission_odd" */>
				...
				<SprTop>
					...
					<SprtIptRecipients>
						...
						<dd>
							<ScrollWrapperReci>
								<ScrollContentReci>
									<textarea name="" id="" cols="30" rows="10" value={this.state.receiverName} onChange={() => {}}>
									</textarea>
								</ScrollContentReci>
							</ScrollWrapperReci>
						</dd>
					...
					<SprMid>
						<ScrollWrapperProcess>
							<ScrollContent>
								<SprmContProcess>
								   <div className={ProcessProperty.cssStyles.sprmAcdn}>
									<dt ref={this.refReceivers} className={this.getReceiversClassName()} onClick={this.onClickCascade}>수신자</dt>
											...
											<SprmTeam>
												<SprmRdo>
													<li>
													  <label>
														<LabelInputRadio>
														   <input type="radio" name="sprmRdo" /* id={ProcessProperty.cssStyles.sprmRdo01} */ checked={this.state.teamType === Receiver.RegularTeam} onChange={() => this.onChangeTeamMode(Receiver.RegularTeam)} />
														</LabelInputRadio>
														정규조직
													  </label>
													</li>
													<li>
													  <label>
														<LabelInputRadio>
															<input type="radio" name="sprmRdo" /* id={ProcessProperty.cssStyles.sprmRdo02} */ checked={this.state.teamType === Receiver.TemporaryNormalTeam} onChange={() => this.onChangeTeamMode(Receiver.TemporaryNormalTeam)} />
														</LabelInputRadio>
														평일 비상조직
													  </label>
													</li>
													<li>
													  <label>
														<LabelInputRadio>
															<input type="radio" name="sprmRdo" /* id={ProcessProperty.cssStyles.sprmRdo03} */ checked={this.state.teamType === Receiver.TemporaryEmergencyTeam} onChange={() => this.onChangeTeamMode(Receiver.TemporaryEmergencyTeam)} />
														</LabelInputRadio>
														야간/휴일 비상조직
													</label>
													</li>
									    ...
									    <dd className={this.getMissionsClassName()}>
										    <SprmUdn>
											    <SprmUdnUp onClick={this.onClickUp}></SprmUdnUp>
											    <SprmUdnDown onClick={this.onClickDown}></SprmUdnDown>
											    <SprmUdnDel onClick={this.onClickDel}></SprmUdnDel>
											</SprmUdn>
											<SprmAdd onClick={this.onClickAddMission}>+ 임무내용 추가</SprmAdd>
											<SprmTb>
												<colgroup>
													<col />
													<col />
											...
											<SprmIport onClick={this.onClickExternalPrograms}>외부 프로그램 불러오기</SprmIport>
											{
											...
											}
											{
												this.state.externalProgram.showList && this.state.externalProgram.datas.length > 0 && (
												    <SprmBtn>
														<li><a onClick={() => this.onClickAddExternalProgram(true)}>추가</a></li>
														<li><a onClick={() => this.onClickAddExternalProgram(false)}>취소</a></li>
												    </SprmBtn>
												)
											}
									    </dd>
							  	    </div>
							   </SprmContProcess>{/*sprmCont*/}
						    </ScrollContent>
					    </ScrollWrapperProcess>
					</SprMid>{/*sprMid*/}
				    <SprBot>
